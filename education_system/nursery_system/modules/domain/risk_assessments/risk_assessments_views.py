...
from education_system.nursery_system.modules.domain.risk_assessments import risk_assessments as data
from education_system.nursery_system.modules.domain.risk_assessments.risk_assessments import ValidationError
...
def open_manager(host) -> None:
    root = _clear(host)
    ttk.Label(root, text="Risk Assessments", font=("", 16, "bold")).pack(anchor="w", pady=(0, 8))
    ...
    cols = ('record_id', 'title', 'area', 'risk_level', 'review_date', 'status',)
    ...
    host.status_var.set("Risk Assessments loaded")
...
def _refresh(tree, summary) -> None:
    ...
    try:
        ...
        return
    for rec in rows:
        tree.insert("", "end", iid=rec.record_id, values=(_cell(getattr(rec, 'record_id', None)), _cell(getattr(rec, 'title', None)), _cell(getattr(rec, 'area', None)), _cell(getattr(rec, 'risk_level', None)), _cell(getattr(rec, 'review_date', None)), _cell(getattr(rec, 'status', None)),))
...
def _sel(tree, host, verb):
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("Risk Assessments", f"Select a record to {verb}.", parent=host.root)
        return None
    return sel
...
def _form_dialog(host, title, *, initial=None):
    ...
    ttk.Label(frm, text="Title:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("title")
    ...
    vars_["area"] = v
    row += 1
    ttk.Label(frm, text="Individual child (optional):").grid(row=row, column=0, sticky="nw", pady=2)
    ...
    vars_["pupil_id"] = v
    row += 1
    ttk.Label(frm, text="Assessor:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("assessor")
    ...
    vars_["assessor"] = v
    row += 1
    ttk.Label(frm, text="Date assessed (YYYY-MM-DD):").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("date_assessed")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["date_assessed"] = v
    row += 1
    ...
    vars_["review_date"] = v
    row += 1
    ttk.Label(frm, text="Hazards:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("hazards")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["hazards"] = v
    row += 1
    ttk.Label(frm, text="Control measures:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("control_measures")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["control_measures"] = v
    row += 1
    ttk.Label(frm, text="Risk level:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("risk_level")
    v = tk.StringVar(value=str(cur or ""))
    ttk.Combobox(frm, textvariable=v, values=[""] + list(data.RISK_LEVELS), state="readonly", width=32).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["risk_level"] = v
    row += 1
...
def open_add(host) -> None:
    fields = _form_dialog(host, "Add assessment")
    if not fields:
        open_manager(host)
        return
    try:
...
def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    ...
    ttk.Label(frame, text="Risk Assessments", font=("", 14, "bold")).pack(anchor="w", pady=(0, 6))
    ttk.Label(frame, text="Open Risk Assessments from the navigation menu.").pack(anchor="w")
    return frame
