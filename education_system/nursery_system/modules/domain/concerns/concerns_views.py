"""Tkinter views for Concerns & Referrals (Nursery System).

Renders into the shared content pane of ``main_gui.NurseryMainGUI`` (the
``host``); the GUI counterpart of ``concerns_cli.py``.
"""
...
from education_system.nursery_system.modules.domain.concerns import concerns as data
from education_system.nursery_system.modules.domain.concerns.concerns import ValidationError
...
def open_manager(host) -> None:
    root = _clear(host)
    ttk.Label(root, text="Concerns & Referrals", font=("", 16, "bold")).pack(anchor="w", pady=(0, 8))
    ...
    cols = ('record_id', 'date_raised', 'child_name', 'concern_type', 'referred_to', 'status',)
    tree = ttk.Treeview(root, columns=cols, show="headings", height=18)
    for c, label, w in [
        ("record_id", "ID", 70),
        ("date_raised", "Date", 110),
        ("child_name", "Child", 150),
        ("concern_type", "Type", 130),
        ("referred_to", "Referred to", 140),
        ("status", "Status", 110),
    ]:
    ...
    host.status_var.set("Concerns & Referrals loaded")
...
def _refresh(tree, summary) -> None:
    ...
    try:
        ...
        return
    for rec in rows:
        tree.insert("", "end", iid=rec.record_id, values=(_cell(getattr(rec, 'record_id', None)), _cell(getattr(rec, 'date_raised', None)), _cell(getattr(rec, 'child_name', None)), _cell(getattr(rec, 'concern_type', None)), _cell(getattr(rec, 'referred_to', None)), _cell(getattr(rec, 'status', None)),))
...
def _sel(tree, host, verb):
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("Concerns & Referrals", f"Select a record to {verb}.", parent=host.root)
        return None
    return sel
...
def _form_dialog(host, title, *, initial=None):
    ...
    ttk.Label(frm, text="Child (optional):").grid(row=row, column=0, sticky="nw", pady=2)
    ...
    vars_["pupil_id"] = v
    row += 1
    ttk.Label(frm, text="Concern type:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("concern_type")
    v = tk.StringVar(value=str(cur or ""))
    ttk.Combobox(frm, textvariable=v, values=[""] + list(data.CONCERN_TYPES), state="readonly", width=32).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["concern_type"] = v
    row += 1
    ttk.Label(frm, text="Raised by:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("raised_by")
    ...
    vars_["raised_by"] = v
    row += 1
    ttk.Label(frm, text="Date raised (YYYY-MM-DD):").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("date_raised")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["date_raised"] = v
    row += 1
    ttk.Label(frm, text="Description:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("description")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["description"] = v
    row += 1
    ttk.Label(frm, text="Referred to:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("referred_to")
    v = tk.StringVar(value=str(cur or ""))
    ttk.Combobox(frm, textvariable=v, values=[""] + list(data.REFERRED_TO), state="readonly", width=32).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["referred_to"] = v
    row += 1
    ttk.Label(frm, text="Referral date (YYYY-MM-DD):").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("referral_date")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["referral_date"] = v
    row += 1
    ttk.Label(frm, text="Outcome:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("outcome")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["outcome"] = v
    row += 1
...
def open_add(host) -> None:
    fields = _form_dialog(host, "Add concern")
    if not fields:
        open_manager(host)
        return
    try:
...
def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    ...
    ttk.Label(frame, text="Concerns & Referrals", font=("", 14, "bold")).pack(anchor="w", pady=(0, 6))
    ttk.Label(frame, text="Open Concerns & Referrals from the navigation menu.").pack(anchor="w")
    return frame
