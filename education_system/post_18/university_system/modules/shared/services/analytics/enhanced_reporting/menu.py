...
from education_system.post_18.university_system.modules.shared.services.analytics.enhanced_reporting.config import CONFIG, AVAILABLE_SECTIONS, get_reporting_db_connection, logger
from education_system.post_18.university_system.modules.shared.services.analytics.enhanced_reporting._compat import get_log_file
from education_system.post_18.university_system.modules.shared.services.analytics.enhanced_reporting.models import ReportTemplate, AdvancedScheduledReport
...
from education_system.post_18.university_system.modules.shared.services.analytics.enhanced_reporting.predictive import PredictiveAnalytics
from education_system.post_18.university_system.modules.shared.services.analytics.enhanced_reporting.visualization import AdvancedVisualization
from education_system.post_18.university_system.modules.shared.services.analytics.enhanced_reporting.templates_db import save_template, load_templates, get_template
...
from education_system.post_18.university_system.modules.shared.services.analytics.enhanced_reporting.scheduler import (
    run_system_maintenance, cleanup_old_reports,
    load_scheduled_reports, save_scheduled_reports, start_scheduler,
)
from education_system.post_18.university_system.modules.shared.services.analytics.enhanced_reporting.api import app
        ...
        if action == '1':
            ...
            try:
                ...
                if report_path and report.get('recipients'):
                    success = send_email is not None
                    ...
                    if send_email is not None:
                        ...
                        for recipient in report['recipients']:
                            if not send_email(
                                ...
                                success = False
                                message = "email delivery failed for one or more recipients"
...
def show_performance_monitor():
    ...
    try:
        ...
        # Users (counted directly from the users table; the old UserManager
        # JSON-store helper was removed during the system reorg).
        try:
            user_conn = get_reporting_db_connection()
            try:
                user_cursor = user_conn.cursor()
                user_cursor.execute("SELECT COUNT(*) FROM users")
                total_users = user_cursor.fetchone()[0]
                try:
                    user_cursor.execute("SELECT COUNT(*) FROM users WHERE is_active = 1")
                    active_users = user_cursor.fetchone()[0]
                except Exception:
            ...
            pass  # users table may live in a different database
