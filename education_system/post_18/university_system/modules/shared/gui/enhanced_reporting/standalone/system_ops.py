...
# Enhanced-reporting helpers are only defined in constants when the enhanced
# service package is importable. They are used behind `if ENHANCED_AVAILABLE:`
# guards below, so bind them to None when unavailable to avoid a NameError.
try:
    from education_system.post_18.university_system.modules.shared.gui.enhanced_reporting.standalone.constants import (
        CacheManager,
        DataQualityMonitor,
...
University Reporting System"""
        ...
        if success_count > 0:
            ...
            logging.error("Failed to email report to any recipients")
