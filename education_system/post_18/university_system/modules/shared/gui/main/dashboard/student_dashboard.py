...
def _launch_profile(parent, auth):
    ...
    from education_system.post_18.university_system.modules.domain.student_affairs.gui.student_profile.profile_gui import StudentProfileGUI
    StudentProfileGUI(parent, auth=auth)


def _launch_security(parent, auth):
    """Launch Account Security Dashboard."""
    from education_system.post_18.university_system.modules.domain.student_affairs.gui.account_security.security_gui import AccountSecurityGUI
    AccountSecurityGUI(parent, auth=auth)


def _launch_notifications(parent, auth):
    """Launch Notification Preferences."""
    from education_system.post_18.university_system.modules.domain.student_affairs.gui.notification_prefs.notification_prefs_gui import NotificationPrefsGUI
    NotificationPrefsGUI(parent, auth=auth)


def _launch_grades_breakdown(parent, auth):
    """Launch Grades Breakdown by Module."""
    from education_system.post_18.university_system.modules.domain.academics.academic_progress.gui.grades_breakdown_gui import GradesBreakdownGUI
    GradesBreakdownGUI(parent, auth=auth)


def _launch_degree_progress(parent, auth):
    """Launch Degree Progress Tracker."""
    from education_system.post_18.university_system.modules.domain.academics.academic_progress.gui.degree_progress_gui import DegreeProgressGUI
    DegreeProgressGUI(parent, auth=auth)


def _launch_course_catalog(parent, auth):
    """Launch Course Catalog & Registration."""
    from education_system.post_18.university_system.modules.domain.academics.gui.course_catalog.course_catalog_gui import CourseCatalogGUI
    CourseCatalogGUI(parent, auth=auth)


def _launch_gpa_calculator(parent, auth):
    """Launch What-If GPA Calculator."""
    from education_system.post_18.university_system.modules.domain.academics.academic_progress.gui.gpa_calculator_gui import GPACalculatorGUI
...
def _launch_messaging(parent, auth):
    ...
    import tkinter as tk
    from education_system.post_18.university_system.modules.shared.gui.email.email_gui.email_manager_main import EmailManagerGUI
    EmailManagerGUI(tk.Toplevel(parent), auth=auth)


def _launch_forums(parent, auth):
    """Launch Course Discussion Forums."""
    from education_system.post_18.university_system.modules.domain.academics.gui.course_forums.course_forums_gui import CourseForumsGUI
...
def _launch_roommate_finder(parent, auth):
    ...
    """
    import tkinter as tk
    from education_system.post_18.university_system.modules.domain.campus.housing.gui.housing_accommodation_gui.main_gui import HousingGUI
...
def _launch_documents(parent, auth):
    ...
    from education_system.post_18.university_system.modules.domain.student_affairs.gui.document_center.document_center_gui import DocumentCenterGUI
    DocumentCenterGUI(parent, auth=auth)


def _launch_graduation(parent, auth):
    """Launch the Graduation Ceremony GUI (opens the student self-service tab)."""
    from education_system.post_18.university_system.modules.domain.academics.gui.course_management_gui.graduation_ceremony_gui import GraduationCeremonyGUI
    GraduationCeremonyGUI(parent, auth=auth)
...
def _launch_feature(root, auth, feature_name):
    ...
    feature_map = {
        'marketplace': (
            'Marketplace',
            'education_system.post_18.university_system.modules.domain.commerce.marketplace.gui.marketplace_gui',
            'MarketplaceGUI', 'kwarg', True,
        ),
        'lost_found': (
            'Lost & Found',
            'education_system.post_18.university_system.modules.domain.campus.lost_found.gui.lost_found_gui',
            'LostFoundGUI', 'kwarg', True,
        ),
        'campus_navigation': (
            'Campus Navigation',
            'education_system.post_18.university_system.modules.domain.campus.campus_navigation.gui.navigation_gui',
            'NavigationGUI', 'none', True,
        ),
        'social_matching': (
            'Social Matching',
            'education_system.post_18.university_system.modules.domain.student_affairs.social_matching.gui.social_matching_gui',
            'SocialMatchingGUI', 'none', True,
        ),
        'mail_post': (
            'Mail & Post',
            'education_system.post_18.university_system.modules.domain.operations.communications.mail.gui.mail_post_gui',
            'MailPostGUI', 'positional', True,
        ),
        'printing_services': (
            'Printing Services',
            'education_system.post_18.university_system.modules.domain.campus.printing.gui.printing_gui',
            'PrintingServicesGUI', 'none', True,
        ),
        'study_room_booking': (
            'Study Room Booking',
            'education_system.post_18.university_system.modules.domain.campus.study_rooms.gui.study_room_gui',
            'StudyRoomBookingGUI', 'none', True,
        ),
        'student_id': (
            'Student ID Card',
            'education_system.post_18.university_system.modules.domain.student_affairs.student_id.gui.student_id_gui',
            'StudentIDGUI', 'none', True,
        ),
        'achievement_badges': (
            'Achievement Badges',
            'education_system.post_18.university_system.modules.domain.student_affairs.achievement_badges.gui.achievement_badge_gui',
            'AchievementBadgeGUI', 'kwarg', True,
        ),
        'wellness_hub': (
            'Wellness Hub',
            'education_system.post_18.university_system.modules.domain.student_affairs.wellness.gui.wellness_gui',
            'WellnessGUI', 'kwarg', True,
        ),
        'todo_app': (
            'Todo App',
            'education_system.post_18.university_system.modules.shared.gui.tools.todo_app_gui',
            'TodoApp', 'none', False,
        ),
        'my_visa_status': (
            'My Visa Status',
            'education_system.post_18.university_system.modules.domain.student_affairs.international_compliance.gui.my_visa_status_gui',
            'MyVisaStatusGUI', 'none', True,
        ),
        'enrolment_letters': (
            'Status Letters',
            'education_system.post_18.university_system.modules.domain.student_affairs.student_app.documentation.gui.documentation_gui',
...
def create_student_dashboard(parent_frame, auth, service):
    ...
    # Announcements posted via the Communications (Email) GUI, filtered to
    # audiences a student can see.
    try:
        ...
        create_announcements_panel(scrollable, auth, limit=5)
    except Exception as e:
        logger.error(f"Error loading announcements panel: {e}")
    ...
    buttons = [
        ...
        ("My Graduation", lambda: _launch_graduation(root, auth)),
    ...
    from education_system.post_18.university_system.modules.shared.gui.main.dashboard.evaluation_launchers import (
    ...
    try:
        from education_system.post_18.university_system.modules.shared.gui.main.dashboard.student_widgets import (
