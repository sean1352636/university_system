...
def open_student_union_portal_gui(self):
    ...
    if self.student_union_gui:
        ...
        messagebox.showerror(_t("student_affairs.errors.title"), _t("student_affairs.student_union.not_available"))
# CLI fallback for the parent portal (not exported by gui_imports).
try:
    from education_system.post_18.university_system.modules.domain.academics.services.parent_portal.menu import (
        display_parent_portal_menu,
    )
except ImportError:
    def display_parent_portal_menu(*args, **kwargs):
        raise RuntimeError("Parent portal CLI menu is unavailable")
...
def show_career_services_gui(self):
    ...
    if callable(opener):
        from education_system.post_18.university_system.modules.domain.student_affairs.gui.career_services.career_services_gui import CareerServicesGUI
...
def open_internship_portal_gui(self):
    ...
    try:
        from education_system.post_18.university_system.modules.domain.student_affairs.gui.internship_management import InternshipGUI
    except Exception as e:
...
def show_email_sms_gui(self):
    ...
    try:
        ...
        from education_system.post_18.university_system.modules.shared.gui.email.email_manager_management_gui import EmailManagerManagementGUI
...
def show_admissions_crm_gui(self):
    ...
    opener = getattr(self, "open_in_workspace", None)
    if callable(opener):
        from education_system.post_18.university_system.modules.domain.admissions.gui.admissions_crm_gui import AdmissionsCRMGUI
...
def show_police_station_gui(self):
    ...
    try:
        from education_system.post_18.university_system.modules.domain.campus.gui.security.police_station_gui import PoliceStationApp
...
def show_security_desk_gui(self):
    ...
    otherwise — same pattern as Student Records (8.117.38)."""
    try:
        from education_system.post_18.university_system.modules.domain.campus.gui.security.security_desk_gui import SecurityDesk
    except Exception as e:
...
def show_church_management_gui(self):
    ...
    try:
        from education_system.post_18.university_system.modules.domain.campus.gui.community.church_management_gui import ChurchManagementSystem
        window = tk.Toplevel(self.root)
        _install_clean_close(window)
