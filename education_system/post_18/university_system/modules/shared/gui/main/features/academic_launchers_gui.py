...
from education_system.post_18.university_system.modules.shared.gui.main.features.academic_link_bar import (
...
try:
    from education_system.post_18.university_system.modules.shared.gui.pdf_export_gui import show_pdf_export_gui as _pdf_export_gui_func
    PDF_EXPORT_GUI_AVAILABLE = True
except ImportError:
    _pdf_export_gui_func = None
    PDF_EXPORT_GUI_AVAILABLE = False

# LIBRARY_GUI_IMPORT_ERROR is referenced in the library CLI-fallback message but
# is not exported by gui_imports; default it so that message path cannot NameError.
LIBRARY_GUI_IMPORT_ERROR = None
...
def show_academic_calendar(self):
    ...
    try:
        ...
        if messagebox.askyesno(_t("academic_launchers.fallback.title"), _t("academic_launchers.fallback.try_cli_calendar")):
            try:
                from education_system.post_18.university_system.modules.domain.academics.services.academic_calendar.cli import display_academic_calendar_menu
    ...
    try:
        if MODULE_SCHEDULING_GUI_AVAILABLE and ModuleSchedulingGUI:
            ...
            try:
                ...
                from education_system.post_18.university_system.modules.domain.academics.services.module_scheduling import display_enhanced_scheduling_menu
...
def show_timetable_management(self):
    """Open the standalone Timetable Manager window (student & instructor
    timetables: generate / view / email / export)."""
    try:
        from education_system.post_18.university_system.modules.domain.academics.gui.timetable_management import (
            TimetableManagementGUI,
        )
    except Exception as e:
        logger.error(f"Timetable Manager unavailable: {e}")
        messagebox.showerror(_t("academic_launchers.errors.title"),
                             f"Timetable Manager unavailable: {e}")
        return
    try:
        ...
        try:
            ...
            logger.debug(f"Could not set timetable window transient: {e}")
    except Exception as e:
        logger.error(f"Error launching Timetable Manager: {e}")
        messagebox.showerror(_t("academic_launchers.errors.title"),
                             f"Failed to launch Timetable Manager: {e}")
...
def show_plagiarism_checker(self):
    ...
    try:
        ...
        from education_system.post_18.university_system.modules.domain.academics.gui.plagiarism_main_gui import launch_gui_from_main_system
...
def open_absence_tracker_gui(self):
    ...
    try:
        python = sys.executable
        cmd = [python, "-m",
               "education_system.post_18.university_system.modules.domain.academics.services.attendance.absence_tracking.absence_tracker"]
...
def show_graduation_ceremony_gui(self):
    ...
    gown orders, seat plan, stage script). Opens as a standalone full-size
    window rather than an embedded workspace tab."""
    if not self.auth.current_user:
...
def show_virtual_classroom_gui(self):
    ...
    try:
        from education_system.post_18.university_system.modules.domain.academics.gui.virtual_classroom_gui import VirtualClassroomGUI
...
def show_exam_scheduler_gui(self):
    ...
    try:
        ...
        if role == 'student':
            ...
            from education_system.post_18.university_system.modules.domain.academics.gui.exam_management import StudentExamViewer
            StudentExamViewer(window, auth=self.auth)
        else:
            # Staff/admin get full exam scheduler
            from education_system.post_18.university_system.modules.domain.academics.gui.exam_management import ExamSchedulerApp
...
def show_exam_portal(self):
    ...
    try:
        from education_system.post_18.university_system.modules.domain.academics.gui.exam_management import ExamPortalGUI
        window = tk.Toplevel(self.root)
        _install_clean_close(window)
...
def show_hesa_export_gui(self):
    ...
    try:
        from education_system.post_18.university_system.modules.domain.admissions.hesa_export.gui.hesa_export_gui import HESAExportGUI
    except ImportError as e:
...
def show_clearing_adjustment_gui(self):
    ...
    Toplevel otherwise — same pattern as Student Records (8.117.38)."""
    try:
        from education_system.post_18.university_system.modules.domain.academics.clearing_adjustment.gui.clearing_adjustment_gui import ClearingAdjustmentGUI
    except ImportError as e:
...
def show_prior_learning_gui(self):
    ...
    tab accordingly."""
    try:
        from education_system.post_18.university_system.modules.domain.academics.prior_learning_recognition.gui.prior_learning_gui import PriorLearningGUI
        from education_system.post_18.university_system.modules.shared.gui.main.features.academic_link_bar import (
            consume_context as _consume,
        )
    except ImportError as e:
...
def show_mitigating_circumstances_gui(self):
    ...
    try:
        from education_system.post_18.university_system.modules.domain.academics.mitigating_circumstances.gui.mitigating_circumstances_gui import MitigatingCircumstancesGUI
    except ImportError as e:
