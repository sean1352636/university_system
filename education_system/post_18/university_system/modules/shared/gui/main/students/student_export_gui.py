...
def export_individual_student_data(self, student_id, first_name, last_name):
    ...
    try:
        ...
        def perform_export():
            ...
            try:
                cursor.execute("""
                    SELECT ticket_id, subject, category, status, created_at, resolved_at
    ...
    def export_data():
        ...
        try:
            ...
            try:
                ...
                # Index of the consolidated module columns within each row.
                MODULE_COUNT_IDX = 12
                MODULES_IDX = 13
                ...
                for row in student_rows:
                    ...
                    # Consolidate ALL of the student's modules into one tidy
                    # cell (one module per line), plus a count — instead of a
                    # fixed, truncating set of module_1..module_6 columns.
                    row_data.append(len(modules_list))
                    row_data.append("\n".join(modules_list))
                ...
                if export_format == "CSV":
                    ...
                    # Tidy the workbook: wrap the multi-line "Enrolled Modules"
                    # cell, widen it, and top-align rows so every module shows.
                    try:
                        ...
                        ws = wb.active
                        modules_col = MODULES_IDX + 1  # 1-based for openpyxl
                        count_col = MODULE_COUNT_IDX + 1
                        ws.column_dimensions[get_column_letter(modules_col)].width = 40
                        ws.column_dimensions[get_column_letter(count_col)].width = 10
                        for cell in ws[1]:  # header row
                            cell.font = Font(bold=True)
                            cell.alignment = Alignment(vertical='top')
                        for r in range(2, ws.max_row + 1):
                            ws.cell(row=r, column=modules_col).alignment = Alignment(
                                wrap_text=True, vertical='top')
                            ws.cell(row=r, column=count_col).alignment = Alignment(
                                horizontal='center', vertical='top')
                        ws.freeze_panes = 'A2'
                        wb.save(filename)
                    except Exception:
                        # Formatting is best-effort; the data export already succeeded.
                        pass
                    ...
                    for row in processed_rows:
                        ...
                        for idx, value in enumerate(row):
                            ...
                            # The modules cell holds newline-separated entries;
                            # render each on its own line within the cell.
                            if idx == MODULES_IDX:
                    ...
                    # Trailing columns: "# Modules" (narrow) + "Enrolled
                    # Modules" (takes all remaining width so the list is legible).
                    count_width = 0.55 * inch
                    fixed_total = sum(fixed_widths) + count_width
                    modules_width = max(page_width - fixed_total, 1.5 * inch)
                    col_widths = fixed_widths + [count_width, modules_width]
                    ...
                    with open(filename, 'w', encoding='utf-8') as txtfile:
                        for row in processed_rows:
                            ...
                            module_count = row[MODULE_COUNT_IDX]
                            txtfile.write(f"{_t('student_export.text_labels.modules')} ({module_count}):\n")
                            # Modules are newline-separated in the consolidated cell.
                            module_lines = [m for m in str(row[MODULES_IDX]).split("\n") if m]
                            if module_lines:
                                for n, module in enumerate(module_lines, start=1):
                                    txtfile.write(f"  {_t('student_export.text_labels.module')} {n}: {module}\n")
                            else:
                                txtfile.write("  —\n")
                            txtfile.write("-" * 60 + "\n\n")
