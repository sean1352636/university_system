from education_system.post_18.university_system.infrastructure.database.db import DEFAULT_DB_PATH, get_connection  # injected
from education_system.post_18.university_system.core.sql_safety import validate_identifier, validate_table_name, validate_field_for_query, validate_column_name  # nosec B608
...
try:
    from education_system.post_18.university_system.modules.shared.utils.chart_generator import (
...
try:
    from education_system.post_18.university_system.modules.shared.services.analytics.advanced_search import (
...
try:
    ...
    from education_system.post_18.university_system.infrastructure.email.admin import search_users, list_all_users
...
def get_connection():
    ...
    try:
        # Prefer the central connection from education_system.post_18.university_system.infrastructure.database.db if available
        from education_system.post_18.university_system.infrastructure.database.db import get_connection as central_get_connection
...
from education_system.post_18.university_system.modules.shared.gui.advanced_search.base import AdvancedSearchGUI

def show_analytics_dashboard(self):
    """Show analytics dashboard in a separate window"""
    self.update_status(_t("advanced_search.menus.loading_analytics"))

    try:
        result = self.capture_function_output(search_analytics_dashboard)
        self._show_report_viewer(result, _t("advanced_search.menus.analytics_dashboard_title"))
        self.update_status(_t("advanced_search.menus.analytics_loaded"))
    except Exception as e:
        messagebox.showerror(_t('common.error'), _t('advanced_search.menus.error_loading_analytics', error=str(e)))
        self.update_status(_t('advanced_search.menus.failed_load_analytics'))
AdvancedSearchGUI.show_analytics_dashboard = show_analytics_dashboard

def show_multi_criteria_search(self):
    """Show multi-criteria search form"""
    self.create_search_form()
AdvancedSearchGUI.show_multi_criteria_search = show_multi_criteria_search

def show_advanced_text_search_menu(self):
    """Show advanced text search submenu"""
    dialog = tk.Toplevel(self.master)
    dialog.title(f"📝 {_t('advanced_search.advanced_text_search_menu_title')}")
    ...
    search_options = [
        (f"🔍 {_t('advanced_search.menus.regex_search')}", self.show_regex_search),
        (f"🃏 {_t('advanced_search.menus.wildcard_search')}", self.show_wildcard_search),
        (f"📋 {_t('advanced_search.menus.search_all_fields')}", self.show_search_all_fields),
...
def show_admin_features_menu(self):
    ...
    dialog = tk.Toplevel(self.master)
    dialog.title(f"👑 {_t('advanced_search.admin_features_dialog_title')}")
    dialog.geometry("400x350")
    dialog.transient(self.master)
    dialog.grab_set()
...
def show_smart_features_menu(self):
    ...
    smart_options = [
        (f"💡 {_t('advanced_search.menus.auto_complete')}", self.show_auto_complete_search),
        (f"🧠 {_t('advanced_search.menus.smart_suggestions')}", self.show_smart_suggestions),
...
def show_enhanced_import_export_menu(self):
    ...
    dialog = tk.Toplevel(self.master)
    dialog.title(_t('advanced_search.import_export_dialog_title'))
    ...
    # Import section
    import_frame = ttk.LabelFrame(frame, text=_t('advanced_search.menus.import_options'), padding="10")
    import_frame.pack(fill=tk.X, pady=(0, 20))

    import_options = [
        (_t('advanced_search.menus.import_csv'), lambda: self.import_data("csv", "students")),
        (_t('advanced_search.menus.import_json'), lambda: self.import_data("json", "students")),
        (_t('advanced_search.menus.import_excel'), lambda: self.import_data("xlsx", "students")),
    ...
    export_options = [
        (_t('advanced_search.menus.export_students'), lambda: self.export_all_data("students")),
        (_t('advanced_search.menus.export_modules'), lambda: self.export_all_data("modules")),
        (_t('advanced_search.menus.export_analytics'), lambda: self.export_all_data("analytics")),
...
def show_demographics_reports(self):
    """Show demographics reports in a separate window"""
    ...
    try:
        ...
        messagebox.showerror(_t('common.error'), _t('advanced_search.menus.error_demographics', error=str(e)))
        self.update_status(_t('advanced_search.menus.failed_demographics'))
AdvancedSearchGUI.show_demographics_reports = show_demographics_reports
...
def show_advanced_demographic_report(self):
    """Open the comprehensive advanced demographic analysis in a new window"""
    self.update_status(_t("advanced_search.menus.generating_advanced_demographics"))
    try:
        conn = get_connection()
        cursor = conn.cursor()
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        result = self.generate_demographics_analysis_report(cursor, timestamp)
        conn.close()
        self._show_report_viewer(result, _t("advanced_search.menus.advanced_demographics_title"))
        self.update_status(_t("advanced_search.menus.advanced_demographics_complete"))
    except Exception as e:
        messagebox.showerror(_t('common.error'), _t('advanced_search.menus.failed_demographic_analysis', error=str(e)))
        self.update_status(_t('advanced_search.menus.failed_analysis'))
AdvancedSearchGUI.show_advanced_demographic_report = show_advanced_demographic_report

def show_performance_analysis(self):
    """Show performance analysis in a separate window"""
    self.update_status(_t("advanced_search.menus.analyzing_performance"))

    try:
        result = self.capture_function_output(academic_performance_analysis)
        self._show_report_viewer(result, _t("advanced_search.menus.performance_analysis_title"))
        self.update_status(_t("advanced_search.menus.performance_complete"))
    except Exception as e:
        messagebox.showerror(_t('common.error'), _t('advanced_search.menus.error_performance', error=str(e)))
        self.update_status(_t('advanced_search.menus.failed_performance'))
AdvancedSearchGUI.show_performance_analysis = show_performance_analysis
