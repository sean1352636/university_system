...
def show_comprehensive_reports(self):
    ...
    def generate_reports():
        ...
        if not selected:
            messagebox.showwarning(_t("advanced_search.reports.no_reports_selected_title"), _t("advanced_search.reports.no_reports_selected_msg"))
            return
        ...
        try:
            ...
            messagebox.showerror(_t("advanced_search.error_title"), _t("advanced_search.reports.report_generation_error", error=str(e)))
            self.update_status(_t("advanced_search.reports.report_generation_failed"))
...
def _save_report_as_txt(self, report_content, report_title):
    ...
    if file_path:
        try:
            ...
            messagebox.showinfo(_t("advanced_search.success_title"), _t("advanced_search.reports.report_saved_msg", path=file_path))
            self.update_status(f"Report saved to {file_path}")
        except Exception as e:
            messagebox.showerror(_t("advanced_search.error_title"), _t("advanced_search.reports.failed_to_save_report", error=str(e)))
    ...
    try:
        ...
        if not admin_emails:
            messagebox.showwarning(
                _t("advanced_search.reports.no_admin_emails_title"),
                _t("advanced_search.reports.no_admin_emails_msg")
            )
            return
        ...
        try:
            ...
            messagebox.showerror(_t("advanced_search.error_title"), _t("advanced_search.reports.email_service_not_available"))
            return
...
"""
        ...
        if success_count == len(admin_emails):
            messagebox.showinfo(
                _t("advanced_search.success_title"),
                _t("advanced_search.reports.report_sent_success", count=success_count)
            )
            self.update_status(f"Report sent to {success_count} admin(s)")
        elif success_count > 0:
            messagebox.showwarning(
                _t("advanced_search.reports.partial_success_title"),
                _t("advanced_search.reports.partial_success_msg", count=success_count, failed=', '.join(failed_emails))
            )
        else:
            messagebox.showerror(
                _t("advanced_search.reports.failed_title"),
                _t("advanced_search.reports.failed_to_send_all", failed=', '.join(failed_emails))
            )

    except Exception as e:
        messagebox.showerror(_t("advanced_search.error_title"), _t("advanced_search.reports.failed_to_send_report", error=str(e)))
        import traceback
        traceback.print_exc()
...
def generate_module_enrollment_report(self, cursor, timestamp):
    ...
    result = "MODULE ENROLLMENT REPORT\n"
    result += "=" * 50 + "\n"
    result += f"Generated: {timestamp}\n\n"
    ...
    result += "ENROLLMENT OVERVIEW:\n"
    result += f"Total Module Enrollments: {total_enrollments}\n\n"

    result += "TOP MODULES BY ENROLLMENT:\n"
...
def generate_demographics_analysis_report(self, cursor, timestamp):
    ...
    result = "DEMOGRAPHICS ANALYSIS REPORT\n"
    result += "=" * 50 + "\n"
    result += f"Generated: {timestamp}\n\n"
    ...
    result += "GENDER DISTRIBUTION BY COURSE:\n"
    current_course = None
...
def generate_performance_analysis_report(self, cursor, timestamp):
    ...
    result = "PERFORMANCE ANALYSIS REPORT\n"
    result += "=" * 50 + "\n"
    result += f"Generated: {timestamp}\n\n"
    ...
    if performance_stats:
        ...
        result += "OVERALL PERFORMANCE METRICS:\n"
...
def generate_custom_sql_report(self, cursor, timestamp):
    ...
    result += "Note: In full implementation, users can input custom SQL queries here.\n"
...
def export_report_to_file(self, content, report_type):
    ...
    messagebox.showinfo(_t("advanced_search.reports.export_complete_title"), _t("advanced_search.reports.export_complete_msg", filename=filename))
