...
def refresh_data(self):
    ...
    try:
        # Check if the window still exists before accessing widgets
    ...
    try:
        ...
        if hasattr(self, 'results_label'):
            self.results_label.config(text=_t("advanced_search.results.data_refreshed"))

    except tk.TclError:
        pass  # Window was destroyed during refresh
    except Exception as e:
        self.log_output(f"Error refreshing data: {e}")
AdvancedSearchGUI.refresh_data = refresh_data
...
def display_search_results(self, results):
    ...
    # Update UI
    count = len(results)
    self.results_label.config(text=_t("advanced_search.results.search_results_count", count=count))
    self.export_btn.config(state='normal' if count > 0 else 'disabled')
AdvancedSearchGUI.display_search_results = display_search_results
...
AdvancedSearchGUI.update_results_display = update_results_display
...
def previous_page(self):
    ...
    if self.current_page > 0:
        self.current_page -= 1
        self.update_results_display()
AdvancedSearchGUI.previous_page = previous_page
...
def next_page(self):
    ...
    if self.current_page < total_pages - 1:
        self.current_page += 1
        self.update_results_display()
AdvancedSearchGUI.next_page = next_page

def change_results_per_page(self, event=None):
    """Change number of results per page"""
    self.current_page = 0
    self.update_results_display()
AdvancedSearchGUI.change_results_per_page = change_results_per_page
