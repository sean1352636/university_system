...
class EmailManagerManagementGUI:
    ...
    def __init__(self, parent_root, auth_manager):
        ...
        self._email_window = None

        # Theme integration — optional; degrade gracefully if unavailable.
        try:
    ...
    def on_theme_changed(self, *args, **kwargs):
        """Re-apply the active theme to the open email-manager window, if any."""
        if not self.theme_manager:
            return
        try:
            if self._email_window is not None:
                self.theme_manager.apply_theme_to_window(self._email_window)
        except Exception:
            pass
    ...
    def show_email_manager(self):
        ...
        try:
            ...
            # Apply the active theme to the child window.
            self._email_window = top
