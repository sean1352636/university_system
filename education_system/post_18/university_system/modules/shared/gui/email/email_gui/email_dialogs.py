...
from education_system.post_18.university_system.modules.shared.utils.config import config, save_config, test_email_configuration
from education_system.post_18.university_system.infrastructure.email.email_db_utilities import execute_db_operation
from education_system.post_18.university_system.infrastructure.email.template_utils import create_template
...
try:
    from education_system.post_18.university_system.infrastructure.email.template_utils import list_templates, load_template
except ImportError:
    pass

# send_email is referenced as `if send_email:` in send_now(); bind it to None
# when the service is unavailable so the truthiness check is safe.
try:
    from education_system.post_18.university_system.infrastructure.email.email_service import send_email
except ImportError:
    send_email = None

# send_batch_announcement is used behind an `if 'send_batch_announcement' in
# globals():` guard, so only bind the name when the import actually succeeds.
try:
...
class TemplateManagerDialog:
    ...
    def load_templates(self):
        ...
        try:
            # Import template functions
            from education_system.post_18.university_system.infrastructure.email.template_utils import list_templates
            templates = list_templates()
    ...
    def delete_selected_template(self):
        ...
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete the template '{template_name}'?"):
            try:
                from education_system.post_18.university_system.infrastructure.email.template_utils import delete_template
...
class TemplateEditDialog:
    ...
    def load_template_data(self):
        ...
        try:
            from education_system.post_18.university_system.infrastructure.email.template_utils import load_template
    ...
    def save_template(self):
        ...
        try:
            if self.template_name:
                ...
                from education_system.post_18.university_system.infrastructure.email.template_utils import update_template
