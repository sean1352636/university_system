...
class PhoneShopCLI:
    ...
    def display_menu(self):
        ...
        while True:
            ...
            print(" 21. Email Admin Report")
            print(" 22. Export Transactions to CSV")
    ...
    def _email_admin_report(self):
        """Generate the full admin report and email it to an admin."""
        ...
        if not (result and result[0]):
            print("\nNo admin email address found. Report not sent.")
            return
        ...
        send_email(
            ...
            subject=f"Phone Shop Admin Report - {date}",
            body=report,
        )
        print(f"\nAdmin report emailed successfully to {recipient_email}.")

    def _export_transactions_csv(self):
        """Export phone shop transactions to a CSV file."""
        import csv
        import os
        ...
        default_name = (
            f"phoneshop_transactions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        path = input(f"Output file path (default {default_name}): ").strip() or default_name
        ...
        with open(path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            writer.writerow(
                ["Transaction ID", "Order ID", "Date", "Customer",
        ...
        print(f"\nExported {count} transaction(s) to {os.path.abspath(path)}.")
