...
from education_system.post_18.university_system.infrastructure.shared_context import get_current_user
...
class MarketplaceCLI:
    ...
    def display_menu(self):
        ...
        print("11. Upload/Attach Photo to Listing")
    ...
    def upload_photo(self):
        """Attach a photo to one of your listings by file path.

        Mirrors the GUI's photo-upload feature. The GUI's "Upload Photos" button is
        a Tkinter placeholder with no service call, so this flow uses the real
        persistence method MarketplaceService.add_listing_photo(): it reads the
        image bytes from the given path, stores them under the marketplace upload
        directory, and records the row in the listing_photos table.
        """
        if not self.user_id:
        ...
        print("\n" + "=" * 70)
        print("UPLOAD/ATTACH PHOTO".center(70))
        print("=" * 70)
        ...
        listing = self.service.get_listing(listing_id, increment_views=False)
        if not listing:
            print("\nListing not found.")
            return

        if str(listing.get('seller_id')) != self.user_id:
            print("\nYou can only add photos to your own listings.")
            return

        file_path = input("Photo file path: ").strip()
        if not file_path:
        ...
        is_primary = input("Set as primary photo? (yes/no): ").strip().lower() == 'yes'
        ...
        photo_id = self.service.add_listing_photo(
        ...
        if photo_id:
            print(f"\n✓ Photo attached successfully! (Photo ID: {photo_id})")
        else:
