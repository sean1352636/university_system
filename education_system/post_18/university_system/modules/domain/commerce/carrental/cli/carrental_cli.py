...
class CarRentalCLI:
    ...
    def _email_admin_report(self):
        """Email the admin report to an administrator."""
        ...
        if not result or not result[0]:
            print("\nNo admin email found. Cannot send report.")
            return
        ...
        send_email(
            ...
            subject=f"Car Rental Admin Report - {date}",
            body=report,
        )
        print(f"\nAdmin report emailed to {admin_email}")

    def _export_transactions_csv(self):
        """Export car rental transactions to a CSV file."""
        import csv
        import os
        ...
        default_path = os.path.abspath(
            f"carrental_transactions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        path = input(f"Output file path [{default_path}]: ").strip() or default_path
        ...
        print(f"\nExported {len(transactions)} transaction(s) to {os.path.abspath(path)}")
