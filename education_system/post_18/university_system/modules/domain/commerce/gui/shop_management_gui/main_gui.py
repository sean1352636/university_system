...
class UniversityShopGUI:
    ...
    def launch_cli_mode(self):
        ...
        try:
            ...
            cli_text.insert(tk.END, "python -m education_system.post_18.university_system.modules.domain.commerce.services.shop_management\n\n")
            ...
            def launch_external_cli():
                try:
                    ...
                    subprocess.Popen(
                        [sys.executable, "-m", "education_system.post_18.university_system.modules.domain.commerce.services.shop_management"],
                        cwd=project_root,
                        env=env
                    )
                    messagebox.showinfo(
    ...
    def generate_sales_reports(self, days=30):
        """Generate and display a sales report in a new window.

        Wraps the shared sales-statistics logic in
        ``services.shop_management.reports`` and renders it using the standard
        report window (with export / email support).
        """
        # Permission gate mirrors the CLI sales-reports menu.
        if self.auth is not None and hasattr(self.auth, 'check_permission'):
            if not self.auth.check_permission('generate_sales_reports'):
                messagebox.showwarning(
                    "Permission Denied",
                    "You don't have permission to generate sales reports."
                )
                return

        try:
            # Function-local import avoids a circular import at module load time.
            from education_system.post_18.university_system.modules.domain.commerce.services.shop_management.reports import (
            ...
            if not stats:
                messagebox.showinfo(
                    "Sales Report",
                    "No sales data is available for the selected period."
                )
                return

            def _val(row, key):
                """Read a value from a sqlite3.Row or dict, defaulting to 0."""
                try:
                    value = row[key]
            ...
            period = stats.get('period_days', days)
            basic = stats.get('basic')

            report = "SALES REPORT\n"
            report += "=" * 80 + "\n\n"
            report += f"Period: Last {period} days\n\n"

            report += "OVERVIEW\n"
            report += "-" * 40 + "\n"
            report += f"{'Transactions':<25}{_val(basic, 'transaction_count')}\n"
            report += f"{'Total Revenue':<25}£{_val(basic, 'total_revenue'):.2f}\n"
            report += f"{'Average Transaction':<25}£{_val(basic, 'avg_transaction'):.2f}\n"
            report += f"{'Smallest Transaction':<25}£{_val(basic, 'min_transaction'):.2f}\n"
            report += f"{'Largest Transaction':<25}£{_val(basic, 'max_transaction'):.2f}\n\n"

            payment_methods = stats.get('payment_methods') or []
            if payment_methods:
                report += "PAYMENT METHODS\n"
                report += "-" * 40 + "\n"
                report += f"{'Method':<20}{'Count':<10}{'Amount'}\n"
                for method in payment_methods:
                    report += (
                        f"{str(_val(method, 'payment_method')):<20}"
                        f"{_val(method, 'count'):<10}"
                        f"£{_val(method, 'amount'):.2f}\n"
                    )
                report += "\n"
            ...
            if top_categories:
                ...
                report += "-" * 40 + "\n"
                report += f"{'Category':<20}{'Revenue':<15}{'Units'}\n"
                for category in top_categories:
                    report += (
                        f"{str(_val(category, 'category')):<20}"
                        f"£{_val(category, 'revenue'):<14.2f}"
                        f"{_val(category, 'quantity')}\n"
                    )
                report += "\n"

            report += "=" * 80 + "\n"

            title = f"Sales Report (Last {period} Days)"
            if hasattr(self, 'show_report_window'):
                self.show_report_window(title, report)
            else:
                messagebox.showinfo(title, report)

        except Exception as e:
            messagebox.showerror(
                "Report Error", f"Error generating sales report:\n{e}"
            )
    ...
    def get_cli_functions(self):
        ...
        try:
            ...
            cli_functions['generate_sales_reports'] = self.generate_sales_reports
            # Add more as needed
        except Exception as e:
...
try:
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import dashboard_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import product_browser
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import product_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import cart_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import checkout_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import order_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import inventory_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import discount_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import refund_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import report_manager
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import bulk_operations
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import utils
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import ui_components
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import email_notifications
    from education_system.post_18.university_system.modules.domain.commerce.gui.shop_management_gui import textbook_manager
    ...
    if hasattr(report_manager, 'show_report_window'):
        UniversityShopGUI.show_report_window = report_manager.show_report_window
