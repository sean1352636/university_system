...
class GymGUI:
    ...
    def create_membership_tab(self):
        ...
        ttk.Button(action_frame, text=_t("gym.btn.check_out", default="Check Out"), command=self.check_out).pack(side=tk.LEFT, padx=5)
        self._refresh_membership_info()
...
University Gym & Fitness Center"""
    ...
    def check_out(self):
        """Function 10b: Check out of gym (close the open attendance)."""
        if not self.membership or self.membership['status'] != 'active':
            messagebox.showwarning(_t("gym.window_title"), _t("gym.errors.no_active_membership"))
            return
        attendance = MembershipManager.get_active_attendance(self.membership['membership_id'])
        if not attendance:
            messagebox.showinfo(_t("gym.window_title"), _t("gym.messages.not_checked_in", default="You are not currently checked in."))
            return
        if MembershipManager.check_out(attendance['attendance_id']):
            messagebox.showinfo(_t("gym.window_title"), _t("gym.messages.checked_out", default="Checked out successfully."))
