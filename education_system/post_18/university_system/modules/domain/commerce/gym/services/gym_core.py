...
class MembershipManager:
    ...
    @staticmethod
    def get_active_attendance(membership_id: int) -> Optional[Dict]:
        """Return the member's open attendance (checked in, not yet out), if any."""
        try:
            with get_connection() as conn:
                cursor = conn.execute('''
            ...
            print(f"Error getting active attendance: {e}")
            return None
