...
class ButcherGUI:
    ...
    def _init_database(self):
        ...
        from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import init_butcher_db
        try:
            init_butcher_db()
        except Exception as e:
    ...
    def create_orders_tab(self):
        ...
        # Lookups / filters row
        lookup_frame = ttk.Frame(orders_frame)
        lookup_frame.pack(fill=tk.X, pady=(0, 5))

        ttk.Label(lookup_frame, text=_t("butcher.labels.status") + ":").pack(side=tk.LEFT)
        from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import ORDER_STATUSES
        self.orders_status_filter = ttk.Combobox(lookup_frame, state="readonly", width=15,
                                                 values=list(ORDER_STATUSES))
        self.orders_status_filter.pack(side=tk.LEFT, padx=5)
        ttk.Button(lookup_frame, text=_t("butcher.btn.filter_by_status"),
                  command=self.filter_orders_by_status).pack(side=tk.LEFT, padx=5)
        ttk.Button(lookup_frame, text=_t("butcher.btn.customer_orders"),
                  command=self.lookup_customer_orders).pack(side=tk.LEFT, padx=5)
        ttk.Button(lookup_frame, text=_t("butcher.btn.todays_orders"),
                  command=self.load_todays_orders).pack(side=tk.LEFT, padx=5)
        ttk.Button(lookup_frame, text=_t("butcher.btn.show_all_orders"),
                  command=self._load_orders).pack(side=tk.LEFT, padx=5)
        ttk.Button(lookup_frame, text=_t("butcher.btn.daily_sales"),
                  command=self.show_daily_sales).pack(side=tk.RIGHT, padx=5)
...
"""
    ...
    def _get_categories(self) -> dict:
        ...
        from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import MEAT_CATEGORIES
    ...
    def _populate_orders_tree(self, orders):
        """Render a list of order dicts into the orders treeview."""
        for item in self.orders_tree.get_children():
            self.orders_tree.delete(item)
        for row in orders:
    ...
    def filter_orders_by_status(self):
        """Filter the order history by the selected status."""
        status = self.orders_status_filter.get().strip()
        if not status:
            messagebox.showwarning(_t("common.warning"), "Please select a status.")
            return
        try:
            from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import OrderManager
            orders = OrderManager.get_orders_by_status(status)
            self._populate_orders_tree(orders)
            if not orders:
                messagebox.showinfo(_t("common.info"), f"No orders with status '{status}'.")
        except Exception as e:
            messagebox.showerror(_t("common.error"), str(e))

    def lookup_customer_orders(self):
        """Look up all orders for a given customer ID."""
        customer_id = simpledialog.askstring(_t("butcher.btn.customer_orders"),
                                             "Customer ID:", parent=self.parent)
        if not customer_id:
            return
        ...
        try:
            ...
            self._populate_orders_tree(orders)
            if not orders:
                messagebox.showinfo(_t("common.info"), f"No orders found for customer '{customer_id}'.")
        except Exception as e:
            messagebox.showerror(_t("common.error"), str(e))

    def load_todays_orders(self):
        """Load today's orders into the order history."""
        try:
            from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import OrderManager
            orders = OrderManager.get_todays_orders()
            self._populate_orders_tree(orders)
            if not orders:
                messagebox.showinfo(_t("common.info"), "No orders for today.")
        except Exception as e:
            messagebox.showerror(_t("common.error"), str(e))

    def show_daily_sales(self):
        """Show a one-tap daily sales summary for today."""
        try:
            from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import TransactionManager
            sales = TransactionManager.get_daily_sales()
            details = (
                f"Daily Sales for {sales['date']}\n"
                f"{'=' * 30}\n"
                f"Transactions: {sales['transaction_count']}\n"
                f"Total Sales:  £{sales['total_sales']:.2f}\n"
                f"Refunds:      £{sales['total_refunds']:.2f}\n"
                f"Net Sales:    £{sales['net_sales']:.2f}"
            )
            messagebox.showinfo(_t("butcher.btn.daily_sales"), details)
        except Exception as e:
            messagebox.showerror(_t("common.error"), str(e))
    ...
    def add_product(self):
        ...
        try:
            ...
            from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import ProductManager
...
"""
    ...
    def process_order_payment(self):
        ...
        from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import OrderManager, TransactionManager
        order = OrderManager.get_order_by_number(order_number)
...
University Butcher Shop"""
    ...
    def generate_admin_report(self):
        ...
        try:
            from education_system.post_18.university_system.modules.domain.commerce.butcher.services.butcher_core import ReportManager
