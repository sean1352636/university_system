...
class LostFoundService:
    ...
    def report_lost_item(self, reporter_id: str, item_name: str, category: str,
        ...
        with transaction() as conn:
            ...
            # Automatically search for potential matches (reuse the open
            # connection — opening a nested one would deadlock on the write
            # lock this transaction already holds).
            self._find_potential_matches_for_lost(item_id, conn)
    ...
    def report_found_item(self, finder_id: str, item_name: str, category: str,
        ...
        with transaction() as conn:
            ...
            # connection — opening a nested one would deadlock on the write
            # lock this transaction already holds).
            self._find_potential_matches_for_found(item_id, conn)
    ...
    def _find_potential_matches_for_lost(self, lost_item_id: int, conn):
        """Find potential matches in found items for a lost item.

        Always invoked from within ``report_lost_item``'s open transaction, so
        it reuses that connection. Opening a separate connection here would
        block on the write lock the caller already holds.
        """
        lost = conn.execute(
            "SELECT * FROM lost_items WHERE item_id = ?",
            (lost_item_id,)
        ...
        # Save matches on the same connection/transaction.
    ...
    def _find_potential_matches_for_found(self, found_item_id: int, conn):
        """Find potential matches in lost items for a found item.

        Always invoked from within ``report_found_item``'s open transaction, so
        it reuses that connection. Opening a separate connection here would
        block on the write lock the caller already holds.
        """
        found = conn.execute(
