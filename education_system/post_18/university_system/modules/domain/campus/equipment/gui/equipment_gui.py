...
class EquipmentRentalGUI:
    ...
    def create_returns_tab(self):
        ...
        # Status filter - 'active' keeps the default (checked_out + reserved) view,
        # any other value filters the list to a single RENTAL_STATUSES value
        filter_frame = ttk.Frame(left_frame)
        filter_frame.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(filter_frame, text=_t("equipment.labels.status") + ":").pack(side=tk.LEFT)
        self.returns_status_filter = ttk.Combobox(
            filter_frame, values=['active'] + RENTAL_STATUSES, width=15, state='readonly')
        self.returns_status_filter.set('active')
        self.returns_status_filter.pack(side=tk.LEFT, padx=5)
        self.returns_status_filter.bind('<<ComboboxSelected>>', lambda e: self._load_active_rentals())
        ...
        btn_frame3 = ttk.Frame(right_frame)
        btn_frame3.grid(row=5, column=0, columnspan=2, pady=5)

        ttk.Button(btn_frame3, text="Refund Deposit",
                   command=self.refund_deposit).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame3, text="View Transactions",
                   command=self.view_transactions).pack(side=tk.LEFT, padx=5)
    ...
    def _load_active_rentals(self):
        """Load rentals for the returns tab, honouring the status filter"""
        for item in self.active_rentals_tree.get_children():
            self.active_rentals_tree.delete(item)

        status = getattr(self, 'returns_status_filter', None)
        selected = status.get() if status else 'active'
        if selected == 'active':
            rentals = RentalManager.get_rentals_by_status('checked_out')
            rentals += RentalManager.get_rentals_by_status('reserved')
        else:
            rentals = RentalManager.get_rentals_by_status(selected)
    ...
    def refund_deposit(self):
        """Refund a deposit for the selected rental"""
        selected = self.active_rentals_tree.selection()
        if not selected:
            messagebox.showwarning(_t("common.warning"), _t("equipment.errors.no_rental_selected"))
            return
        ...
        # Refund dialog
        dialog = tk.Toplevel(self.root)
        dialog.title("Refund Deposit")
        dialog.geometry("320x200")
        dialog.transient(self.root)
        dialog.grab_set()

        ttk.Label(dialog, text=f"Rental: {rental.get('rental_number', rental_id)}").pack(pady=(15, 5))
        default_deposit = rental.get('deposit_amount') or rental.get('deposit_required') or 0
        ttk.Label(dialog, text="Refund amount (£):").pack(pady=5)
        amount_entry = ttk.Entry(dialog, width=20)
        amount_entry.insert(0, f"{float(default_deposit):.2f}")
        ...
        def confirm_refund():
            try:
                ...
                messagebox.showerror(_t("common.error"), "Invalid amount", parent=dialog)
                return

            borrower_id = rental.get('borrower_id')
            txn_id = TransactionManager.refund_deposit(
                rental_id=rental_id,
                borrower_id=borrower_id,
                amount=amount,
                processed_by=self.current_user.get('username')
            ...
            if txn_id:
                messagebox.showinfo(_t("common.success"),
                    f"Deposit of £{amount:.2f} refunded (Transaction ID: {txn_id})")
                dialog.destroy()
            else:
                messagebox.showerror(_t("common.error"), "Failed to refund deposit", parent=dialog)

        ttk.Button(dialog, text=_t("common.confirm"), command=confirm_refund).pack(pady=15)

    def view_transactions(self):
        """View transactions for the selected rental (or all recent)"""
        selected = self.active_rentals_tree.selection()
        rental_id = self.active_rentals_tree.item(selected[0])['values'][0] if selected else None

        transactions = TransactionManager.get_transactions(rental_id)

        window = tk.Toplevel(self.root)
        window.title("Transactions" if rental_id else "Recent Transactions")
        window.geometry("760x400")
        window.transient(self.root)

        columns = ('id', 'reference', 'type', 'amount', 'method', 'status', 'date')
        tree = ttk.Treeview(window, columns=columns, show='headings')
        headings = {
            'id': 'ID', 'reference': 'Reference', 'type': 'Type', 'amount': 'Amount',
            'method': 'Method', 'status': 'Status', 'date': 'Date'
        }
        ...
        if not transactions:
            ttk.Label(window, text="No transactions found.").pack(pady=5)
        ttk.Button(window, text=_t("equipment.btn.close"), command=window.destroy).pack(pady=(0, 10))
