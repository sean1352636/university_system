...
def generate_financial_report():
    ...
    try:
        ...
        total_collected = sum((r[2] or 0) for r in rows)
        payment_count = sum((r[1] or 0) for r in rows)
        ...
        if rows:
            ...
            for pt, cnt, amt in rows:
                # Deposits are liabilities (held, refundable), not collected revenue —
                # flag them in the listing so the operator doesn't conflate.
                tag = " (held — liability)" if pt == 'Deposit' else ""
                print(f"  {pt}: {cnt} payment(s), £{(amt or 0):,.2f}{tag}")
        print()
