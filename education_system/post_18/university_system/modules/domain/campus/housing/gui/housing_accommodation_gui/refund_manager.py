...
try:
    from education_system.post_18.university_system.core.activity_logger import log_dynamic_activity
except ImportError:
...
def create_payments_refunds_tab(self, parent):
        ...
        columns = ('payment_id', 'student_id', 'type', 'amount', 'method', 'date', 'period_start', 'period_end', 'status')
        ...
        self.refund_payments_tree.heading('type', text='Type')
        ...
        self.refund_payments_tree.column('type', width=100)
...
def _refresh_refund_payments_list(self):
        ...
        try:
            ...
            if search_term:
                cursor.execute('''
                    SELECT source_payment_id, student_id, COALESCE(payment_type, 'Rent'),
            ...
            for row in cursor.fetchall():
                values = list(row)
                # Format amount (now index 3 after inserting type at index 2)
                if values[3]:
                    try:
        ...
        try:
            ...
            if filepath:
                with open(filepath, 'w', newline='') as csvfile:
                    ...
                    writer.writerow(['Payment ID', 'Student ID', 'Type', 'Amount', 'Method',
...
def process_housing_refund(self):
        ...
        try:
            ...
            # Deposits must go through the structured move-out flow so deductions
            # are computed against held — not just marked Refunded.
            if (payment_type or '').lower() == 'deposit':
                messagebox.showwarning(
                    "Use Deposit Refund Flow",
                    "This is a held deposit. Refund it via:\n\n"
                    "  Payment Menu → Process Deposit Refund (Move-out)\n\n"
                    "That flow links the move-out inspection, captures itemised "
                    "deductions, computes held − damages = refund, and posts the "
                    "correct GL journal."
                )
                return
...
def _notify_finance_for_housing_refund(self, student_id: str, amount: float, method: str, refund_ref: str):
        ...
        try:
            from education_system.post_18.university_system.infrastructure.database.db import transaction as db_transaction
            from datetime import datetime
