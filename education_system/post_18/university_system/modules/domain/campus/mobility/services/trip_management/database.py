...
from education_system.post_18.university_system.modules.domain.campus.mobility.services.trip_management._common import sqlite3, get_text, logging, time, traceback, DEFAULT_DB_PATH, datetime
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if conn:
                try:
                    conn.rollback()
                    logging.debug("Successfully rolled back transaction after operational error")
            ...
            if conn:
                try:
                    conn.rollback()
                    logging.debug("Successfully rolled back transaction after database error")
                except sqlite3.Error as rollback_error:
