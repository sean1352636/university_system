...
# Data / persistence layer + shared constants now live in the GUI-free
# ``risk_service`` module so non-GUI callers (CLI, API, tests) can reuse
# the exact same SQL. Re-exported here for backward compatibility. Imported
# after the sys.path bootstrap above so it resolves when the GUI is launched
# as a subprocess by file path.
from education_system.post_18.university_system.modules.domain.operations.legal.risk_management.risk_service import (  # noqa: E402,F401
    CATEGORIES,
    DEPARTMENTS,
    STATUSES,
    LIKELIHOOD_LEVELS,
