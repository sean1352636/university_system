...
class ProspectManager:
    ...
    @staticmethod
    def list_prospects(status: Optional[str] = None,
                       search: str = "") -> List[Dict[str, Any]]:
        """List prospects, optionally filtered by status and/or name/email."""
        try:
            with get_connection() as conn:
                query = "SELECT * FROM admission_prospects WHERE 1=1"
                params: List[Any] = []
                if status:
                    query += " AND status = ?"
            ...
            raise Exception(f"Error listing prospects: {e}")

    @staticmethod
    def get_prospect(prospect_id: int) -> Optional[Dict[str, Any]]:
        """Fetch a single prospect by id, or None."""
        try:
            with get_connection() as conn:
                cursor = conn.execute(
                    "SELECT * FROM admission_prospects WHERE prospect_id = ?",
                    (prospect_id,))
                row = cursor.fetchone()
                return dict(row) if row else None
        except Exception as e:
            raise Exception(f"Error getting prospect: {e}")

    @staticmethod
    def get_interactions(prospect_id: int) -> List[Dict[str, Any]]:
        """List logged interactions for a prospect (newest first)."""
        try:
            with get_connection() as conn:
                cursor = conn.execute(
                    "SELECT * FROM prospect_interactions "
                    "WHERE prospect_id = ? ORDER BY interaction_id DESC",
                    (prospect_id,))
                return [dict(row) for row in cursor.fetchall()]
        except Exception as e:
            raise Exception(f"Error getting interactions: {e}")
...
class ApplicationManager:
    ...
    def list_applications(status: Optional[str] = None,
                          prospect_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """List applications with applicant name/email, newest first."""
        try:
            with get_connection() as conn:
                query = (
                    "SELECT a.*, p.first_name, p.last_name, p.email "
                    "FROM admission_applications a "
                    "LEFT JOIN admission_prospects p "
                    "ON p.prospect_id = a.prospect_id WHERE 1=1")
                params: List[Any] = []
                if status:
                    query += " AND a.status = ?"
                    params.append(status)
    ...
    def get_application(application_id: int) -> Optional[Dict[str, Any]]:
        """Fetch a single application (with applicant name/email), or None."""
        try:
            with get_connection() as conn:
                cursor = conn.execute(
                    "SELECT a.*, p.first_name, p.last_name, p.email "
...
class CampaignManager:
    ...
    def list_campaigns() -> List[Dict[str, Any]]:
        """List recruitment campaigns, newest first."""
        try:
            with get_connection() as conn:
                cursor = conn.execute(
            ...
            raise Exception(f"Error listing campaigns: {e}")
...
class TourManager:
    ...
    @staticmethod
    def list_tours(upcoming_only: bool = False) -> List[Dict[str, Any]]:
        """List campus tours. When *upcoming_only*, only today-or-later."""
        try:
            with get_connection() as conn:
                query = "SELECT * FROM campus_tours"
                params: List[Any] = []
                if upcoming_only:
                    query += " WHERE tour_date >= ?"
                    params.append(datetime.now().date().isoformat())
                query += " ORDER BY tour_date, tour_time"
                cursor = conn.execute(query, params)
                return [dict(row) for row in cursor.fetchall()]
        except Exception as e:
            raise Exception(f"Error listing tours: {e}")

    @staticmethod
    def get_tour_registrations(tour_id: int) -> List[Dict[str, Any]]:
        """List registrations for a tour, with prospect name/email."""
        try:
            with get_connection() as conn:
                cursor = conn.execute(
                    ...
                    "LEFT JOIN admission_prospects p "
                    "ON p.prospect_id = tr.prospect_id "
                    "WHERE tr.tour_id = ? ORDER BY tr.registered_at",
                    (tour_id,))
                return [dict(row) for row in cursor.fetchall()]
        except Exception as e:
            raise Exception(f"Error getting tour registrations: {e}")
...
class YieldPredictionManager:
    ...
    @staticmethod
    def list_predictions(application_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """List yield predictions, optionally for one application."""
        try:
            with get_connection() as conn:
                query = "SELECT * FROM yield_predictions"
                params: List[Any] = []
                if application_id:
            ...
            raise Exception(f"Error listing predictions: {e}")


def display_admissions_crm_menu(auth):
    """Display the Admissions & Recruitment CRM CLI menu.

    Delegates to the interactive CLI in ``admissions.cli.admissions_crm_cli``,
    which is wired to the managers above (same ``student_records.db`` the
    Admissions CRM GUI uses). Imported lazily to avoid an import cycle.
    """
    from education_system.post_18.university_system.modules.domain.admissions.cli.admissions_crm_cli import (
        run_admissions_crm_menu,
    )
    run_admissions_crm_menu(auth)
...
def launch_admissions_crm_gui(root, auth):
    ...
    try:
        from education_system.post_18.university_system.modules.domain.admissions.gui.admissions_crm_gui import launch_admissions_crm_gui as _launch
        _launch(root, auth)
