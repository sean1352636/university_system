...
# Allows digits, spaces, and the usual phone punctuation; needs >= 7 digits.
...
def _is_valid_phone(value: str) -> bool:
    v = (value or "").strip()
    return bool(_PHONE_RE.match(v)) and sum(c.isdigit() for c in v) >= 7
...
# DATA LAYER
# ---------------------------------------------------------------------------
from education_system.post_18.university_system.modules.domain.health.first_aid.first_aid_service import (  # noqa: E402
    EMERGENCY_CONTACTS,
    IncidentDB,
)
...
class FirstAidPortal:
    ...
    def _emergency_banner_text(self):
        """Build the Home-tab emergency banner from the contact data so it
        never drifts from the Emergency Contacts tab. Prefers the two most
        critical contacts; falls back to whatever is available."""
        wanted = ("Emergency Services", "Campus Security",
        ...
        for name in wanted:
            ...
            if len(picked) >= 2:
                break
        if not picked:
            picked = self.emergency_contacts[:2]
        return "  |  ".join(f"{c['name']}: {c['number']}" for c in picked)
    ...
    def _create_home_tab(self):
        ...
        tk.Label(
            emergency_frame,
            text=self._emergency_banner_text(),
            font=("Segoe UI", 12),
            bg=self.colors["primary"],
            fg="white",
        ).pack(pady=5)
    ...
    def _create_emergency_tab(self):
        ...
        # Scrollable area so every contact card is reachable regardless
        # of window height (previously the bottom row could be clipped).
        outer = tk.Frame(em_tab, bg=self.colors["bg"])
        ...
        # Keep the inner frame as wide as the canvas so the two columns
        # share the full width and expand with the window.
        canvas.bind("<Configure>",
                    lambda e: canvas.itemconfigure(window_id, width=e.width))

        # Mousewheel scrolling, scoped to while the cursor is over this
        # tab so it doesn't hijack scrolling elsewhere in the portal.
        ...
        def _bind_wheel(_e=None):
            ...
            canvas.bind_all("<Button-4>",
                            lambda e: canvas.yview_scroll(-1, "units"))  # Linux
        ...
        container.grid_columnconfigure(0, weight=1, uniform="contact")
        container.grid_columnconfigure(1, weight=1, uniform="contact")
    ...
    def _create_report_tab(self):
        ...
        # Pre-fill reporter identity from the signed-in user. The email is
        # used to send the reporter a confirmation receipt on submit, so
        # pre-fill it from the resolved address where possible.
        if self.user:
            self.form_vars["name"].set(self.user_display)
            uid = (self.user.get('user_id') or self.user.get('id') or '')
            if uid:
                self.form_vars["id"].set(str(uid))
        prefill_email = self._resolve_user_email(
            self.form_vars["id"].get().strip())
        if prefill_email:
            self.form_vars["email"].set(prefill_email)
    ...
    def _create_resources_tab(self):
        ...
        resources = [
            ("🎓", "CPR Certification", "Sign up for CPR/AED certification courses held every semester at the University Health Center.", "Register", lambda: self._resource_register_course("CPR / AED Certification")),
            ("📖", "First Aid Handbook", "Download the official university first aid handbook with detailed procedures and emergency contacts.", "Download", self._resource_download_handbook),
            ("🎥", "Training Videos", "Watch training videos on common emergency procedures and response techniques.", "Watch", self._resource_watch_videos),
            ("🗓", "Workshop Schedule", "View upcoming first aid workshops, Red Cross events, and safety seminars on campus.", "View Schedule", self._resource_view_schedule),
            ("🧰", "First Aid Kit Locations", "Campus directory of all first aid kit and AED locations.", "Open Map", self._resource_kit_locations),
            ("📱", "Mobile App", "Get the University Safety App for on-the-go emergency information.", "Get App", self._resource_get_app),
        ]

        for i, (icon, title, desc, btn_text, command) in enumerate(resources):
            row = i // 2
            col = i % 2
        ...
        # Admin/staff: review who has signed up for training courses.
        if self._is_staff():
            tk.Button(
                res_tab,
                text="👥 View Course Registrations (Admin)",
                font=("Segoe UI", 10, "bold"),
                bg=self.colors["text"],
                fg="white",
                relief="flat",
                ...
                pady=6,
                cursor="hand2",
                command=self._view_registrations,
            ).pack(pady=(0, 15))

    def _view_registrations(self):
        """Admin/staff view of training-course sign-ups, with CSV export."""
        win = self._resource_window("Course Registrations",
                                    "👥  Training Course Registrations",
                                    width=760, height=540)

        log_text = scrolledtext.ScrolledText(win, font=("Consolas", 10),
                                             wrap="word")
        ...
        if self.db:
            try:
                regs = self.db.fetch_registrations()
            except sqlite3.Error:
                logger.exception("Could not load course registrations")

        if not regs:
            log_text.insert("1.0", "No course registrations yet.\n")
        else:
            for r in regs:
                log_text.insert(
                    "end",
                    f"── Registration REG-{r['id']:04d} ──\n"
                    f"Course:      {r['course']}\n"
                    f"Registered:  {r['submitted_at']}\n"
                    f"Name:        {r['name']} (ID: {r.get('user_id') or '—'})\n"
                    f"Email:       {r.get('email') or '—'}\n"
                    f"Phone:       {r.get('phone') or '—'}\n"
                    f"Preferred:   {r.get('preferred_date') or '—'}\n\n",
                )
        log_text.config(state="disabled")

        def _export():
            if not regs:
                messagebox.showinfo("Export", "Nothing to export.", parent=win)
                return
            import csv
            fname = (f"first_aid_course_registrations_"
                     f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
            downloads = os.path.join(os.path.expanduser("~"), "Downloads")
            target_dir = downloads if os.path.isdir(downloads) else tempfile.gettempdir()
            path = os.path.join(target_dir, fname)
            cols = ["id", "submitted_at", "course", "name", "user_id",
            ...
            try:
                ...
                logger.exception("Could not export registrations")
                messagebox.showerror("Export Failed",
                                     "Could not write the export file.",
                                     parent=win)
                return
            logger.info("Registrations exported (%s rows) to %s by user=%s",
                        len(regs), path, self.user_display)
            self._open_path(path)
            messagebox.showinfo("Export Complete",
                                f"✓ Exported {len(regs)} registration(s) to:"
                                f"\n\n{path}", parent=win)

        tk.Button(win, text="⤓ Export CSV", font=("Segoe UI", 9, "bold"),
                  bg=self.colors["secondary"], fg="white", relief="flat",
                  padx=12, pady=5, cursor="hand2",
                  command=_export).pack(pady=(0, 12))

    # ---------- Resources tab actions ----------

    def _resource_window(self, title, heading, width=640, height=520):
        """Create a styled Toplevel for a resource, returning the
        scrollable body frame. Matches the portal's colour scheme."""
    ...
    def _resource_register_course(self, course):
        """Open a registration form for a training course and persist the
        sign-up to the central DB (``first_aid_training_registrations``)."""
        win = self._resource_window(f"Register — {course}", f"🎓  {course}",
                                    width=520, height=440)

        form = tk.Frame(win, bg=self.colors["card"], relief="raised", bd=1)
        form.pack(fill="both", expand=True, padx=25, pady=10)

        tk.Label(
            form,
            text="Reserve a place on the next available course. The "
                 "Health Center will confirm your slot by email.",
            font=("Segoe UI", 9), bg=self.colors["card"], fg="#7f8c8d",
        ...
        if self.user:
            vars_["name"].set(self.user_display)
            uid = (self.user.get('user_id') or self.user.get('id') or '')
            if uid:
                vars_["user_id"].set(str(uid))
            if self.user.get('email'):
                vars_["email"].set(str(self.user['email']))
        ...
        def _submit():
            ...
            if not name:
                messagebox.showwarning("Name Required",
                                       ...
                                       parent=win)
                return
            reg_email = vars_["email"].get().strip()
            reg_phone = vars_["phone"].get().strip()
            if reg_email and not _is_valid_email(reg_email):
                messagebox.showwarning(
                    "Invalid Email",
                    "Please enter a valid email address so we can confirm "
                    "your place (or leave it blank).", parent=win)
                return
            if reg_phone and not _is_valid_phone(reg_phone):
                messagebox.showwarning(
                    "Invalid Phone Number",
                    "Please enter a valid contact number with at least 7 "
                    "digits (or leave it blank).", parent=win)
                return
            if not self.db:
                messagebox.showerror(
                    "Database Unavailable",
                    "Cannot save your registration — the database is not "
                    "reachable. Please contact the Health Center.",
                    parent=win)
                return
            reg = {
                "submitted_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "course": course,
                "user_id": vars_["user_id"].get().strip(),
                "name": name,
                "email": reg_email,
                "phone": reg_phone,
                "preferred_date": vars_["preferred_date"].get().strip(),
                "notes": "",
            }
            try:
                reg_id = self.db.add_registration(reg)
            except sqlite3.Error:
                logger.exception("Course registration failed (user=%s)",
                                 self.user_display)
                messagebox.showerror("Save Error",
                                     "Could not save your registration.",
                                     parent=win)
                return
            logger.info("Training registration id=%s course=%s user=%s",
                        reg_id, course, self.user_display)

            # Email integration: send the user a sign-up confirmation.
            reference = f"REG-{reg_id:04d}"
            recipient = reg["email"] or self._resolve_user_email(reg["user_id"])
            confirmation_sent = False
            if recipient:
                confirmation_sent = self._send_template_email(
                    recipient, "health/first_aid_course_confirmation", {
                        "name": reg["name"],
                        "reference": reference,
                        "course": course,
                        "preferred_date": reg["preferred_date"] or "To be confirmed",
                        "submitted_at": reg["submitted_at"],
                    })

            if confirmation_sent:
                tail = f"A confirmation email has been sent to {recipient}."
            else:
                tail = ("The University Health Center will email you to "
                        "confirm your place.")
            messagebox.showinfo(
                "Registration Received",
                f"✓ You're registered for {course}.\n\n"
        ...
        btns.pack(pady=18)
        tk.Button(btns, text="Submit Registration",
                  font=("Segoe UI", 10, "bold"), bg=self.colors["accent"],
                  fg="white", relief="flat", padx=20, pady=6, cursor="hand2",
                  command=_submit).pack(side="left", padx=5)
        tk.Button(btns, text="Cancel", font=("Segoe UI", 10),
                  bg="#bdc3c7", fg="white", relief="flat", padx=20, pady=6,
                  cursor="hand2", command=win.destroy).pack(side="left", padx=5)

    def _resource_download_handbook(self):
        """Generate the first aid handbook from the portal's own content
        and open it with the OS default application."""
        lines = [
            "UNIVERSITY FIRST AID HANDBOOK",
            "=" * 60,
        ...
        lines.append("")
        lines.append("FIRST AID PROCEDURES")
        lines.append("-" * 60)
        for condition, instructions in self.first_aid_data.items():
            lines.append("")
            lines.append(condition.upper())
            lines.append(instructions.strip())
            lines.append("")

        lines.append("")
        lines.append("This handbook is a quick-reference guide and is not a "
                     "substitute for professional medical training.")

        content = "\n".join(lines)
        fname = (f"University_First_Aid_Handbook_"
                 f"{datetime.now().strftime('%Y%m%d')}.txt")
        # Prefer the user's Downloads folder; fall back to the temp dir.
        downloads = os.path.join(os.path.expanduser("~"), "Downloads")
        target_dir = downloads if os.path.isdir(downloads) else tempfile.gettempdir()
        path = os.path.join(target_dir, fname)
        try:
            ...
            logger.exception("Could not write handbook file")
            messagebox.showerror("Download Failed",
                                 "Could not save the handbook file.")
            return
        logger.info("First aid handbook generated at %s (user=%s)",
                    path, self.user_display)
        self._open_path(path)
        messagebox.showinfo(
            "Handbook Downloaded",
            f"✓ The first aid handbook has been saved to:\n\n{path}\n\n"
            f"It should now open automatically.")

    def _resource_watch_videos(self):
        """List reputable first aid training videos; each opens in the
        default web browser."""
        win = self._resource_window("Training Videos",
                                    "🎥  First Aid Training Videos",
                                    width=620, height=440)
        videos = [
            ("CPR & AED — British Heart Foundation",
             "https://www.youtube.com/watch?v=ILxjxfB4zNk"),
            ("How to do CPR — British Red Cross",
             "https://www.youtube.com/watch?v=2PngCv7NjaI"),
            ("Recovery position — British Red Cross",
             "https://www.youtube.com/watch?v=dPYUViGTVMM"),
            ("Choking (adult) — British Red Cross",
             "https://www.youtube.com/watch?v=7CgtIgSyAiU"),
            ("Treating severe bleeding — British Red Cross",
        ...
        for title, url in videos:
            card = tk.Frame(body, bg=self.colors["card"], relief="raised", bd=1)
            card.pack(fill="x", pady=6)
            tk.Label(card, text=title, font=("Segoe UI", 11, "bold"),
                     bg=self.colors["card"], fg=self.colors["text"]).pack(
                side="left", padx=15, pady=12)
            tk.Button(card, text="▶ Watch", font=("Segoe UI", 9, "bold"),
                      bg=self.colors["secondary"], fg="white", relief="flat",
                      padx=15, pady=4, cursor="hand2",
                      command=lambda u=url: self._open_url(u)).pack(
                side="right", padx=15)

    def _resource_view_schedule(self):
        """Show upcoming workshops with dates relative to today."""
        win = self._resource_window("Workshop Schedule",
                                    "🗓  Upcoming Workshops & Events",
                                    width=640, height=480)
        today = datetime.now()
        workshops = [
            (7, "CPR & AED Certification", "University Health Center, Bldg H",
             "Hands-on certification with the campus first aid team."),
            (14, "Basic First Aid Workshop", "Student Services, Bldg C",
             "Bleeding, burns, sprains and the recovery position."),
            (21, "Mental Health First Aid", "Counselling Centre, Bldg C",
             "Recognising and supporting peers in crisis."),
            (35, "Red Cross Community Day", "Main Quad",
             "Open demonstrations, free kits and Q&A sessions."),
            (49, "AED & Choking Response", "Sports Centre",
             "Defibrillator use and adult/child choking procedures."),
        ]
        body = tk.Frame(win, bg=self.colors["bg"])
        body.pack(fill="both", expand=True, padx=25, pady=10)
        for offset, name, location, desc in workshops:
            when = today + timedelta(days=offset)
            card = tk.Frame(body, bg=self.colors["card"], relief="raised", bd=1)
            card.pack(fill="x", pady=6)
            tk.Label(card, text=when.strftime("%a %d %b %Y"),
                     font=("Segoe UI", 10, "bold"), bg=self.colors["card"],
                     fg=self.colors["primary"], width=16, anchor="w").pack(
            ...
            tk.Label(info, text=name, font=("Segoe UI", 11, "bold"),
                     bg=self.colors["card"], fg=self.colors["text"]).pack(
                anchor="w")
            tk.Label(info, text=f"📍 {location}", font=("Segoe UI", 9),
                     bg=self.colors["card"], fg="#7f8c8d").pack(anchor="w")
            tk.Label(info, text=desc, font=("Segoe UI", 9),
                     bg=self.colors["card"], fg="#7f8c8d", wraplength=380,
                     justify="left").pack(anchor="w")
        tk.Button(win, text="Register for a Course",
                  font=("Segoe UI", 9, "bold"), bg=self.colors["accent"],
                  fg="white", relief="flat", padx=15, pady=5, cursor="hand2",
                  command=lambda: self._resource_register_course(
                      "First Aid Workshop")).pack(pady=10)

    def _resource_kit_locations(self):
        """Show the campus directory of first aid kits and AEDs."""
        win = self._resource_window("First Aid Kit & AED Locations",
                                    "🧰  First Aid Kit & AED Locations",
                                    width=640, height=500)
        locations = [
            ("Main Reception, Building A", "Kit + AED", "Ground floor, by the desk"),
            ("Library, Building B", "Kit + AED", "Entrance lobby, left wall"),
            ("Student Services, Building C", "Kit", "Help desk, 1st floor"),
            ("Science Labs, Building D", "Kit + AED", "Corridor outside Lab 3"),
            ("Sports Centre", "Kit + AED", "Reception & poolside"),
            ("University Health Center, Building H", "Kit + AED", "Treatment room"),
            ("Cafeteria / Dining Hall", "Kit", "Behind the serving counter"),
            ("Halls of Residence", "Kit + AED", "Each block's ground-floor lobby"),
        ]
        body = tk.Frame(win, bg=self.colors["bg"])
        body.pack(fill="both", expand=True, padx=25, pady=10)
        for place, kind, detail in locations:
            card = tk.Frame(body, bg=self.colors["card"], relief="raised", bd=1)
            card.pack(fill="x", pady=4)
            badge = self.colors["primary"] if "AED" in kind else self.colors["accent"]
            tk.Label(card, text=kind, font=("Segoe UI", 8, "bold"),
                     bg=badge, fg="white", width=10).pack(side="left",
                                                          padx=10, pady=10)
            info = tk.Frame(card, bg=self.colors["card"])
            info.pack(side="left", fill="x", expand=True, pady=6)
            tk.Label(info, text=place, font=("Segoe UI", 11, "bold"),
                     bg=self.colors["card"], fg=self.colors["text"]).pack(
                anchor="w")
            tk.Label(info, text=detail, font=("Segoe UI", 9),
                     bg=self.colors["card"], fg="#7f8c8d").pack(anchor="w")
        tk.Label(win, text="AED = Automated External Defibrillator",
                 font=("Segoe UI", 8, "italic"), bg=self.colors["bg"],
                 fg="#7f8c8d").pack(pady=(0, 4))
        tk.Button(win, text="Open Campus Map in Browser",
                  font=("Segoe UI", 9, "bold"), bg=self.colors["secondary"],
                  fg="white", relief="flat", padx=15, pady=5, cursor="hand2",
                  command=lambda: self._open_url(
                      "https://www.openstreetmap.org/")).pack(pady=8)

    def _resource_get_app(self):
        """Show the University Safety App details and store links."""
        win = self._resource_window("University Safety App",
                                    "📱  University Safety App",
                                    width=520, height=420)
        card = tk.Frame(win, bg=self.colors["card"], relief="raised", bd=1)
        card.pack(fill="both", expand=True, padx=25, pady=10)
        tk.Label(card, text="Carry emergency information, first aid guides "
                            "and one-tap campus security calling wherever "
                            "you go.",
                 font=("Segoe UI", 10), bg=self.colors["card"],
                 fg=self.colors["text"], wraplength=440,
                 justify="left").pack(padx=20, pady=(20, 10))
        features = [
            "• Offline first aid procedures",
            "• One-tap call to Campus Security & 911",
            "• Nearest AED & first aid kit finder",
            "• Push alerts for campus emergencies",
        ]
        for f in features:
            tk.Label(card, text=f, font=("Segoe UI", 10),
                     bg=self.colors["card"], fg="#34495e",
                     anchor="w").pack(fill="x", padx=40, pady=2)
        store = tk.Frame(card, bg=self.colors["card"])
        store.pack(pady=20)
        tk.Button(store, text=" App Store", font=("Segoe UI", 10, "bold"),
                  bg="#2c3e50", fg="white", relief="flat", padx=20, pady=8,
                  cursor="hand2",
                  command=lambda: self._open_url(
                      "https://www.apple.com/app-store/")).pack(side="left",
                                                                padx=8)
        tk.Button(store, text="▶ Google Play", font=("Segoe UI", 10, "bold"),
                  bg="#27ae60", fg="white", relief="flat", padx=20, pady=8,
                  cursor="hand2",
                  command=lambda: self._open_url(
                      "https://play.google.com/store")).pack(side="left",
                                                             padx=8)

    def _open_url(self, url):
        """Open a URL in the default browser, reporting failure to the user."""
        try:
            webbrowser.open(url)
            logger.info("Opened resource URL: %s", url)
        except Exception:
            logger.exception("Could not open URL %s", url)
            messagebox.showerror("Cannot Open Link",
                                 f"Could not open your browser.\n\n{url}")

    def _open_path(self, path):
        """Open a local file with the OS default application (best-effort)."""
        try:
            ...
            logger.exception("Could not auto-open file %s", path)

    # ---------- Email integration ----------

    def _resolve_user_email(self, user_id=""):
        """Best-effort email lookup for the signed-in user.

        Prefers the email carried on the auth session, then falls back to
        the ``users`` / ``students`` tables keyed by the supplied id.
        Returns '' if nothing can be resolved."""
        if self.user and self.user.get("email"):
            return str(self.user["email"]).strip()
        ...
        try:
            ...
            logger.debug("Could not resolve email for %s", uid, exc_info=True)
        return ""

    def _admin_emails(self):
        """Return the list of admin email addresses from the ``users``
        table (empty list if none / DB unavailable)."""
        try:
            from education_system.post_18.university_system.infrastructure.database.db import (
                get_connection,
            )
            ...
            logger.debug("Could not load admin emails", exc_info=True)
            return []

    def _send_template_email(self, recipient, template, variables):
        """Render a JSON template and queue it to ``recipient``.

        Best-effort — logs and returns False on any failure so the
        calling flow (incident submit / course sign-up) is never blocked
        ...
        try:
            ...
            if not (subject and body):
                logger.warning("Empty render for template %s -> %s",
                               template, recipient)
                return False
        except Exception:
            logger.exception("Failed to render template %s", template)
            return False
        try:
            from education_system.post_18.university_system.modules.shared.utils.email_service import (
                queue_email,
            )
            ok = bool(queue_email(recipient, subject, body))
            if ok:
                logger.info("Queued %s email to %s", template, recipient)
            else:
                logger.warning("queue_email returned falsy for %s -> %s",
                               template, recipient)
            return ok
        except Exception:
            logger.exception("Failed to queue %s email to %s",
                             template, recipient)
            return False
    ...
    def _submit_report(self):
        ...
        report_id = self.form_vars["id"].get().strip()
        report_email = self.form_vars["email"].get().strip()
        report_phone = self.form_vars["phone"].get().strip()

        # Validate optional contact fields (only when filled in).
        if report_id and not _is_valid_id(report_id):
            messagebox.showwarning(
                "Invalid ID",
                "Student/Staff ID may only contain letters, numbers and "
                "hyphens.")
            return
        if report_email and not _is_valid_email(report_email):
            messagebox.showwarning(
                "Invalid Email",
                "Please enter a valid email address (or leave it blank).")
            return
        if report_phone and not _is_valid_phone(report_phone):
            messagebox.showwarning(
                "Invalid Phone Number",
                "Please enter a valid contact number with at least 7 "
        ...
        # Email integration: notify admins of the incident and send the
        # reporter a confirmation receipt. Best-effort — never blocks the
        # submit flow.
        reference = f"INC-{new_id:04d}"
        admin_vars = {
            "reference": reference,
            ...
            "reporter_id": report["reporter_id"] or "—",
            "reporter_user": report["reporter_user"] or "—",
        ...
        for admin_email in admins:
            if self._send_template_email(
                    admin_email, "health/first_aid_incident_report", dict(admin_vars)):
                admins_notified += 1

        reporter_email = report["email"] or self._resolve_user_email(
            report["reporter_id"])
        receipt_sent = False
        if reporter_email:
            receipt_sent = self._send_template_email(
                reporter_email, "health/first_aid_incident_receipt", {
                    "reporter_name": report["reporter_name"],
                    "reference": reference,
                    "incident_type": report["incident_type"],
                    "severity": report["severity"],
        ...
        if admins_notified:
            notify_line = (f"The Health & Safety team has been notified by "
                           f"email ({admins_notified} recipient"
                           f"{'s' if admins_notified != 1 else ''}).")
        else:
            notify_line = "A first aid officer will be notified."
        if receipt_sent:
            notify_line += f"\nA confirmation receipt was sent to {reporter_email}."
    ...
    def _view_incident_log(self):
        ...
        # Filter bar.
        bar = tk.Frame(log_window, bg=self.colors["bg"])
        bar.pack(fill="x", padx=15)
        tk.Label(bar, text="Severity:", font=("Segoe UI", 10, "bold"),
                 bg=self.colors["bg"], fg=self.colors["text"]).pack(side="left")
        sev_var = tk.StringVar(value="All")
        ttk.Combobox(bar, textvariable=sev_var, state="readonly", width=12,
                     values=["All", "Low", "Medium", "High", "Critical"]
                     ).pack(side="left", padx=(5, 15))
        tk.Label(bar, text="Status:", font=("Segoe UI", 10, "bold"),
                 bg=self.colors["bg"], fg=self.colors["text"]).pack(side="left")
        ...
        # Holds the currently-displayed incidents so the action buttons can
        # operate on them.
        state = {"incidents": []}
        ...
        def _load():
            ...
            if self.db:
                try:
                    incidents = self.db.fetch_all(severity=sev, status=status)
                except sqlite3.Error:
        ...
        sev_var.trace("w", lambda *a: _load())
        status_var.trace("w", lambda *a: _load())

        def _mark_resolved():
            incidents = [r for r in state["incidents"]
                         if (r.get("status") or "Open") != "Resolved"]
            if not incidents:
                messagebox.showinfo("Incident Log",
                                    "No open incidents to resolve.",
                                    parent=log_window)
                return
            ids = ", ".join(f"INC-{r['id']:04d}" for r in incidents)
            if not messagebox.askyesno(
                    "Mark Resolved",
                    f"Mark the {len(incidents)} currently-listed open "
                    f"incident(s) as Resolved?\n\n{ids}",
                    parent=log_window):
                return
            done = 0
            for r in incidents:
                try:
                    if self.db.update_status(r["id"], "Resolved"):
                        done += 1
                except sqlite3.Error:
                    logger.exception("Could not update status for %s", r["id"])
            logger.info("Marked %s incidents resolved by user=%s",
                        done, self.user_display)
            _load()
        ...
        def _export():
            ...
            if not incidents:
                ...
                return
            import csv
            fname = (f"first_aid_incident_log_"
                     f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
            downloads = os.path.join(os.path.expanduser("~"), "Downloads")
            target_dir = downloads if os.path.isdir(downloads) else tempfile.gettempdir()
            path = os.path.join(target_dir, fname)
            cols = ["id", "submitted_at", "status", "severity",
                    "incident_type", "location", "reporter_name",
                    "reporter_id", "reporter_user", "email", "phone",
                    "description"]
            try:
                with open(path, "w", newline="", encoding="utf-8") as fh:
                    writer = csv.DictWriter(fh, fieldnames=cols,
                ...
                logger.exception("Could not export incident log")
                messagebox.showerror("Export Failed",
                                     "Could not write the export file.",
                                     parent=log_window)
                return
            logger.info("Incident log exported (%s rows) to %s by user=%s",
                        len(incidents), path, self.user_display)
            self._open_path(path)
            messagebox.showinfo("Export Complete",
                                f"✓ Exported {len(incidents)} incident(s) to:"
                                f"\n\n{path}", parent=log_window)

        btns = tk.Frame(log_window, bg=self.colors["bg"])
        btns.pack(fill="x", padx=15, pady=(0, 12))
        # Resolving incidents is an admin/staff action.
        if self._is_staff():
            tk.Button(btns, text="✓ Mark Listed Resolved",
                      font=("Segoe UI", 9, "bold"), bg=self.colors["accent"],
                      fg="white", relief="flat", padx=12, pady=5,
                      cursor="hand2", command=_mark_resolved).pack(side="left")
        tk.Button(btns, text="⤓ Export CSV", font=("Segoe UI", 9, "bold"),
                  bg=self.colors["secondary"], fg="white", relief="flat",
                  padx=12, pady=5, cursor="hand2",
    ...
    def _is_staff(self):
        """True if the signed-in user is an admin or staff member."""
        role = str((self.user or {}).get("role") or "").lower()
        return role in ("admin", "staff", "superadmin", "super_admin")
    ...
    def _load_emergency_contacts(self):
        return [dict(c) for c in EMERGENCY_CONTACTS]
