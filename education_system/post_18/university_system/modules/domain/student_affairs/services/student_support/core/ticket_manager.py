...
from education_system.post_18.university_system.modules.domain.student_affairs.services.student_support.core.attachment_manager import _get_attachment_count, _get_last_response_info, _process_attachments
from education_system.post_18.university_system.modules.domain.student_affairs.services.student_support.automation.sentiment_analysis import _analyze_sentiment, _suggest_category, _get_auto_assignment, _estimate_resolution_time
from education_system.post_18.university_system.modules.domain.student_affairs.services.student_support.features.templates import _create_auto_response
from education_system.post_18.university_system.modules.domain.student_affairs.services.student_support.features.notifications import _create_ticket_notifications
    ...
    try:
        ...
        try:
            ...
            INSERT INTO support_tickets (
                ...
                created_at, assigned_to, sentiment, estimated_resolution, tags
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
...
def _build_ticket_query(student_id, filters, current_user):
    ...
    if filters:
        ...
        if filters.get('search'):
            base_query += " AND (subject LIKE ? OR description LIKE ?)"
...
def get_ticket_details(ticket_id):
    ...
    try:
        ...
        if _auth_mod.auth.current_user['role'] == 'student':
            ...
            if not result or result[0] != ticket['user_id']:
...
def get_ticket_attachments(ticket_id):
    ...
    try:
        ...
        cursor.execute('SELECT * FROM ticket_attachments WHERE ticket_id = ? ORDER BY created_at DESC', (ticket_id,))
        attachments = [dict(row) for row in cursor.fetchall()]
...
def view_my_tickets_enhanced(support):
    ...
    try:
        ...
        for ticket in tickets:
            ...
            print(f"{status_emoji} #{ticket['ticket_id']} - {ticket['subject']}")
            print(f"   📂 {ticket['category']} | {priority_emoji} {ticket['priority']} | 📅 {ticket['created_at']}")
        ...
        if tickets:
            ticket_choice = input("View ticket details (enter ticket #) or press Enter to go back: ").strip()
...
def view_all_tickets_enhanced(support):
    ...
    try:
        ...
        for ticket in tickets:
            ...
            print(f"{status_emoji} #{ticket['ticket_id']} - {ticket['subject']}")
            print(f"   👤 Student: {ticket['user_id']} | 📂 {ticket['category']} | {priority_emoji} {ticket['priority']}")
...
def display_ticket_details_enhanced(support, ticket_id):
    ...
    try:
        ...
        print("="*50)
        print(f"📋 Title: {ticket['subject']}")
        print(f"👤 Student: {ticket['user_id']}")
