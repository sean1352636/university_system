...
from education_system.post_18.university_system.modules.domain.student_affairs.services.student_support.features.knowledge_base import _search_knowledge_base
...
def _search_tickets(query, filters, page, per_page):
    ...
    try:
        ...
        if _auth_mod.auth.current_user['role'] == 'student':
            ...
            if result:
                base_query += "user_id = ? AND "
                params.append(result[0])
            else:
                return {'tickets': [], 'total_count': 0}

        # Add search condition
        base_query += "(subject LIKE ? OR description LIKE ?)"
    ...
    try:
        ...
        if 'tickets' in results:
            ...
            for ticket in tickets[:5]:
                print(f"  #{ticket['ticket_id']} - {ticket['subject']} ({ticket['status']})")
