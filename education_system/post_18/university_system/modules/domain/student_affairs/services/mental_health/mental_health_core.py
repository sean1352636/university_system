...
try:
    ...
    def send_email(*args, **kwargs):
        print("Email would be sent (email service not available)")
    def render_template(template_name, vars):
        return f"Template: {template_name}", "Email body would be here"


def _resolve_counselor_email(cursor, identifier) -> Optional[str]:
    """Resolve a counselor's email from a user identifier (username, id, or email)."""
    if not identifier:
        return None
...
class WellnessCheckInManager:
    ...
    def _trigger_follow_up(student_id: str, checkin_id: int) -> None:
        """Trigger follow-up notification to the counseling team.

        Emails every active counselor with a deliverable address. Any mail
        failure is logged but never propagated — the check-in is already saved.
        """
        conn = get_connection()
        cursor = conn.cursor()
        try:
            ...
            student_name = (f"{srow['first_name']} {srow['last_name']}".strip()
                            if srow else student_id)

            # Active counselors -> their email via users/staff.
            cursor.execute(
                "SELECT user_id, name FROM mental_health_counselors WHERE is_active = 1")
            counselors = cursor.fetchall()

            recipients = []
            for c in counselors:
                email = _resolve_counselor_email(cursor, c['user_id'])
                if email:
                    recipients.append(email)

            subject, body = render_template('wellness_follow_up', {
                'student_name': student_name,
                'student_id': student_id,
                'checkin_id': checkin_id,
            })
            if not subject:
                subject = f"Wellness follow-up required: {student_name}"
            if not body:
                body = (
                    f"A wellness check-in has flagged a student for follow-up.\n\n"
                    f"Student: {student_name} ({student_id})\n"
                    f"Check-in ID: {checkin_id}\n\n"
                    f"Please review and reach out per the counseling protocol.\n"
                )

            if recipients and HAS_EMAIL:
                for email in recipients:
                    try:
                        send_email(email, subject, body)
                    except Exception as e:
                        print(f"Failed to send wellness follow-up to {email}: {e}")
            else:
                print(f"Follow-up triggered for student {student_id} "
                      f"(Check-in ID: {checkin_id}) - no counselor email available")
        finally:
            conn.close()
    ...
    while True:
        try:
            ...
            if choice in ['1', '2', '3', '4', '5', '6', '7']:
                ...
                print("Use: from education_system.post_18.university_system.modules.domain.student_affairs.services.mental_health import CounselorManager")
            elif choice == '8':
                break
            else:
