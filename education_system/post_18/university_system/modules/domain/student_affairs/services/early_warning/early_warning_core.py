...
def _resolve_staff_email(cursor, identifier: str) -> Optional[str]:
    """Resolve a staff/user email from an identifier that may be a username,
    a numeric user/staff id, or already an email address."""
    if not identifier:
        return None
    ident = str(identifier).strip()
    if "@" in ident:
        return ident
    # Try the users table (username or id), then the staff table.
    for table, col in (("users", "username"), ("users", "id"),
                       ("staff", "username"), ("staff", "id")):
        try:
            cursor.execute(
                f"SELECT email FROM {table} WHERE {col} = ? "  # nosec B608 - literals only
                f"AND email IS NOT NULL AND TRIM(email) <> '' LIMIT 1",
                (ident,))
            row = cursor.fetchone()
            if row and row[0]:
                return row[0]
    ...
    def _notify_intervention(intervention_id: int) -> None:
        ...
        try:
            ...
            if intervention['assigned_to']:
                ...
                # Fall back to a plain message if the template is unavailable.
                if not subject:
                    subject = (f"[{intervention['priority']}] Intervention assigned: "
                               f"{intervention['first_name']} {intervention['last_name']}")
                if not body:
                    body = (
                        f"An early-warning intervention has been assigned to you.\n\n"
                        f"Student: {intervention['first_name']} {intervention['last_name']} "
                        f"({intervention['student_id']})\n"
                        f"Priority: {intervention['priority']}\n"
                        f"Type: {intervention['intervention_type']}\n"
                        f"Scheduled: {scheduled}\n\n"
                        f"{intervention['description'] or ''}\n"
                    )

                staff_email = _resolve_staff_email(cursor, intervention['assigned_to'])
                if staff_email and HAS_EMAIL:
                    try:
                        send_email(staff_email, subject, body)
                    except Exception as e:  # never let a mail failure break the workflow
                        print(f"Failed to send intervention notification to {staff_email}: {e}")
                else:
                    # No deliverable address / email offline — record rather than silently drop.
                    print(f"Intervention notification (undelivered - no email): {subject}")
    ...
    while True:
        try:
            ...
            if choice in ['1', '2', '3', '4', '5', '6', '7']:
                ...
                print("Use: from education_system.post_18.university_system.modules.domain.student_affairs.services.early_warning import RiskAssessmentManager")
            elif choice == '8':
                display_language_menu_option()
            elif choice == '9':
...
def launch_early_warning_gui(root, auth):
    ...
    from education_system.post_18.university_system.infrastructure.database.schemas.attendance_warning_schemas import init_early_warning_system_db
