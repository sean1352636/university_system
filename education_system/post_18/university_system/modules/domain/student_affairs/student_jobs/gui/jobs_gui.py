...
class StudentJobsGUI:
    ...
    def __init__(self, parent=None, auth=None, container=None):
        """Initialize the GUI.

        container: when provided, the board is built INSIDE that existing frame
        (embedded mode) rather than its own Tk/Toplevel window — used to host
        the board in another app's content area. ``parent``/no-arg behaviour is
        unchanged (Toplevel / standalone Tk window).
        """
        self.embedded = container is not None
        if self.embedded:
            self.root = tk.Frame(container, bg='#f0f0f0')
        ...
        # Create UI (a menubar only applies to a real window, not embedded)
    ...
    def view_application_details(self):
        ...
        text.insert(tk.END, f"{'=' * 60}\n")
        text.insert(tk.END, "APPLICATION DETAILS\n")
        text.insert(tk.END, f"{'=' * 60}\n\n")
