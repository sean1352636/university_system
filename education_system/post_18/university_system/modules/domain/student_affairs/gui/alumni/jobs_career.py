...
class JobsCareerMixin:
    ...
    """
        ...
        def search_jobs(self):
            ...
            if search_term or category != "All":
                self.job_text.delete(1.0, tk.END)
                self.job_text.insert(tk.END, "Searching for jobs...\n")
    ...
    """
        ...
        def show_job_board(self):
            """Job board is unified with the student job board — embed that GUI
            directly in the content area instead of a separate board/window."""
            self.clear_content()
            ...
            try:
                ...
                # Embedded mode: build the board inside the alumni content frame.
                self._embedded_job_board = StudentJobsGUI(
                    container=self.content_frame, auth=self.auth)
            except Exception as e:
                ttk.Label(self.content_frame,
                          text=f"Could not load the university job board: {e}",
                          foreground='#b3261e').pack(pady=20)
        ...
        def _resolve_user_email(self, user):
            """Best-effort email for the current user dict."""
            email = (user.get('email') or user.get('email_address') or "").strip()
            if email:
                return email
            ...
            try:
                with db_get_connection() as conn:
                    row = conn.execute(
                        ...
                        (str(username), username)).fetchone()
                    if row and row[0]:
                        return row[0]
            except sqlite3.Error:
                pass
        ...
        def _send_counseling_email(self, recipient_email, scheduler_name, counselor,
                                   session_type, session_dt, duration):
            """Email counselling session details to the user who scheduled it,
            using the JSON template alumni/counseling_scheduled."""
            recipient_email = (recipient_email or "").strip()
            if not recipient_email:
                return False
            try:
                from education_system.post_18.university_system.infrastructure.email.email_service import send_email
                subject, body = render_template("alumni/counseling_scheduled", {
                    "full_name": scheduler_name,
                ...
                if not subject or not body:
                    return False
                return bool(send_email(recipient_email=recipient_email, subject=subject, body=body))
            except Exception as e:
                import logging
                logging.warning(f"Counseling email failed: {e}")
                return False

        def submit_counseling_request(self):
            """Submit counseling session request: persist it and email details
            to the user who scheduled it."""
            counselor = self.counseling_vars['counselor'].get().strip()
            session_type = self.counseling_vars['session_type'].get().strip()
            session_dt = self.counseling_vars['datetime'].get().strip()
            duration = self.counseling_vars['duration'].get().strip() or "60"
            notes = self.counseling_notes.get(1.0, tk.END).strip()
            ...
            scheduler_name = (user.get('first_name') or user.get('display_name')
                              or user.get('username') or 'Alumni User')
            scheduler_id = str(user.get('username') or user.get('id') or 'alumni_gui')
            recipient_email = self._resolve_user_email(user)
                ...
                with db_get_connection() as conn:
                    conn.execute(
                        ...
                        INSERT INTO career_counseling
                            (counselor_id, client_id, session_date, session_type,
                             duration, notes, status)
                        VALUES (?, ?, ?, ?, ?, ?, 'scheduled')
                        """,
                        (counselor, scheduler_id, session_dt, session_type, dur_int, notes),
                    )
                    conn.commit()
            except sqlite3.Error as e:
                messagebox.showerror("Database Error",
                                     f"Could not schedule the session: {e}")
                return

            # Email the details to whoever scheduled it.
            emailed = self._send_counseling_email(
                recipient_email, scheduler_name, counselor, session_type, session_dt, dur_int)

            messagebox.showinfo(
                "Session Scheduled",
                "Career counseling session scheduled." +
                ("\nDetails have been emailed to you." if emailed
