...
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.dashboard import DashboardMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.alumni_crud import AlumniCRUDMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.directory import DirectoryMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.business_directory import BusinessDirectoryMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.events import EventsMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.forum import ForumMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.stories_photos import StoriesPhotosMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.donations import DonationsMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.reports import ReportsMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.chapters import ChaptersMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.mentorship import MentorshipMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.jobs_career import JobsCareerMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.reunions import ReunionsMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.gamification import GamificationMixin
from education_system.post_18.university_system.modules.domain.student_affairs.gui.alumni.notifications import NotificationsMixin
...
class AlumniGUIApp(
    ...
    def __init__(self, root, auth=None, show_return_button=True):
        self.root = root
        # When an alumnus logs in, this app is the primary portal window and
        # there is no parent dashboard to return to (the portal header already
        # offers Return to Login / Shutdown). The launcher passes
        # show_return_button=False in that case. Staff who open the alumni
        # system as a child window (e.g. from the graduation ceremony GUI)
        # keep the button so they can close it and return.
        self.show_return_button = show_return_button
    ...
    def create_navigation_menu(self, parent):
        ...
        # "Return to Homescreen" is only meaningful when this app was opened
        # from a parent menu (staff Toplevel). For a logged-in alumnus it would
        # drop them into the main GUI with no way back, so it is hidden.
    ...
    def create_sidebar(self, parent):
        ...
        display_name = (self.current_user.get('first_name')
                        or self.current_user.get('display_name')
                        or self.current_user.get('username')
                        or 'User')
        ttk.Label(user_frame, text=_t("common.welcome_user", default="Welcome, {name}!").format(name=display_name),
                 font=('Arial', 10, 'bold')).pack()
        ttk.Label(user_frame, text=_t("common.username_display", default="Username: {username}").format(username=self.current_user.get('username', ''))).pack()
    ...
    def create_widgets(self):
        ...
        # Add return to main menu button at the top — only when there is a
        # menu/dashboard to go back to (i.e. not when an alumnus is logged in
        # and this is the primary portal window).
        if self.show_return_button:
