...
class WellnessGUI:
    ...
    def create_checkin_tab(self):
        """Create wellness check-in tab."""
        checkin_tab = ttk.Frame(self.notebook)
        self.notebook.add(checkin_tab, text=_t("wellness.tabs.checkin", default="📝 Check-in"))

        form_frame = ttk.LabelFrame(checkin_tab, text=_t("wellness.checkin.form.title", default="Complete Wellness Check-in"), padding="10")
        form_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        ttk.Label(form_frame, text=_t("wellness.checkin.form.instructions", default="Rate each area from 1 (Poor) to 10 (Excellent):"),
                 font=('Arial', 11, 'bold')).grid(row=0, column=0, columnspan=2, sticky=tk.W, pady=(0, 10))

        self.checkin_vars = {}
        checkin_fields = [
            ('overall_mood', _t("wellness.checkin.form.overall_mood", default="Overall Mood (1-10):")),
            ('stress_level', _t("wellness.checkin.form.stress_level", default="Stress Level (1=Low, 10=High):")),
            ('sleep_quality', _t("wellness.checkin.form.sleep_quality", default="Sleep Quality (1-10):")),
            ('energy_level', _t("wellness.checkin.form.energy_level", default="Energy Level (1-10):")),
            ('anxiety_level', _t("wellness.checkin.form.anxiety_level", default="Anxiety Level (1=Low, 10=High):")),
        ]
        for i, (key, label) in enumerate(checkin_fields, start=1):
            ttk.Label(form_frame, text=label).grid(row=i, column=0, sticky=tk.W, pady=5)
            var = tk.IntVar(value=5)
            self.checkin_vars[key] = var
            ttk.Scale(form_frame, from_=1, to=10, variable=var,
                     orient=tk.HORIZONTAL).grid(row=i, column=1, pady=5, sticky=(tk.W, tk.E))

        notes_row = len(checkin_fields) + 1
        ttk.Label(form_frame, text=_t("wellness.checkin.form.notes_label", default="Additional notes:")).grid(row=notes_row, column=0, sticky=tk.W, pady=5)
        self.checkin_notes = scrolledtext.ScrolledText(form_frame, height=4, width=30)
        self.checkin_notes.grid(row=notes_row, column=1, pady=5)

        ttk.Button(form_frame, text=_t("wellness.checkin.form.submit", default="Complete Check-in"),
                  command=self.submit_checkin).grid(row=notes_row + 1, column=0, columnspan=2, pady=10)
    ...
    def submit_checkin(self):
        ...
        try:
            values = {k: int(v.get()) for k, v in self.checkin_vars.items()}
            notes = self.checkin_notes.get("1.0", tk.END).strip() or None

            student_id = self.auth.get_current_user()['username']
            checkin_id = self.service.create_checkin(
                student_id, values['overall_mood'], values['stress_level'],
                values['sleep_quality'], values['energy_level'],
                values['anxiety_level'], notes
            )

            messagebox.showinfo(
                _t("wellness.checkin.messages.title_success", default="Success"),
                _t("wellness.checkin.messages.success", default="Wellness check-in completed! (ID: {id})\n+10 Wellness Points earned!").format(id=checkin_id)
            )

            if values['stress_level'] >= 8 or values['anxiety_level'] >= 8:
                messagebox.showwarning(
                    _t("wellness.checkin.messages.title_concern", default="High Stress Detected"),
                    _t("wellness.checkin.messages.concern", default="We noticed you're experiencing high stress or anxiety.\nConsider booking a counseling appointment or reviewing crisis resources.")
                )

            # Reset form
            for var in self.checkin_vars.values():
                var.set(5)
            self.checkin_notes.delete("1.0", tk.END)

        except Exception as e:
            messagebox.showerror(
                _t("wellness.checkin.messages.title_error", default="Error"),
                _t("wellness.checkin.messages.error", default="Failed to complete check-in: {error}").format(error=str(e))
            )

    def create_report_tab(self):
        """Create comprehensive wellness report tab."""
        report_tab = ttk.Frame(self.notebook)
        self.notebook.add(report_tab, text=_t("wellness.tabs.report", default="📈 Report"))

        report_frame = ttk.LabelFrame(report_tab, text=_t("wellness.report.title", default="Comprehensive Wellness Report (Past 30 Days)"), padding="10")
        report_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        self.report_text = scrolledtext.ScrolledText(report_frame, height=25, width=60, wrap=tk.WORD)
        self.report_text.pack(fill=tk.BOTH, expand=True)

        ttk.Button(report_frame, text=_t("wellness.buttons.refresh", default="Refresh"),
                  command=self.refresh_report).pack(pady=5)

        self.refresh_report()

    def refresh_report(self):
        """Refresh comprehensive wellness report display."""
        self.report_text.config(state=tk.NORMAL)
        self.report_text.delete("1.0", tk.END)

        student_id = self.auth.get_current_user()['username']
        report = self.service.get_comprehensive_wellness_report(student_id, 30)

        patterns = report['wellness_patterns']
        if 'message' not in patterns:
            self.report_text.insert(tk.END, "WELLNESS CHECK-IN SUMMARY\n", 'header')
            self.report_text.insert(tk.END, "=" * 50 + "\n")
            self.report_text.insert(tk.END, f"Total Check-ins: {patterns['total_checkins']}\n\n")
            avgs = patterns['averages']
            self.report_text.insert(tk.END, f"  Overall Mood:   {avgs['mood']:.1f}/10\n")
            self.report_text.insert(tk.END, f"  Stress Level:   {avgs['stress']:.1f}/10\n")
            self.report_text.insert(tk.END, f"  Sleep Quality:  {avgs['sleep_quality']:.1f}/10\n")
            self.report_text.insert(tk.END, f"  Energy Level:   {avgs['energy']:.1f}/10\n")
            self.report_text.insert(tk.END, f"  Anxiety Level:  {avgs['anxiety']:.1f}/10\n")
            if patterns['concerns']:
                self.report_text.insert(tk.END, "\nAreas of Concern:\n")
                for concern in patterns['concerns']:
                    self.report_text.insert(tk.END, f"  - {concern}\n")

        sleep = report['sleep_analytics']
        if 'message' not in sleep:
            self.report_text.insert(tk.END, "\nSLEEP SUMMARY\n", 'header')
            self.report_text.insert(tk.END, "=" * 50 + "\n")
            self.report_text.insert(tk.END, f"Nights Tracked: {sleep['nights_tracked']}\n")
            self.report_text.insert(tk.END, f"Average Hours: {sleep['average_hours']:.1f} hours/night\n")
            self.report_text.insert(tk.END, f"Average Quality: {sleep['average_quality']:.1f}/5\n")

        self.report_text.insert(tk.END, "\nACTIVE GOALS\n", 'header')
        self.report_text.insert(tk.END, "=" * 50 + "\n")
        self.report_text.insert(tk.END, f"Active Goals: {report['active_goals_count']}\n")
        for goal in report['active_goals']:
            percentage = (goal['current_value'] / goal['target_value']) * 100 if goal['target_value'] > 0 else 0
            self.report_text.insert(tk.END, f"\n  {goal['goal_type']}: {percentage:.1f}% complete\n")
            self.report_text.insert(tk.END, f"  ({goal['current_value']:.1f} / {goal['target_value']:.1f})\n")

        self.report_text.insert(tk.END, "\nGAMIFICATION\n", 'header')
        self.report_text.insert(tk.END, "=" * 50 + "\n")
        self.report_text.insert(tk.END, f"Total Wellness Points: {report['total_wellness_points']}\n")

        self.report_text.tag_config('header', font=('Arial', 11, 'bold'))
...
def main():
    ...
    from education_system.post_18.university_system.modules.shared.gui.auth.launch_guard import (
