...
def display_documentation_menu(auth) -> None:
    ...
    while True:
        ...
        print("  4. Save a letter as .txt")
        print("  5. Verify a letter (third-party)")
        if staff:
            print("\n  -- Registry Staff --")
            print("  6. Review pending requests")
            print("  7. Revoke an issued letter")
        print("\n  0. Back")
        print("=" * 70)
...
def _save_letter(service: DocumentationService, student_id: str) -> None:
    rows = service.list_letters(student_id)
    if not rows:
        print("\nNo issued letters.")
        return
    for r in rows:
        ...
        print(
            f"  {r['id']}: {r['reference_code']} — {r['letter_type']} "
            f"issued {r['issued_at'][:10]}{flag}"
        )
    sel = input("\nLetter ID to save (Enter to skip): ").strip()
    if not sel.isdigit():
        return
    letter = service.get_letter(int(sel))
    if not letter:
        print("Letter not found.")
        return
    path = input("Save to path (e.g. letter.txt): ").strip()
    if not path:
        return
    content = (
        letter["content"]
