...
from education_system.post_18.university_system.modules.domain.student_affairs.student_union.services import union_context as ctx
from education_system.post_18.university_system.modules.domain.student_affairs.student_union.services.union_context import auto_award_points

logger = logging.getLogger(__name__)

def manage_peer_support_system():
    """Main peer support system interface"""
    from education_system.post_18.university_system.modules.domain.student_affairs.student_union.administration.admin_management import (
...
def _ensure_peer_matching_tables(cursor):
    """Create the peer-matching queue tables if they do not already exist."""
    cursor.execute('''
...
def anonymous_peer_matching(student_id, cursor, conn):
    """Anonymous peer matching for one-on-one support (queue + pairing)"""
    try:
        _ensure_peer_matching_tables(cursor)
        ...
        # If the student already has an active or pending match, resume it.
        cursor.execute('''
        SELECT match_id, requester_id, matched_id, requester_anon_id,
               matched_anon_id, topic, status
        FROM peer_matches
        ...
        if existing:
            match_id, requester_id, matched_id, req_anon, matched_anon, topic, status = existing
            my_anon = req_anon if requester_id == student_id else matched_anon
            peer_anon = matched_anon if requester_id == student_id else req_anon

            if status == 'pending':
                print(f"\nYou are already in the matching queue for: {topic}")
                print(f"Your anonymous ID: {my_anon}")
                print("You will be paired as soon as a compatible peer joins.")
                leave = input("Leave the queue? (y/n): ").strip().lower()
                if leave == 'y':
                    cursor.execute(
                        "UPDATE peer_matches SET status = 'cancelled' WHERE match_id = ?",
                        (match_id,))
                    conn.commit()
                    print("You have left the matching queue.")
                return

            # status == 'matched'
            print(f"\nYou have an active peer match for: {topic}")
            print(f"Your anonymous ID: {my_anon}")
            print(f"Your peer's anonymous ID: {peer_anon}")
            _peer_match_conversation(match_id, my_anon, cursor, conn)
            return
        ...
        my_anon = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))

        # Try to pair with the oldest waiting peer in the same support area.
        cursor.execute('''
        SELECT match_id, requester_anon_id
        FROM peer_matches
        WHERE status = 'pending' AND topic = ? AND requester_id != ?
        ORDER BY match_id ASC
        LIMIT 1
        ''', (selected_area, student_id))
        waiting = cursor.fetchone()

        if waiting:
            waiting_match_id, waiting_anon = waiting
            cursor.execute('''
            UPDATE peer_matches
            SET matched_id = ?, matched_anon_id = ?, status = 'matched', matched_date = ?
            WHERE match_id = ?
            ''', (student_id, my_anon, now, waiting_match_id))
            conn.commit()

            print("\nYou have been matched with a waiting peer!")
            print(f"Your anonymous ID: {my_anon}")
            print(f"Your peer's anonymous ID: {waiting_anon}")
            print("Use these IDs for confidential communication.")
            _peer_match_conversation(waiting_match_id, my_anon, cursor, conn)
        else:
            cursor.execute('''
            INSERT INTO peer_matches (
                requester_id, requester_anon_id, topic, match_type, status, created_date
            ) VALUES (?, ?, ?, ?, ?, ?)
            ''', (student_id, my_anon, selected_area, 'One-on-one', 'pending', now))
            conn.commit()

            print("\nYou have been added to the anonymous peer support matching queue.")
            print(f"Your anonymous ID: {my_anon}")
            print("You will be paired as soon as a compatible peer joins the queue.")
            print("Meanwhile, please review our crisis resources if you need immediate support.")
...
def _peer_match_conversation(match_id, my_anon, cursor, conn):
    """Show anonymous messages for a match and optionally send one."""
    try:
        cursor.execute('''
    ...
    try:
        while True:
            ...
            if choice == '1':
                ...
                # Request tutoring - real DB-backed request against an offer
                cursor.execute('''
                CREATE TABLE IF NOT EXISTS tutoring_requests (
                    ...
                    FOREIGN KEY (offer_id) REFERENCES tutoring_offers (offer_id)
                )
                ''')

                offer_id = input("Enter the tutoring offer ID to request: ").strip()
                if not offer_id.isdigit():
                    print("Invalid offer ID.")
                    continue

                cursor.execute('''
                SELECT offer_id, tutor_id, subject, status
                FROM tutoring_offers
                WHERE offer_id = ?
                ''', (offer_id,))
                ...
                if not offer:
                    print("Tutoring offer not found.")
                    continue

                if offer[1] == student_id:
                    print("You cannot request your own tutoring offer.")
                    continue

                if offer[3] != 'available':
                    print("This tutoring offer is not currently available.")
                    continue

                # Prevent duplicate outstanding requests for the same offer.
                cursor.execute('''
                SELECT COUNT(*) FROM tutoring_requests
                WHERE offer_id = ? AND requester_id = ? AND status = 'pending'
                ''', (offer_id, student_id))
                if cursor.fetchone()[0] > 0:
                    print("You already have a pending request for this offer.")
                    continue

                message = input("Message to the tutor (topics, availability): ").strip()
                created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

                cursor.execute('''
                INSERT INTO tutoring_requests (
                    offer_id, requester_id, tutor_id, subject, message, status, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (offer[0], student_id, offer[1], offer[2], message, 'pending', created_at))
                conn.commit()

                request_id = cursor.lastrowid
                print(f"Tutoring request submitted! Request ID: {request_id}")
                print("The tutor will be notified and can accept your request.")

                auto_award_points(student_id, "Academic Support", 5,
                                f"Requested tutoring in {offer[2]}", cursor, conn)
...
def exam_preparation_groups(student_id, cursor, conn):
    """Manage exam preparation groups (create/list/join) + revision room booking"""
    try:
        while True:
            print("\nExam Preparation Groups")
            print("=======================")
            print("1. Browse exam prep groups")
            print("2. Create exam prep group")
            print("3. Join exam prep group")
            print("4. Book a revision room")
            print("5. Return to academic support")

            choice = input("Choose option: ").strip()

            if choice == '1':
                _browse_exam_prep_groups(cursor)
                ...
                meeting_schedule = input("Meeting schedule (e.g., 'Mon 5pm'): ").strip() or "TBD"
                location = input("Location (or press Enter for TBD): ").strip() or "TBD"

                group_name = f"Exam Prep: {subject} (exam {exam_date})"
                created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

                cursor.execute('''
                INSERT INTO study_groups (
                    course_id, group_name, creator_id, max_members, current_members,
                    meeting_schedule, location, status, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    subject, group_name, student_id, 8, 1,
                    meeting_schedule, location, 'Active', created_at
                ))
                conn.commit()
                group_id = cursor.lastrowid

                # Register the creator as the organising member.
                cursor.execute('''
                INSERT OR IGNORE INTO study_group_members (group_id, student_id, role)
                VALUES (?, ?, 'Organizer')
                ''', (group_id, student_id))
                conn.commit()

                print(f"Exam preparation group created! Group ID: {group_id}")
                ...
                cursor.execute('''
                SELECT group_name, current_members, max_members, creator_id, status
                FROM study_groups
                WHERE group_id = ? AND group_name LIKE 'Exam Prep:%'
                ''', (group_id,))
                group = cursor.fetchone()

                if not group:
                    print("Exam prep group not found.")
                    continue
                if group[4] != 'Active':
                    print("This group is no longer active.")
                    continue
                if group[1] >= group[2]:
                ...
                ''', (group_id, student_id))
                cursor.execute('''
                UPDATE study_groups SET current_members = current_members + 1
                WHERE group_id = ?
                ''', (group_id,))
                conn.commit()
                ...
                print(f"Successfully joined: {group[0]}")
                auto_award_points(student_id, "Academic Support", 15,
                                f"Joined exam prep group: {group[0]}", cursor, conn)

            elif choice == '4':
                _book_revision_room(student_id, cursor, conn)
...
def _browse_exam_prep_groups(cursor):
    """List active exam preparation groups."""
    try:
        cursor.execute('''
        SELECT sg.group_id, sg.course_id, sg.group_name, s.first_name, s.last_name,
               sg.current_members, sg.max_members, sg.meeting_schedule, sg.location
        FROM study_groups sg
        LEFT JOIN students s ON sg.creator_id = s.student_id
        WHERE sg.status = 'Active' AND sg.group_name LIKE 'Exam Prep:%'
        ORDER BY sg.course_id, sg.created_at
        ''')
        groups = cursor.fetchall()

        if not groups:
            print("No exam preparation groups currently available.")
            return

        print("\nExam Preparation Groups:")
        print(f"{'ID':<6} {'Subject':<15} {'Name':<30} {'Organizer':<20} {'Members':<10} {'Schedule':<15}")
        print("-" * 100)
        for g in groups:
            organizer = f"{g[3]} {g[4]}" if g[3] else "TBD"
            members = f"{g[5]}/{g[6]}"
            print(f"{g[0]:<6} {g[1][:15]:<15} {g[2][:30]:<30} {organizer[:20]:<20} "
                  f"{members:<10} {(g[7] or 'TBD')[:15]:<15}")
...
def _book_revision_room(student_id, cursor, conn):
    """Book a study room for revision, preventing double-booking of a room+timeslot."""
    try:
        cursor.execute('''
        ...
        if not rooms:
            print("No study rooms are currently available for booking.")
            return

        print("\nAvailable Study Rooms:")
        print(f"{'ID':<6} {'Room':<12} {'Building':<20} {'Capacity':<10}")
        print("-" * 50)
        for r in rooms:
            print(f"{r[0]:<6} {(r[1] or ''):<12} {(r[2] or ''):<20} {r[3]:<10}")
        ...
        cursor.execute("SELECT room_number FROM study_rooms WHERE room_id = ? AND status = 'available'",
                       (room_id,))
        room = cursor.fetchone()
        if not room:
            print("Room not found or unavailable.")
            return
        ...
        # Normalise to zero-padded HH:MM for reliable string comparison.
        start_time = start_dt.strftime('%H:%M')
        end_time = end_dt.strftime('%H:%M')

        # Reject overlapping confirmed bookings for the same room and date.
        cursor.execute('''
        ...
        if cursor.fetchone()[0] > 0:
            print("That room is already booked for an overlapping time slot.")
            return
        ...
        INSERT INTO study_room_bookings (
            ...
            purpose, group_size, notes, status
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (room_id, student_id, booking_date, start_time, end_time,
              'exam revision', group_size, notes, 'confirmed'))
        conn.commit()

        booking_id = cursor.lastrowid
        print(f"Room {room[0]} booked for {booking_date} {start_time}-{end_time}.")
        print(f"Booking ID: {booking_id}")

        auto_award_points(student_id, "Academic Support", 10,
                        f"Booked revision room {room[0]}", cursor, conn)
...
def view_academic_workshops(cursor):
    """View upcoming academic workshops from the events system"""
    try:
        ...
        # Union events tagged as workshops / academic sessions.
        try:
            cursor.execute('''
            SELECT event_name, COALESCE(description, ''), COALESCE(event_date, 'TBD'),
                   COALESCE(location, 'TBD')
            FROM union_events
            WHERE status IN ('upcoming', 'scheduled', 'open')
              AND (LOWER(category) LIKE '%workshop%'
                   OR LOWER(category) LIKE '%academic%'
                   OR LOWER(event_name) LIKE '%workshop%')
            ORDER BY event_date
            ''')
            workshops.extend(cursor.fetchall())
        except sqlite3.Error:
            pass

        # Academic calendar entries classified as workshops.
        try:
            cursor.execute('''
            SELECT name, COALESCE(description, ''),
                   COALESCE(date, date_start, 'TBD'), COALESCE(location, 'TBD')
            FROM academic_calendar_events
            WHERE is_active = 1
              AND (LOWER(event_type) LIKE '%workshop%'
                   OR LOWER(name) LIKE '%workshop%')
        ...
        if not workshops:
            print("No upcoming academic workshops are currently scheduled.")
        else:
            print(f"{'Workshop':<28} {'Description':<35} {'Date':<12} {'Location':<15}")
            print("-" * 92)
            for w in workshops:
                print(f"{(w[0] or '')[:28]:<28} {(w[1] or '')[:35]:<35} "
                      f"{(w[2] or 'TBD')[:12]:<12} {(w[3] or 'TBD')[:15]:<15}")
