...
def _current_moderator():
    """Return (id, username) for the acting moderator, with safe fallbacks."""
    try:
        current = auth.current_user if auth is not None else None
        if current:
            return current.get('id'), current.get('username', 'moderator')
    except Exception:
        pass
    return None, 'system'

def ensure_content_moderation_table(cursor, conn):
    """Create the content moderation reports table if it does not exist."""
    cursor.execute('''
...
def content_moderation_queue(cursor, conn):
    """Content moderation workflow: review, inspect and action reported content."""
    try:
        ensure_content_moderation_table(cursor, conn)

        while True:
            print("\nContent Moderation Queue")
            print("========================")
            print("1. View pending reports")
            print("2. View report details")
            print("3. Take action on a report (approve / remove / dismiss)")
            print("4. File a new content report")
            print("5. View resolved reports history")
            print("6. Return to previous menu")

            choice = input("Choose option: ").strip()

            if choice == '1':
                # View pending / flagged reports
                cursor.execute('''
                SELECT report_id, content_type, content_id, reported_by, reason, status, created_at
                FROM content_moderation_reports
                WHERE status IN ('pending', 'flagged', 'escalated')
                ...
                print(f"{'ID':<6} {'Type':<15} {'Content':<12} {'Reported By':<15} {'Status':<12} {'Reason':<25}")
                print("-" * 90)
                for rep in reports:
                    reason = (rep[4] or "")[:25]
                    content_id = str(rep[2]) if rep[2] is not None else "N/A"
                    reported_by = str(rep[3]) if rep[3] is not None else "anonymous"
                    print(f"{rep[0]:<6} {rep[1][:15]:<15} {content_id[:12]:<12} {reported_by[:15]:<15} {rep[5]:<12} {reason:<25}")

            elif choice == '2':
                # View a single report's details
                report_id = input("Enter report ID to view: ").strip()
                if not report_id.isdigit():
                    print("Invalid report ID.")
                    continue

                cursor.execute('''
                SELECT report_id, content_type, content_id, content_summary, reported_by,
                       reason, status, moderator_id, moderator_action, moderator_notes,
                ...
                print(f"\nReport #{rep[0]}")
                print("-" * 40)
                print(f"Content type:    {rep[1]}")
                print(f"Content ID:      {rep[2] if rep[2] is not None else 'N/A'}")
                print(f"Summary:         {rep[3] or 'N/A'}")
                print(f"Reported by:     {rep[4] if rep[4] is not None else 'anonymous'}")
                print(f"Reason:          {rep[5] or 'N/A'}")
                print(f"Status:          {rep[6]}")
                print(f"Created at:      {rep[11]}")
                if rep[7] or rep[8] or rep[10]:
                    print(f"Moderator:       {rep[7] or 'N/A'}")
                    print(f"Action taken:    {rep[8] or 'N/A'}")
                    print(f"Moderator notes: {rep[9] or 'N/A'}")
                    print(f"Moderated at:    {rep[10] or 'N/A'}")

            elif choice == '3':
                # Take a moderation action
                report_id = input("Enter report ID to action: ").strip()
                if not report_id.isdigit():
                    print("Invalid report ID.")
                    continue

                cursor.execute(
                    'SELECT content_type, content_id, status FROM content_moderation_reports WHERE report_id = ?',
                    (report_id,)
                )
                rep = cursor.fetchone()
                if not rep:
                ...
                if rep[2] not in ('pending', 'flagged', 'escalated'):
                    print(f"This report is already resolved (status: {rep[2]}).")
                    continue

                print(f"Report #{report_id} — {rep[0]} (content {rep[1] if rep[1] is not None else 'N/A'})")
                print("Available actions:")
                print("1. approve  (content is acceptable, keep it)")
                print("2. remove   (content violates guidelines, take it down)")
                print("3. dismiss  (report is invalid / no action needed)")
                print("4. escalate (refer for further review)")

                action_choice = input("Select action (enter number): ").strip()
                action_map = {
                    '1': ('approved', 'approve'),
                    '2': ('removed', 'remove'),
                    '3': ('dismissed', 'dismiss'),
                    '4': ('escalated', 'escalate'),
                }
                ...
                new_status, action_label = action_map[action_choice]
                notes = input("Optional moderator notes: ").strip()
                moderator_id, moderator_name = _current_moderator()
                moderated_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

                cursor.execute('''
                UPDATE content_moderation_reports
                SET status = ?, moderator_action = ?, moderator_id = ?,
                    moderator_notes = ?, moderated_at = ?
                WHERE report_id = ?
                ''', (new_status, action_label, str(moderator_id) if moderator_id is not None else moderator_name,
                      notes, moderated_at, report_id))
                conn.commit()
                print(f"Report #{report_id} marked '{new_status}' by {moderator_name}.")

            elif choice == '4':
                # File a new report / flag content
                content_type = input("Content type (e.g. group_message, post, comment): ").strip()
                if not content_type:
                    print("Content type is required.")
                    continue
                content_id = input("Content ID (leave blank if not applicable): ").strip() or None
                content_summary = input("Short summary of the content: ").strip() or None
                reported_by = input("Reporter ID (leave blank for anonymous): ").strip() or None
                reason = input("Reason for report: ").strip() or None
                created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

                cursor.execute('''
                INSERT INTO content_moderation_reports
                    (content_type, content_id, content_summary, reported_by, reason, status, created_at)
                VALUES (?, ?, ?, ?, ?, 'pending', ?)
                ''', (content_type, content_id, content_summary, reported_by, reason, created_at))
                conn.commit()
                print(f"Report filed successfully (report ID {cursor.lastrowid}).")

            elif choice == '5':
                # Resolved reports history
                cursor.execute('''
                SELECT report_id, content_type, status, moderator_id, moderator_action, moderated_at
                FROM content_moderation_reports
                WHERE status NOT IN ('pending', 'flagged', 'escalated')
                ...
                print(f"\nResolved Reports ({len(history)}):")
                print(f"{'ID':<6} {'Type':<15} {'Status':<12} {'Moderator':<15} {'Action':<10} {'When':<20}")
                print("-" * 85)
                for h in history:
                    moderator = str(h[3]) if h[3] is not None else "N/A"
                    print(f"{h[0]:<6} {h[1][:15]:<15} {h[2]:<12} {moderator[:15]:<15} {(h[4] or ''):<10} {(h[5] or ''):<20}")
    ...
    try:
        while True:
            ...
            if choice == '1':
                ...
                if safety_choice == '1':
                    # Review reported/flagged content via the moderation queue
                    content_moderation_queue(cursor, conn)
                ...
                if group_id.isdigit():
                    ...
                    if group_info:
                        print("Facilitator training assigned for:")
                ...
                print("5. Review reported content (moderation queue)")
                ...
                if mod_choice == '1':
                    ...
                    if group_id.isdigit() and student_id:
                        cursor.execute('''
                        UPDATE support_group_members SET status = 'removed'
                        WHERE group_id = ? AND student_id = ? AND status = 'active'
                        ''', (group_id, student_id))
                        if cursor.rowcount > 0:
                            cursor.execute('''
                            UPDATE peer_support_groups
                            SET current_members = MAX(current_members - 1, 0)
                            WHERE group_id = ?
                            ''', (group_id,))
                            conn.commit()
                            print("Member removed from group.")
                        else:
                            print("Active membership not found for that student.")
                    else:
                        print("Invalid group ID or student ID.")

                elif mod_choice == '3':
                    group_id = input("Enter group ID: ").strip()
                    new_facilitator = input("Enter new facilitator student ID: ").strip()
                    if group_id.isdigit() and new_facilitator:
                        cursor.execute('''
                        UPDATE peer_support_groups SET facilitator_id = ?
                        WHERE group_id = ?
                        ''', (new_facilitator, group_id))
                        conn.commit()
                        if cursor.rowcount > 0:
                            print("Group facilitator updated.")
                        else:
                            print("Group not found.")
                    else:
                        print("Invalid group ID or facilitator ID.")

                elif mod_choice == '4':
                    group_id = input("Enter group ID to archive: ").strip()
                    if group_id.isdigit():
                        cursor.execute('''
                        UPDATE peer_support_groups SET status = 'archived'
                        WHERE group_id = ?
                        ''', (group_id,))
                        conn.commit()
                        if cursor.rowcount > 0:
                            print("Group archived.")
                        else:
                            print("Group not found.")
                    else:
                        print("Invalid group ID.")
...
def display_admin_menu():
    ...
    global auth
    from education_system.post_18.university_system.modules.domain.student_affairs.student_union.administration.student_union_core import (
        set_up_election,
        manage_union_reps,
