...
class AchievementBadgeGUI:
    def __init__(self, parent=None, auth=None):
        ...
        try:
            from education_system.post_18.university_system.modules.domain.student_affairs.achievement_badges.services.achievement_badge_service import AchievementBadgeService
    ...
    def _create_progress_tab(self, notebook):
        tab = ttk.Frame(notebook)
        notebook.add(tab, text="Progress")

        cols = ("badge", "progress", "percent")
        self.progress_tree = ttk.Treeview(tab, columns=cols, show="headings", height=15)
        for c in cols:
            self.progress_tree.heading(c, text=c.replace("_", " ").title())
            self.progress_tree.column(c, width=180)
        self.progress_tree.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        btn_frame = ttk.Frame(tab)
        btn_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Button(btn_frame, text="Refresh", command=self._refresh_progress).pack(side=tk.RIGHT, padx=2)
    ...
    def _refresh_progress(self):
        ...
        if not self.service:
            return
        try:
            progress = self.service.get_progress(self.student_id)
            for p in progress:
                target = p.get("target_progress", 0) or 0
                current = p.get("current_progress", 0) or 0
                pct = int(current / target * 100) if target > 0 else 0
                self.progress_tree.insert("", tk.END, values=(
                    p.get("name", "Badge %s" % p.get("badge_id", "")),
                    f"{current}/{target}", f"{pct}%",
                ))
        except Exception as e:
            logger.error(f"Error loading badge progress: {e}")
    ...
    def _create_statistics_tab(self, notebook):
        ...
        self.stats_text = tk.Text(tab, height=20, width=60, state="disabled")
        self.stats_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        btn_frame = ttk.Frame(tab)
        btn_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Button(btn_frame, text="Refresh", command=self._refresh_statistics).pack(side=tk.RIGHT, padx=2)
    ...
    def _refresh_statistics(self):
        if not self.service:
            return
        try:
            stats = self.service.get_badge_statistics()
            lines = [
                f"Total badge types: {stats['total_badges']}",
                f"Total awarded: {stats['total_awarded']}",
            ...
            logger.error(f"Error loading badge statistics: {e}")
