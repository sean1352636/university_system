...
from education_system.post_18.university_system.modules.domain.finance.reporting.revenue_analytics.forecasting import generate_revenue_forecast
...
class AnalyticsManager:
    ...
    def create_analytics_tab(self):
        ...
        # Create matplotlib figure. Use a standalone Figure() rather than
        # plt.subplots(): pyplot registers figures in a global manager, so an
        # embedded GUI figure can outlive its Tk widget and get redrawn while
        # detached (-> "'NoneType' object has no attribute 'dpi'").
        self.fig = Figure(figsize=(12, 8))
        ((self.ax1, self.ax2), (self.ax3, self.ax4)) = self.fig.subplots(2, 2)
        self.fig.tight_layout(pad=3.0)

        self.canvas = FigureCanvasTkAgg(self.fig, charts_frame)
        self.canvas.get_tk_widget().pack(fill='both', expand=True)

        # Tear the canvas down when its frame is destroyed so no queued
        # idle_draw fires on a detached figure (-> NoneType .dpi crash).
        self._charts_frame = charts_frame
        charts_frame.bind("<Destroy>", self._on_charts_destroy)

        # Load initial dashboard
        self.update_dashboard_charts()

    def _on_charts_destroy(self, event=None):
        """Release the embedded chart canvas/figure on widget teardown."""
        # The <Destroy> event bubbles up from child widgets too; only act
        # when the charts frame itself is going away.
        import matplotlib.pyplot as plt
        if event is not None and event.widget is not getattr(self, '_charts_frame', event.widget):
            return
    ...
    def _safe_draw(self):
        """Draw the chart canvas only if it still exists and is mapped.

        Guards against canvas.draw() running after the Tk widget has been
        destroyed (tab switch / window close), which detaches the figure
        and raises "'NoneType' object has no attribute 'dpi'".
        """
        canvas = getattr(self, 'canvas', None)
        if canvas is None:
            return
    ...
    def update_dashboard_charts(self):
        ...
        try:
            ...
            # Bail if the canvas was torn down (tab/window closed) — a
            # deferred after(0) update can still land after destroy.
            ...
            if hasattr(self, 'ax1') and getattr(self, 'canvas', None) is not None:
                for ax in [self.ax1, self.ax2, self.ax3, self.ax4]:
                    ax.clear()
