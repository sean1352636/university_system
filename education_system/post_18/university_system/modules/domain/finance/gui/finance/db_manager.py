...
# The collections / payment-arrangement / db-maintenance workflow functions live
# across several finance modules (they are NOT in financial_core, which is why the
# previous single import here always failed and every function silently degraded to
# a "not implemented" print stub). Import each group from its real location, with a
# per-group stub fallback so the GUI still loads if one module is unavailable.
...
try:
    from education_system.post_18.university_system.modules.domain.finance.reporting.revenue_analytics.collections import (
        assign_to_collection_agency, track_collection_progress,
        update_collection_case_status,
    )
except ImportError as _e:
    print(f"⚠️ collections workflow unavailable: {_e}")
    assign_to_collection_agency = _make_missing_stub('assign_to_collection_agency')
    track_collection_progress = _make_missing_stub('track_collection_progress')
    update_collection_case_status = _make_missing_stub('update_collection_case_status')

try:
    from education_system.post_18.university_system.modules.domain.finance.reporting.revenue_analytics.agencies import (
        setup_collection_workflows,
    )
except ImportError as _e:
    print(f"⚠️ collection workflows unavailable: {_e}")
    setup_collection_workflows = _make_missing_stub('setup_collection_workflows')

try:
    from education_system.post_18.university_system.modules.domain.finance.core.aid import (
        create_payment_arrangement,
    )
except ImportError as _e:
    print(f"⚠️ payment arrangements unavailable: {_e}")
    create_payment_arrangement = _make_missing_stub('create_payment_arrangement')

try:
    from education_system.post_18.university_system.modules.domain.finance.core.communications import (
        send_arrangement_confirmation,
    )
except ImportError as _e:
    print(f"⚠️ arrangement confirmations unavailable: {_e}")
    send_arrangement_confirmation = _make_missing_stub('send_arrangement_confirmation')
...
try:
    ...
    print(f"⚠️ db maintenance helpers unavailable: {_e}")
    check_required_packages = _make_missing_stub('check_required_packages')
    ensure_database_exists = _make_missing_stub('ensure_database_exists')
    verify_fix = _make_missing_stub('verify_fix')
    ...
    def backup_database(self):
        ...
        try:
            ...
            if university_system_root:
                ...
                from education_system.post_18.university_system.core.paths import BACKUP_DIR
