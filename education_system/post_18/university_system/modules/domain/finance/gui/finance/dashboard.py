...
class DashboardManager:
    ...
    def gui_generate_financial_dashboard(self):
        ...
        try:
            ...
            # Run on the Tk main loop, not a worker thread: update_dashboard_charts
            # mutates matplotlib axes and calls canvas.draw(), which is not
            # thread-safe. Drawing off-thread races Tk's idle_draw and crashes
            # with "'NoneType' object has no attribute 'dpi'" on a detached
            # figure. Mirrors update_dashboard_stats/load_recent_activity above.
            self.root.after(0, generate_dashboard)
