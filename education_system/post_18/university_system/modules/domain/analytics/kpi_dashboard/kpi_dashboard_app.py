...
from education_system.post_18.university_system.modules.domain.analytics.kpi_dashboard import (  # noqa: E402
    KpiDashboardService,
    KpiDashboardError,
)
...
class _Frame(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Button(ctrl, text="View KPI Detail", command=self._view_kpi_detail).pack(side="left", padx=4)
        tk.Button(ctrl, text="View Dashboard", command=self._view_dashboard).pack(side="left", padx=4)
        tk.Button(ctrl, text="List Dashboards", command=self._list_dashboards).pack(side="left", padx=4)
        ...
        self._tree.bind("<Double-1>", lambda e: self._view_kpi_detail())
    ...
    def _view_kpi_detail(self):
        """Show detail fields for the selected KPI (mirrors CLI _view_kpi)."""
        sel = self._tree.selection()
        if not sel:
            messagebox.showinfo("View KPI Detail", "Select a KPI row first.")
            return
        try:
            kid = int(self._tree.item(sel[0])["values"][0])
        except (ValueError, IndexError):
            messagebox.showerror("Error", "Could not read the selected KPI id.")
            return
        try:
            kpi = self._svc.get_kpi(kid)
            st = self._svc.kpi_status(kpi)
        except KpiDashboardError as e:
            messagebox.showerror("Error", str(e)); return
        win = tk.Toplevel(self); win.title(f"KPI #{kid}"); win.geometry("460x360")
        txt = tk.Text(win, wrap="word"); txt.pack(fill="both", expand=True, padx=10, pady=10)
        prog = f"{st['progress_pct']}%" if st.get("progress_pct") is not None else "-"
        txt.insert("end", f"KPI #{kpi['kpi_id']} — {kpi.get('kpi_name')}\n")
        txt.insert("end", "-" * 40 + "\n")
        txt.insert("end", f"Category:        {kpi.get('kpi_category') or '-'}\n")
        txt.insert("end", f"Period:          {kpi.get('period') or '-'}\n")
        txt.insert("end", f"Trend:           {kpi.get('trend') or '-'}\n")
        txt.insert("end", f"Current value:   {kpi.get('current_value')}\n")
        txt.insert("end", f"Target value:    {kpi.get('target_value')}\n")
        txt.insert("end", f"Measured on:     {kpi.get('measurement_date') or '-'}\n")
        txt.insert("end", f"Status:          {st.get('status')}\n")
        txt.insert("end", f"Progress:        {prog}\n")
        txt.insert("end", f"Gap to target:   {st.get('gap_to_target')}\n")
        txt.config(state="disabled")

    def _list_dashboards(self):
        """List dashboards by owner / public visibility (mirrors CLI _list_dashboards)."""
        owner = simpledialog.askstring(
            "List Dashboards", "Filter by owner_id (blank for all):", parent=self)
        if owner is None:
            return
        owner = owner.strip() or None
        public_only = messagebox.askyesno("List Dashboards", "Public dashboards only?")
        try:
            rows = self._svc.list_dashboards(owner_id=owner, public_only=public_only)
        except KpiDashboardError as e:
            messagebox.showerror("Error", str(e)); return
        win = tk.Toplevel(self); win.title("Dashboards"); win.geometry("760x360")
        cols = ("id", "name", "type", "owner", "public", "updated")
        widths = {"id": 50, "name": 210, "type": 120, "owner": 120, "public": 60, "updated": 170}
        tree = ttk.Treeview(win, columns=cols, show="headings")
        for c in cols:
            tree.heading(c, text=c.title()); tree.column(c, width=widths[c])
        tree.pack(fill="both", expand=True, padx=10, pady=10)
        if not rows:
            tree.insert("", "end", values=("-", "No dashboards found.", "-", "-", "-", "-"))
            return
        for d in rows:
            tree.insert("", "end", values=(
                d.get("dashboard_id"),
                d.get("dashboard_name") or "-",
                d.get("dashboard_type") or "-",
                d.get("owner_id") or "-",
                "Yes" if d.get("is_public") else "No",
