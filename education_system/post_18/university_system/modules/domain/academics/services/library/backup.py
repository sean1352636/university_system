...
from education_system.post_18.university_system.modules.domain.academics.services.library.settings import get_current_user_id, get_library_settings, update_library_setting, validate_library_configuration
...
def _build_backup_script(script_path):
    """Return the text of a self-contained shell backup script.

    The script mirrors the real backup routine in :func:`enhanced_system_backup`
    (copy the consolidated database plus the qr_codes / digital_library /
    cover_images asset directories into a timestamped folder under BACKUP_DIR),
    writes a manifest, and prunes folders older than 7 days.
    """
    backup_root = str(BACKUP_DIR)
    return f'''#!/bin/bash
# Library System Backup Script
# Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} by setup_scheduled_backups()
set -euo pipefail
...
cp "{DATABASE_FILE}" "$BACKUP_PATH/library_database.db"

# Backup additional asset directories if present
...
def _install_cron_job(script_path, schedule="0 2 * * *"):
    """Install a cron entry running *script_path* on *schedule* (POSIX only).

    Reads the current crontab, appends the job only if an entry for this exact
    script is not already present, and writes the result back via ``crontab -``.
    Returns a human-readable status string. Never raises for the common
    "no crontab yet" case.
    """
    job_line = f"{schedule} {script_path}  # library_backup"
    try:
        ...
        return "cron is not available on this system; install the job manually."

    # `crontab -l` exits non-zero with "no crontab" when none exists yet.
    current = existing.stdout if existing.returncode == 0 else ""

    if script_path in current:
        return "cron entry already present; left unchanged."

    new_crontab = current
    if new_crontab and not new_crontab.endswith("\n"):
        new_crontab += "\n"
    new_crontab += job_line + "\n"
    ...
    if result.returncode != 0:
        return f"failed to install cron entry: {result.stderr.strip()}"
    return f"installed cron entry: {job_line}"


def setup_scheduled_backups():
    """Setup scheduled backups: write a real backup script and offer cron install."""
    print("\nScheduled Backup Setup:")
    print("======================")

    # Write the generated backup script to a real, executable file on disk.
    scripts_dir = BACKUP_DIR / "scripts"
    scripts_dir.mkdir(parents=True, exist_ok=True)
    script_path = scripts_dir / "library_backup.sh"

    script_text = _build_backup_script(str(script_path))
    with open(script_path, "w") as f:
        f.write(script_text)

    # Make the script executable (rwx for owner, rx for group/other).
    ...
    print(f"✅ Backup script written and marked executable: {script_path}")

    if platform.system() == "Windows":
        print("Windows detected: cron is unavailable.")
        print("Schedule the script with Task Scheduler, for example:")
        print(f'   schtasks /Create /SC DAILY /ST 02:00 /TN "LibraryBackup" '
              f'/TR "bash {script_path}"')
        return

    # POSIX: offer to install a real cron entry.
    answer = input("Install a daily 2 AM cron job for this script now? (y/N): ").strip().lower()
    if answer in ("y", "yes"):
        status = _install_cron_job(str(script_path))
        print(f"cron: {status}")
    else:
        print("Skipped cron installation. To schedule it yourself, add to crontab:")
        print(f"   0 2 * * * {script_path}  # Daily at 2 AM")
