...
from education_system.post_18.university_system.infrastructure.security.password_generator import generate_temp_password
import datetime
import random
...
class AccountsMixin:
    ...
    def create_parent_account_interactive(auth):
        ...
        from education_system.post_18.university_system.modules.domain.academics.services.parent_portal.portal import ParentPortal
    ...
    def update_profile_photo(self):
        ...
        try:
            ...
            if photo_path.lower() == 'remove':
                ...
                source = Path(photo_path.strip()).expanduser()
                if not source.is_file():
                    print(f"Error: no file found at '{source}'. Profile photo not changed.")
                    return

                if source.stat().st_size > MAX_PHOTO_BYTES:
                    print("Error: image exceeds the 5 MB size limit. Profile photo not changed.")
                    return

                # Verify the file is a genuine image before accepting it.
                try:
                ...
                # Copy the validated image into the managed uploads directory.
                dest_dir = paths.UPLOADS_DIR / "parent_photos"
                dest_dir.mkdir(parents=True, exist_ok=True)
                suffix = source.suffix.lower() or ".img"
                timestamp = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
                dest_path = dest_dir / f"{parent_id}_{timestamp}{suffix}"
                shutil.copy2(source, dest_path)

                photo_path = str(dest_path)
                print(f"Profile photo uploaded and stored at: {photo_path}")
    ...
    def enable_two_factor_auth(self):
        ...
        try:
            ...
            # Determine an account label for the provisioning URI
            cursor.execute('SELECT email FROM parent_accounts WHERE parent_id = ?', (parent_id,))
            email_row = cursor.fetchone()
            account_name = (email_row[0] if email_row and email_row[0]
                            else self.auth.current_user.get('username', parent_id))

            # Generate a real TOTP secret (RFC 6238 base32)
            import pyotp
            secret = pyotp.random_base32()
            totp = pyotp.TOTP(secret)
            provisioning_uri = totp.provisioning_uri(
                name=account_name, issuer_name="University Parent Portal"
            )
            ...
            print("or scan the following provisioning URI as a QR code:")
            print(f"{provisioning_uri}")

            # Require the user to prove they added the secret before enabling.
            code = input("\nEnter the current 6-digit code from your authenticator app: ").strip()
            if not totp.verify(code, valid_window=1):
                print("Verification failed. The code did not match. Two-factor "
                      "authentication was NOT enabled.")
                return

            # Verified — persist the secret and enable 2FA.
            cursor.execute('''
            UPDATE parent_accounts
            ...
            print("You will be asked for a code from your authenticator app on future logins.")
