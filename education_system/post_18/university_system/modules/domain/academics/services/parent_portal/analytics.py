    ...
    def generate_qr_code(self):
        ...
        try:
            ...
            try:
                ...
                # Render the QR matrix to a real PNG file with matplotlib.
                import matplotlib
                ...
                ax.set_axis_off()
                ax.set_title(f"Pickup QR - {selected_child[1]} {selected_child[3]}", fontsize=9)

                paths.QR_CODES_DIR.mkdir(parents=True, exist_ok=True)
                timestamp = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
                out_path = paths.QR_CODES_DIR / f"pickup_{parent_id}_{student_id}_{timestamp}.png"
                fig.savefig(str(out_path), dpi=150, bbox_inches="tight")

                print(f"\nQR Code generated for {selected_child[1]} {selected_child[3]}")
                print("QR Code data:", qr_string)
                print(f"QR code image saved to: {out_path}")
                print("This QR code can be used for secure student pickup.")

            except Exception as e:
                print(f"Error generating QR code: {e}")
                print("QR code generation requires the qrcode and matplotlib libraries.")
