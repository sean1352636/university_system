...
class MobileAPIManager:
    ...
    def __init__(self, calendar_manager):
        ...
        # The Flask app (and its CORS config + routes) is built lazily on
        # first access to .app. Constructing the calendar happens at startup,
        # but the mobile API server is rarely started — building Flask eagerly
        # here spun up an app and emitted the production CORS warning on every
        # launch for a server nobody was serving. Deferring it keeps startup
        # quiet and moves the (legitimate) CORS check to when the API is
        # actually used.
        self._app = None

    @property
    def app(self):
        """The Flask application, built on first access."""
        if self._app is None:
            if not FLASK_AVAILABLE:
                raise RuntimeError(
                    "Flask is not available; the mobile API cannot be served")
            self._app = Flask(__name__)
            CORS(self._app, origins=_get_cors_origins(), supports_credentials=True)
