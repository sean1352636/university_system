...
class ConflictsMixin:
    ...
    def detect_all_conflicts(self):
        ...
        # Room-capacity overflow conflicts (enrolment exceeds room size)
    ...
    def _detect_capacity_conflicts(self):
        ...
        room's capacity. Complements the double-booking detectors: a room
        can be free yet still too small for the class booked into it."""
        conn = get_connection(self.db_path, row_factory=False)
        cursor = conn.cursor()
        ...
        try:
            ...
            # Missing tables (e.g. student_modules) → no capacity data to
            # check. Mirror _detect_student_conflicts' defensive behaviour.
            conn.close()
            return []
