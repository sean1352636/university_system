...
def display_external_examiner_menu(auth):
    from education_system.post_18.university_system.modules.domain.academics.external_examiners.services.external_examiner_service import ExternalExaminerService
    ...
    while True:
        ...
        print("  9. Edit Examiner")
        print(" 10. View Module Descriptor")
        print("\n  0. Back")
        print("=" * 70)
        ...
        if choice == '0':
            ...
            eid = int(input("Examiner ID: ").strip())
            updates = {}
            for label, field in (("Name", "name"), ("Email", "email"),
                ...
                if val:
                    updates[field] = val
            if updates:
                service.update_examiner(eid, **updates)
                print("\nExaminer updated.")
            else:
                print("\nNothing to update.")
        elif choice == '10':
            ...
            matches = [d for d in curr.list_module_descriptors() if d['module_code'] in modules_reviewed]
            if not matches:
                print("\nNo curriculum-spec descriptors match the modules for this visit.")
            else:
                print(f"\n{'Code':<10} {'Ver':<6} {'Title':<35} {'Cr':<5} {'Leader':<20}")
                print("-" * 76)
                for d in matches:
                    print(f"{d['module_code']:<10} {d['version']:<6} "
                          f"{(d['title'] or '')[:33]:<35} {(d.get('credits') or 0):<5} "
                          f"{(d.get('module_leader') or ''):<20}")
