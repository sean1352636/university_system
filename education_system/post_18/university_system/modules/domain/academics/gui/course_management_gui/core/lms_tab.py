...
try:
    ...
    from education_system.post_18.university_system.modules.domain.academics.services.lms.db_schema import initialize_lms_database
...
class LMSTabMixin:
    ...
    def _create_lms_discussions_subtab(self):
        ...
        # Double-clicking a forum opens the interactive discussion board.
        self.lms_forums_tree.bind('<Double-1>', lambda e: self._lms_view_posts())

        ttk.Label(
            tab,
            text="Tip: select a forum and click 'View Posts' (or double-click it) "
                 "to read the thread, reply, and like messages.",
            foreground="#7f8c8d",
    ...
    def _lms_view_posts(self):
        ...
        forum_vals = self.lms_forums_tree.item(sel[0])['values']
        forum_id = forum_vals[0]
        forum_topic = forum_vals[1] if len(forum_vals) > 1 else f"Forum {forum_id}"

        dlg = tk.Toplevel(self.root)
        dlg.title(f"Discussion: {forum_topic}")
        dlg.geometry("920x700")
        dlg.minsize(760, 560)
        dlg.transient(self.root)

        header = ttk.Frame(dlg, padding=(10, 8)); header.pack(fill=tk.X)
        ttk.Label(header, text=forum_topic, font=("Arial", 14, "bold")).pack(side=tk.LEFT)

        # Threaded post tree: the tree column shows the message snippet, with
        # replies nested under the post they answer.
        body = ttk.Frame(dlg, padding=(10, 0)); body.pack(fill=tk.BOTH, expand=True)
        cols = ("author", "date", "likes")
        tree = ttk.Treeview(body, columns=cols, show="tree headings", height=13)
        tree.heading("#0", text="Message"); tree.column("#0", width=420, stretch=True)
        tree.heading("author", text="Author"); tree.column("author", width=150, anchor=tk.W)
        tree.heading("date", text="Date"); tree.column("date", width=150, anchor=tk.W)
        tree.heading("likes", text="Likes"); tree.column("likes", width=60, anchor=tk.CENTER)
        vsb = ttk.Scrollbar(body, orient=tk.VERTICAL, command=tree.yview)
        tree.configure(yscrollcommand=vsb.set)
        tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        vsb.pack(side=tk.RIGHT, fill=tk.Y)

        # Full text of the selected post.
        detail = ttk.LabelFrame(dlg, text="Selected message", padding=8)
        detail.pack(fill=tk.X, padx=10, pady=5)
        detail_txt = ScrolledText(detail, wrap=tk.WORD, height=6)
        ...
        def load_posts():
            ...
            by_parent = {}
            for p in posts:
                by_parent.setdefault(p.get('parent_post_id'), []).append(p)
            inserted = set()

            def insert_children(parent_key, tk_parent):
                for p in by_parent.get(parent_key, []):
                    pid = p.get('post_id')
                    snippet = (p.get('content') or '').replace('\n', ' ').strip()
                    if len(snippet) > 70:
                        snippet = snippet[:67] + '...'
                    iid = str(pid)
                    tree.insert(tk_parent, tk.END, iid=iid, text=snippet or "(no text)",
                                values=(p.get('user_id'), p.get('created_at'), p.get('likes_count', 0)))
                    post_map[iid] = p
                    inserted.add(pid)
                    insert_children(pid, iid)

            insert_children(None, "")
            # Surface any posts whose parent is missing so nothing is hidden.
            for p in posts:
                if p.get('post_id') not in inserted:
                    pid = p.get('post_id')
                    snippet = (p.get('content') or '').replace('\n', ' ').strip()
                    if len(snippet) > 70:
                        snippet = snippet[:67] + '...'
                    iid = str(pid)
                    tree.insert("", tk.END, iid=iid, text=snippet or "(no text)",
                                values=(p.get('user_id'), p.get('created_at'), p.get('likes_count', 0)))
                    post_map[iid] = p
            ...
            if post_map:
                ...
                _set_detail("Select a message to read it in full, then use the buttons below to reply or like.")
            else:
                _set_detail("No messages yet. Write the first message below and click 'Post as New Topic'.")
        ...
        def on_select(_e=None):
            iid = _selected_iid()
            if iid and iid in post_map:
                p = post_map[iid]
                _set_detail(
                    f"{p.get('user_id')}  ·  {p.get('created_at')}  ·  "
                    f"{p.get('likes_count', 0)} likes\n"
                    f"{'-' * 60}\n{p.get('content') or ''}")

        tree.bind('<<TreeviewSelect>>', on_select)

        # Compose area.
        compose = ttk.LabelFrame(dlg, text="Write a message", padding=8)
        compose.pack(fill=tk.X, padx=10, pady=(0, 10))
        compose_txt = tk.Text(compose, wrap=tk.WORD, height=4)
        compose_txt.pack(fill=tk.X, pady=(0, 6))

        def _read_compose():
            text = compose_txt.get('1.0', tk.END).strip()
            if not text:
                messagebox.showwarning(_("common.warning"), "Enter a message first.")
                return None
            return text
        ...
        def post_new():
            ...
            if not content:
                return
            try:
                pid = LMSDiscussionManager.add_post(
                    forum_id=forum_id, user_id=self._lms_user_id, content=content)
                log_activity('create', 'lms_post', str(pid), {'forum_id': forum_id})
                compose_txt.delete('1.0', tk.END)
                load_posts()
            except Exception as e:
                messagebox.showerror(_("common.error"), f"{_('lms.messages.failed_add_post')}: {e}")

        def post_reply():
            parent = _selected_iid()
            if not parent:
                messagebox.showwarning(_("common.warning"), "Select a message to reply to."); return
            content = _read_compose()
            if not content:
                return
            try:
                pid = LMSDiscussionManager.add_post(
                    forum_id=forum_id, user_id=self._lms_user_id, content=content,
                    parent_post_id=int(parent))
                log_activity('create', 'lms_post', str(pid), {'forum_id': forum_id, 'parent': parent})
                compose_txt.delete('1.0', tk.END)
                load_posts()
                if parent in tree.get_children("") or parent in post_map:
                    try:
                        tree.see(parent); tree.item(parent, open=True)
                    except Exception:
                        pass
            except Exception as e:
                messagebox.showerror(_("common.error"), f"{_('lms.messages.failed_add_post')}: {e}")
        ...
        def like_selected():
            iid = _selected_iid()
            if not iid:
                messagebox.showwarning(_("common.warning"), "Select a message to like."); return
            try:
                LMSDiscussionManager.like_post(int(iid))
                load_posts()
                try:
        ...
        btns = ttk.Frame(compose); btns.pack(fill=tk.X)
        ttk.Button(btns, text="Post as New Topic", command=post_new).pack(side=tk.LEFT, padx=3)
        ttk.Button(btns, text="Reply to Selected", command=post_reply).pack(side=tk.LEFT, padx=3)
        ttk.Button(btns, text="👍 Like Selected", command=like_selected).pack(side=tk.LEFT, padx=3)
        ttk.Button(btns, text="Refresh", command=load_posts).pack(side=tk.LEFT, padx=3)
        ttk.Button(btns, text="Close", command=dlg.destroy).pack(side=tk.RIGHT, padx=3)
        ...
        def do_enrol():
            ...
            try:
                with get_connection() as conn:
                    ...
                    conn.commit()
                messagebox.showinfo(_("common.success"), "Successfully enrolled!")
    ...
    def _create_lms_modules_subtab(self):
        ...
        ttk.Label(top, text="Course ID / Code:").pack(side=tk.LEFT, padx=5)
        self._sh_mod_course_var = tk.StringVar()
        ttk.Entry(top, textvariable=self._sh_mod_course_var, width=14).pack(side=tk.LEFT, padx=5)
    ...
    def _sh_resolve_course(self, raw, silent=False):
        """Resolve a user-entered course identifier to the canonical
        ``courses.id`` value, or ``None`` when it doesn't match a real course.

        The shared ``lms_modules.course_id`` column has no foreign-key
        constraint and, in the university database, ``courses.id`` is a text
        code (e.g. ``CS``) rather than an integer. A plain ``isdigit()`` check
        therefore both accepts any number and rejects every real course. This
        instead matches the entry against the course id, code, or name so only
        genuine courses are accepted. Unless ``silent``, a warning is shown
        explaining what to enter.
        """
        val = (raw or "").strip()
        if not val:
            if not silent:
                messagebox.showwarning("LMS", "Please enter a Course ID or code.")
            return None
        try:
            with get_connection() as conn:
                cursor = conn.cursor()
                ...
                if not cursor.fetchone():
                    if not silent:
                        messagebox.showwarning("LMS", "No courses are available to assign content to.")
                    return None
                row = cursor.execute(
                    """
                    SELECT id FROM courses
                    WHERE CAST(id AS TEXT) = ?
                       OR LOWER(COALESCE(course_code, code)) = LOWER(?)
                       OR LOWER(COALESCE(course_name, name)) = LOWER(?)
                    LIMIT 1
                    """,
                    (val, val, val),
                ).fetchone()
            ...
            if not silent:
                messagebox.showwarning("LMS", f"Could not validate the course: {e}")
            return None
        if not row:
            if not silent:
                messagebox.showwarning(
                    "LMS",
                    f"'{val}' is not an existing course. Enter a valid Course "
                    "ID or code (e.g. CS).",
                )
            return None
        return row[0]
    ...
    def _sh_load_modules(self):
        if not self._sh_content_svc:
            return
        cid = self._sh_resolve_course(self._sh_mod_course_var.get())
        if cid is None:
            return
    ...
    def _create_lms_staff_progress_subtab(self):
        ...
        ttk.Entry(top, textvariable=self._sh_prog_course_var, width=14).pack(side=tk.LEFT, padx=5)
    ...
    def _sh_load_course_stats(self):
        if not self._sh_progress_svc:
            return
        cid = self._sh_resolve_course(self._sh_prog_course_var.get())
        if cid is None:
            return
    ...
    def _create_lms_current_lesson_subtab(self):
        ...
        ttk.Entry(top, textvariable=self._sh_cur_course_var, width=14).pack(side=tk.LEFT, padx=5)
    ...
    def _sh_load_next_lesson(self):
        ...
        for w in self._sh_lesson_display.winfo_children():
            w.destroy()
        cid = self._sh_resolve_course(self._sh_cur_course_var.get())
        if cid is None:
            return
    ...
    def _create_lms_student_progress_subtab(self):
        ...
        ttk.Entry(top, textvariable=self._sh_my_prog_course_var, width=14).pack(side=tk.LEFT, padx=5)
    ...
    def _sh_view_my_progress(self):
        ...
        for w in self._sh_my_prog_frame.winfo_children():
            w.destroy()
        cid = self._sh_resolve_course(self._sh_my_prog_course_var.get())
        if cid is None:
            return
        sid = self._lms_user_id or ""
