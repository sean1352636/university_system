...
class DialogsMixin:
    ...
    def show_export_pdf(self):
        ...
        from education_system.post_18.university_system.modules.domain.academics.gui.course_management_gui.import_export.import_export import export_pdf
        try:
            export_pdf(self)
        except Exception as e:
    ...
    def show_recommendations(self):
        ...
        from education_system.post_18.university_system.modules.domain.academics.gui.course_management_gui.recommendations.recommendations_dialog import RecommendationsDialog
        dialog = RecommendationsDialog(self.root)
        if dialog.result:
            # Display recommendations in analytics tab (text surface)
            self.notebook.select(2)  # Analytics tab
            if hasattr(self, "show_analytics_text"):
                self.show_analytics_text()
