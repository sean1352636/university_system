...
class LibraryGUI:
    def __init__(self, master, auth=None):
        ...
        # Clock poll loop bookkeeping. Cancelling the pending after() on
        # teardown (rather than letting master.destroy() delete its callback
        # command out from under a queued timer) prevents a Tcl "invalid
        # command name ...update_time" background error.
        ...
        if hasattr(self.master, 'wm_state'):
            # Match Finance Management's sizing approach: a fixed
            # 1400x900 with a 1200x800 minsize, no programmatic
            # maximise. On a typical 1366x768 desktop Tk clips the
            # request to the work area, which the user perceives as
            # "fills the screen" — same shape Finance uses.
            #
            # Pre-8.117.36 tried a 3-stage chain (state('zoomed') →
            # wm_attributes('-zoomed') → screen-sized geometry).
            # That was inconsistent with the rest of the app, and on
            # WMs where the maximised state stuck, the window
            # animated to maximised after construction — visible
            # flicker. The plain geometry call is what Finance does
    ...
    def get_db_connection():
        ...
        if ORIGINAL_LIBRARY_AVAILABLE:
            ...
            try:
                from education_system.post_18.university_system.modules.domain.academics.services.library.database import get_db_connection as original_get_db_connection
    ...
    def update_time(self):
        ...
        try:
            ...
            if not self._closing:
                self._time_after_id = self.master.after(1000, self.update_time)
        except (tk.TclError, Exception):
            # Window destroyed, don't schedule next update
            pass

    def _on_master_destroy(self, event):
        """Stop the clock loop when the host window/frame is destroyed.

        Bound to <Destroy> so it runs on every teardown path (WM close,
        return-to-menu, or the workspace notebook closing our tab). Cancelling
        here — while the after callback command still exists — avoids the Tcl
        'invalid command name' race that master.destroy() would otherwise cause.
        """
        ...
        if self._time_after_id is not None:
            try:
                self.master.after_cancel(self._time_after_id)
            except tk.TclError:
                pass
            self._time_after_id = None
        ...
        try:
            ...
            if not self.auth:
                if SHARED_AUTH_AVAILABLE:
                    ...
                    self.auth = get_auth()
                    print("Library GUI connected to global auth context")
                else:
                    # Create new auth instance if shared context not available
                    self.auth = UserAuth()
                    print("Library GUI created standalone auth instance")
    ...
    def setup_event_handlers(self):
        ...
        # Stop the clock poll loop on any teardown path (window close, tab
        # close, return-to-menu). add="+" so we don't clobber other bindings.
        try:
