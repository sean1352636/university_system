...
class AddScheduleDialog:
    ...
    def save_schedule(self):
        try:
            ...
            # Cross-model clash check: the module scheduler's own check_conflicts
            # only sees other module_schedule rows. This also warns when the room
            # is used at the same time by a Course Management section meeting.
            try:
                from education_system.post_18.university_system.modules.domain.academics.services.timetable_bridge import (
                    find_timetable_conflicts,
                )
                room_label = room_str.split(' - ', 1)[1] if ' - ' in room_str else room_str
                section_clashes = [
                    c for c in find_timetable_conflicts(
                        day, start_time, end_time, location=room_label, room_id=room_id)
                    if c.startswith("Section ")]
            except Exception:
                section_clashes = []
            if section_clashes and self.status_var.get() != "draft":
                if not messagebox.askyesno(
                        "Section clash",
                        "This room/time clashes with course section meetings:\n\n"
                        + "\n".join(f"  • {c}" for c in section_clashes[:6])
                        + "\n\nSave anyway?",
...
class AddInstructorDialog:
    ...
    def save_instructor(self):
        try:
            ...
            # Delegate to the unified service so an instructor added here also
            # lands in the staff directory / login stores — not just the
            # instructors table. (One code path shared with course management.)
            ...
            result = create_instructor_service(
                ...
                last_name=last_name,
                email=email,  # user-typed address
            ...
            if not result.ok:
                messagebox.showerror("Error", result.error or "Failed to save instructor.",
                                     parent=self.dialog)
                return

            self.result = True
            msg = f"Instructor {first_name} {last_name} added successfully."
...
class EditScheduleDialog:
    ...
    def _show_history(self):
        ...
        try:
            from education_system.post_18.university_system.modules.domain.academics.gui.module_scheduling.history_dialog import ScheduleHistoryDialog
