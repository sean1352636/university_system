...
try:
    from education_system.post_18.university_system.modules.domain.academics.services.module_scheduling.constants import (
        DAYS_OF_WEEK,
        TIME_SLOTS,
    )
except ImportError:
...
class StudentTimetableGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Button(
            btn_frame, text=_t("Export to Calendar (.ics)"),
    ...
    def export_to_ics(self):
        """Export the current weekly timetable to an .ics calendar file."""
        if not self.schedule_rows:
            messagebox.showinfo(
                _t("Export Timetable"),
                _t("No schedule data to export. Try Refresh first."),
                parent=self.parent,
            )
            return
        ...
        for row in self.schedule_rows:
            # schedule_rows tuples: (module_code, module_name, day_of_week,
            # start_time, end_time, building, room_number, first_name,
            # last_name, session_type)
            (module_code, module_name, day, start, end, building,
             room_number, first_name, last_name, session_type) = row
            room = (
                f"{building}-{room_number}"
                if building and room_number else (room_number or "TBA")
            )
            instructor = " ".join(p for p in (first_name, last_name) if p)
            title = module_name or module_code
            if session_type:
                title = f"{title} ({session_type})"
            desc_bits = [f"Module: {module_code}"]
            if instructor:
                desc_bits.append(f"Instructor: {instructor}")
            sessions.append({
                "day": day,
                "start": start,
                "end": end,
        ...
        from education_system.post_18.university_system.modules.domain.academics.services.timetable_ics import (
            prompt_and_save_ics,
        )
        cal_name = f"Timetable — {self.student_id}" if self.student_id else "Timetable"
        prompt_and_save_ics(
            self.parent, sessions,
            calendar_name=cal_name,
            default_filename=f"timetable_{self.student_id or 'student'}.ics",
        )
