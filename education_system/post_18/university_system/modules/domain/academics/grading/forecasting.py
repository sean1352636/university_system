...
from education_system.post_18.university_system.modules.domain.academics.grading.grade_calculation.conversions import letter_to_gpa
...
def create_dashboard_visualizations(dashboard_data):
    ...
    # collect_dashboard_data() returns a nested structure; also tolerate a
    # pre-flattened mapping so either shape renders correctly.
    overview = dashboard_data.get('overview', {})
    total_students = overview.get('total_students', dashboard_data.get('total_students', 0))
    total_courses = overview.get('total_modules', dashboard_data.get('total_courses', 0))
    total_grades = overview.get('total_grades', dashboard_data.get('total_grades', 0))

    # Grade distribution: {'distribution': {grade: count}, ...} or a flat {grade: count}.
    grade_dist_raw = dashboard_data.get('grade_distribution', {}) or {}
    if isinstance(grade_dist_raw, dict) and 'distribution' in grade_dist_raw:
        grade_dist = grade_dist_raw.get('distribution', {}) or {}
    else:
        grade_dist = grade_dist_raw

    # Course performance: list of (course, students, avg_score) rows or list of dicts.
    course_avgs = dashboard_data.get('course_performance', dashboard_data.get('course_averages', []))

    # At-risk count: a risk_stats summary dict, or a list/int under at_risk_students.
    risk_stats = dashboard_data.get('risk_stats', {})
    if isinstance(risk_stats, dict) and risk_stats:
        at_risk_count = risk_stats.get('at_risk_students', 0) or 0
    else:
        at_risk = dashboard_data.get('at_risk_students', [])
        at_risk_count = len(at_risk) if isinstance(at_risk, list) else (at_risk or 0)

    # Trend points: recent_trends rows (date, avg_pct) or monthly_trends dicts.
    monthly = dashboard_data.get('recent_trends', dashboard_data.get('monthly_trends', []))
    ...
    if course_avgs:
        courses = [
            (c.get('course', '?') if isinstance(c, dict) else c[0])
            for c in course_avgs[:8]
        ]
        avg_scores = [
            ((c.get('avg_score', 0) if isinstance(c, dict)
              else (c[2] if len(c) > 2 else 0)) or 0)
            for c in course_avgs[:8]
        ]
    ...
    if monthly:
        months = [
            (t.get('month', '') if isinstance(t, dict) else t[0])
            for t in monthly
        ]
        scores = [
            ((t.get('avg_score', 0) if isinstance(t, dict)
              else (t[1] if len(t) > 1 else 0)) or 0)
            for t in monthly
        ]
