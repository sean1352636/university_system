...
def display_clearing_adjustment_menu(auth):
    ...
    while True:
        ...
        print("  9. Statistics")
        print(" 10. Update Vacancy")
        print(" 11. Create APL claim")
        print("\n  0. Back")
        print("=" * 70)
        ...
        if choice == '0':
            ...
            places = input("Available places (Enter to skip): ").strip()
            tariff = input("Minimum tariff (Enter to skip): ").strip()
            updates = {}
            if places:
                updates['available_places'] = int(places)
            if tariff:
                updates['minimum_tariff'] = int(tariff)
            if updates:
                service.update_vacancy(vid, **updates)
                print("\nVacancy updated.")
            else:
                print("\nNothing to update.")
        elif choice == '11':
            aid = int(input("Clearing application ID: ").strip())
            ...
            cid = PriorLearningService().create_claim_from_clearing(aid)
            if cid:
                print(f"\nDraft APL claim #{cid} created from application #{aid}.")
            else:
                print("\nCould not load the clearing application.")
