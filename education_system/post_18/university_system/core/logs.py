...
def _get_state():
    ...
    if state is None:
        try:
            from education_system.post_18.university_system.infrastructure.email import state as _state
            state = _state
        except ImportError:
            try:
                from education_system.post_18.university_system.modules.shared.utils import state as _state
                state = _state
            except ImportError:
                pass
    return state
...
def display_communication_analytics_menu(dashboard):
    ...
    from education_system.post_18.university_system.modules.shared.utils.database import execute_db_operation  # Local import to avoid circular dependency
    from education_system.post_18.university_system.modules.shared.utils.email_service import get_stored_emails  # Local import to avoid circular dependency
