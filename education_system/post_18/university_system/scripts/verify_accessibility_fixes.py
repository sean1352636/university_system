...
def verify_student_lookup():
    ...
    try:
        with get_connection() as conn:
            ...
            row = cursor.fetchone()
            if row:
                print("   ✅ Student lookup query works")
                ...
                try:
                    ...
                    try:
                        if row['notes']:
                            ...
                            print("      Notes field accessible (empty/NULL)")
                    except (KeyError, IndexError):
                        print("      Notes field handled by exception (as designed)")

                except Exception as e:
                    print(f"   ❌ Dictionary-style access failed: {e}")
                    return False
            else:
                print("   ⚠️  No exam accommodations in database to test")
    except Exception as e:
    ...
    try:
        with get_connection() as conn:
            ...
            if fks:
                ...
                if correct_fk:
                    print("   ✅ Foreign key references users(id)")
                else:
                    print("   ⚠️  Foreign key may reference wrong column")
                    for fk in fks:
                        print(f"      Found: {fk['table']}({fk['to']})")
            else:
                print("   ⚠️  No foreign keys found on accessibility_profiles")
    except Exception as e:
