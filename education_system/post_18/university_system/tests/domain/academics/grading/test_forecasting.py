...
class TestDashboardVisualization:
    ...
    @pytest.fixture(autouse=True)
    def _force_agg_backend(self):
        """GUI modules elsewhere call matplotlib.use('TkAgg') at import, which
        leaks process-wide and makes headless rendering fail. Pin Agg for these
        file-rendering tests regardless of prior backend mutations."""
        import matplotlib
    ...
    def test_create_dashboard_visualizations(self, setup_forecast_data, tmp_path, monkeypatch):
        ...
        # A previous GUI test may have replaced matplotlib in sys.modules with a
        ...
        if not hasattr(_plt_check, '__file__'):
            for k in [k for k in sys.modules if k.startswith('matplotlib')]:
                del sys.modules[k]
            import matplotlib  # noqa: F401  # deliberate reimport after MagicMock purge
            matplotlib.use('Agg')
            import matplotlib.pyplot  # noqa: F401, F811
            forecasting_key = 'education_system.post_18.university_system.modules.domain.academics.grading.forecasting'
            if forecasting_key in sys.modules:
                importlib.reload(sys.modules[forecasting_key])

        from education_system.post_18.university_system.modules.domain.academics.grading.progress import collect_dashboard_data
        # Re-import after potential reload so we use the real-matplotlib version.
        from education_system.post_18.university_system.modules.domain.academics.grading.forecasting import create_dashboard_visualizations as _create_viz

        with get_connection() as conn:
            cursor = conn.cursor()
            dashboard_data = collect_dashboard_data(cursor)

        _create_viz(dashboard_data)
    ...
    def test_generate_dashboard_report(self, setup_forecast_data, tmp_path, monkeypatch):
        ...
        from education_system.post_18.university_system.modules.domain.academics.grading.forecasting import generate_dashboard_report as _gen_report
