...
from education_system.post_18.university_system.modules.domain.academics.services.attendance import (
...
class TestQRAttendanceSystem:
    ...
    def setUp(self):
        ...
        # attendance_sessions has a FK to modules(module_code); ensure CS101 exists.
        conn = get_connection()
        try:
            conn.execute(
                "CREATE TABLE IF NOT EXISTS modules "
                "(module_code TEXT PRIMARY KEY, module_name TEXT)"
            )
            conn.execute(
                "INSERT OR IGNORE INTO modules (module_code, module_name) "
                "VALUES ('CS101', 'Intro CS')"
            )
            # attendance_records has a FK to students(student_id); ensure the
            # check-in student exists so inserts aren't rejected. first_name /
            # last_name are NOT NULL in the test schema, so supply them —
            # otherwise INSERT OR IGNORE silently drops the row.
            conn.execute(
                "INSERT OR IGNORE INTO students (student_id, first_name, last_name) "
                "VALUES ('STU001', 'Test', 'Student')"
            )
            conn.commit()
        finally:
            conn.close()
    ...
    @patch('education_system.post_18.university_system.modules.domain.academics.services.attendance.attendance_tracker.update_gamification_points')
