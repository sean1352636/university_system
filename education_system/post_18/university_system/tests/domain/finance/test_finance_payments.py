...
# stripe is an optional dependency; tests that patch stripe.* can only run when
# it's installed (the patch target is imported at setup time).
_HAS_STRIPE = importlib.util.find_spec("stripe") is not None
requires_stripe = pytest.mark.skipif(not _HAS_STRIPE, reason="stripe not installed")

from education_system.post_18.university_system.modules.domain.finance.core.payments import (
    manual_rate_update,
...
class TestManualRateUpdate:
    ...
    @patch('builtins.input', side_effect=['1.0', '1.0', '1.0', '1.0'])
    @patch('builtins.print')
...
class TestProcessStripePayment:
    ...
    def test_process_stripe_payment_success(self, mock_create):
    ...
    @patch('stripe.PaymentIntent.create')
    def test_process_stripe_payment_failed_status(self, mock_create):
    ...
    @patch('stripe.PaymentIntent.create')
    def test_process_stripe_payment_exception(self, mock_create):
    ...
    @patch('stripe.PaymentIntent.create')
    def test_process_stripe_payment_correct_amount_conversion(self, mock_create):
    ...
    @patch('stripe.PaymentIntent.create')
    def test_process_stripe_payment_currency_lowercase(self, mock_create):
    ...
    @patch('stripe.PaymentIntent.create')
    def test_process_stripe_payment_includes_metadata(self, mock_create):
    ...
    @patch('stripe.PaymentIntent.create')
    def test_process_stripe_payment_pending_status(self, mock_create):
...
class TestGenerateQRPaymentCode:
    ...
    @patch('education_system.post_18.university_system.modules.domain.finance.core.payments.qrcode')
...
class TestPaymentIntegration:
    ...
    @patch('stripe.PaymentIntent.create')
    @patch('education_system.post_18.university_system.modules.domain.finance.core.payments.qrcode')
    def test_combined_payment_flow(self, mock_qrcode, mock_create):
