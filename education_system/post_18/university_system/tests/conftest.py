...
# (tests/ -> university_system -> post_18 -> education_system)
...
def _setup_headless_tkinter():
    ...
    try:
        ...
        # Stash the real implementations of every method we're about to
        # neuter, so that @pytest.mark.gui tests which genuinely need live
        # widgets (e.g. counting notebook tabs) can restore them locally
        # via a directory-scoped conftest. Recording them here is a no-op
        # for every other test.
        tk._HEADLESS_REAL = {
            name: getattr(tk.Tk, name)
            for name in (
                "__init__", "mainloop", "destroy", "update", "update_idletasks",
                "winfo_screenwidth", "winfo_screenheight", "winfo_exists",
                "withdraw", "deiconify", "title", "geometry", "protocol",
                "after", "configure", "config",
            )
        }
...
def _initialize_test_auth():
    ...
    try:
        ...
        import education_system.post_18.university_system.infrastructure.shared_context as _ctx
...
def _isolate_db(_template_db, monkeypatch):
    ...
    import education_system.post_18.university_system.infrastructure.database.db as _db_mod
