...
from education_system.post_18.university_system.infrastructure.auth.mfa_integration import (
    MFAIntegration,
    integrate_mfa_check,
...
class TestMFAIntegration:
    ...
    def test_init(self):
        ...
        with patch('education_system.post_18.university_system.infrastructure.auth.mfa_integration.MFAService'):
            integration = MFAIntegration()

            assert integration.mfa_service is not None

    @patch('education_system.post_18.university_system.infrastructure.auth.mfa_integration.MFAService')
    ...
    def test_check_mfa_requirement_error(self, mock_service_class):
        ...
        # Fail closed: a policy lookup error must not report MFA as not-required.
        assert result['required'] is True
        assert result['enabled'] is False
        assert result['needs_setup'] is False
        assert result['must_verify'] is False
...
class TestIntegrateMFACheck:
    ...
    @patch('education_system.post_18.university_system.infrastructure.auth.mfa_integration.MFAIntegration')
    ...
    def test_integrate_mfa_check_error(self, mock_integration_class):
        ...
        # Fail closed: block login when the MFA policy cannot be evaluated.
        assert result['action'] == 'locked'
...
class TestCreateMFAPatch:
    ...
    @patch('education_system.post_18.university_system.infrastructure.auth.mfa_integration.integrate_mfa_check')
...
class TestGUIIntegrationHelpers:
    ...
    @patch('education_system.post_18.university_system.infrastructure.auth.mfa_integration.MFAIntegration')
    @patch('education_system.post_18.university_system.infrastructure.auth.mfa_gui.show_mfa_verification')
    ...
    @patch('education_system.post_18.university_system.infrastructure.auth.mfa_gui.show_mfa_setup')
