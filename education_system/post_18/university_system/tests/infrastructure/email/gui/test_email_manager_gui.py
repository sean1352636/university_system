...
try:
    from education_system.post_18.university_system.modules.shared.gui.email.email_gui import (
        EmailManagerGUI,
        main,
        run_gui_mode,
        handle_gui_error,
...
def mock_root():
    ...
    # tkinter widgets (e.g. tk.Toplevel(parent)) call parent._root(), which
    # walks the master chain until master is None. A bare MagicMock's .master
    # is itself another truthy mock, so that walk never terminates and the test
    # hangs until the timeout. Pin master to None to end the chain.
    root.master = None
    return root
