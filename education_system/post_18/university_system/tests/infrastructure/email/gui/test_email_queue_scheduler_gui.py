...
from education_system.post_18.university_system.modules.shared.gui.email import email_queue_scheduler_gui
...
class TestEmailQueueSchedulerGUIInitialization:
    ...
    def test_gui_initialization_with_parent(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            try:
...
class TestWorkerControlTab:
    ...
    def test_start_workers_button(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.start_email_workers', return_value=True):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
    ...
    def test_stop_workers_button(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.stop_email_workers', return_value=True):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
                    with patch('tkinter.messagebox.showinfo') as mock_info:
                        gui.stop_workers()
    ...
    def test_refresh_status_button(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.worker_threads', []):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
                    gui.refresh_status()
...
class TestQueueEmailTab:
    ...
    def test_queue_direct_email(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.queue_email', return_value=True):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
    ...
    def test_queue_template_email(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.queue_template_email', return_value=True):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
...
class TestScheduleEmailTab:
    ...
    def test_schedule_emails(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.schedule_send', return_value=True):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
    ...
    def test_process_due_emails(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.process_scheduled_emails', return_value=5):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
...
class TestMonitorTab:
    ...
    def test_refresh_monitor(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.email_queue') as mock_queue:
                with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.worker_threads', []):
...
class TestUtilitiesTab:
    ...
    def test_wait_for_queue(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.wait_for_email_queue', return_value=True):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
    ...
    def test_fix_inbox_display(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.fix_inbox_display_issue', return_value=True):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
    ...
    def test_update_email_status(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.update_scheduled_email_status', return_value=True):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
...
class TestTemplateLoading:
    ...
    def test_load_templates(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.list_templates', return_value=['test_template', 'welcome']):
                try:
                    gui = email_queue_scheduler_gui.EmailQueueSchedulerGUI(parent=root_window)
                    gui.load_templates()
...
class TestErrorHandling:
    ...
    def test_queue_email_smtp_error(self, root_window, temp_db):
        ...
        with patch('education_system.post_18.university_system.infrastructure.database.db.DEFAULT_DB_PATH', temp_db):
            with patch('education_system.post_18.university_system.modules.shared.gui.email.email_queue_scheduler_gui.queue_email',
