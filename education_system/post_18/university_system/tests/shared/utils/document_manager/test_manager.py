...
class TestStandaloneFunctions:
    ...
    def test_main_calls_display_main_menu(self, patched_connection, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        with patch.object(manager_mod, "DocumentManager") as DMClass, \
             patch("education_system.post_18.university_system.infrastructure.auth.UserAuth") as UA, \
             patch("education_system.post_18.university_system.infrastructure.auth.set_auth_instance"), \
             patch("education_system.post_18.university_system.infrastructure.auth.get_current_user",
