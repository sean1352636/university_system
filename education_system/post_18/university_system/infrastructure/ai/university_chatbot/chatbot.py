...
from education_system.post_18.university_system.infrastructure.ai.university_chatbot.models import (
    AuthenticatedSession,
    ConversationContext,
    UserSession,
)
from education_system.post_18.university_system.infrastructure.ai.university_chatbot.voice_interface import VoiceInterface
from education_system.post_18.university_system.infrastructure.ai.university_chatbot.config import load_config
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import nlp_processor
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import intent_handlers
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import authenticated_handlers
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import recommendation_engine
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import database_utils
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import logging_tracking
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import voice_support
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import api_routes as api_routes_mod
from education_system.post_18.university_system.infrastructure.ai.university_chatbot import background_tasks
...
class UniversityChatbot:
    def __init__(self, db_path=None, config_path=None):
        ...
        try:
            ...
            from education_system.post_18.university_system.modules.services.chatbot_inbox import (
                queue_message_for,
            )
    ...
    def process_message(self, message: str, user_id: str, session_id: str | None = None, is_voice: bool = False, **kwargs) -> str:
        ...
        try:
            ...
            if pending and message.strip().lower() in (
                ...
                from education_system.post_18.university_system.modules.services.student_union_bus import (
                    request_advocacy,
                )
        ...
        try:
            ...
            self._augment_module_context(message, nlp_result, context)
    ...
    def _augment_module_context(self, message, nlp_result, context):
        """Pull a module code out of free text and carry grades context forward.

        The NLP entity extractor never produces ``module_code`` from typed text,
        so a message like "CS101" or "CIS0001" is otherwise invisible to the
        grades/timetable handlers. We also treat a bare module code that arrives
        right after a grades turn (e.g. the bot just asked "Tell me a module
        code") as a grade lookup rather than an unrecognised message.
        """
        module_code = intent_handlers.nlp_result_safe_module(message)
        if not module_code:
            return
        nlp_result.setdefault("entities", {})["module_code"] = module_code
        bare = len(message.split()) <= 2
        if (bare and context.intent_history
                and context.intent_history[-1] == "grades"):
            nlp_result["intent"] = "grades"
    ...
    def get_user_email(self, username):
        return database_utils.get_user_email(self, username)

    def create_facility_booking(self, booker, facility_name, booking_date,
                                start_time, end_time, purpose=""):
        return database_utils.create_facility_booking(
            self, booker, facility_name, booking_date, start_time, end_time, purpose)
