...
from education_system.post_18.university_system.infrastructure.ai.university_chatbot.intent_handlers import (
    find_best_faq_match,
...
def generate_authenticated_response(chatbot, nlp_result: Dict, context: ConversationContext, session: AuthenticatedSession) -> str:
    ...
    if intent == "course_inquiry":
        ...
        from education_system.post_18.university_system.infrastructure.ai.university_chatbot.intent_handlers import handle_wellbeing_query
...
def handle_authenticated_academic_support(chatbot, nlp_result: Dict, context: ConversationContext, session: AuthenticatedSession) -> str:
    ...
    from education_system.post_18.university_system.infrastructure.ai.university_chatbot.intent_handlers import handle_academic_support_query
...
def handle_authenticated_admissions(chatbot, nlp_result: Dict, context: ConversationContext, session: AuthenticatedSession) -> str:
    ...
    from education_system.post_18.university_system.infrastructure.ai.university_chatbot.intent_handlers import handle_admissions_query
...
def handle_authenticated_administrative(chatbot, nlp_result: Dict, context: ConversationContext, session: AuthenticatedSession) -> str:
    ...
    if any(kw in user_message for kw in ["transcript", "certificate", "enrollment verification"]):
        if session.role == "student":
            ...
            if requests:
                response = "Your Document Requests:\n\n"
                for r in requests:
    ...
    from education_system.post_18.university_system.infrastructure.ai.university_chatbot.intent_handlers import handle_administrative_query
