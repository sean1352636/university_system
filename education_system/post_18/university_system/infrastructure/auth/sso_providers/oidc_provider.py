...
Usage:
    from education_system.post_18.university_system.infrastructure.auth.sso_providers.oidc_provider import OIDCProvider
...
class OIDCProvider:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # Security: when the ID token signature cannot be verified (JWKS
        # endpoint unreachable, no matching key id, or no JWT library present)
        # the token is rejected by default (fail-closed). Unverified decoding
        # is only permitted when this flag is explicitly enabled for local
        # development against a trusted IdP.
        self.allow_insecure_tokens = bool(config.get("allow_insecure_tokens", False))
        self._config = config
    ...
    def _signature_unverifiable_error(self, reason: str) -> Dict[str, Any]:
        """Build a fail-closed validation result for an unverifiable token.

        Called whenever the ID token's signature cannot be cryptographically
        verified. Rejects the login so an attacker who blocks/spoofs the JWKS
        endpoint or presents an unknown ``kid`` cannot have a forged token
        accepted.

        Args:
            reason: Short explanation of why verification was unavailable.

        Returns:
            Failure dict with empty claims.
        """
        logger.error(
            "Rejecting ID token: signature could not be verified (%s). "
            "Enable allow_insecure_tokens only for local development.",
            reason,
        )
        return {
            "success": False,
            "error": (
                f"ID token signature could not be verified ({reason}); "
                "rejecting login"
            ),
            "claims": {},
        }
    ...
    def _validate_id_token_authlib(
        ...
        try:
            ...
            jwks = self._get_jwks()
            if not jwks:
                if not self.allow_insecure_tokens:
                    return self._signature_unverifiable_error("JWKS unavailable")
                logger.warning(
                    "Could not fetch JWKS; decoding without signature "
                    "verification (allow_insecure_tokens is enabled)"
                )
                ...
                if jwks_data:
                    try:
                        ...
                        if signing_key:
                            ...
                            if not self.allow_insecure_tokens:
                                return self._signature_unverifiable_error(
                                    "no matching key id in JWKS"
                                )
                            logger.warning(
                                "Matching key ID not found in JWKS; decoding "
                                "without signature verification "
                                "(allow_insecure_tokens is enabled)"
                            )
                        ...
                        # A failure here includes genuine signature/claim
                        # verification errors; never silently downgrade to an
                        # unverified decode.
                        if not self.allow_insecure_tokens:
                            return self._signature_unverifiable_error(
                                f"JWKS validation error: {jwk_err}"
                            )
                        logger.warning(
                            "JWKS-based validation failed, falling back to "
                            "unverified decode (allow_insecure_tokens is "
                            "enabled): %s",
                            jwk_err,
                        )
                        claims_dict = pyjwt.decode(  # nosemgrep: python.jwt.security.jwt-decode-without-verification
