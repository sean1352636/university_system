...
class MFAIntegration:
    ...
    def check_mfa_requirement(self, user_id: int, role: str) -> Dict:
        ...
        if not policy_result['success']:
            # Fail closed: if we cannot determine the MFA policy we must not
            # silently treat MFA as not-required, otherwise a lookup error
            # becomes an MFA bypass. Signal the error so the caller blocks login.
            logger.error(
                "Failed to check MFA policy; failing closed: %s",
...
def integrate_mfa_check(user_id: int, role: str, device_id: str = None, trust_token: str = None) -> Dict:
    ...
    if mfa_check.get('error'):
        # Fail closed: could not evaluate MFA policy, so block the login
        # rather than allowing it without MFA. 'locked' is treated as a hard
        # login block by all callers.
        logger.error(
            "Blocking login: MFA policy could not be evaluated: %s",
            mfa_check['error'],
        )
        return {
            'action': 'locked',
            'message': (
                'Unable to verify security requirements. '
...
def create_mfa_patch_for_user_auth():
    ...
    from education_system.post_18.university_system.infrastructure.auth.mfa_integration import create_mfa_patch_for_user_auth
...
def show_mfa_for_login(parent, user_id: int, username: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        from education_system.post_18.university_system.infrastructure.auth.mfa_gui import show_mfa_verification
...
def show_mfa_setup_for_login(parent, user_id: int, username: str, required: bool = False) -> bool:
    ...
    """
    try:
        from education_system.post_18.university_system.infrastructure.auth.mfa_gui import show_mfa_setup
...
from education_system.post_18.university_system.infrastructure.auth.mfa_integration import integrate_mfa_check, show_mfa_for_login, show_mfa_setup_for_login
