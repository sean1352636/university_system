    ...
    def create_announcement(self, title, content, target_audience, is_urgent=0, start_date=None, end_date=None):
        ...
        def _create_announcement_op(cursor):
            ...
            for recipient in recipients:
                ...
                # Skip users with no email on file — otherwise send_template_email
                # raises "Recipient email is required" for each such row.
                if not (recipient_email and str(recipient_email).strip()):
                    skipped_no_email += 1
                    continue

                if email_notifications and announcement_notifications:
                    recipient_emails.append(str(recipient_email).strip())
                    template_vars = {
            ...
            if skipped_no_email:
                log_event('warning',
                          f"Announcement: skipped {skipped_no_email} recipient(s) "
                          f"with no email address on file.")

            # NOTE: do NOT send here. This runs inside the DatabaseManager
            # transaction, and send_bulk opens its own connection to the same
            # DB — sending now would deadlock ("database is locked"). Return the
            # recipients and send after the transaction has committed.
            return announcement_id, recipient_emails, template_vars_list
        ...
        # Unpack; the announcement row is now committed and the lock released.
        if isinstance(result, tuple):
            announcement_id, recipient_emails, template_vars_list = result
        else:
            announcement_id, recipient_emails, template_vars_list = result, [], []
        ...
        # Send notifications outside the transaction so send_bulk can open its
        # own connection without contending with our write lock.
        if recipient_emails:
            try:
                send_bulk(recipient_emails, 'general_announcement', template_vars_list)
            except Exception as e:
                log_event('error', f"Announcement saved but notifications failed: {e}")
