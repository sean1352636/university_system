...
def create_session():
    ...
    return jsonify({"message": "Session created.", "data": session}), 201
...
def record_attendance():
    ...
    return jsonify({"message": "Attendance recorded.", "data": result}), 201
...
def bulk_record():
    ...
    return jsonify({"message": f"Recorded attendance for {count} students.", "count": count}), 201
...
def generate_registers():
    ...
    return jsonify({
        "message": f"Generated {len(sessions)} register(s).",
        "data": sessions,
        "count": len(sessions),
    }), 201
...
def create_session_from_slot():
    ...
    return jsonify({"message": "Session created from slot.", "data": session}), 201
...
def populate_register(session_id):
    ...
    return jsonify({"message": f"Pre-populated {count} absent record(s).", "count": count}), 201
