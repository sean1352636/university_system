...
class DocumentHubService:
    ...
    def create_document(self, **kwargs) -> dict:
        ...
        try:
            conn.execute(
                """INSERT INTO documents (title, category, file_path, file_type, file_size, version, uploaded_by, target_role, download_count)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (kwargs.get('title'), kwargs.get('category'), kwargs.get('file_path'), kwargs.get('file_type'), kwargs.get('file_size'), kwargs.get('version'), kwargs.get('uploaded_by'), kwargs.get('target_role'), kwargs.get('download_count'),),
            )
            conn.commit()
            row = conn.execute(
    ...
    def update_document(self, pk: int, **kwargs) -> dict:
        ...
        allowed = {"title", "category", "file_path", "file_type", "file_size", "version", "uploaded_by", "target_role", "download_count"}
