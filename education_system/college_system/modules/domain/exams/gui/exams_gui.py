...
class ExamsFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Exams & Assessment", font=("Helvetica", 14, "bold"),
        ...
        self._nb.add(self._entries_tab, text="Exam Entries")
        self._build_entries_tab()

        # Timetable tab
        self._tt_tab = tk.Frame(self._nb)
        self._nb.add(self._tt_tab, text="Exam Timetable")
        self._build_timetable_tab()

        # Access tab
        self._access_tab = tk.Frame(self._nb)
        self._nb.add(self._access_tab, text="Access Arrangements")
    ...
    def _build_entries_tab(self):
        ...
        ttk.Button(toolbar, text="Refresh", command=self._load_entries).pack(side="left", padx=5)
        ttk.Button(toolbar, text="New Entry", command=self._new_entry).pack(side="right", padx=5)
    ...
    def _build_timetable_tab(self):
        ...
        ttk.Button(toolbar, text="Search", command=self._load_timetable).pack(side="left", padx=5)
        ttk.Button(toolbar, text="Add Slot", command=self._new_tt_slot).pack(side="right", padx=5)
    ...
    def _build_access_tab(self):
        ...
        toolbar.pack(fill="x", padx=5, pady=5)
        ttk.Button(toolbar, text="Refresh", command=self._load_access).pack(side="left", padx=5)
        ttk.Button(toolbar, text="New Arrangement", command=self._new_access).pack(side="right", padx=5)
    ...
    def _build_results_tab(self):
        ...
        ttk.Button(toolbar, text="Search", command=self._load_results).pack(side="left", padx=5)
        ttk.Button(toolbar, text="Record Result", command=self._new_result).pack(side="right", padx=5)
    ...
    def _new_entry(self):
        ...
        win.title("New Exam Entry")
        win.geometry("400x350")
        fields = {}
        row = 0
        for label, key in [("Student ID*:", "student_id"), ("Exam Board*:", "exam_board"),
                           ("Subject*:", "subject"), ("Level*:", "qualification_level"),
                           ("Component Code:", "component_code"), ("Series:", "series")]:
    ...
    def _new_tt_slot(self):
        win = tk.Toplevel(self)
        win.title("New Exam Timetable Slot")
        win.geometry("400x400")
        fields = {}
        row = 0
        for label, key in [("Exam Board*:", "exam_board"), ("Subject*:", "subject"),
                           ("Component*:", "component_code"), ("Date* (YYYY-MM-DD):", "exam_date"),
                           ("Start Time* (HH:MM):", "start_time"), ("Duration (mins)*:", "duration_mins"),
    ...
    def _new_access(self):
        win = tk.Toplevel(self)
        win.title("New Access Arrangement")
        win.geometry("400x300")
        fields = {}
        row = 0
        for label, key in [("Student ID*:", "student_id"), ("Type*:", "arrangement_type"),
                           ("Approved By:", "approved_by"), ("JCQ Form:", "jcq_form"),
                           ("Extra Time %:", "extra_time_percent"), ("Details:", "details")]:
            tk.Label(win, text=label).grid(row=row, column=0, padx=10, pady=5, sticky="e")
            e = tk.Entry(win, width=25)
            e.grid(row=row, column=1, padx=10, pady=5)
            fields[key] = e
        ...
        def save():
            try:
                ...
                messagebox.showinfo("Success", "Arrangement created")
                win.destroy()
                self._load_access()
            except Exception as e:
                messagebox.showerror("Error", str(e))
    ...
    def _new_result(self):
        ...
        for label, key in [("Student ID*:", "student_id"), ("Exam Board*:", "exam_board"),
                           ("Subject*:", "subject"), ("Level*:", "qualification_level"),
                           ("Grade*:", "grade"), ("UMS Mark:", "ums_mark"),
                           ("Raw Mark:", "raw_mark"), ("Series:", "series")]:
            tk.Label(win, text=label).grid(row=row, column=0, padx=10, pady=5, sticky="e")
            e = tk.Entry(win, width=25)
            e.grid(row=row, column=1, padx=10, pady=5)
            fields[key] = e
