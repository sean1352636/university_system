...
class _SessionDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text=t("study_planner.topic"), anchor="w",
...
class StudyPlannerFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("study_planner.management"),
        ...
        self._tree.heading("subject", text=t("common.subject"))
        self._tree.column("subject", width=120, anchor="center")
        self._tree.heading("topic", text=t("study_planner.topic"))
        self._tree.column("topic", width=150, anchor="center")
        self._tree.heading("planned_date", text=t("common.date"))
        self._tree.column("planned_date", width=100, anchor="center")
        self._tree.heading("planned_duration", text=t("study_planner.planned"))
        self._tree.column("planned_duration", width=60, anchor="center")
        self._tree.heading("actual_duration", text=t("study_planner.actual"))
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._tree, "study_planner.csv")
    ...
    def _on_add(self):
        dlg = _SessionDialog(self, title=t("study_planner.add_session"))
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_session(**dlg.result)
            messagebox.showinfo(t("common.success"), t("study_planner.session_created"))
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if not item:
            messagebox.showerror(t("common.error"), t("study_planner.session_not_found"))
            return
        dlg = _SessionDialog(self, title=t("study_planner.edit_session"), item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_session(pk, **dlg.result)
            messagebox.showinfo(t("common.success"), t("study_planner.session_updated"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno(t("common.confirm"), t("study_planner.delete_session_confirm")):
            return
        try:
            self._svc.delete_session(pk)
            messagebox.showinfo(t("common.success"), t("study_planner.session_deleted"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
