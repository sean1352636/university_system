"""GUI for skills passport management."""
...
from education_system.college_system.core.exceptions import SkillsPassportError
...
class _CategoryDialog(tk.Toplevel):
    ...
    def __init__(self, parent, title="Category", item=None):
...
class SkillsPassportFrame(tk.Frame):
    """Skills Passport management screen."""

    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        super().__init__(parent, **kwargs)
        self._db_path = db_path
        self._auth = auth
        self._svc = SkillsPassportService(db_path)
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("skills_passport.management"),
        ...
        columns = ('name', 'description', 'display_order')
        ...
        self._tree.heading("display_order", text=t("skills_passport.order"))
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._tree, "skills_passport_export.csv")
    ...
    def _load_items(self):
        ...
        try:
            items = self._svc.list_categories()
            for item in items:
                self._tree.insert("", "end", iid=item["id"], values=(
                    item.get("name", ""), item.get("description", ""), item.get("display_order", ""),
                ))
    ...
    def _on_add(self):
        dlg = _CategoryDialog(self, title=t("skills_passport.add_category"))
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_category(**dlg.result)
            messagebox.showinfo(t("common.success"), t("skills_passport.category_created"))
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if pk is None:
            return
        item = self._svc.get_category(pk)
        if not item:
            messagebox.showerror(t("common.error"), t("skills_passport.category_not_found"))
            return
        dlg = _CategoryDialog(self, title=t("skills_passport.edit_category"), item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_category(pk, **dlg.result)
            messagebox.showinfo(t("common.success"), t("skills_passport.category_updated"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno(t("common.confirm"), t("skills_passport.delete_category_confirm")):
            return
        try:
            self._svc.delete_category(pk)
            messagebox.showinfo(t("common.success"), t("skills_passport.category_deleted"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
