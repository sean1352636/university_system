"""GUI for activity feed management."""
...
from education_system.college_system.core.exceptions import ActivityFeedError
from education_system.college_system.core.i18n import t


class _Feed_ItemDialog(tk.Toplevel):
    """Modal dialog for adding or editing a feed_item."""

    def __init__(self, parent, title="Feed_Item", item=None):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text=t("activity_feed.entity"), anchor="w",
...
class ActivityFeedFrame(tk.Frame):
    """Activity Feed management screen."""

    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        super().__init__(parent, **kwargs)
        self._db_path = db_path
        self._auth = auth
        self._svc = ActivityFeedService(db_path)
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("activity_feed.management"),
                 font=("Helvetica", 15, "bold"),
                 bg="#2c3e50", fg="white").pack(side="left", padx=20, pady=10)

        toolbar = tk.Frame(self, bg="#ecf0f1", pady=8)
        toolbar.pack(fill="x", padx=15)
        ttk.Button(toolbar, text=t("activity_feed.add"), command=self._on_add).pack(side="left", padx=4)
        ...
        columns = ('user_id', 'activity_type', 'title', 'description', 'entity_type', 'entity_id')
        ...
        self._tree.heading("entity_type", text=t("activity_feed.entity"))
        self._tree.column("entity_type", width=80, anchor="center")
        self._tree.heading("entity_id", text=t("activity_feed.entity_id"))
        self._tree.column("entity_id", width=60, anchor="center")
    ...
    def _load_items(self):
        ...
        try:
            items = self._svc.list_feed_items()
            for item in items:
                self._tree.insert("", "end", iid=item["id"], values=(
                    item.get("user_id", ""), item.get("activity_type", ""), item.get("title", ""), item.get("description", ""), item.get("entity_type", ""), item.get("entity_id", ""),
                ))
            self._status_var.set(t("activity_feed.count_loaded", count=len(items)))
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._tree, "activity_feed.csv")

    def _on_add(self):
        dlg = _Feed_ItemDialog(self, title="Add Feed_Item")
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_feed_item(**dlg.result)
            messagebox.showinfo(t("common.success"), t("activity_feed.created"))
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if pk is None:
            return
        item = self._svc.get_feed_item(pk)
        if not item:
            messagebox.showerror(t("common.error"), t("activity_feed.not_found"))
            return
        dlg = _Feed_ItemDialog(self, title="Edit Feed_Item", item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_feed_item(pk, **dlg.result)
            messagebox.showinfo(t("common.success"), t("activity_feed.updated"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno(t("common.confirm"), t("activity_feed.delete_confirm")):
            return
        try:
            self._svc.delete_feed_item(pk)
            messagebox.showinfo(t("common.success"), t("activity_feed.deleted"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
