"""GUI for work journal management."""
...
from education_system.college_system.core.exceptions import WorkJournalError
from education_system.college_system.core.i18n import t


class _PlacementDialog(tk.Toplevel):
    """Modal dialog for adding or editing a placement."""

    def __init__(self, parent, title="Placement", item=None):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text=t("work_journal.employer"), anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=1, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("employer_name", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=1, column=1, sticky="ew", **pad)
        self._vars["employer_name"] = var
        tk.Label(container, text=t("work_journal.role"), anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=2, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("role_title", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=2, column=1, sticky="ew", **pad)
        self._vars["role_title"] = var
        ...
        tk.Label(container, text=t("work_journal.supervisor"), anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=5, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("supervisor_name", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=5, column=1, sticky="ew", **pad)
        self._vars["supervisor_name"] = var
...
class WorkJournalFrame(tk.Frame):
    """Work Journal management screen."""

    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        super().__init__(parent, **kwargs)
        self._db_path = db_path
        self._auth = auth
        self._svc = WorkJournalService(db_path)
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("work_journal.management"),
        ...
        columns = ('student_id', 'employer_name', 'role_title', 'start_date', 'end_date', 'supervisor_name')
        ...
        self._tree.heading("employer_name", text=t("work_journal.employer"))
        self._tree.column("employer_name", width=200, anchor="center")
        self._tree.heading("role_title", text=t("work_journal.role"))
        self._tree.column("role_title", width=150, anchor="center")
        ...
        self._tree.heading("supervisor_name", text=t("work_journal.supervisor"))
        self._tree.column("supervisor_name", width=120, anchor="center")
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._tree, "work_journal.csv")
    ...
    def _load_items(self):
        ...
        try:
            items = self._svc.list_placements()
            for item in items:
                self._tree.insert("", "end", iid=item["id"], values=(
                    item.get("student_id", ""), item.get("employer_name", ""), item.get("role_title", ""), item.get("start_date", ""), item.get("end_date", ""), item.get("supervisor_name", ""),
                ))
    ...
    def _on_add(self):
        dlg = _PlacementDialog(self, title=t("work_journal.add_placement"))
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_placement(**dlg.result)
            messagebox.showinfo(t("common.success"), t("work_journal.placement_created"))
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if pk is None:
            return
        item = self._svc.get_placement(pk)
        if not item:
            messagebox.showerror(t("common.error"), t("work_journal.placement_not_found"))
            return
        dlg = _PlacementDialog(self, title=t("work_journal.edit_placement"), item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_placement(pk, **dlg.result)
            messagebox.showinfo(t("common.success"), t("work_journal.placement_updated"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno(t("common.confirm"), t("work_journal.delete_placement_confirm")):
            return
        try:
            self._svc.delete_placement(pk)
            messagebox.showinfo(t("common.success"), t("work_journal.placement_deleted"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
