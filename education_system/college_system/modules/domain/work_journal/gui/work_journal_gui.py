...
class _PlacementDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text="Employer", anchor="w",
        ...
        tk.Label(container, text="Role", anchor="w",
        ...
        tk.Label(container, text="End", anchor="w",
        ...
        tk.Label(container, text="Supervisor", anchor="w",
...
class WorkJournalFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Work Journal",
        ...
        self._tree.heading("employer_name", text="Employer")
        self._tree.column("employer_name", width=200, anchor="center")
        self._tree.heading("role_title", text="Role")
        ...
        self._tree.heading("end_date", text="End")
        self._tree.column("end_date", width=100, anchor="center")
        self._tree.heading("supervisor_name", text="Supervisor")
    ...
    def _on_add(self):
        dlg = _PlacementDialog(self, title="Add Placement")
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_placement(**dlg.result)
            messagebox.showinfo("Success", "Placement created.")
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if not item:
            ...
            return
        dlg = _PlacementDialog(self, title="Edit Placement", item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_placement(pk, **dlg.result)
            messagebox.showinfo("Success", "Placement updated.")
            self._load_items()
        except Exception as exc:
            messagebox.showerror("Error", str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno("Confirm", "Delete this placement?"):
            return
        try:
            self._svc.delete_placement(pk)
            messagebox.showinfo("Success", "Placement deleted.")
            self._load_items()
        except Exception as exc:
            messagebox.showerror("Error", str(exc))
