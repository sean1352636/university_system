...
class _SurveyDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text=t("surveys.target"), anchor="w",
        ...
        tk.Label(container, text=t("surveys.opens"), anchor="w",
        ...
        tk.Label(container, text=t("surveys.closes"), anchor="w",
...
class SurveyFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("surveys.management"),
        ...
        self._tree.heading("created_by", text=t("surveys.created_by"))
        self._tree.column("created_by", width=80, anchor="center")
        self._tree.heading("survey_type", text=t("common.type"))
        self._tree.column("survey_type", width=100, anchor="center")
        self._tree.heading("is_anonymous", text=t("surveys.anonymous"))
        self._tree.column("is_anonymous", width=60, anchor="center")
        self._tree.heading("target_role", text=t("surveys.target"))
        self._tree.column("target_role", width=80, anchor="center")
        self._tree.heading("open_date", text=t("surveys.opens"))
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._tree, "surveys.csv")
    ...
    def _on_add(self):
        dlg = _SurveyDialog(self, title=t("surveys.add_survey"))
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_survey(**dlg.result)
            messagebox.showinfo(t("common.success"), t("surveys.survey_created"))
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if not item:
            messagebox.showerror(t("common.error"), t("surveys.survey_not_found"))
            return
        dlg = _SurveyDialog(self, title=t("surveys.edit_survey"), item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_survey(pk, **dlg.result)
            messagebox.showinfo(t("common.success"), t("surveys.survey_updated"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno(t("common.confirm"), t("surveys.delete_survey_confirm")):
            return
        try:
            self._svc.delete_survey(pk)
            messagebox.showinfo(t("common.success"), t("surveys.survey_deleted"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
