...
class SENDFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="SEND / ALS", font=("Helvetica", 14, "bold"),
        ...
        self._nb.add(self._records_tab, text="SEND Records")
        self._build_records_tab()

        # Interventions tab
        self._int_tab = tk.Frame(self._nb)
        self._nb.add(self._int_tab, text="Interventions")
        self._build_interventions_tab()

    def _build_records_tab(self):
        toolbar = tk.Frame(self._records_tab)
        toolbar.pack(fill="x", padx=5, pady=5)
        ttk.Button(toolbar, text="Refresh", command=self._load_records).pack(side="left", padx=5)
    ...
    def _build_interventions_tab(self):
        ...
        ttk.Button(toolbar, text="Search", command=self._load_interventions).pack(side="left", padx=5)
        ttk.Button(toolbar, text="New Intervention", command=self._new_intervention).pack(side="right", padx=5)
    ...
    def _new_record(self):
        ...
        win.title("New SEND Record")
        win.geometry("400x350")
        fields = {}
        row = 0
        for label, key in [("Student ID*:", "student_id"), ("Need Type*:", "need_type")]:
        ...
        row += 1
        tk.Label(win, text="Support Plan:").grid(row=row, column=0, padx=10, pady=5, sticky="ne")
        plan = tk.Text(win, width=30, height=3)
        plan.grid(row=row, column=1, padx=10, pady=5)
        row += 1
        ehcp_var = tk.BooleanVar()
        ttk.Checkbutton(win, text="EHCP in place", variable=ehcp_var).grid(row=row, column=1, sticky="w", padx=10)
        row += 1
    ...
    def _new_intervention(self):
        win = tk.Toplevel(self)
        win.title("New Intervention")
        win.geometry("400x300")
        fields = {}
        row = 0
        for label, key in [("Student ID*:", "student_id"), ("Type*:", "intervention_type"),
                           ("Start Date:", "start_date"), ("Target End Date:", "target_end_date")]:
            tk.Label(win, text=label).grid(row=row, column=0, padx=10, pady=5, sticky="e")
            e = tk.Entry(win, width=25)
            e.grid(row=row, column=1, padx=10, pady=5)
            fields[key] = e
