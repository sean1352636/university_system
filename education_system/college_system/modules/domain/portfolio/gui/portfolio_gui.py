...
class _ItemDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text=t("portfolio.file"), anchor="w",
...
class PortfolioFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("portfolio.management"),
        ...
        self._tree.heading("item_type", text=t("common.type"))
        ...
        self._tree.heading("file_path", text=t("portfolio.file"))
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._tree, "portfolio_export.csv")
    ...
    def _on_add(self):
        dlg = _ItemDialog(self, title=t("portfolio.add_item"))
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_item(**dlg.result)
            messagebox.showinfo(t("common.success"), t("portfolio.item_created"))
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if not item:
            messagebox.showerror(t("common.error"), t("portfolio.item_not_found"))
            return
        dlg = _ItemDialog(self, title=t("portfolio.edit_item"), item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_item(pk, **dlg.result)
            messagebox.showinfo(t("common.success"), t("portfolio.item_updated"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno(t("common.confirm"), t("portfolio.delete_item_confirm")):
            return
        try:
            self._svc.delete_item(pk)
            messagebox.showinfo(t("common.success"), t("portfolio.item_deleted"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
