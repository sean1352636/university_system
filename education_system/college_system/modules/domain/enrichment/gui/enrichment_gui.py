...
class _ActivityDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text="Name", anchor="w",
        ...
        tk.Label(container, text="Day", anchor="w",
        ...
        tk.Label(container, text="Time", anchor="w",
        ...
        tk.Label(container, text="Location", anchor="w",
...
class EnrichmentFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Enrichment",
        ...
        self._tree.heading("activity_type", text="Type")
        self._tree.column("activity_type", width=100, anchor="center")
        self._tree.heading("lead_staff_id", text="Lead Staff")
        self._tree.column("lead_staff_id", width=80, anchor="center")
        self._tree.heading("day_of_week", text="Day")
        self._tree.column("day_of_week", width=80, anchor="center")
        self._tree.heading("time_slot", text="Time")
    ...
    def _on_add(self):
        dlg = _ActivityDialog(self, title="Add Activity")
    ...
    def _on_edit(self):
        ...
        if not item:
            ...
            return
        dlg = _ActivityDialog(self, title="Edit Activity", item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno("Confirm", "Delete this activity?"):
            return
        try:
            self._svc.delete_activity(pk)
            messagebox.showinfo("Success", "Activity deleted.")
            self._load_items()
        except Exception as exc:
            messagebox.showerror("Error", str(exc))
