...
from education_system.college_system.core.exceptions import EnrichmentError
from education_system.college_system.core.i18n import t


class _ActivityDialog(tk.Toplevel):
    """Modal dialog for adding or editing a activity."""

    def __init__(self, parent, title="Activity", item=None):
    ...
    def _build_ui(self):
        ...
        var = tk.StringVar(value=self._item.get("activity_type", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=1, column=1, sticky="ew", **pad)
        self._vars["activity_type"] = var
        tk.Label(container, text=t("enrichment.day"), anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=2, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("day_of_week", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=2, column=1, sticky="ew", **pad)
        self._vars["day_of_week"] = var
        tk.Label(container, text=t("enrichment.time"), anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=3, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("time_slot", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=3, column=1, sticky="ew", **pad)
        self._vars["time_slot"] = var
        tk.Label(container, text=t("common.location"), anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=4, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("location", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=4, column=1, sticky="ew", **pad)
        self._vars["location"] = var
...
class EnrichmentFrame(tk.Frame):
    """Enrichment management screen."""

    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        super().__init__(parent, **kwargs)
        self._db_path = db_path
        self._auth = auth
        self._svc = EnrichmentService(db_path)
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("enrichment.management"),
        ...
        columns = ('name', 'activity_type', 'lead_staff_id', 'day_of_week', 'time_slot', 'location')
        self._tree = ttk.Treeview(tree_frame, columns=columns, show="headings", selectmode="browse")

        self._tree.heading("name", text=t("common.name"))
        self._tree.column("name", width=200, anchor="center")
        self._tree.heading("activity_type", text=t("common.type"))
        self._tree.column("activity_type", width=100, anchor="center")
        self._tree.heading("lead_staff_id", text=t("enrichment.lead_staff"))
        self._tree.column("lead_staff_id", width=80, anchor="center")
        self._tree.heading("day_of_week", text=t("enrichment.day"))
        self._tree.column("day_of_week", width=80, anchor="center")
        self._tree.heading("time_slot", text=t("enrichment.time"))
        self._tree.column("time_slot", width=80, anchor="center")
        self._tree.heading("location", text=t("common.location"))
        self._tree.column("location", width=120, anchor="center")
    ...
    def _load_items(self):
        ...
        try:
            items = self._svc.list_activities()
            for item in items:
                self._tree.insert("", "end", iid=item["id"], values=(
                    item.get("name", ""), item.get("activity_type", ""), item.get("lead_staff_id", ""), item.get("day_of_week", ""), item.get("time_slot", ""), item.get("location", ""),
                ))
    ...
    def _on_add(self):
        dlg = _ActivityDialog(self, title=t("enrichment.add_activity"))
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_activity(**dlg.result)
            messagebox.showinfo(t("common.success"), t("enrichment.activity_created"))
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if pk is None:
            return
        item = self._svc.get_activity(pk)
        if not item:
            messagebox.showerror(t("common.error"), t("enrichment.activity_not_found"))
            return
        dlg = _ActivityDialog(self, title=t("enrichment.edit_activity"), item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_activity(pk, **dlg.result)
            messagebox.showinfo(t("common.success"), t("enrichment.activity_updated"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno(t("common.confirm"), t("enrichment.delete_activity_confirm")):
            return
        try:
            self._svc.delete_activity(pk)
            messagebox.showinfo(t("common.success"), t("enrichment.activity_deleted"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._tree, "enrichment_export.csv")
