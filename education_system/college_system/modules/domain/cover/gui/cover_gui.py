...
class CoverFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Cover Arrangements", font=("Helvetica", 14, "bold"),
        ...
        self._nb.add(self._today_tab, text="Today's Cover")
        self._build_today_tab()

        self._all_tab = tk.Frame(self._nb)
        self._nb.add(self._all_tab, text="All Arrangements")
        self._build_all_tab()

    def _build_today_tab(self):
        toolbar = tk.Frame(self._today_tab)
        toolbar.pack(fill="x", padx=5, pady=5)
        ttk.Button(toolbar, text="Refresh", command=self._load_today).pack(side="left", padx=5)
        ttk.Button(toolbar, text="New Arrangement", command=self._new_arrangement).pack(side="right", padx=5)
    ...
    def _build_all_tab(self):
        ...
        ttk.Button(toolbar, text="Filter", command=self._load_all).pack(side="left", padx=5)
        ...
        ttk.Button(btn_frame, text="Update", command=self._update_arrangement).pack(side="left", padx=5)
        ttk.Button(btn_frame, text="Delete", command=self._delete_arrangement).pack(side="left", padx=5)
    ...
    def _new_arrangement(self):
        ...
        win.title("New Cover Arrangement")
    ...
    def _update_arrangement(self):
        ...
        if not sel:
            messagebox.showwarning("Warning", "Select an arrangement")
            return
        arr_id = int(sel[0])
        win = tk.Toplevel(self)
        win.title("Update Arrangement")
    ...
    def _delete_arrangement(self):
        ...
        if not sel:
            messagebox.showwarning("Warning", "Select an arrangement")
            return
        if messagebox.askyesno("Confirm", "Delete this cover arrangement?"):
            try:
