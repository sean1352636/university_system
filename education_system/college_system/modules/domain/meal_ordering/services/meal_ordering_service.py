...
class MealOrderingService:
    ...
    def create_item(self, **kwargs) -> dict:
        ...
        try:
            ...
            _insert_vals: list = []
            for col in ('name', 'category', 'price', 'dietary_tags', 'description', 'is_available'):
                val = kwargs.get(col)
                if val is not None:
            ...
            conn.execute(
                f"INSERT INTO menu_items ({cols_sql}) VALUES ({ph_sql})",
                _insert_vals,
            )
            conn.commit()
            row = conn.execute(
    ...
    def update_item(self, pk: int, **kwargs) -> dict:
        ...
        conn = self._conn()
        try:
            conn.execute(f"UPDATE menu_items SET {set_clause} WHERE id = ?", params)  # nosec B608
            conn.commit()
