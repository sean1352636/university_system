"""GUI for announcements management."""
...
from education_system.college_system.core.exceptions import AnnouncementError
from education_system.college_system.core.i18n import t


class _AnnouncementDialog(tk.Toplevel):
    """Modal dialog for adding or editing a announcement."""

    def __init__(self, parent, title="Announcement", item=None):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text=t("common.content"), anchor="w",
        ...
        tk.Label(container, text=t("announcements.published"), anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=4, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("publish_date", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=4, column=1, sticky="ew", **pad)
        self._vars["publish_date"] = var
        tk.Label(container, text=t("announcements.expires"), anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=5, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("expiry_date", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=5, column=1, sticky="ew", **pad)
        self._vars["expiry_date"] = var
...
class AnnouncementFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("announcements.management"),
                 font=("Helvetica", 15, "bold"),
                 bg="#2c3e50", fg="white").pack(side="left", padx=20, pady=10)

        toolbar = tk.Frame(self, bg="#ecf0f1", pady=8)
        toolbar.pack(fill="x", padx=15)
        ttk.Button(toolbar, text=t("announcements.add"), command=self._on_add).pack(side="left", padx=4)
        ...
        columns = ('title', 'content', 'author_id', 'category', 'target_role', 'is_pinned')
        self._tree = ttk.Treeview(tree_frame, columns=columns, show="headings", selectmode="browse")

        self._tree.heading("title", text=t("common.title"))
        self._tree.column("title", width=200, anchor="center")
        self._tree.heading("content", text=t("common.content"))
        self._tree.column("content", width=300, anchor="center")
        self._tree.heading("author_id", text=t("announcements.author"))
        ...
        self._tree.heading("target_role", text=t("common.target"))
        self._tree.column("target_role", width=80, anchor="center")
        self._tree.heading("is_pinned", text=t("announcements.pinned"))
        self._tree.column("is_pinned", width=50, anchor="center")
    ...
    def _load_items(self):
        ...
        try:
            items = self._svc.list_announcements()
            for item in items:
                self._tree.insert("", "end", iid=item["id"], values=(
                    item.get("title", ""), item.get("content", ""), item.get("author_id", ""), item.get("category", ""), item.get("target_role", ""), item.get("is_pinned", ""),
                ))
            self._status_var.set(t("announcements.count_loaded", count=len(items)))
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._tree, "announcements.csv")

    def _on_add(self):
        dlg = _AnnouncementDialog(self, title="Add Announcement")
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_announcement(**dlg.result)
            messagebox.showinfo(t("common.success"), t("announcements.created"))
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if pk is None:
            return
        item = self._svc.get_announcement(pk)
        if not item:
            messagebox.showerror(t("common.error"), t("announcements.not_found"))
            return
        dlg = _AnnouncementDialog(self, title="Edit Announcement", item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.update_announcement(pk, **dlg.result)
            messagebox.showinfo(t("common.success"), t("announcements.updated"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno(t("common.confirm"), t("announcements.delete_confirm")):
            return
        try:
            self._svc.delete_announcement(pk)
            messagebox.showinfo(t("common.success"), t("announcements.deleted"))
            self._load_items()
        except Exception as exc:
            messagebox.showerror(t("common.error"), str(exc))
