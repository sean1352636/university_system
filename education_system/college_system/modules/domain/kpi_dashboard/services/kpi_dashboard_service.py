...
class KPIDashboardService:
    ...
    def create_target(self, **kwargs) -> dict:
        ...
        try:
            conn.execute(
                """INSERT INTO kpi_targets (academic_year, kpi_name, kpi_category, target_value, actual_value, national_benchmark)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (kwargs.get('academic_year'), kwargs.get('kpi_name'), kwargs.get('kpi_category'), kwargs.get('target_value'), kwargs.get('actual_value'), kwargs.get('national_benchmark'),),
            )
            conn.commit()
            row = conn.execute(
    ...
    def update_target(self, pk: int, **kwargs) -> dict:
        ...
        conn = self._conn()
        try:
            conn.execute(f"UPDATE kpi_targets SET {set_clause} WHERE id = ?", params)
            conn.commit()
