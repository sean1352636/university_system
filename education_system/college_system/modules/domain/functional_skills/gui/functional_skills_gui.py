...
class FunctionalSkillsFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(
            header, text="Functional Skills & GCSE Resits",
    ...
    def _build_enrollments_tab(self):
        ...
        tk.Label(filt_frame, text="Subject:", bg="#ecf0f1").pack(side="left", padx=(0, 4))
        ...
        tk.Label(filt_frame, text="Qual Type:", bg="#ecf0f1").pack(side="left", padx=(0, 4))
        ...
        ttk.Button(filt_frame, text="Filter", command=self._load_enrollments).pack(side="left", padx=4)
        ttk.Button(filt_frame, text="Clear", command=self._clear_enr_filters).pack(side="left", padx=4)
        ...
        for c, h, w in [
            ("id", "ID", 40), ("student_id", "Student ID", 70),
            ("subject", "Subject", 100), ("qual_type", "Qual Type", 110),
            ("level", "Level", 50), ("entry_grade", "Entry Grade", 80),
            ("target", "Target", 60), ("current", "Current", 60),
            ("exam_date", "Exam Date", 90), ("status", "Status", 90),
        ]:
        ...
        for text, cmd in [
            ("New", self._new_enrollment), ("View", self._view_enrollment),
            ("Update", self._update_enrollment), ("Book Exam", self._book_exam),
            ("Record Result", self._record_result), ("Delete", self._delete_enrollment),
        ]:
    ...
    def _build_assessments_tab(self):
        ...
        ttk.Button(filt_frame, text="Filter", command=self._load_assessments).pack(side="left", padx=4)
        ttk.Button(filt_frame, text="Clear", command=self._clear_asmt_filters).pack(side="left", padx=4)
        ...
        for c, h, w in [
            ("id", "ID", 40), ("enrollment_id", "Enrollment ID", 90),
            ("type", "Type", 90), ("date", "Date", 90),
            ("score", "Score", 60), ("grade", "Grade", 60),
        ...
        btn_frame.pack(fill="x", pady=(8, 0))
        for text, cmd in [
            ("New", self._new_assessment),
            ("Update", self._update_assessment),
            ("Delete", self._delete_assessment),
        ]:
            ttk.Button(btn_frame, text=text, command=cmd).pack(side="left", padx=4)
    ...
    def _load_stats(self):
        ...
        try:
            ...
            row = 0
            tk.Label(self._stats_frame, text="Overview",
            ...
            row += 1
            tk.Label(self._stats_frame, text="By Subject",
            ...
            row += 1
            tk.Label(self._stats_frame, text="By Qualification Type",
                     font=("Helvetica", 12, "bold"), bg="#ecf0f1").grid(
                row=row, column=0, columnspan=2, sticky="w", pady=(6, 4))
            row += 1
    ...
    def _new_enrollment(self):
        ...
        for label, key, widget_type, options in [
            ...
            ("Subject:", "subject", "combo", self.SUBJECTS[1:]),
            ("Qualification Type:", "qualification_type", "combo", self.QUAL_TYPES[1:]),
            ("Level:", "level", "combo", self.LEVELS[1:]),
        ...
        def _save():
            try:
                ...
                if not subj:
                    messagebox.showerror("Error", "Subject is required.", parent=dlg)
                    return
                kwargs = {}
                ...
                dlg.destroy()
                self._load_enrollments()
                messagebox.showinfo("Success", "Enrollment created.")
            except ValueError:
                messagebox.showerror("Error", "Student ID must be a number.", parent=dlg)
            except Exception as e:
                messagebox.showerror("Error", str(e), parent=dlg)

        ttk.Button(dlg, text="Save", command=_save).grid(row=r, column=0, columnspan=2, pady=12)
    ...
    def _view_enrollment(self):
        ...
        try:
            ...
            for label, value in [
                ("ID:", enr["id"]),
                ("Student:", f"{enr['student_id']} - {name}"),
                ("Subject:", enr.get("subject", "-")),
                ("Qualification Type:", enr.get("qualification_type", "-")),
                ("Level:", enr.get("level", "-")),
    ...
    def _update_enrollment(self):
        ...
        for label, key, widget_type, options, default in [
            ("Subject:", "subject", "combo", self.SUBJECTS[1:], enr.get("subject", "")),
            ("Qualification Type:", "qualification_type", "combo", self.QUAL_TYPES[1:], enr.get("qualification_type", "")),
            ("Level:", "level", "combo", self.LEVELS[1:], enr.get("level", "")),
            ...
            ("Status:", "status", "combo", self.STATUSES[1:], enr.get("status", "")),
            ("Notes:", "notes", "entry", None, enr.get("notes") or ""),
        ]:
            tk.Label(dlg, text=label, bg="#ecf0f1").grid(row=r, column=0, sticky="w", padx=10, pady=4)
                ...
                for k, var in fields.items():
                    ...
                    if val:
                        if k == "attendance_pct":
                            ...
                            kwargs[k] = val
                if not kwargs:
                    messagebox.showwarning("Warning", "No changes to save.", parent=dlg)
                    return
                self._svc.update_enrollment(int(eid), **kwargs)
                dlg.destroy()
                self._load_enrollments()
                messagebox.showinfo("Success", "Enrollment updated.")
            except Exception as e:
                messagebox.showerror("Error", str(e), parent=dlg)
    ...
    def _book_exam(self):
        ...
        def _save():
            try:
                ...
                if not exam_date:
                    messagebox.showerror("Error", "Exam date is required.", parent=dlg)
                    return
                series = series_var.get().strip() or None
                self._svc.book_exam(int(eid), exam_date, series)
                dlg.destroy()
                self._load_enrollments()
                messagebox.showinfo("Success", "Exam booked.")
            except Exception as e:
                messagebox.showerror("Error", str(e), parent=dlg)

        ttk.Button(dlg, text="Book", command=_save).grid(row=2, column=0, columnspan=2, pady=12)
    ...
    def _record_result(self):
        ...
        tk.Label(dlg, text="Result:", bg="#ecf0f1").grid(
            row=0, column=0, sticky="w", padx=10, pady=8)
        ...
        tk.Label(dlg, text="Grade:", bg="#ecf0f1").grid(
            row=1, column=0, sticky="w", padx=10, pady=8)
        ...
        def _save():
            try:
                ...
                if not result:
                    messagebox.showerror("Error", "Result is required.", parent=dlg)
                    return
                grade = grade_var.get().strip() or None
                self._svc.record_result(int(eid), result, grade)
                dlg.destroy()
                self._load_enrollments()
                messagebox.showinfo("Success", "Result recorded.")
            except Exception as e:
                messagebox.showerror("Error", str(e), parent=dlg)

        ttk.Button(dlg, text="Save", command=_save).grid(row=2, column=0, columnspan=2, pady=12)

    def _delete_enrollment(self):
        eid = self._get_selected_enrollment_id()
        if not eid:
            return
        if not messagebox.askyesno("Confirm", f"Delete enrollment #{eid} and all its assessments?"):
            return
        try:
            self._svc.delete_enrollment(int(eid))
            self._load_enrollments()
            self._load_assessments()
            messagebox.showinfo("Success", "Enrollment deleted.")
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def _get_selected_assessment_id(self):
        ...
        if not sel:
            messagebox.showwarning("Warning", "Please select an assessment.")
            return None
        return self._asmt_tree.item(sel[0], "values")[0]

    def _new_assessment(self):
        dlg = tk.Toplevel(self)
        dlg.title("New Assessment")
        ...
        for label, key, widget_type, options in [
            ...
            ("Type:", "assessment_type", "combo", self.ASSESSMENT_TYPES[1:]),
            ("Date (YYYY-MM-DD):", "assessment_date", "entry", None),
            ("Score:", "score", "entry", None),
            ("Grade:", "grade", "entry", None),
        ...
        def _save():
            try:
                ...
                messagebox.showerror("Error", "Enrollment ID must be a number, score must be numeric.", parent=dlg)
            except Exception as e:
                messagebox.showerror("Error", str(e), parent=dlg)
    ...
    def _update_assessment(self):
        ...
        r = 0
        for label, key, widget_type, options, default in [
            ("Type:", "assessment_type", "combo", self.ASSESSMENT_TYPES[1:], asmt.get("assessment_type", "")),
            ("Date (YYYY-MM-DD):", "assessment_date", "entry", None, asmt.get("assessment_date") or ""),
            ("Score:", "score", "entry", None, str(asmt.get("score") or "")),
            ("Grade:", "grade", "entry", None, asmt.get("grade") or ""),
    ...
    def _delete_assessment(self):
        ...
        try:
            ...
            self._load_assessments()
            messagebox.showinfo("Success", "Assessment deleted.")
        except Exception as e:
            messagebox.showerror("Error", str(e))
