...
class MobileDashboardService:
    ...
    def create_widget(self, **kwargs) -> dict:
        ...
        try:
            ...
            conn.execute(
                f"INSERT INTO mobile_widget_config ({cols}) VALUES ({placeholders})",
                list(fields.values()),
            )
            conn.commit()
            row = conn.execute(
    ...
    def update_widget(self, pk: int, **kwargs) -> dict:
        ...
        set_parts: list[str] = []
        params: list = []
        for col in ("user_id", "widget_type", "display_order", "is_visible", "config"):
            val = kwargs.get(col)
            if val is not None:
        ...
        conn = self._conn()
        try:
            conn.execute(f"UPDATE mobile_widget_config SET {set_clause} WHERE id = ?", params)  # nosec B608
            conn.commit()
