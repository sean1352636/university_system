"""GUI for staff wellbeing management."""
...
class _CheckinDialog(tk.Toplevel):
    """Modal dialog for adding or editing a checkin."""

    def __init__(self, parent, title="Checkin", item=None):
    ...
    def _build_ui(self):
        ...
        var = tk.StringVar(value=self._item.get("checkin_date", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=1, column=1, sticky="ew", **pad)
        self._vars["checkin_date"] = var
        tk.Label(container, text="Stress", anchor="w",
                 font=("Helvetica", 9, "bold")).grid(row=2, column=0, sticky="w", **pad)
        var = tk.StringVar(value=self._item.get("stress_level", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=2, column=1, sticky="ew", **pad)
        self._vars["stress_level"] = var
...
class StaffWellbeingFrame(tk.Frame):
    """Staff Wellbeing management screen."""

    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        super().__init__(parent, **kwargs)
        self._db_path = db_path
        self._auth = auth
        self._svc = StaffWellbeingService(db_path)
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        columns = ('staff_id', 'checkin_date', 'mood_rating', 'workload_rating', 'stress_level', 'notes')
        self._tree = ttk.Treeview(tree_frame, columns=columns, show="headings", selectmode="browse")

        self._tree.heading("staff_id", text="Staff ID")
        self._tree.column("staff_id", width=80, anchor="center")
        self._tree.heading("checkin_date", text="Date")
        self._tree.column("checkin_date", width=100, anchor="center")
        self._tree.heading("mood_rating", text="Mood")
        self._tree.column("mood_rating", width=60, anchor="center")
        self._tree.heading("workload_rating", text="Workload")
        self._tree.column("workload_rating", width=70, anchor="center")
        self._tree.heading("stress_level", text="Stress")
        self._tree.column("stress_level", width=80, anchor="center")
    ...
    def _on_add(self):
        dlg = _CheckinDialog(self, title="Add Checkin")
