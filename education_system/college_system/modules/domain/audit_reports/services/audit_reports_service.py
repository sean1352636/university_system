...
class AuditReportService:
    ...
    def create_report(self, **kwargs) -> dict:
        ...
        try:
            ...
            _insert_vals: list = []
            for col in ('report_type', 'title', 'generated_by', 'result_summary', 'findings', 'status'):
                val = kwargs.get(col)
                if val is not None:
            ...
            conn.execute(
                f"INSERT INTO audit_report_records ({cols_sql}) VALUES ({ph_sql})",
                _insert_vals,
            )
            conn.commit()
            row = conn.execute(
    ...
    def update_report(self, pk: int, **kwargs) -> dict:
        ...
        conn = self._conn()
        try:
            conn.execute(f"UPDATE audit_report_records SET {set_clause} WHERE id = ?", params)  # nosec B608
            conn.commit()
