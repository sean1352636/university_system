"""GUI for audit & compliance reports management."""
...
class _ReportDialog(tk.Toplevel):
    """Modal dialog for adding or editing a report."""

    def __init__(self, parent, title="Report", item=None):
    ...
    def _build_ui(self):
        ...
        var = tk.StringVar(value=self._item.get("report_type", "") if self._item else "")
        ttk.Entry(container, textvariable=var, width=36).grid(row=0, column=1, sticky="ew", **pad)
        self._vars["report_type"] = var
...
class AuditReportFrame(tk.Frame):
    """Audit & Compliance Reports management screen."""

    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        super().__init__(parent, **kwargs)
        self._db_path = db_path
        self._auth = auth
        self._svc = AuditReportService(db_path)
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        columns = ('report_type', 'title', 'generated_by', 'result_summary', 'findings', 'status')
        self._tree = ttk.Treeview(tree_frame, columns=columns, show="headings", selectmode="browse")

        self._tree.heading("report_type", text="Type")
        self._tree.column("report_type", width=120, anchor="center")
        self._tree.heading("title", text="Title")
        self._tree.column("title", width=200, anchor="center")
        self._tree.heading("generated_by", text="By")
        self._tree.column("generated_by", width=80, anchor="center")
        self._tree.heading("result_summary", text="Summary")
        self._tree.column("result_summary", width=200, anchor="center")
        self._tree.heading("findings", text="Findings")
        self._tree.column("findings", width=200, anchor="center")
