...
class HelpdeskFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text=t("helpdesk.title"),
        ...
        self._nb.add(my_tab, text=t("helpdesk.my_tickets"))

        my_top = tk.Frame(my_tab, bg="#ecf0f1")
        my_top.pack(fill="x", pady=(0, 5))
        ttk.Button(my_top, text=t("helpdesk.new_ticket"),
                   command=self._on_new_ticket).pack(side="left")
        ttk.Button(my_top, text=t("common.refresh"),
                   command=self._load_my_tickets).pack(side="left", padx=10)
        ttk.Button(my_top, text="Export CSV", command=self._export_csv).pack(side="left", padx=4)
        ...
        my_detail = tk.LabelFrame(my_tab, text=t("helpdesk.ticket_detail"),
        ...
        ttk.Button(resp_row, text=t("helpdesk.reply"),
        ...
        self._nb.add(self._all_tab, text=t("helpdesk.all_tickets"))

        all_filter = tk.Frame(self._all_tab, bg="#ecf0f1")
        all_filter.pack(fill="x", pady=(0, 5))
        tk.Label(all_filter, text=t("helpdesk.status_colon"), bg="#ecf0f1").pack(side="left")
        ...
        ttk.Button(all_filter, text=t("common.load"),
                   command=self._load_all_tickets).pack(side="left", padx=5)
        ttk.Button(all_filter, text="Export CSV",
                   command=self._export_all_csv).pack(side="left", padx=4)
        ...
        # Admin detail pane
        all_detail = tk.LabelFrame(self._all_tab, text=t("helpdesk.manage_ticket"),
                                   bg="#ecf0f1", font=("Helvetica", 10, "bold"))
        ...
        tk.Label(admin_row, text=t("helpdesk.set_status_colon"), bg="#ecf0f1").pack(side="left")
        ...
        ttk.Combobox(admin_row, textvariable=self._admin_status_var,
                     ...
                     width=12).pack(side="left", padx=5)
        ttk.Button(admin_row, text=t("helpdesk.update_status"),
        ...
        ttk.Button(admin_row, text=t("helpdesk.reply"),
    ...
    def _on_new_ticket(self):
        ...
        if dlg.result:
            try:
                ...
                messagebox.showinfo(t("common.success"), t("helpdesk.ticket_created"))
                self._load_my_tickets()
            except Exception as e:
                messagebox.showerror(t("common.error"), str(e))
    ...
    def _on_reply_my_ticket(self):
        sel = self._my_tree.selection()
        if not sel:
            messagebox.showwarning(t("common.selection_required"), t("helpdesk.select_ticket_first"))
            return
    ...
    def _export_csv(self):
        ...
        export_treeview_to_csv(self._my_tree, "helpdesk_my_tickets_export.csv")

    def _export_all_csv(self):
        from education_system.college_system.modules.shared.csv_export import export_treeview_to_csv
        export_treeview_to_csv(self._all_tree, "helpdesk_all_tickets_export.csv")
...
class _TicketDialog(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.title(t("helpdesk.new_ticket"))
        ...
        tk.Label(frame, text=t("helpdesk.subject_colon")).grid(row=0, column=0, sticky="w", pady=3)
        self._subject_var = tk.StringVar()
        ttk.Entry(frame, textvariable=self._subject_var, width=35).grid(
            row=0, column=1, pady=3)

        tk.Label(frame, text=t("helpdesk.category_colon")).grid(row=1, column=0, sticky="w", pady=3)
        ...
        tk.Label(frame, text=t("helpdesk.priority_colon")).grid(row=2, column=0, sticky="w", pady=3)
        ...
        tk.Label(frame, text=t("common.description_colon")).grid(row=3, column=0, sticky="nw", pady=3)
        ...
        ttk.Button(btn_frame, text=t("common.submit"), command=self._save).pack(
            side="left", padx=5)
    ...
    def _save(self):
        ...
        if not subject:
            messagebox.showwarning(t("common.input"), t("helpdesk.subject_required"), parent=self)
            return
        if not desc:
            messagebox.showwarning(t("common.input"), t("helpdesk.description_required"), parent=self)
            return
        self.result = {
            "subject": subject,
            "description": desc,
