...
class LibraryFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Library", font=("Helvetica", 14, "bold"),
        ...
        self._nb.add(self._catalogue_tab, text="Catalogue")
        self._build_catalogue_tab()

        self._loans_tab = tk.Frame(self._nb)
        self._nb.add(self._loans_tab, text="Loans")
        self._build_loans_tab()

        self._overdue_tab = tk.Frame(self._nb)
        self._nb.add(self._overdue_tab, text="Overdue")
    ...
    def _build_catalogue_tab(self):
        ...
        ttk.Button(toolbar, text="Search", command=self._load_catalogue).pack(side="left", padx=5)
        ttk.Button(toolbar, text="Add Item", command=self._add_item).pack(side="right", padx=5)
    ...
    def _build_loans_tab(self):
        ...
        toolbar.pack(fill="x", padx=5, pady=5)
        ttk.Button(toolbar, text="Refresh", command=self._load_loans).pack(side="left", padx=5)
        ttk.Button(toolbar, text="Checkout", command=self._checkout).pack(side="right", padx=5)
        ttk.Button(toolbar, text="Return", command=self._return_item).pack(side="right", padx=5)
        ttk.Button(toolbar, text="Renew", command=self._renew).pack(side="right", padx=5)
    ...
    def _build_overdue_tab(self):
        ...
        toolbar.pack(fill="x", padx=5, pady=5)
        ttk.Button(toolbar, text="Refresh", command=self._load_overdue).pack(side="left", padx=5)
    ...
    def _add_item(self):
        ...
        for label, key in [("Title*:", "title"), ("Author:", "author"),
                           ...
                           ("Location:", "location"), ("Copies:", "total_copies")]:
    ...
    def _checkout(self):
        win = tk.Toplevel(self)
        win.title("Checkout Item")
        win.geometry("300x200")
        fields = {}
        row = 0
        for label, key in [("Item ID*:", "item_id"), ("Borrower ID*:", "borrower_id"),
                           ("Due Date:", "due_date")]:
            tk.Label(win, text=label).grid(row=row, column=0, padx=10, pady=5, sticky="e")
        ...
        def save():
            try:
                ...
                messagebox.showerror("Error", str(e))
        ttk.Button(win, text="Checkout", command=save).grid(row=row, column=0, columnspan=2, pady=15)
    ...
    def _renew(self):
        sel = self._loan_tree.selection()
        if not sel:
            messagebox.showwarning("Warning", "Select a loan to renew")
            return
        try:
            self._svc.renew_loan(int(sel[0]))
            messagebox.showinfo("Success", "Loan renewed")
            self._load_loans()
        except Exception as e:
            messagebox.showerror("Error", str(e))
