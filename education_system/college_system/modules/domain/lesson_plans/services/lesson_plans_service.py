...
class LessonPlanService:
    ...
    def update_plan(self, pk: int, **kwargs) -> dict:
        """Update plan record."""
        allowed = {"course_id", "teacher_id", "lesson_date", "topic", "learning_objectives", "starter_activity", "main_activity", "plenary", "differentiation", "resources", "homework", "reflection", "status"}
        ...
        conn = self._conn()
        try:
            conn.execute(f"UPDATE lesson_plans SET {set_clause} WHERE id = ?", params)
            conn.commit()
