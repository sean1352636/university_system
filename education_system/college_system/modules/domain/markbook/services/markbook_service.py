...
class MarkbookService:
    ...
    def create_column(self, **kwargs) -> dict:
        ...
        try:
            ...
            _insert_vals: list = []
            for col in ('course_id', 'column_name', 'column_type', 'max_score', 'weight', 'due_date', 'display_order'):
                val = kwargs.get(col)
                if val is not None:
            ...
            conn.execute(
                f"INSERT INTO markbook_columns ({cols_sql}) VALUES ({ph_sql})",
                _insert_vals,
            )
            conn.commit()
            row = conn.execute(
    ...
    def update_column(self, pk: int, **kwargs) -> dict:
        ...
        conn = self._conn()
        try:
            conn.execute(f"UPDATE markbook_columns SET {set_clause} WHERE id = ?", params)  # nosec B608
            conn.commit()
