...
class ResourceBookingFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Resource Booking",
        ...
        self._tree.heading("resource_type", text="Type")
        ...
        self._tree.heading("is_available", text="Available")
    ...
    def _on_add(self):
        dlg = _ResourceDialog(self, title="Add Resource")
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
            self._svc.create_resource(**dlg.result)
            messagebox.showinfo("Success", "Resource created.")
            self._load_items()
        except Exception as exc:
    ...
    def _on_edit(self):
        ...
        if not item:
            ...
            return
        dlg = _ResourceDialog(self, title="Edit Resource", item=item)
        self.wait_window(dlg)
        if dlg.result is None:
            return
        try:
    ...
    def _on_delete(self):
        ...
        if pk is None:
            return
        if not messagebox.askyesno("Confirm", "Delete this resource?"):
            return
        try:
            self._svc.delete_resource(pk)
            messagebox.showinfo("Success", "Resource deleted.")
            self._load_items()
        except Exception as exc:
            messagebox.showerror("Error", str(exc))
