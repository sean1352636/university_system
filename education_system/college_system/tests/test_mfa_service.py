...
from education_system.college_system.core.exceptions import MFAError, AuthError


class TestMFAService:

    def test_setup_totp(self, mfa_service):
        result = mfa_service.setup_totp(1, "admin")
    ...
    def test_verify_invalid_totp(self, mfa_service):
        mfa_service.setup_totp(1, "admin")
    ...
    def test_auth_login_with_mfa(self, db_path):
        auth = UserAuth(db_path)
        mfa_svc = MFAService(db_path)
        ...
        auth2 = UserAuth(db_path)
        mfa_result = auth2.login("admin", "Admin@123")
