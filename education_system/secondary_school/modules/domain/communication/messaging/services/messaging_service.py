...
class MessagingService:
    ...
    def create(self, **kwargs):
        ...
        values: list = []
        for col in ('sender_id', 'sender_type', 'recipient_id', 'recipient_type', 'subject', 'body', 'is_read', 'read_at', 'parent_message_id'):
        ...
        try:
            cursor = conn.execute(
                f"INSERT INTO messages ({cols_sql}) VALUES ({ph_sql})",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
    ...
    def list_all(self, limit=100, offset=0, **filters):
        ...
        params: list = []
        for col in ('id', 'sender_id', 'sender_type', 'recipient_id', 'recipient_type', 'subject', 'body', 'is_read', 'read_at', 'parent_message_id'):
            val = filters.get(col)
            if val is not None:
        ...
        try:
            sql = (
                f"SELECT * FROM messages WHERE {where_clause} "  # noqa: S608
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            conn.execute(
                f"UPDATE messages SET {set_clause} WHERE id = ?",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
    ...
    def delete(self, record_id):
        conn = self._conn()
        try:
            conn.execute("DELETE FROM messages WHERE id = ?", (record_id,))
            conn.commit()
            return True
        finally:
            conn.close()
