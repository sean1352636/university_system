...
class DepartmentsService:
    ...
    def create(self, **kwargs):
        ...
        values: list = []
        for col in ('name', 'head_of_department', 'description', 'budget', 'email', 'phone', 'room', 'status'):
        ...
        try:
            cursor = conn.execute(
                f"INSERT INTO departments ({cols_sql}) VALUES ({ph_sql})",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
    ...
    def list_all(self, limit=100, offset=0, **filters):
        ...
        params: list = []
        for col in ('id', 'name', 'head_of_department', 'description', 'budget', 'email', 'phone', 'room', 'status'):
            val = filters.get(col)
            if val is not None:
        ...
        try:
            sql = (
                f"SELECT * FROM departments WHERE {where_clause} "  # noqa: S608
    ...
    def get(self, record_id):
        conn = self._conn()
        try:
            row = conn.execute("SELECT * FROM departments WHERE id = ?", (record_id,)).fetchone()
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            conn.execute(
                f"UPDATE departments SET {set_clause} WHERE id = ?",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM departments WHERE id = ?", (record_id,)
            ).fetchone()
            return dict(row) if row else None
        except Exception:
            conn.rollback()
