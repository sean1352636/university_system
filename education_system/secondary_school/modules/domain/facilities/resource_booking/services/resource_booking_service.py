...
class ResourceBookingService:
    ...
    def create(self, **kwargs):
        ...
        values: list = []
        for col in ('name', 'resource_type', 'description', 'location', 'capacity', 'is_available', 'notes'):
        ...
        try:
            cursor = conn.execute(
                f"INSERT INTO resources ({cols_sql}) VALUES ({ph_sql})",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
    ...
    def list_all(self, limit=100, offset=0, **filters):
        ...
        params: list = []
        for col in ('id', 'name', 'resource_type', 'description', 'location', 'capacity', 'is_available', 'notes'):
            val = filters.get(col)
            if val is not None:
        ...
        try:
            sql = (
                f"SELECT * FROM resources WHERE {where_clause} "  # noqa: S608
    ...
    def get(self, record_id):
        conn = self._conn()
        try:
            row = conn.execute("SELECT * FROM resources WHERE id = ?", (record_id,)).fetchone()
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            conn.execute(
                f"UPDATE resources SET {set_clause} WHERE id = ?",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM resources WHERE id = ?", (record_id,)
            ).fetchone()
            return dict(row) if row else None
        except Exception:
            conn.rollback()
    ...
    def delete(self, record_id):
        conn = self._conn()
        try:
            conn.execute("DELETE FROM resources WHERE id = ?", (record_id,))
            conn.commit()
            return True
        finally:
            conn.close()
