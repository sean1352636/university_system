...
class TargetSettingService:
    ...
    def create(self, **kwargs):
        ...
        values: list = []
        for col in ('student_id', 'subject_id', 'target_grade', 'current_grade', 'predicted_grade', 'set_by', 'review_date', 'status', 'notes'):
        ...
        try:
            cursor = conn.execute(
                f"INSERT INTO student_targets ({cols_sql}) VALUES ({ph_sql})",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM student_targets WHERE id = ?", (cursor.lastrowid,)
            ).fetchone()
    ...
    def list_all(self, limit=100, offset=0, **filters):
        ...
        params: list = []
        for col in ('id', 'student_id', 'subject_id', 'target_grade', 'current_grade', 'predicted_grade', 'set_by', 'review_date', 'status', 'notes'):
            val = filters.get(col)
            if val is not None:
        ...
        try:
            sql = (
                f"SELECT * FROM student_targets WHERE {where_clause} "  # noqa: S608
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            conn.execute(
                f"UPDATE student_targets SET {set_clause} WHERE id = ?",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM student_targets WHERE id = ?", (record_id,)
            ).fetchone()
            return dict(row) if row else None
        except Exception:
            conn.rollback()
