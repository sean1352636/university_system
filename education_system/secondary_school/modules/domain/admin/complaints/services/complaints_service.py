"""ComplaintsService service for the Secondary School."""
...
class ComplaintsService:
    ...
    def create(self, **kwargs):
        ...
        try:
            ...
            cursor.execute("SELECT * FROM complaints WHERE id = ?", (row_id,))
            row = cursor.fetchone()
    ...
    def list_all(self, limit=100, offset=0, **filters):
        """List complaint records with optional filters."""
        conn = self._conn()
        try:
    ...
    def get(self, record_id):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM complaints WHERE id = ?", (record_id,))
            row = cursor.fetchone()
