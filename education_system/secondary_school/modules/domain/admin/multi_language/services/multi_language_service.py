...
class MultiLanguageService:
    ...
    def create(self, **kwargs):
        ...
        values: list = []
        for col in ('language_code', 'translation_key', 'translation_value', 'context', 'is_verified'):
        ...
        try:
            cursor = conn.execute(
                f"INSERT INTO translations ({cols_sql}) VALUES ({ph_sql})",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM translations WHERE id = ?", (cursor.lastrowid,)
            ).fetchone()
    ...
    def list_all(self, limit=100, offset=0, **filters):
        ...
        params: list = []
        for col in ('id', 'language_code', 'translation_key', 'translation_value', 'context', 'is_verified'):
            val = filters.get(col)
            if val is not None:
        ...
        try:
            sql = (
                f"SELECT * FROM translations WHERE {where_clause} "  # noqa: S608
    ...
    def get(self, record_id):
        conn = self._conn()
        try:
            row = conn.execute("SELECT * FROM translations WHERE id = ?", (record_id,)).fetchone()
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            conn.execute(
                f"UPDATE translations SET {set_clause} WHERE id = ?",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
