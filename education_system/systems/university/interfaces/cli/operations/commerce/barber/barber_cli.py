...
class BarberCLI:
    ...
    def _process_refund(self):
        ...
        refund_type = input("Refund method (cash/card/student_account) [cash]: ").strip() or 'cash'
        reason = input(f"Reason [Refund via {refund_type}]: ").strip() or f"Refund via {refund_type}"
        processed_by = input("Processed by [cli-user]: ").strip() or 'cli-user'

        refund_id = RefundManager.issue_refund(
            transaction_id=transaction_id, amount=amount, reason=reason,
            refund_type=refund_type, processed_by=processed_by,
        )
        print(f"\nRefund processed (Refund ID: {refund_id})")
    ...
    def _email_admin_report(self):
        """Generate the admin report and email it to an admin."""
        ...
        if not row or not row['email']:
            print("\nNo admin email found. Report not sent.")
            return
        ...
        send_email(
            ...
            subject=f"Barber Shop Admin Report - {date}",
            body=report,
        )
        print(f"\nAdmin report emailed to {recipient_email}")

    def _export_transactions_csv(self):
        """Export barber transactions to a CSV file."""
        default_path = f"barber_transactions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        file_path = input(f"Output file path [{default_path}]: ").strip() or default_path

        conn = get_db_connection()
        try:
            rows = conn.execute(
                """
                SELECT bt.transaction_id, bt.created_at, bt.customer_id,
                  COALESCE(s.first_name || ' ' || s.last_name, bt.customer_id),
                  bt.amount, bt.tip_amount, bt.payment_method, bt.status, bt.reference_number
        ...
        headers = [
            ...
            'Service Amount', 'Tip Amount', 'Total Amount',
        ...
        with open(file_path, 'w', newline='') as f:
            ...
            for r in rows:
                txn_id, created_at, customer_id, customer_name, amount, tip, method, status, ref = r
                total = (amount or 0) + (tip or 0)
                writer.writerow([
                    txn_id, created_at, customer_id, customer_name,
                    amount, tip, total, method, status, ref,
                ])

        print(f"\nExported {len(rows)} transaction(s) to {os.path.abspath(file_path)}")
