...
class ChaptersMixin:
        ...
        def join_regional_chapter(self):
            ...
            ttk.Button(button_frame, text="My Chapters",
                      command=self.view_my_chapters).pack(side=tk.LEFT, padx=(0, 10))
            ttk.Button(button_frame, text="Create Chapter",
                      command=self.create_regional_chapter).pack(side=tk.LEFT, padx=(0, 10))
            ttk.Button(button_frame, text="🔄 Refresh",
                      command=self.join_regional_chapter).pack(side=tk.LEFT)

        def _ensure_chapters_schema(self):
            """Ensure regional_chapters has the columns the chapter features use
            and that the chapter_members table exists. Idempotent."""
            try:
                with db_get_connection() as conn:
                    for ddl in (
                        "ALTER TABLE regional_chapters ADD COLUMN coordinator_name TEXT",
                        "ALTER TABLE regional_chapters ADD COLUMN contact_email TEXT",
                        "ALTER TABLE regional_chapters ADD COLUMN created_by TEXT",
                        "ALTER TABLE regional_chapters ADD COLUMN status TEXT DEFAULT 'active'",
                    ):
                        try:
                            conn.execute(ddl)
                ...
                import logging
                logging.warning(f"Chapters schema ensure failed: {e}")

        def show_regional_chapters(self):
            """Regional chapters — functional, DB-backed (browse + join, with
            links to My Chapters and Create Chapter)."""
            self._ensure_chapters_schema()
            self.join_regional_chapter()
