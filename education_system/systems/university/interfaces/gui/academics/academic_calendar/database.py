...
from education_system.systems.university.interfaces.gui.academics.academic_calendar.exceptions import DatabaseError, ExportError
...
def init_calendar_database(db_path: Optional[str] = None) -> DatabaseManager:
    ...
    try:
        with db.transaction() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_scheduled_events_date ON scheduled_events(date)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_scheduled_events_type ON scheduled_events(event_type)")
