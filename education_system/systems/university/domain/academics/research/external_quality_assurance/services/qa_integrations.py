...
def enqueue_qa_outbox(event_type: str, payload: dict) -> bool:
    ...
    isn't installed."""
    try:
        from education_system.platform.offline_sync import outbox  # type: ignore
    except Exception:
        return False
    try:
...
def aggregate_qa_across_systems() -> dict:
    ...
    candidates = [
        ("university",
         "education_system.systems.university.domain.academics.research."
...
def draft_tef_narrative(section: str, year: int,
    ...
    try:
        from education_system.systems.university.interfaces.cli.shell.chatbot_integration import (
