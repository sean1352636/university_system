...
def initialize_degree_audit_database():
    ...
    with transaction() as conn:
        ...
        # ---- Migrations -------------------------------------------------
        # Payment tracking for gown orders. Added after the table shipped, so
        # we ALTER existing databases in-place (CREATE TABLE IF NOT EXISTS
        # above never adds new columns to a pre-existing table).
        _add_columns_if_missing(cursor, 'ceremony_gown_orders', {
...
def _add_columns_if_missing(cursor, table: str, columns: dict) -> None:
    """Idempotently add any missing *columns* to *table*.

    ``columns`` maps column name -> column definition (type + default). Columns
    that already exist are skipped, so this is safe to run on every startup.
    """
