...
from education_system.systems.university.infrastructure.institution_settings import get_elections_phone
...
def track_campaign_expenses(cursor, conn):
    ...
    try:
        ...
        if choice == '1':
            ...
            # Upload a receipt file and attach it to an existing expense.
            import shutil
            from pathlib import Path

            from education_system.systems.university.infrastructure import paths

            candidate_ids = [c[0] for c in candidacies]
            placeholders = ','.join('?' * len(candidate_ids))
            cursor.execute(f'''
            SELECT expense_id, amount, description, expense_date, receipt_path
            FROM campaign_expenses
            ...
            if not expenses:
                print("You have no recorded expenses to attach a receipt to.")
                return

            print("Your recorded expenses:")
            for i, exp in enumerate(expenses):
                status = "has receipt" if exp[4] else "no receipt"
                print(f"{i+1}. £{exp[1]:.2f} - {exp[2]} ({exp[3]}) [{status}]")

            exp_choice = input("Select expense to attach a receipt (enter number): ").strip()
            if (not exp_choice.isdigit() or int(exp_choice) < 1
                    or int(exp_choice) > len(expenses)):
                print("Invalid selection.")
                return

            expense_id = expenses[int(exp_choice) - 1][0]

            source_path = input("Path to receipt file (image or PDF): ").strip()
            if not source_path:
            ...
            # Validate that the file is a genuine image or PDF.
            ...
            if suffix == '.pdf':
                ...
                try:
                    ...
                    print("File is not a valid image or supported receipt format.")
                    return

            # Copy the validated file into the managed uploads directory.
            dest_dir = paths.UPLOADS_DIR / "campaign_receipts"
            dest_dir.mkdir(parents=True, exist_ok=True)
            dest_name = (f"expense_{expense_id}_"
                         f"{datetime.now().strftime('%Y%m%d%H%M%S')}{suffix}")
            dest_path = dest_dir / dest_name
            try:
                shutil.copy2(src, dest_path)
            except OSError as err:
                print(f"Failed to store receipt: {err}")
                return
            ...
            SET receipt_path = ?
            WHERE expense_id = ?
            ''', (str(dest_path), expense_id))
            conn.commit()

            print(f"✅ Receipt uploaded and attached to expense #{expense_id}.")
...
def monitor_campaign_compliance(cursor):
    ...
    global auth
    from education_system.systems.university.domain.pastoral.student_life.student_union.services.voting import (
...
def election_security_audit(cursor):
    ...
    global auth
    from education_system.systems.university.domain.pastoral.student_life.student_union.services.voting import access_control_review
    from education_system.systems.university.domain.pastoral.student_life.student_union.administration.admin_management import (
