"""Tkinter views for EYFS Profile (Nursery System).

Renders into the shared content pane of ``main_gui.NurseryMainGUI`` (the
``host``). Shows the EYFS profile list with a tree + toolbar and an add/edit
form dialog — the GUI counterpart of ``eyfs_profile_cli.py``.
"""
...
from education_system.systems.nursery.domain.assessment.eyfs_profile import (
    eyfs_profile as data,
)
from education_system.systems.nursery.domain.assessment.eyfs_profile.eyfs_profile import (
    AREA_FIELDS,
    AREA_LABELS,
    BANDS,
    STATUSES,
...
# (key, label, kind) — kind one of: entry / band / status / staff
_FIELDS: list[tuple[str, str, str]] = [
    ("assessment_date",       "Assessment date (YYYY-MM-DD)", "entry"),
    ("term",                  "Term",                         "entry"),
    ("communication_language","Communication & Language",     "band"),
    ("physical_development",  "Physical Development",         "band"),
    ("pse_development",       "PSE Development",              "band"),
    ("literacy",              "Literacy",                     "band"),
    ("mathematics",           "Mathematics",                  "band"),
    ("understanding_world",   "Understanding the World",      "band"),
    ("expressive_arts",       "Expressive Arts & Design",     "band"),
    ("summary",               "Summary",                      "entry"),
    ("assessor",              "Assessor",                     "staff"),
...
def open_manager(host) -> None:
    logger.debug("GUI: eyfs_profile open_manager")
    root = _clear(host)
    _header(root, "EYFS Profile")

    bar = ttk.Frame(root)
    bar.pack(fill="x", pady=(0, 8))
    ttk.Button(bar, text="Add Profile",
    ...
    cols = ("id", "child", "date", "term", "status", "assessor")
    tree = ttk.Treeview(root, columns=cols, show="headings", height=18)
    for c, label, w in [
        ("id", "ID", 70), ("child", "Child", 180), ("date", "Date", 100),
        ("term", "Term", 120), ("status", "Status", 90), ("assessor", "Assessor", 160),
    ]:
    ...
    _refresh(tree)
    host.status_var.set("EYFS profiles loaded")
...
def _refresh(tree: ttk.Treeview) -> None:
    for i in tree.get_children():
        tree.delete(i)
    try:
        rows = data.list_profiles()
    except Exception:
        logger.exception("Could not refresh EYFS profiles")
        return
    for p in rows:
        tree.insert("", "end", iid=p.profile_id, values=(
            p.profile_id, p.child_name or "-", p.assessment_date or "-",
            p.term or "-", p.status or "-", p.assessor_name or "-"))


def _selected(tree: ttk.Treeview, host, verb: str) -> str | None:
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("EYFS Profile", f"Select a profile to {verb}.",
                            parent=host.root)
        return None
    return sel
...
def _delete_selected(tree: ttk.Treeview, host) -> None:
    ...
    if not sel:
        return
    if not messagebox.askyesno("Delete profile", f"Delete profile {sel}?",
                               parent=host.root):
        return
    try:
        data.delete_profile(sel)
    except Exception as e:  # noqa: BLE001
        logger.exception("Failed to delete EYFS profile %s", sel)
        messagebox.showerror("Delete profile", f"Could not delete:\n\n{e}",
                             parent=host.root)
        return
    open_manager(host)
    host.status_var.set(f"Deleted profile {sel}")
...
def _form_dialog(host, title: str, *, initial: dict[str, Any] | None = None,
    ...
    for key, label, kind in _FIELDS:
        ...
        if kind == "band":
            ...
            ttk.Combobox(frm, textvariable=v, values=[""] + list(BANDS),
...
def open_add(host) -> None:
    fields = _form_dialog(host, "Add EYFS Profile",
                          pupil_choices=_pupil_choices())
    if not fields:
        host.status_var.set("Add profile cancelled")
        open_manager(host)
        return
    if not fields.get("pupil_id"):
        messagebox.showerror("Add profile", "Please choose a child.",
                             parent=host.root)
        open_manager(host)
        return
    try:
        ...
        messagebox.showerror("Add profile", str(e), parent=host.root)
        open_manager(host)
        return
    host.status_var.set(f"Added EYFS profile {p.profile_id}")
    open_manager(host)


@_safe_view
def open_edit(host, profile_id: str) -> None:
    p = data.get_profile(profile_id)
    if p is None:
        messagebox.showerror("Edit profile", f"No profile with id {profile_id}",
                             parent=host.root)
        return
    initial = {key: getattr(p, key) for key, _l, _k in _FIELDS}
    fields = _form_dialog(host, f"Edit profile — {p.child_name}",
                          initial=initial, is_edit=True)
    if not fields:
        return
    try:
        data.update_profile(profile_id, fields)
    except ValidationError as e:
        messagebox.showerror("Edit profile", str(e), parent=host.root)
        return
    host.status_var.set(f"Updated profile {profile_id}")
    open_manager(host)


def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    frame = ttk.Frame(parent, padding=16)
    ttk.Label(frame, text="EYFS Profile", font=("", 14, "bold")).pack(
        anchor="w", pady=(0, 6))
    ttk.Label(frame, text="Open EYFS Profile from the navigation menu.").pack(
        anchor="w")
    return frame
