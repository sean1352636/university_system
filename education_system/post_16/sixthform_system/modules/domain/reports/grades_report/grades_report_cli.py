...
from education_system.post_16.sixthform_system.modules.domain.reports.grades_report import grades_report
from education_system.post_16.sixthform_system.modules.domain.reports.grades_report import grades_report as data
from education_system.post_16.sixthform_system.modules.domain.students.students import students as student_data
from education_system.post_16.sixthform_system.modules.domain.academics.subjects import subjects as subject_data
from education_system.post_16.sixthform_system.modules.domain.reports.grades_report.grades_report import (
    GradeDistribution,
    GRADE_POINTS,
    LETTER_GRADES,
    PASS_GRADES,
...
def _pick_subject() -> str:
    ...
    for i, s in enumerate(subs, 1):
        ...
        print(f"    {i:>3}) {name}")
    while True:
        raw = _input("Pick # or type name", allow_empty=False)
        if raw.isdigit():
            n = int(raw)
            if 1 <= n <= len(subs):
