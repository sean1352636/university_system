...
def group_rollup(group_id: int) -> GroupRollup:
    ...
    open_concerns = 0
    try:
        from education_system.post_16.sixthform_system.modules.domain.reports.progress import (
            progress as pr,
        )
        for sid in members:
    ...
    try:
        from education_system.post_16.sixthform_system.modules.domain.pastoral.attendance_concerns import (
...
def timetable_clashes() -> list[Clash]:
    ...
    from education_system.post_16.sixthform_system.modules.domain.academics.timetable import (
        timetable as tt,
    )
    slots = tt.list_slots()
    out: list[Clash] = []
...
def student_group_index() -> dict[str, tuple[int | None, str | None]]:
    ...
    try:
        from education_system.post_16.sixthform_system.modules.domain.pastoral.tutor_groups import (
            tutor_groups as tg,
        )
...
def _search_accommodations(parsed: ParsedQuery,
                           opts: QueryOptions,
                           _filters: dict) -> list[Hit]:
    from education_system.post_16.sixthform_system.modules.domain.pastoral.accessibility import (
        accessibility as m,
    )
...
def _search_safeguarding(parsed: ParsedQuery,
                         opts: QueryOptions,
                         _filters: dict) -> list[Hit]:
    from education_system.post_16.sixthform_system.modules.domain.pastoral.safeguarding import (
        safeguarding as m,
    )
    rows = m.list_concerns()
    out: list[Hit] = []
...
def _search_notices(parsed: ParsedQuery,
                    opts: QueryOptions,
                    _filters: dict) -> list[Hit]:
    from education_system.post_16.sixthform_system.modules.domain.staff_comms.notices import (
        notices as m,
    )
    rows = m.list_notices()
    out: list[Hit] = []
...
def _search_homework(parsed: ParsedQuery,
                     opts: QueryOptions,
                     _filters: dict) -> list[Hit]:
    from education_system.post_16.sixthform_system.modules.domain.academics.homework import (
        homework as m,
    )
...
def _search_parents_evenings(parsed: ParsedQuery,
                             opts: QueryOptions,
                             _filters: dict) -> list[Hit]:
    from education_system.post_16.sixthform_system.modules.domain.staff_comms.parents_evenings import (
...
def _search_surveys(parsed: ParsedQuery,
                    opts: QueryOptions,
                    _filters: dict) -> list[Hit]:
    from education_system.post_16.sixthform_system.modules.domain.pastoral.surveys import (
        surveys as m,
    )
    rows = m.list_surveys()
    out: list[Hit] = []
...
def _search_ucas(parsed: ParsedQuery,
    ...
    try:
        from education_system.post_16.sixthform_system.modules.domain.progression.references import (
            references as r,
        )
...
def _search_finance(parsed: ParsedQuery,
    ...
    out: list[Hit] = []
    try:
        from education_system.post_16.sixthform_system.modules.domain.finance.fees import (
            fees as f,
        )
...
def _search_documents(parsed: ParsedQuery,
                      opts: QueryOptions,
                      _filters: dict) -> list[Hit]:
    from education_system.post_16.sixthform_system.modules.domain.staff_comms.document_hub import (
        document_hub as m,
    )
...
def _search_audit_logs(parsed: ParsedQuery,
                       opts: QueryOptions,
                       _filters: dict) -> list[Hit]:
    from education_system.post_16.sixthform_system.core import log_store as m
...
def _students_without_consent(purpose: str) -> set[str]:
    ...
    out: set[str] = set()
    try:
        from education_system.post_16.sixthform_system.modules.domain.governance.gdpr import (
            gdpr as g,
        )
...
def _try_push_notification(sched: ScheduledSearch, delta: int,
    ...
    try:
        from education_system.post_16.sixthform_system.modules.domain.staff_comms.notifications import (
