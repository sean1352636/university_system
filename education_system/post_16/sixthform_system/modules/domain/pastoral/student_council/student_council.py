"""Student Council data layer for Sixth Form System.

Three tables:
- `student_council_members`: per-student membership with role,
  term-of-office and status. FK cascade on students.
- `student_council_meetings`: free-standing meeting records with
  agenda, minutes, attendees and decisions.
- `student_council_motions`: motions tied to a meeting (FK cascade)
...
def _connect() -> Iterator[sqlite3.Connection]:
    ...
    conn = sqlite3.connect(_paths.STUDENT_COUNCIL_DB)
...
def init_db() -> None:
    with _connect() as conn:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS student_council_members (
        ...
        """)
        conn.execute("""
            CREATE TABLE IF NOT EXISTS student_council_meetings (
        ...
        """)
        conn.execute("""
            CREATE TABLE IF NOT EXISTS student_council_motions (
                ...
                FOREIGN KEY(meeting_id)
                  REFERENCES student_council_meetings(meeting_id)
                  ON DELETE CASCADE
            )
        """)
...
def _validate_member(p: dict[str, Any], *,
    ...
    if status == "Active" and role != "Member":
        with _connect() as conn:
            row = conn.execute(
                "SELECT member_id FROM student_council_members "
...
def create_member(payload: dict[str, Any]) -> Member:
    ...
    with _connect() as conn:
        cur = conn.execute("""
            INSERT INTO student_council_members (
...
def update_member(member_id: int,
    ...
    with _connect() as conn:
        conn.execute("""
            UPDATE student_council_members SET
...
def delete_member(member_id: int) -> bool:
    ...
    with _connect() as conn:
        cur = conn.execute(
            "DELETE FROM student_council_members "
...
def get_member(member_id: int) -> Member | None:
    ...
    with _connect() as conn:
        r = conn.execute(
            "SELECT * FROM student_council_members "
...
def list_members(*,
    ...
    init_db()
    sql = "SELECT * FROM student_council_members WHERE 1=1"
    params: list[Any] = []
    if student_id:
        sql += " AND student_id=?"
...
def create_meeting(payload: dict[str, Any]) -> Meeting:
    ...
    with _connect() as conn:
        cur = conn.execute("""
            INSERT INTO student_council_meetings (
...
def update_meeting(meeting_id: int,
    ...
    with _connect() as conn:
        conn.execute("""
            UPDATE student_council_meetings SET
...
def delete_meeting(meeting_id: int) -> bool:
    ...
    with _connect() as conn:
        cur = conn.execute(
            "DELETE FROM student_council_meetings "
...
def get_meeting(meeting_id: int) -> Meeting | None:
    ...
    with _connect() as conn:
        r = conn.execute(
            "SELECT * FROM student_council_meetings "
...
def list_meetings(*,
    ...
    init_db()
    sql = "SELECT * FROM student_council_meetings WHERE 1=1"
    params: list[Any] = []
    if meeting_type:
...
def _next_motion_number(meeting_id: int) -> int:
    with _connect() as conn:
        row = conn.execute(
            ...
            "FROM student_council_motions WHERE meeting_id=?",
            (meeting_id,)).fetchone()
...
def add_motion(meeting_id: int,
    ...
    with _connect() as conn:
        cur = conn.execute("""
            INSERT INTO student_council_motions (
...
def update_motion(motion_id: int,
    ...
    with _connect() as conn:
        conn.execute("""
            UPDATE student_council_motions SET
...
def delete_motion(motion_id: int) -> bool:
    ...
    with _connect() as conn:
        cur = conn.execute(
            "DELETE FROM student_council_motions "
...
def get_motion(motion_id: int) -> Motion | None:
    ...
    with _connect() as conn:
        r = conn.execute(
            "SELECT * FROM student_council_motions "
...
def list_motions(*,
    ...
    init_db()
    sql = "SELECT * FROM student_council_motions WHERE 1=1"
    params: list[Any] = []
