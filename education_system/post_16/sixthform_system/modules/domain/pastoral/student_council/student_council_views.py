"""Tkinter views for Sixth Form Student Council."""
...
from education_system.post_16.sixthform_system.modules.domain.pastoral.student_council import (
    student_council as data,
)
from education_system.post_16.sixthform_system.modules.domain.pastoral.student_council.student_council import (
...
def open_student_council_window(parent=None) -> None:
    data.init_db()
    master = getattr(parent, "root", parent)
    win = tk.Toplevel(master) if master is not None else tk.Tk()
    win.title(f"Student Council — {branding.SYSTEM_NAME}")
...
class MembersTab:
    ...
    def _selected(self) -> Member | None:
        ...
        if not sel:
            messagebox.showinfo("Student Council",
    ...
    def _end_term(self) -> None:
        ...
        try:
            ...
            messagebox.showerror("Student Council", str(exc),
                                    parent=self.frame)
            return
        self.refresh()
...
class SummaryTab:
    ...
    def refresh(self) -> None:
        ...
        self.text.insert("end", "Student Council Summary\n")
...
class MemberEditor:
    ...
    def _save(self) -> None:
        ...
        if idx < 0:
            messagebox.showerror("Student Council",
                                    "Pick a student.",
                                    parent=self.win)
            return
        payload: dict = {
...
__all__ = ["open_student_council_window"]
