...
def create_operations_tab(parent_frame, service):
    ...
    # Logs sub-tab — moved from System Administration → Operations →
    # Monitoring & Logs (8.117.81). One-click entry into the unified
    # Operations Console plus shortcuts to each individual tool.
    logs_frame = ttk.Frame(notebook)
    notebook.add(logs_frame, text="Logs")
    _build_logs_tab(logs_frame)


def _build_logs_tab(parent):
    """Render the Logs section: launchers for the unified Operations
    Console and each of its five tools."""
    container = ttk.Frame(parent, padding=15)
    container.pack(fill=tk.BOTH, expand=True)

    ttk.Label(container, text="Monitoring & Logs",
              font=('Arial', 14, 'bold')).pack(anchor='w', pady=(0, 5))
    ttk.Label(
        container,
        text=("Opens the unified Operations Console — Activity Log, "
              "Audit Viewer, Activity Logger, Log Analyzer and System "
              "Monitoring all in one window."),
        foreground="#555", wraplength=700).pack(anchor='w', pady=(0, 15))
    ...
    # Resolve the launcher methods off the UnifiedManagementGUI instance
    # via the parent chain. The dashboard is hosted in self.content_frame
    # of that GUI; ``parent.winfo_toplevel()`` gives us the root window,
    # and we look the GUI instance up via a private attribute the main
    # GUI sets at construction. Falls back to a friendly error if absent.
    def _resolve_gui():
        widget = parent
        while widget is not None:
            gui = getattr(widget, '_unified_gui', None)
            if gui is not None:
                return gui
            widget = getattr(widget, 'master', None)
        return None

    def _launch(method_name):
        from tkinter import messagebox
        gui = _resolve_gui()
        if gui is None:
            messagebox.showerror(
                "Logs",
                "Could not locate the main GUI instance from this tab.")
            return
        method = getattr(gui, method_name, None)
        if method is None:
            messagebox.showerror(
                "Logs", f"{method_name} is not available on the main GUI.")
            return
        try:
            method()
        except Exception as e:
            logger.exception(f"Logs launcher failed: {method_name}")
            messagebox.showerror("Logs", f"Failed to open: {e}")

    ttk.Button(grid, text="Operations Console",
               command=lambda: _launch('show_operations_console'),
