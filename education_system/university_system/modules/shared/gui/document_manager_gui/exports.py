        ...
        try:
            ...
            with open(file_path, 'w', newline='') as f:
                ...
                with get_connection() as conn:
                    ...
                    writer.writerows([tuple(r) for r in results])
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            headers = ['Document ID', 'Student ID', 'Document Type', 'File Name',
                      'Status', 'Upload Date', 'Expiry Date', 'File Size (KB)']
            ...
            with get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    LIMIT 1000
                """)
                documents = [tuple(r) for r in cursor.fetchall()]
                ...
                cursor.execute("""
                    ...
                    ORDER BY count DESC
                """)
                type_stats = [tuple(r) for r in cursor.fetchall()]
