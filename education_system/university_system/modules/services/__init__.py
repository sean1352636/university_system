...
``education_system.university_system.modules.services`` — including
...
def __getattr__(name):
    if name in _LAZY_GUI:
        try:
            mod = importlib.import_module(
                "education_system.university_system.modules.services.gui"
            )
    ...
    if name in _LAZY_CLI:
        try:
            mod = importlib.import_module(
                "education_system.university_system.modules.services.cli"
            )
            value = getattr(mod, name, None)
        except ImportError:
            value = None
