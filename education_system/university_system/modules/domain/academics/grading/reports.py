...
def _get_grading_imports():
    ...
    from education_system.university_system.modules.domain.academics.grading.grade_calculation import calculate_student_gpa, GRADE_SYSTEMS
...
def generate_module_stats_report(cursor, module_code, reports_dir, timestamp):
    ...
    try:
        ...
        elements.append(Paragraph(f"Module Statistical Report", title_style))
...
def generate_all_courses_stats_report(cursor, courses, reports_dir, timestamp):
    ...
    try:
        ...
        elements.append(Paragraph(f"Course Comparison Report", title_style))
        elements.append(Paragraph(f"Generated on {datetime.now().strftime('%B %d, %Y')}", styles['Normal']))
        elements.append(Spacer(1, 12))
...
def generate_comprehensive_stats_report(cursor, reports_dir, timestamp):
    ...
    try:
        ...
        elements.append(Paragraph(f"Comprehensive Performance Analysis", title_style))
        elements.append(Paragraph(f"Generated on {datetime.now().strftime('%B %d, %Y')}", styles['Normal']))
        elements.append(Spacer(1, 12))
        ...
        # Add insights
        elements.append(Paragraph(f"<b>Overall Academic Performance:</b>", styles['Normal']))
        ...
        elements.append(Spacer(1, 6))
        elements.append(Paragraph(f"<b>Student Achievement:</b>", styles['Normal']))
        ...
        if top_modules and bottom_modules:
            elements.append(Spacer(1, 6))
            elements.append(Paragraph(f"<b>Module Performance:</b>", styles['Normal']))
