...
class AnalyticsManager:
    ...
    def email_custom_report(self):
        ...
        try:
            ...
            # Format report data
            column_headers = " | ".join(report_data['columns'])
            separator = "-" * len(column_headers)

            # Format rows (limit to first 100 for email)
            report_rows = ""
            for i, row in enumerate(report_data['rows'][:100]):
                report_rows += " | ".join(str(val) for val in row) + "\n"
            ...
            try:
                ...
                subject, email_body = render_template("assignment_analytics_report", {
                ...
                email_body += column_headers + "\n"
                email_body += separator + "\n"
                email_body += report_rows
                if row_count_info:
                    email_body += "\n" + row_count_info + "\n"
