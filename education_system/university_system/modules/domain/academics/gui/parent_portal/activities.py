...
def request_activity_enrollment(self, activity_name):
    """Request enrollment in an activity with DB record and email confirmation"""
    if not self.children:
        messagebox.showwarning("No Students",
                               "No students linked to your guardian account.")
        return

    # If multiple children, ask which one
    if len(self.children) == 1:
        child = self.children[0]
    else:
        # Build selection dialog
        sel_dialog = tk.Toplevel(self.root)
        sel_dialog.title("Select Student")
        sel_dialog.geometry("400x200")
        sel_dialog.transient(self.root)
        sel_dialog.grab_set()

        ttk.Label(sel_dialog, text="Select the student to enroll:",
                 font=('Arial', 11)).pack(pady=15)
        child_var = tk.StringVar()
        child_combo = ttk.Combobox(sel_dialog, textvariable=child_var, width=40, state="readonly")
        child_combo['values'] = [f"{c[1]} {c[3]} (ID: {c[0]})" for c in self.children]
        ...
        def on_select():
            ...
            if idx >= 0:
                selected[0] = self.children[idx]
            sel_dialog.destroy()

        ttk.Button(sel_dialog, text="Select", command=on_select).pack(pady=15)
        sel_dialog.wait_window()
    ...
    student_id = child[0]
    student_name = f"{child[1]} {child[3]}"

    try:
        conn = sqlite3.connect(str(DEFAULT_DB_PATH))
        cursor = conn.cursor()

        # Look up the activity id
        cursor.execute("""
        SELECT id FROM extracurricular_activities
        WHERE activity_name = ? AND (status = 'active' OR status IS NULL)
        LIMIT 1
        """, (activity_name,))
        activity_row = cursor.fetchone()

        if not activity_row:
            messagebox.showwarning("Activity Not Found",
                                   f"Could not find activity '{activity_name}' in the database.")
            conn.close()
            return
        ...
        if existing:
            status = existing[1]
            if status == 'active':
                messagebox.showinfo("Already Enrolled",
                                    f"{student_name} is already enrolled in '{activity_name}'.")
            else:
                messagebox.showinfo("Request Pending",
                                    f"An enrollment request for {student_name} in '{activity_name}' "
        ...
        cursor.execute("""
        INSERT INTO student_activities (student_id, activity_id, enrollment_date, status)
        VALUES (?, ?, ?, 'pending')
        """, (student_id, activity_id, enrollment_date))

        conn.commit()

        # Look up student email for confirmation
        cursor.execute("""
        SELECT email_address FROM students WHERE student_id = ?
        """, (student_id,))
        email_row = cursor.fetchone()
        student_email = email_row[0] if email_row and email_row[0] else None

        conn.close()

        # Send confirmation email to student
        if student_email and EMAIL_SERVICE_AVAILABLE:
            try:
                email_subject = f"Activity Enrollment Request: {activity_name}"
                email_body = (
                    f"Dear {student_name},\n\n"
                    f"A request to enroll you in the extracurricular activity "
                    f"'{activity_name}' has been submitted by your parent/guardian.\n\n"
                    f"Request Date: {enrollment_date}\n"
                    f"Status: Pending Review\n\n"
                    f"The university administration will review this request and "
                    f"confirm your enrollment. You will be notified once a decision "
                    f"has been made.\n\n"
                    f"If you did not expect this request, please contact your "
                    f"parent/guardian or the student affairs office.\n\n"
                    f"Best regards,\n"
                    f"University Administration"
                )
                send_email(student_email, email_subject, email_body)
            except Exception as e:
                # Email failure shouldn't block the enrollment request
                print(f"Warning: Could not send confirmation email: {e}")

        messagebox.showinfo("Enrollment Request Submitted",
                           f"Enrollment request for {student_name} in '{activity_name}' "
                           f"has been submitted successfully.\n\n"
                           f"Status: Pending Review\n"
                           + (f"A confirmation email has been sent to {student_email}."
                              if student_email and EMAIL_SERVICE_AVAILABLE
                              else "No email address on file for this student."))

    except Exception as e:
        messagebox.showerror("Error", f"Failed to submit enrollment request: {str(e)}")
