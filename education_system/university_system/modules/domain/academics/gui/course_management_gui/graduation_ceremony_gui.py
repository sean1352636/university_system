...
from education_system.university_system.modules.domain.academics.services.degree_audit.graduation_ceremony import (
    CeremonyManager, GownOrderManager, RsvpManager, SeatPlanManager,
    StageScriptManager,
)
from education_system.university_system.modules.domain.academics.services.degree_audit.graduation_records import (
...
class GraduationCeremonyGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            outer, text="🎓 Graduation Ceremony Management",
            font=("Arial", 16, "bold"),
    ...
    def _build_ceremonies_tab(self):
        ...
        labels = ["Name", "Date (YYYY-MM-DD)", "Time (HH:MM)", "Venue",
                  "Capacity", "Cohort filter (e.g. BSc CS 2025/26)",
                  "RSVP deadline", "Guests per graduand"]
        today = datetime.now().strftime("%Y-%m-%d")
        defaults = ["Summer Graduation", today, "14:00", "Main Hall",
                    "300", "", "", "2"]
        self._cer_entries = {}
        for i, (lbl, dft) in enumerate(zip(labels, defaults)):
            ttk.Label(form, text=lbl + ":").grid(row=i // 2, column=(i % 2) * 2, sticky="e", padx=6, pady=4)
            e = ttk.Entry(form, width=28)
            e.insert(0, dft)
            e.grid(row=i // 2, column=(i % 2) * 2 + 1, sticky="w", padx=6, pady=4)
            self._cer_entries[lbl] = e
    ...
    def _on_cer_tree_select(self, _evt=None):
        ...
        for c in self._ceremonies:
            if c['ceremony_id'] == cid:
                ...
                self._cer_entries["Date (YYYY-MM-DD)"].delete(0, tk.END); self._cer_entries["Date (YYYY-MM-DD)"].insert(0, c['ceremony_date'])
                self._cer_entries["Time (HH:MM)"].delete(0, tk.END); self._cer_entries["Time (HH:MM)"].insert(0, c['ceremony_time'])
                self._cer_entries["Venue"].delete(0, tk.END); self._cer_entries["Venue"].insert(0, c['venue'])
                ...
                self._cer_entries["RSVP deadline"].delete(0, tk.END); self._cer_entries["RSVP deadline"].insert(0, c.get('rsvp_deadline') or '')
    ...
    def _form_ceremony_payload(self) -> dict:
        ...
        return {
            ...
            'ceremony_date': self._cer_entries["Date (YYYY-MM-DD)"].get().strip(),
            'ceremony_time': self._cer_entries["Time (HH:MM)"].get().strip(),
            'venue':         self._cer_entries["Venue"].get().strip(),
            'capacity':      capacity,
            'cohort_filter': self._cer_entries["Cohort filter (e.g. BSc CS 2025/26)"].get().strip(),
            'rsvp_deadline': self._cer_entries["RSVP deadline"].get().strip(),
    ...
    def _on_create_ceremony(self):
        ...
        missing = [name for name, key in (
            ("Name", "ceremony_name"),
            ("Date (YYYY-MM-DD)", "ceremony_date"),
            ("Time (HH:MM)", "ceremony_time"),
            ("Venue", "venue"),
        ) if not p.get(key)]
        if missing:
    ...
    def _build_gown_tab(self):
        ...
        cols = ("oid", "sid", "name", "gown", "hat", "hood", "supplier", "slot", "status")
        self.gown_tree = ttk.Treeview(table, columns=cols, show="headings", height=18)
        for c, w in zip(cols, (50, 90, 200, 60, 60, 160, 160, 160, 90)):
    ...
    def _on_issue_package(self):
        ...
        if result.get('alumni'):
            lines.append(f"Alumni record: provisioned (alumni_id={result['alumni'].get('alumni_id')})")
