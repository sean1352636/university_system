...
def _email_admin(db, subject: str, body: str, sender_username: str = "") -> int:
    """Deliver the report to admin/superadmin in-app inboxes.

    Writes directly to the same tables the rest of the system uses:
      - `messages`       — the in-app inbox the admin actually reads
      - `stored_emails`  — the 'Sent' archive
      - `email_log`      — audit trail
    Also attempts SMTP via infrastructure.email.send_email as a secondary
    delivery channel. Returns the number of inboxes delivered to.
    """
    ...
    # Resolve the sender user_id (for messages.sender_id).
    sender_id = None
    if sender_username:
        r = db.cur.execute("SELECT id FROM users WHERE username = ?",
    ...
    # Probe the real schemas once.
    msg_cols = {r[1] for r in db.cur.execute("PRAGMA table_info(messages)").fetchall()}
    se_cols  = {r[1] for r in db.cur.execute("PRAGMA table_info(stored_emails)").fetchall()}
    el_cols  = {r[1] for r in db.cur.execute("PRAGMA table_info(email_log)").fetchall()}
    ...
    for rid, uname, addr in admins:
        ...
        if se_cols:
            ...
            if usable:
                try:
                    ...
                    logger.debug("stored_emails insert skipped", exc_info=True)

        # --- audit log (schema varies) ---
        if el_cols:
            el_vals = {
                "recipient": addr,
                "subject": subject,
        ...
        # --- best-effort SMTP (won't raise into the caller) ---
        if send_email is not None:
            try:
                send_email(recipient=addr, subject=subject, body=body_tagged)
            except Exception:
                logger.debug("send_email skipped/failed for %s", addr, exc_info=True)
...
def _rows_to_pdf(path, title, columns, rows) -> None:
    """Write rows to a simple PDF using reportlab."""
...
def _report_window(parent, title, columns, rows, *,
    ...
    """Open a report in its own window with a CSV/TXT/PDF/Email/Close toolbar.

    `db` and `user` are optional — supplying them enables the Email-to-admin
    button. If omitted, only the save-to-file actions are offered.
    """
    win = tk.Toplevel(parent)
    win.title(title)
    ...
    tk.Button(bar, text="Close", command=win.destroy, bg="#6b7280",
              fg="white", relief="flat", padx=12, pady=6).pack(side="right",
                                                               padx=4)
    return win
...
def feat_19_at_risk(ctx: AdminContext):
    ...
    _report_window(ctx.parent, f"At-risk (<{threshold:.0f}%)",
                   ("student_id", "name", "pct", "sessions"), rows,
                   db=ctx.db, user=ctx.user, widths=[110, 260, 90, 90])
...
def feat_20_module_health(ctx: AdminContext):
    ...
    _report_window(ctx.parent, "Module health",
                   ("module", "name", "avg %", "rows"), rows,
                   db=ctx.db, user=ctx.user, widths=[110, 360, 90, 90])
...
def feat_21_cohort_compare(ctx: AdminContext):
    ...
    _report_window(ctx.parent, "Cohort comparison",
                   ("cohort", "students", "avg %"), rows,
                   db=ctx.db, user=ctx.user, widths=[320, 110, 110])
...
def feat_22_trend_chart(ctx: AdminContext):
    ...
    _report_window(ctx.parent, "Weekly % per module",
                   ("week", "module", "pct"), weeks,
                   db=ctx.db, user=ctx.user, widths=[110, 140, 120])
...
def feat_23_term_compare(ctx: AdminContext):
    ...
    _report_window(ctx.parent, "Term-over-term",
...
def feat_25_consecutive_absences(ctx: AdminContext):
    ...
    _report_window(ctx.parent, f"Top {n} by absence count",
                   ("student", "absences"), rows,
                   db=ctx.db, user=ctx.user, widths=[180, 120])
...
def feat_26_heatmap(ctx: AdminContext):
    ...
    _report_window(ctx.parent, "Absences by day-of-week",
                   ("day", "absences"), rows,
                   db=ctx.db, user=ctx.user, widths=[140, 140])
