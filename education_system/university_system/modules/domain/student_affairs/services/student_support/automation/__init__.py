...
try:
    from education_system.university_system.modules.domain.student_affairs.services.student_support.automation.escalations import (
            ...
            for rule in rules:
                ...
                try:
                    ...
                    if condition_type == 'time_based':
                        ...
                        dt_col = 'created_datetime' if 'created_datetime' in columns else (
                    ...
                    for ticket in tickets:
                        ...
                        if action_type == 'escalate':
                            cursor.execute(
                                "UPDATE support_tickets SET status = 'Escalated', last_updated_datetime = ? WHERE ticket_id = ?",
                                (escalation_time, ticket_id)
                            )
                        elif action_type == 'reassign':
                            cursor.execute(
                                "UPDATE support_tickets SET assigned_to = ?, last_updated_datetime = ? WHERE ticket_id = ?",
...
try:
    from education_system.university_system.modules.domain.student_affairs.services.student_support.automation.sentiment_analysis import (
        _analyze_sentiment,
        _suggest_category,
...
try:
    from education_system.university_system.modules.domain.student_affairs.services.student_support.automation.background_tasks import (
    ...
    def _update_metrics():
        ...
        try:
            ...
            if 'resolved_at' in columns and 'created_datetime' in columns:
                cursor.execute('''
                    SELECT AVG(julianday(resolved_at) - julianday(created_datetime)) * 24
                    FROM support_tickets
                    WHERE resolved_at IS NOT NULL AND created_datetime IS NOT NULL
                ''')
