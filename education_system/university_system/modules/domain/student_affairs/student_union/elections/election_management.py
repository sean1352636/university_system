...
from education_system.university_system.modules.shared.constants.institution_settings import get_elections_phone
...
def view_election_results(cursor, conn):
    ...
    show_detailed = auth.check_permission('view_election_results')
        ...
        for i, election in enumerate(elections):
            ...
            if election[4] == 'completed' or show_detailed:
                ...
                if candidates:
                    print("\n    Results:")
                    for candidate in candidates:
                        total_votes = candidate[4] if candidate[4] > 0 else 1  # Avoid division by zero
                        percentage = (candidate[3] / total_votes) * 100
                        print(f"    - {candidate[1]} {candidate[2]}: {candidate[3]} votes ({percentage:.1f}%)")
                    ...
                    if election[4] == 'completed' and candidates:
                        ...
                        # Show if winner has been appointed to position
                        cursor.execute('''
