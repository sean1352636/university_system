...
class ReportsMixin:
        def _email_report_to_admin(self):
            """Email current report to admin"""
            report_content = self.report_output.get(1.0, tk.END).strip()
        ...
        def _generate_report_in_window(self, report_type):
            """Generate report and display in window"""
            self.report_output.delete(1.0, tk.END)

            if report_type == 'summary':
                self._generate_summary_report_content()
            elif report_type == 'engagement':
                self._generate_engagement_report_content()
            elif report_type == 'donation':
                self._generate_donation_report_content()
            elif report_type == 'custom':
                self._generate_custom_report_content()
    ...
    """
        ...
        def _save_report_as_txt(self):
            """Save current report as TXT file"""
            report_content = self.report_output.get(1.0, tk.END).strip()
            if not report_content or report_content.startswith("Select a quick report"):
