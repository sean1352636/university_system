...
def generate_custom_report(auth):
    ...
    if choice == '1':
        from education_system.university_system.modules.domain.health.records.analytics.population import generate_student_health_summary
        return generate_student_health_summary(auth)
    elif choice == '2':
        from education_system.university_system.modules.domain.health.records.vaccinations.reports import generate_vaccination_status_report
        return generate_vaccination_status_report(auth)
    elif choice == '3':
        from education_system.university_system.modules.domain.health.appointments.appointment_booking import generate_appointment_schedule_report
        return generate_appointment_schedule_report(auth)
    elif choice == '4':
        from education_system.university_system.modules.domain.health.records.analytics.trends import generate_health_condition_analysis
        return generate_health_condition_analysis(auth)
    elif choice == '5':
        from education_system.university_system.modules.domain.health.appointments.appointment_booking import generate_provider_performance_report
