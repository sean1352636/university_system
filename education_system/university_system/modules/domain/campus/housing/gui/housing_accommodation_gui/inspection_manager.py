...
from education_system.university_system.modules.domain.campus.housing.services.housing_accommodation import generate_id
...
def record_inspection_dialog(self):
        ...
        def save_results():
            ...
            try:
                ...
                # Check if inspection exists (and capture type for downstream hooks).
                cursor.execute(
                    'SELECT inspection_id, inspection_type FROM housing_inspections '
                ...
                # Move-out inspections feed the deposit refund — offer to capture
                # proposed deductions now so they're already itemised by the time
                # staff process the refund.
                if inspection_type == 'Move-out':
                    try:
                        from education_system.university_system.modules.domain.campus.housing.gui.housing_accommodation_gui.deposit_manager import (
                            prompt_move_out_deductions,
                        )
                        prompt_move_out_deductions(self.root, inspection_id, self.auth)
                    except Exception as exc:
                        import logging
                        logging.getLogger(__name__).warning(
                            "Move-out deduction prompt failed: %s", exc,
                        )
