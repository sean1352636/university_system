...
class BuildingManagementApp:
    ...
    def _build_header(self):
        # Quick-switch bar to the other linked academic modules. Uses
        # cross-process IPC because this app runs in its own subprocess.
        try:
            from education_system.university_system.modules.shared.gui.main.features.academic_link_bar import (
                attach_subprocess_quickbar,
            )
            attach_subprocess_quickbar(self.root, current="building_mgmt")
        except Exception:
            logger.exception("Could not attach academic link bar")

        header = tk.Frame(self.root, bg="#2c3e50", height=46)
        header.pack(fill="x")
        header.pack_propagate(False)
        tk.Label(header, text="🏢 Building Management",
                 font=("Arial", 14, "bold"),
    ...
    def run(self):
        logger.info("Building Management starting (db=%s, user=%s)",
...
def main():
    try:
        ...
        logger.exception("Building Management failed to start")
        try:
