...
def email_worker():
    ...
    from education_system.university_system.infrastructure.email.email_service.core import send_email, send_template_email
...
def queue_template_email(template_name, recipient, template_vars, cc=None, bcc=None, attachments=None, scheduled_id=None):
    ...
    # In database-only mode, process immediately instead of queuing
    if config.get('database_only_mode', True):
        from education_system.university_system.infrastructure.email.email_service.core import send_template_email
        from education_system.university_system.infrastructure.email.email_service.scheduling import update_scheduled_email_status
