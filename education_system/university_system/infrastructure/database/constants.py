...
Configuration can be overridden via environment variables for production tuning.
"""
...
# Override with DB_TIMEOUT environment variable
DEFAULT_DB_TIMEOUT = float(os.environ.get('DB_TIMEOUT', 30.0))
...
# Override with DB_BUSY_TIMEOUT environment variable
SQLITE_BUSY_TIMEOUT = int(os.environ.get('DB_BUSY_TIMEOUT', 30000))

# Maximum number of retry attempts for failed operations
# Override with DB_MAX_RETRIES environment variable
MAX_RETRY_ATTEMPTS = int(os.environ.get('DB_MAX_RETRIES', 3))

# Delay between retry attempts (seconds)
# Override with DB_RETRY_DELAY environment variable
...
# Override with DB_PRAGMA_FOREIGN_KEYS environment variable
PRAGMA_FOREIGN_KEYS = os.environ.get('DB_PRAGMA_FOREIGN_KEYS', 'ON')

# Use Write-Ahead Logging for better concurrency
# Override with DB_PRAGMA_JOURNAL_MODE environment variable
PRAGMA_JOURNAL_MODE = os.environ.get('DB_PRAGMA_JOURNAL_MODE', 'WAL')

# Synchronous mode (NORMAL is good balance of safety/performance)
# Options: OFF, NORMAL, FULL, EXTRA
# Override with DB_PRAGMA_SYNCHRONOUS environment variable
PRAGMA_SYNCHRONOUS = os.environ.get('DB_PRAGMA_SYNCHRONOUS', 'NORMAL')
...
# Override with DB_MAX_CONNECTIONS environment variable
# For production with higher concurrency, consider 20-50
MAX_POOL_CONNECTIONS = int(os.environ.get('DB_MAX_CONNECTIONS', 10))

# Minimum number of connections to keep alive
# Override with DB_MIN_CONNECTIONS environment variable
# For production, consider 5-10
MIN_POOL_CONNECTIONS = int(os.environ.get('DB_MIN_CONNECTIONS', 2))

# Connection pool timeout (seconds)
# Override with DB_POOL_TIMEOUT environment variable
POOL_TIMEOUT = float(os.environ.get('DB_POOL_TIMEOUT', 30.0))

# Maximum time to keep an idle connection (seconds)
# Override with DB_MAX_CONNECTION_AGE environment variable
MAX_CONNECTION_AGE = int(os.environ.get('DB_MAX_CONNECTION_AGE', 3600))  # 1 hour

# How often to check for expired connections (seconds)
# Override with DB_POOL_CLEANUP_INTERVAL environment variable
POOL_CLEANUP_INTERVAL = int(os.environ.get('DB_POOL_CLEANUP_INTERVAL', 300))  # 5 minutes
