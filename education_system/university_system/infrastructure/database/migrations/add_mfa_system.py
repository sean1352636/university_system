...
def run_migration(db_path=None):
    ...
    try:
        ...
        # Add columns that may be missing if table was created by init_security_tables.py
        for col_sql in [
            "ALTER TABLE mfa_methods ADD COLUMN method_identifier TEXT",
            "ALTER TABLE mfa_methods ADD COLUMN is_primary BOOLEAN DEFAULT 0",
            "ALTER TABLE mfa_methods ADD COLUMN setup_completed_at TIMESTAMP",
            "ALTER TABLE mfa_methods ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP",
        ]:
            try:
                cursor.execute(col_sql)
            except sqlite3.OperationalError:
                logger.debug("Column already exists, skipping: %s", col_sql)
        ...
        try:
            ...
            logger.debug("Column verification_disabled already exists, skipping")
        ...
        try:
            ...
            logger.debug("Column mfa_status already exists, skipping")
        ...
        try:
            ...
            logger.debug("Column disabled_at already exists, skipping")
        ...
        # 8. Email SMTP Whitelist Table - Controls which emails can receive real SMTP
        cursor.execute("""
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_email_whitelist_active ON email_smtp_whitelist(is_active)")
