...
class TestStudentUnionManagementGUI:
    ...
    def test_initialization_with_theme_manager(self, mock_root, mock_auth):
        ...
        with patch('university_system.modules.domain.student_affairs.gui.student_union_management_gui.get_theme_manager') as mock_theme:
            ...
            # Should register theme observer
            mock_theme_instance.register_observer.assert_called_once()
            assert gui.theme_manager == mock_theme_instance
    ...
    def test_on_theme_changed_method(self, mock_root, mock_auth):
        """Test on_theme_changed method"""
        gui = StudentUnionManagementGUI(mock_root, mock_auth)

        # Should have theme change handler
        assert hasattr(gui, 'on_theme_changed')

        # Call should not raise errors
