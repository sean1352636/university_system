...
def chart_generator_module(mock_matplotlib):
    ...
    mod_key = 'education_system.university_system.modules.shared.utils.chart_generator'

    # Remove if already imported so reimport picks up mocked matplotlib
    saved = sys.modules.pop(mod_key, None)

    # Mock database connection during module load
    ...
    # If the module was previously loaded with real matplotlib, reload it
    # within the mocked sys.modules context so it picks up the mocks
    if saved is not None:
        with patch('education_system.university_system.infrastructure.database.db.get_connection') as mock_conn:
            mock_conn.return_value = MagicMock()
            chart_generator = importlib.reload(chart_generator)
