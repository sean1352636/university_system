"""GUI frame for Inter-System Messaging between staff."""
...
class InterSystemMessagingFrame(tk.Frame):
    """Frame for inter-system staff messaging with Inbox, Sent, and Compose tabs."""
    ...
    def _build_ui(self):
        ...
        tk.Label(
            header, text="Inter-System Messaging",
            font=("Helvetica", 15, "bold"), bg="#1a5276", fg="white",
        ).pack(side="left", padx=20, pady=10)

        # Unread badge
        self._unread_var = tk.StringVar(value="")
        tk.Label(
            header, textvariable=self._unread_var,
    ...
    def _build_compose_tab(self, parent):
        ...
        self._staff_map = {}  # display -> user dict
        row += 1
    ...
    def _on_system_changed(self, event=None):
        """Populate recipient dropdown when system changes."""
    ...
    def refresh(self):
        """Refresh both inbox and sent tabs and update unread count."""
        self._refresh_inbox()
        self._refresh_sent()
    ...
    def _refresh_inbox(self):
        ...
        try:
            msgs = self._svc.get_inbox(user_id, system=system or None)
            unread = 0
            for m in msgs:
                iid = str(m["id"])
            ...
            self._unread_var.set(f"{unread} unread" if unread else "")
        except Exception as e:
            self._unread_var.set(f"Error: {e}")
    ...
    def _refresh_sent(self):
        ...
        try:
            msgs = self._svc.get_sent(user_id, system=system or None)
            for m in msgs:
                iid = str(m["id"])
