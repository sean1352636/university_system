...
from education_system.secondary_school.modules.domain.admin.onboarding.services.onboarding_service import OnboardingService
...
@onboarding_bp.route("/<int:pk>", methods=["GET"])
@token_required
def get_onboarding_item(pk):
    svc = OnboardingService(_db_path)
    item = svc.get(pk)
    if not item:
...
@onboarding_bp.route("/<int:pk>", methods=["PUT"])
@token_required
@role_required("admin", "staff")
def update_onboarding_item(pk):
    data = get_json_body()
    svc = OnboardingService(_db_path)
...
@onboarding_bp.route("/<int:pk>", methods=["DELETE"])
@token_required
@role_required("admin")
def delete_onboarding_item(pk):
    svc = OnboardingService(_db_path)
    svc.delete(pk)
