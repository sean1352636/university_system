...
from education_system.secondary_school.modules.domain.portals.parent_portal.services.parent_portal_service import ParentPortalService
...
@parent_portal_bp.route("", methods=["GET"])
@token_required
def list_parent_portal():
    svc = ParentPortalService(_db_path)
...
@parent_portal_bp.route("/<int:pk>", methods=["GET"])
@token_required
def get_parent_portal_item(pk):
    svc = ParentPortalService(_db_path)
    item = svc.get(pk)
    if not item:
...
@parent_portal_bp.route("", methods=["POST"])
@token_required
@role_required("admin", "staff")
def create_parent_portal_item():
    data = get_json_body()
    svc = ParentPortalService(_db_path)
...
@parent_portal_bp.route("/<int:pk>", methods=["PUT"])
@token_required
@role_required("admin", "staff")
def update_parent_portal_item(pk):
    data = get_json_body()
    svc = ParentPortalService(_db_path)
...
@parent_portal_bp.route("/<int:pk>", methods=["DELETE"])
@token_required
@role_required("admin")
def delete_parent_portal_item(pk):
    svc = ParentPortalService(_db_path)
    svc.delete(pk)
