...
from education_system.secondary_school.modules.domain.facilities.resource_booking.services.resource_booking_service import ResourceBookingService
...
@token_required
def get_resource_booking_item(pk):
    svc = ResourceBookingService(_db_path)
    item = svc.get(pk)
    if not item:
...
def create_resource_booking_item():
    data = get_json_body()
    svc = ResourceBookingService(_db_path)
...
@token_required
@role_required("admin", "staff")
def update_resource_booking_item(pk):
    data = get_json_body()
    svc = ResourceBookingService(_db_path)
...
@token_required
@role_required("admin")
def delete_resource_booking_item(pk):
    svc = ResourceBookingService(_db_path)
    svc.delete(pk)
