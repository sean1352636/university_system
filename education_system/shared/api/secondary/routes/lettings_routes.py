...
from education_system.secondary_school.modules.domain.facilities.lettings.services.lettings_service import LettingsService
...
@lettings_bp.route("/<int:pk>", methods=["GET"])
@token_required
def get_lettings_item(pk):
    svc = LettingsService(_db_path)
    item = svc.get(pk)
    if not item:
...
@lettings_bp.route("", methods=["POST"])
@token_required
@role_required("admin", "staff")
def create_lettings_item():
    data = get_json_body()
    svc = LettingsService(_db_path)
...
@lettings_bp.route("/<int:pk>", methods=["PUT"])
@token_required
@role_required("admin", "staff")
def update_lettings_item(pk):
    data = get_json_body()
    svc = LettingsService(_db_path)
...
@lettings_bp.route("/<int:pk>", methods=["DELETE"])
@token_required
@role_required("admin")
def delete_lettings_item(pk):
    svc = LettingsService(_db_path)
    svc.delete(pk)
