...
from education_system.secondary_school.modules.domain.facilities.departments.services.departments_service import DepartmentsService
...
def init_departments_routes(db_path=None):
    global _db_path
    _db_path = db_path
...
def list_departments():
    svc = DepartmentsService(_db_path)
...
@departments_bp.route("/<int:pk>", methods=["GET"])
@token_required
def get_departments_item(pk):
    svc = DepartmentsService(_db_path)
    item = svc.get(pk)
    if not item:
...
@departments_bp.route("/<int:pk>", methods=["PUT"])
@token_required
@role_required("admin", "staff")
def update_departments_item(pk):
    data = get_json_body()
    svc = DepartmentsService(_db_path)
...
@departments_bp.route("/<int:pk>", methods=["DELETE"])
@token_required
@role_required("admin")
def delete_departments_item(pk):
    svc = DepartmentsService(_db_path)
    svc.delete(pk)
