...
def get_year(pk):
    ...
    if not item:
        return jsonify({"error": t("api.academic_year.not_found")}), 404
...
def create_year():
    ...
    return jsonify({"message": t("api.academic_year.created"), "data": item}), 201
...
def update_year(pk):
    ...
    return jsonify({"message": t("api.academic_year.updated"), "data": item})
...
def delete_year(pk):
    svc = AcademicYearService(_db_path)
    svc.delete_year(pk)
    return jsonify({"message": t("api.academic_year.deleted")})
