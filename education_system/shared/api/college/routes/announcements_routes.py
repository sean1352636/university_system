...
def get_announcement(pk):
    ...
    if not item:
        return jsonify({"error": t("api.announcements.not_found")}), 404
...
def create_announcement():
    ...
    return jsonify({"message": t("api.announcements.created"), "data": item}), 201
...
def update_announcement(pk):
    ...
    return jsonify({"message": t("api.announcements.updated"), "data": item})
...
def delete_announcement(pk):
    ...
    return jsonify({"message": t("api.announcements.deleted")})
