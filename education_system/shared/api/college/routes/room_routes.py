...
def create_room():
    ...
    return jsonify({"message": t("api.room.created"), "data": room}), 201
...
def get_room(room_id):
    ...
    if not room:
        return jsonify({"error": t("api.room.not_found")}), 404
...
def update_room(room_id):
    ...
    return jsonify({"message": t("api.room.updated"), "data": room})
...
def add_resource(room_id):
    ...
    return jsonify({"message": t("api.common.created"), "data": resource}), 201
...
def available_rooms():
    ...
    if not day or not start or not end:
        return jsonify({"error": t("api.common.bad_request")}), 400
    svc = RoomService(_db_path)
