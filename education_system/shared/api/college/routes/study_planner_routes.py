...
def get_session(pk):
    ...
    if not item:
        return jsonify({"error": t("api.study_planner.not_found")}), 404
...
def create_session():
    ...
    return jsonify({"message": t("api.study_planner.created"), "data": item}), 201
...
def update_session(pk):
    ...
    return jsonify({"message": t("api.study_planner.updated"), "data": item})
...
def delete_session(pk):
    ...
    return jsonify({"message": t("api.study_planner.deleted")})
