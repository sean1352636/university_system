...
def get_document(pk):
    ...
    if not item:
        return jsonify({"error": t("api.document_hub.not_found")}), 404
...
def create_document():
    ...
    return jsonify({"message": t("api.document_hub.uploaded"), "data": item}), 201
...
def update_document(pk):
    ...
    return jsonify({"message": t("api.document_hub.updated"), "data": item})
...
def delete_document(pk):
    ...
    return jsonify({"message": t("api.document_hub.deleted")})
