...
def get_checkin(pk):
    ...
    if not item:
        return jsonify({"error": t("api.staff_wellbeing.not_found")}), 404
...
def create_checkin():
    ...
    return jsonify({"message": t("api.staff_wellbeing.created"), "data": item}), 201
...
def update_checkin(pk):
    ...
    return jsonify({"message": t("api.staff_wellbeing.updated"), "data": item})
...
def delete_checkin(pk):
    ...
    return jsonify({"message": t("api.staff_wellbeing.deleted")})
