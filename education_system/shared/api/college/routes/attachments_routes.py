...
def get_attachment(pk):
    ...
    if not item:
        return jsonify({"error": t("api.attachments.not_found")}), 404
...
def create_attachment():
    ...
    return jsonify({"message": t("api.attachments.uploaded"), "data": item}), 201
...
def delete_attachment(pk):
    ...
    return jsonify({"message": t("api.attachments.deleted")})
