...
def get_observation(pk):
    ...
    if not item:
        return jsonify({"error": t("api.observations.not_found")}), 404
...
def create_observation():
    ...
    return jsonify({"message": t("api.observations.created"), "data": item}), 201
...
def update_observation(pk):
    ...
    return jsonify({"message": t("api.observations.updated"), "data": item})
...
def delete_observation(pk):
    ...
    return jsonify({"message": t("api.observations.deleted")})
