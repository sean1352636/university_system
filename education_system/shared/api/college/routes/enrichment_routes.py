...
def get_activity(pk):
    ...
    if not item:
        return jsonify({"error": t("api.enrichment.not_found")}), 404
...
def create_activity():
    ...
    return jsonify({"message": t("api.enrichment.created"), "data": item}), 201
...
def update_activity(pk):
    ...
    return jsonify({"message": t("api.enrichment.updated"), "data": item})
...
def delete_activity(pk):
    ...
    return jsonify({"message": t("api.enrichment.deleted")})
