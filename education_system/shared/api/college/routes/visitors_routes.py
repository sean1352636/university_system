...
def get_visitor(pk):
    ...
    if not item:
        return jsonify({"error": t("api.visitors.not_found")}), 404
...
def create_visitor():
    ...
    return jsonify({"message": t("api.visitors.created"), "data": item}), 201
...
def update_visitor(pk):
    ...
    return jsonify({"message": t("api.visitors.updated"), "data": item})
