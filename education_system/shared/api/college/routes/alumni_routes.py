"""API routes for alumni."""

from flask import Blueprint, jsonify, request, g

from education_system.shared.api.college.auth import token_required, role_required
from education_system.shared.api.college.validators import get_json_body, require_fields
from education_system.shared.api.college.pagination import get_pagination_params, paginated_response
from education_system.college_system.modules.domain.alumni.services.alumni_service import AlumniService
from education_system.college_system.core.i18n import t

alumni_bp = Blueprint("alumni", __name__, url_prefix="/api/alumni")

_db_path = None


def init_alumni_routes(db_path=None):
    global _db_path
    _db_path = db_path


@alumni_bp.route("", methods=["GET"])
@token_required
def list_records():
    svc = AlumniService(_db_path)
    limit, offset = get_pagination_params()
    items = svc.list_records(limit=limit, offset=offset)
    return jsonify(paginated_response(items, len(items)))
@alumni_bp.route("/<int:pk>", methods=["GET"])
@token_required
def get_record(pk):
    svc = AlumniService(_db_path)
    item = svc.get_record(pk)
    if not item:
        return jsonify({"error": t("api.alumni.not_found")}), 404
    return jsonify({"data": item})
@alumni_bp.route("", methods=["POST"])
@token_required
@role_required('admin', 'staff')
def create_record():
    data = get_json_body()
    svc = AlumniService(_db_path)
    result = svc.create_record(**data)
    return jsonify({"message": t("api.alumni.created"), "data": result}), 201
@alumni_bp.route("/<int:pk>", methods=["PUT"])
@token_required
@role_required('admin', 'staff')
def update_record(pk):
    data = get_json_body()
    svc = AlumniService(_db_path)
    result = svc.update_record(pk, **data)
    return jsonify({"message": t("api.alumni.updated"), "data": result})
@alumni_bp.route("/<int:pk>", methods=["DELETE"])
@token_required
@role_required('admin')
def delete_record(pk):
    svc = AlumniService(_db_path)
    svc.delete_record(pk)
    return jsonify({"message": t("api.alumni.deleted")})
@alumni_bp.route("/events", methods=["GET"])
@token_required
def list_events():
    svc = AlumniService(_db_path)
    limit, offset = get_pagination_params()
    items = svc.list_events(limit=limit, offset=offset)
    return jsonify(paginated_response(items, len(items)))
@alumni_bp.route("/events", methods=["POST"])
@token_required
@role_required('admin', 'staff')
def create_event():
    data = get_json_body()
    svc = AlumniService(_db_path)
    result = svc.create_event(**data)
    return jsonify({"message": t("api.alumni.created"), "data": result}), 201
@alumni_bp.route("/stats", methods=["GET"])
@token_required
@role_required('admin', 'staff')
def get_stats():
    svc = AlumniService(_db_path)
    result = svc.get_stats()
    return jsonify({"data": result})
