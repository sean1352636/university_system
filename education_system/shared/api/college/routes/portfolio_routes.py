...
def get_item(pk):
    ...
    if not item:
        return jsonify({"error": t("api.portfolio.not_found")}), 404
...
def create_item():
    ...
    return jsonify({"message": t("api.portfolio.created"), "data": item}), 201
...
def update_item(pk):
    ...
    return jsonify({"message": t("api.portfolio.updated"), "data": item})
...
def delete_item(pk):
    svc = PortfolioService(_db_path)
    svc.delete_item(pk)
    return jsonify({"message": t("api.portfolio.deleted")})
