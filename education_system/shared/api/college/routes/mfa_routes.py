...
from education_system.shared.api.college.auth import (
...
def verify_mfa():
    ...
    token = generate_token(user_info["user_id"], user_info["username"], systems)

    return jsonify({
        "message": t("api.auth.mfa_verified"),
        "token": token,
        "user": {
...
def setup_mfa():
    ...
    return jsonify({
        "message": t("api.auth.mfa_setup_complete", default="MFA setup complete."),
...
def disable_mfa():
    ...
    return jsonify({"message": t("api.auth.mfa_disabled", default="MFA disabled.")})
