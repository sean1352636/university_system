...
def get_plan(pk):
    ...
    if not item:
        return jsonify({"error": t("api.lesson_plans.not_found")}), 404
...
def create_plan():
    ...
    return jsonify({"message": t("api.lesson_plans.created"), "data": item}), 201
...
def update_plan(pk):
    ...
    return jsonify({"message": t("api.lesson_plans.updated"), "data": item})
...
def delete_plan(pk):
    svc = LessonPlanService(_db_path)
    svc.delete_plan(pk)
    return jsonify({"message": t("api.lesson_plans.deleted")})
