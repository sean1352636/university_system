...
def get_drill(pk):
    ...
    if not item:
        return jsonify({"error": t("api.emergency.not_found")}), 404
...
def create_drill():
    ...
    return jsonify({"message": t("api.emergency.created"), "data": item}), 201
...
def update_drill(pk):
    ...
    return jsonify({"message": t("api.emergency.updated"), "data": item})
