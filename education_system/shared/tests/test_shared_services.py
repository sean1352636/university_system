...
class TestConsentService:
    ...
    def test_get_consent_summary(self, auth_db):
        from education_system.shared.gdpr.consent_service import ConsentService, CONSENT_TYPES
        svc = ConsentService(auth_db)
        conn = sqlite3.connect(auth_db)
        conn.execute(
            "INSERT INTO users (username, password_hash) VALUES (?, ?)",
...
class TestEarlyWarningService:
    def test_assess_unknown_system(self):
        from education_system.shared.analytics.early_warning import EarlyWarningService
