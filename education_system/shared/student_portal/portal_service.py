...
_AUTH_DB = _SHARED_DIR / "data" / "db_files" / "auth.db"
...
class PortalService:
    ...
    def _records_university(self, conn, student_name):
        ...
        if _table_exists(conn, "attendance"):
            try:
                total = conn.execute(
                    "SELECT COUNT(*) AS c FROM attendance WHERE student_id = ?", (sid,)
                ).fetchone()["c"]
                present = conn.execute(
                    "SELECT COUNT(*) AS c FROM attendance WHERE student_id = ? AND status IN ('present','late')",
                    (sid,),
                ).fetchone()["c"]
                attendance = {
