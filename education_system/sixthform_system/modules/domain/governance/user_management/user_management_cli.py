"""CLI flow for the Sixth Form User Management module.

Roles- and access-focused complement to the User Accounts CLI:

* show the access matrix (per system × role)
* show role distribution for the sixth-form system
...
def _print_users(rows: list[UserRow]) -> None:
    ...
    print(f"  {'ID':>4}  {'Username':<18}  {'Display':<22}  "
          f"{'Sixth-form':<12}  {'Active':<6}  {'Locked':<6}")
    print("  " + "-" * 78)
    for u in rows:
...
def list_without_access_flow(auth: Any) -> None:
    print("\n═══ Users without sixth-form access ═══")
...
def summary_flow(auth: Any) -> None:
    ...
    print(f"  Sixth-form users      : {s.sixth_form_users}")
    print(f"  Sixth-form admins     : {snap.admins}")
...
_MENU: list[tuple[str, Callable[[Any], None]]] = [
    ...
    ("Sixth-form Role Distribution",  show_role_distribution),
    ("List Admins",                   list_admins_flow),
    ("Users without sixth-form access", list_without_access_flow),
