...
def group_rollup(group_id: int) -> GroupRollup:
    ...
    open_concerns = 0
    try:
        from education_system.sixthform_system.modules.domain.reports.progress import (
            progress as pr,
        )
        for sid in members:
...
def student_group_index() -> dict[str, tuple[int | None, str | None]]:
    ...
    try:
        from education_system.sixthform_system.modules.domain.pastoral.tutor_groups import (
            tutor_groups as tg,
        )
...
def _search_homework(parsed: ParsedQuery,
                     opts: QueryOptions,
                     _filters: dict) -> list[Hit]:
    from education_system.sixthform_system.modules.domain.academics.homework import (
        homework as m,
    )
...
def _search_ucas(parsed: ParsedQuery,
    ...
    try:
        from education_system.sixthform_system.modules.domain.progression.references import (
            references as r,
        )
...
def _search_finance(parsed: ParsedQuery,
    ...
    out: list[Hit] = []
    try:
        from education_system.sixthform_system.modules.domain.finance.fees import (
            fees as f,
        )
...
def _search_documents(parsed: ParsedQuery,
                      opts: QueryOptions,
                      _filters: dict) -> list[Hit]:
    from education_system.sixthform_system.modules.domain.staff_comms.document_hub import (
        document_hub as m,
    )
...
def _search_audit_logs(parsed: ParsedQuery,
                       opts: QueryOptions,
                       _filters: dict) -> list[Hit]:
    from education_system.sixthform_system.core import log_store as m
...
def _students_without_consent(purpose: str) -> set[str]:
    ...
    out: set[str] = set()
    try:
        from education_system.sixthform_system.modules.domain.governance.gdpr import (
            gdpr as g,
        )
...
def _try_push_notification(sched: ScheduledSearch, delta: int,
    ...
    try:
        from education_system.sixthform_system.modules.domain.staff_comms.notifications import (
