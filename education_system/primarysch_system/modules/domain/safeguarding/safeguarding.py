"""Safeguarding data layer for the Primary School System.
...
def raise_concern(data: dict[str, Any]) -> Concern:
    ...
    # follows the pupil between systems (best-effort; never blocks).
    try:
        from education_system.shared.safeguarding import alert_service
        alert_service.raise_flag_for_local_concern(
            "primary", rec.pupil_id, category=rec.category,
            severity=rec.severity, summary=rec.category,
            source_ref=str(rec.concern_id), raised_by=payload.get("raised_by"))
    except Exception:
