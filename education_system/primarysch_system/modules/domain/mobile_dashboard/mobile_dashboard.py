...
def _progress_snapshot() -> tuple[int, int, int] | None:
    ...
    def _():
        from education_system.primarysch_system.modules.domain.progress import (
            progress,
        )
        s = progress.summary()
...
def _high_risk_feed(limit: int = 10) -> list[FeedItem]:
    def _():
        from education_system.primarysch_system.modules.domain.progress_dashboard import (
            progress_dashboard,
        )
