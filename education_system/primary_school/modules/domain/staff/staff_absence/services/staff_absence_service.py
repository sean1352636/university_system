...
class StaffAbsenceService:
    ...
    def create(self, **kwargs):
        ...
        try:
            ...
            cursor = conn.execute(
                f"INSERT INTO staff_absences ({cols}) VALUES ({placeholders})",
                list(kwargs.values()))
            conn.commit()
            row = conn.execute("SELECT * FROM staff_absences WHERE id = ?", (cursor.lastrowid,)).fetchone()
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            ...
            conn.execute(f"UPDATE staff_absences SET {set_clause} WHERE id = ?", values)
            conn.commit()
            row = conn.execute("SELECT * FROM staff_absences WHERE id = ?", (record_id,)).fetchone()
            return dict(row) if row else None
        except Exception as e:
