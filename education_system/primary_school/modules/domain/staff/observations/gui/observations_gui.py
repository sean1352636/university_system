"""ObservationsFrame GUI for the Primary School."""

import tkinter as tk
from tkinter import ttk, messagebox

from education_system.primary_school.modules.domain.staff.observations.services.observations_service import ObservationsService


class ObservationsFrame(tk.Frame):
    """GUI for managing observations."""
    ...
    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        ...
        self._auth = auth
        self._service = ObservationsService(db_path)

        # Header
        header = tk.Frame(self, bg=self.HEADER_BG, height=40)
        header.pack(fill="x")
        header.pack_propagate(False)
        tk.Label(header, text="Observations",
        ...
        self._observations_tab = tk.Frame(self._notebook)
        self._notebook.add(self._observations_tab, text="Observations")
        self._schedule_tab = tk.Frame(self._notebook)
        self._notebook.add(self._schedule_tab, text="Schedule")
        self._feedback_tab = tk.Frame(self._notebook)
        self._notebook.add(self._feedback_tab, text="Feedback")
