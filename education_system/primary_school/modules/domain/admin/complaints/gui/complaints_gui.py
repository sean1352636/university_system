"""ComplaintsFrame GUI for the Primary School."""

import tkinter as tk
from tkinter import ttk, messagebox

from education_system.primary_school.modules.domain.admin.complaints.services.complaints_service import ComplaintsService


class ComplaintsFrame(tk.Frame):
    """GUI for managing complaints."""
    ...
    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        ...
        self._auth = auth
        self._service = ComplaintsService(db_path)

        # Header
        header = tk.Frame(self, bg=self.HEADER_BG, height=40)
        header.pack(fill="x")
        header.pack_propagate(False)
        tk.Label(header, text="Complaints",
        ...
        self._all_complaints_tab = tk.Frame(self._notebook)
        self._notebook.add(self._all_complaints_tab, text="All Complaints")
        self._submit_tab = tk.Frame(self._notebook)
        self._notebook.add(self._submit_tab, text="Submit")
        self._responses_tab = tk.Frame(self._notebook)
        self._notebook.add(self._responses_tab, text="Responses")
