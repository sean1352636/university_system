...
class QualityAssuranceService:
    ...
    def create(self, **kwargs):
        ...
        try:
            ...
            cursor = conn.execute(
                f"INSERT INTO qa_reviews ({cols}) VALUES ({placeholders})",
                list(kwargs.values()))
            conn.commit()
            row = conn.execute("SELECT * FROM qa_reviews WHERE id = ?", (cursor.lastrowid,)).fetchone()
    ...
    def list_all(self, limit=100, offset=0, **filters):
        conn = self._conn()
        try:
            sql = "SELECT * FROM qa_reviews WHERE 1=1"
            params = []
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            ...
            conn.execute(f"UPDATE qa_reviews SET {set_clause} WHERE id = ?", values)
            conn.commit()
            row = conn.execute("SELECT * FROM qa_reviews WHERE id = ?", (record_id,)).fetchone()
            return dict(row) if row else None
        except Exception as e:
