...
class AuditReportsService:
    ...
    def create(self, **kwargs):
        ...
        try:
            ...
            cursor = conn.execute(
                f"INSERT INTO audit_reports ({cols}) VALUES ({placeholders})",
                list(kwargs.values()))
            conn.commit()
            row = conn.execute("SELECT * FROM audit_reports WHERE id = ?", (cursor.lastrowid,)).fetchone()
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            ...
            conn.execute(f"UPDATE audit_reports SET {set_clause} WHERE id = ?", values)
            conn.commit()
            row = conn.execute("SELECT * FROM audit_reports WHERE id = ?", (record_id,)).fetchone()
            return dict(row) if row else None
        except Exception as e:
