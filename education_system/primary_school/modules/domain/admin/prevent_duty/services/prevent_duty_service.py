...
class PreventDutyService:
    ...
    def create(self, **kwargs):
        ...
        values: list = []
        for col in ('pupil_id', 'staff_reporter_id', 'concern_type', 'description', 'risk_level', 'action_taken', 'referral_date', 'status', 'outcome', 'resolved_date'):
        ...
        try:
            cursor = conn.execute(
                f"INSERT INTO prevent_referrals ({cols_sql}) VALUES ({ph_sql})",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
    ...
    def list_all(self, limit=100, offset=0, **filters):
        ...
        params: list = []
        for col in ('id', 'pupil_id', 'staff_reporter_id', 'concern_type', 'description', 'risk_level', 'action_taken', 'referral_date', 'status', 'outcome', 'resolved_date'):
            val = filters.get(col)
            if val is not None:
        ...
        try:
            sql = (
                f"SELECT * FROM prevent_referrals WHERE {where_clause} "  # noqa: S608
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            conn.execute(
                f"UPDATE prevent_referrals SET {set_clause} WHERE id = ?",  # noqa: S608
                values,
            )
            conn.commit()
            row = conn.execute(
