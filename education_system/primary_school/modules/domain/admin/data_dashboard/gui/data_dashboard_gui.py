"""DataDashboardFrame GUI for the Primary School."""

import tkinter as tk
from tkinter import ttk, messagebox

from education_system.primary_school.modules.domain.admin.data_dashboard.services.data_dashboard_service import DataDashboardService


class DataDashboardFrame(tk.Frame):
    """GUI for managing data dashboard."""
    ...
    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        ...
        self._auth = auth
        self._service = DataDashboardService(db_path)

        # Header
        header = tk.Frame(self, bg=self.HEADER_BG, height=40)
        header.pack(fill="x")
        header.pack_propagate(False)
        tk.Label(header, text="Data Dashboard",
        ...
        self._attendance_tab = tk.Frame(self._notebook)
        self._notebook.add(self._attendance_tab, text="Attendance")
        self._assessment_tab = tk.Frame(self._notebook)
        self._notebook.add(self._assessment_tab, text="Assessment")
        self._behaviour_tab = tk.Frame(self._notebook)
        self._notebook.add(self._behaviour_tab, text="Behaviour")
        self._enrollment_tab = tk.Frame(self._notebook)
        self._notebook.add(self._enrollment_tab, text="Enrollment")
