"""PortfolioFrame GUI for the Primary School."""

import tkinter as tk
from tkinter import ttk, messagebox

from education_system.primary_school.modules.domain.academics.portfolio.services.portfolio_service import PortfolioService


class PortfolioFrame(tk.Frame):
    """GUI for managing portfolio."""
    ...
    def __init__(self, parent, db_path=None, auth=None, **kwargs):
        ...
        self._auth = auth
        self._service = PortfolioService(db_path)

        # Header
        header = tk.Frame(self, bg=self.HEADER_BG, height=40)
        header.pack(fill="x")
        header.pack_propagate(False)
        tk.Label(header, text="Portfolio",
        ...
        self._portfolio_tab = tk.Frame(self._notebook)
        self._notebook.add(self._portfolio_tab, text="Portfolio")
        self._add_work_tab = tk.Frame(self._notebook)
        self._notebook.add(self._add_work_tab, text="Add Work")
        self._comments_tab = tk.Frame(self._notebook)
        self._notebook.add(self._comments_tab, text="Comments")
