...
def ilp_menu(auth):
    ...
    from education_system.primary_school.modules.domain.pupil_life.ilp.services.ilp_service import IlpService

    svc = IlpService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            strategies = input("  Strategies: ").strip()
            status = input("  Status: ").strip()
            try:
                svc.create(targets=targets, strategies=strategies, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), targets=targets, strategies=strategies, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
