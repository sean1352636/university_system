"""Subjects CLI menu."""

from education_system.primary_school.infrastructure.database.db import get_db_path


def subjects_menu(auth):
    """Subjects management menu."""
    from education_system.primary_school.cli.cli_main import print_header, print_menu, get_choice
    from education_system.primary_school.modules.domain.academics.subjects.services.subject_service import SubjectService

    svc = SubjectService(get_db_path())

    while True:
        print_header("Subjects")
        print_menu([
            ("1", "List subjects"),
            ("2", "View details"),
            ("3", "Add Subject"),
            ("4", "Update Subject"),
            ("5", "Delete Subject"),
            ("0", "Back"),
        ])

        choice = get_choice()
        if choice == "0":
            break
        elif choice == "1":
            items = svc.list_subjects()
            if not items:
                print("\n  No records found.")
            else:
                for item in items:
                    print(f"  {dict(item) if hasattr(item, 'keys') else item}")
        elif choice == "2":
            pk = input("  Enter ID: ").strip()
            try:
                item = svc.get_subject(int(pk))
                if item:
                    for k, v in (dict(item) if hasattr(item, "keys") else {}).items():
                        print(f"  {k}: {v}")
                else:
                    print("\n  Not found.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "3":
            name = input("  Name: ").strip()
            subject_code = input("  Subject Code: ").strip()
            try:
                svc.create_subject(name=name, subject_code=subject_code)
                print("\n  Subject created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            pk = input("  Enter ID to update: ").strip()
            name = input("  Name: ").strip()
            subject_code = input("  Subject Code: ").strip()
            try:
                svc.update_subject(int(pk), name=name, subject_code=subject_code)
                print("\n  Subject updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
            pk = input("  Enter ID to delete: ").strip()
            try:
                svc.delete_subject(int(pk))
                print("\n  Subject deleted.")
            except Exception as e:
                print(f"\n  Error: {e}")
        else:
            print("\n  Invalid option.")
