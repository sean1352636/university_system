"""Health & Safety CLI menu."""

from education_system.primary_school.infrastructure.database.db import get_db_path


def health_safety_menu(auth):
    """Health & Safety management menu."""
    from education_system.primary_school.cli.cli_main import print_header, print_menu, get_choice
    from education_system.primary_school.modules.domain.admin.health_safety.services.health_safety_service import HealthSafetyService

    svc = HealthSafetyService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                svc.create(title=title, description=description, location=location, severity=severity, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), title=title, description=description, location=location, severity=severity, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
