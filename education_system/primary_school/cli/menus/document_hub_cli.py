"""Document Hub CLI menu."""

from education_system.primary_school.infrastructure.database.db import get_db_path


def document_hub_menu(auth):
    """Document Hub management menu."""
    from education_system.primary_school.cli.cli_main import print_header, print_menu, get_choice
    from education_system.primary_school.modules.domain.portals.document_hub.services.document_hub_service import DocumentHubService

    svc = DocumentHubService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                svc.create(title=title, category=category, file_type=file_type, access_level=access_level)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            access_level = input("  Access Level: ").strip()
            try:
                svc.update(int(pk), title=title, category=category, file_type=file_type, access_level=access_level)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
