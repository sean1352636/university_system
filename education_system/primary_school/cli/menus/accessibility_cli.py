...
def accessibility_menu(auth):
    ...
    svc = AccessibilityService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            provision_type = input("  Provision Type: ").strip()
            description = input("  Description: ").strip()
            status = input("  Status: ").strip()
            try:
                svc.create(provision_type=provision_type, description=description, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), provision_type=provision_type, description=description, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
