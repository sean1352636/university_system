...
def pupil_support_menu(auth):
    ...
    svc = PupilSupportService(get_db_path())

    while True:
        print_header("Pupil Support")
        print_menu([
            ("1", "List records"),
            ("2", "View details"),
            ("3", "Add record"),
        ...
        if choice == "0":
            ...
            try:
                svc.create(referral_type=referral_type, description=description, urgency=urgency, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), referral_type=referral_type, description=description, urgency=urgency, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
