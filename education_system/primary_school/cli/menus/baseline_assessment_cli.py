...
def baseline_assessment_menu(auth):
    ...
    svc = BaselineAssessmentService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            assessment_type = input("  Assessment Type: ").strip()
            score = input("  Score: ").strip()
            band = input("  Band: ").strip()
            try:
                svc.create(assessment_type=assessment_type, score=score, band=band)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            band = input("  Band: ").strip()
            try:
                svc.update(int(pk), assessment_type=assessment_type, score=score, band=band)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
