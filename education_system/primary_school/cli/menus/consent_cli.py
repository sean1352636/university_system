...
def consent_menu(auth):
    """Consent management menu."""
    ...
    svc = ConsentService(get_db_path())

    while True:
        print_header("Consent")
        print_menu([
            ("1", "List records"),
            ("2", "View details"),
            ("3", "Add Record"),
        ...
        if choice == "0":
            ...
            consent_type = input("  Consent Type: ").strip()
            status = input("  Status: ").strip()
            try:
                svc.create_record(pupil_id=pupil_id, consent_type=consent_type, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update_record(int(pk), pupil_id=pupil_id, consent_type=consent_type, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
