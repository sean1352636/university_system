...
def kpi_dashboard_menu(auth):
    ...
    svc = KpiDashboardService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            target_value = input("  Target Value: ").strip()
            unit = input("  Unit: ").strip()
            try:
                svc.create(metric_name=metric_name, category=category, current_value=current_value, target_value=target_value, unit=unit)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            unit = input("  Unit: ").strip()
            try:
                svc.update(int(pk), metric_name=metric_name, category=category, current_value=current_value, target_value=target_value, unit=unit)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
