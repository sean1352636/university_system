...
def todo_menu(auth):
    ...
    from education_system.primary_school.modules.domain.admin.todo.services.todo_service import TodoService

    svc = TodoService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                svc.create(title=title, priority=priority, category=category, due_date=due_date)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            due_date = input("  Due Date: ").strip()
            try:
                svc.update(int(pk), title=title, priority=priority, category=category, due_date=due_date)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
