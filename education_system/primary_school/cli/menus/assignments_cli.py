...
def assignments_menu(auth):
    ...
    svc = AssignmentService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                item = svc.get_assignment(int(pk))
                if item:
            ...
            try:
                svc.create_assignment(title=title, description=description, due_date=due_date)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            due_date = input("  Due Date: ").strip()
            try:
                svc.update_assignment(int(pk), title=title, description=description, due_date=due_date)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
