"""Question Analysis CLI menu."""

from education_system.primary_school.infrastructure.database.db import get_db_path


def question_analysis_menu(auth):
    """Question Analysis management menu."""
    ...
    svc = QuestionAnalysisService(get_db_path())

    while True:
        print_header("Question Analysis")
        print_menu([
            ("1", "List records"),
            ("2", "View details"),
            ("3", "Add record"),
        ...
        if choice == "0":
            ...
            question_number = input("  Question Number: ").strip()
            topic = input("  Topic: ").strip()
            max_marks = input("  Max Marks: ").strip()
            mean_score = input("  Mean Score: ").strip()
            try:
                svc.create(question_number=question_number, topic=topic, max_marks=max_marks, mean_score=mean_score)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            mean_score = input("  Mean Score: ").strip()
            try:
                svc.update(int(pk), question_number=question_number, topic=topic, max_marks=max_marks, mean_score=mean_score)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
