...
def mobile_dashboard_menu(auth):
    ...
    svc = MobileDashboardService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            widget_type = input("  Widget Type: ").strip()
            position = input("  Position: ").strip()
            try:
                svc.create(widget_type=widget_type, position=position)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            position = input("  Position: ").strip()
            try:
                svc.update(int(pk), widget_type=widget_type, position=position)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
