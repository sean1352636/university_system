...
def pupil_portal_menu(auth):
    ...
    svc = PupilPortalService(get_db_path())

    while True:
        print_header("Pupil Portal")
        print_menu([
            ("1", "List records"),
            ("2", "View details"),
            ("3", "Add record"),
        ...
        if choice == "0":
            ...
            notifications_enabled = input("  Notifications Enabled: ").strip()
            try:
                svc.create(theme=theme, notifications_enabled=notifications_enabled)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            notifications_enabled = input("  Notifications Enabled: ").strip()
            try:
                svc.update(int(pk), theme=theme, notifications_enabled=notifications_enabled)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
