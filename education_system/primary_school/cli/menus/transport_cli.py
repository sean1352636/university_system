...
def transport_menu(auth):
    ...
    svc = TransportService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                svc.create_record(pupil_id=pupil_id, route_name=route_name)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            route_name = input("  Route Name: ").strip()
            try:
                svc.update_record(int(pk), pupil_id=pupil_id, route_name=route_name)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
