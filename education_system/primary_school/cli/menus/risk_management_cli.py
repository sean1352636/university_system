"""Risk Management CLI menu."""

from education_system.primary_school.infrastructure.database.db import get_db_path


def risk_management_menu(auth):
    """Risk Management management menu."""
    ...
    svc = RiskManagementService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                svc.create(title=title, category=category, risk_level=risk_level, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), title=title, category=category, risk_level=risk_level, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
