...
def meals_menu(auth):
    ...
    while True:
        ...
        if choice == "0":
            ...
            meal_type = input("  Meal Type: ").strip()
            date = input("  Date: ").strip()
            try:
                svc.create_record(pupil_id=pupil_id, meal_type=meal_type, date=date)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            date = input("  Date: ").strip()
            try:
                svc.update_record(int(pk), pupil_id=pupil_id, meal_type=meal_type, date=date)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
