"""Staff Absence CLI menu."""

from education_system.primary_school.infrastructure.database.db import get_db_path


def staff_absence_menu(auth):
    """Staff Absence management menu."""
    from education_system.primary_school.cli.cli_main import print_header, print_menu, get_choice
    from education_system.primary_school.modules.domain.staff.staff_absence.services.staff_absence_service import StaffAbsenceService

    svc = StaffAbsenceService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                svc.create(absence_type=absence_type, start_date=start_date, end_date=end_date, reason=reason, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), absence_type=absence_type, start_date=start_date, end_date=end_date, reason=reason, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
