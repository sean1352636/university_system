...
def markbook_menu(auth):
    """Markbook management menu."""
    from education_system.primary_school.cli.cli_main import print_header, print_menu, get_choice
    from education_system.primary_school.modules.domain.academics.markbook.services.markbook_service import MarkbookService

    svc = MarkbookService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            assessment_name = input("  Assessment Name: ").strip()
            score = input("  Score: ").strip()
            max_score = input("  Max Score: ").strip()
            grade = input("  Grade: ").strip()
            try:
                svc.create(assessment_name=assessment_name, score=score, max_score=max_score, grade=grade)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            grade = input("  Grade: ").strip()
            try:
                svc.update(int(pk), assessment_name=assessment_name, score=score, max_score=max_score, grade=grade)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
