...
def equality_diversity_menu(auth):
    ...
    svc = EqualityDiversityService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            record_type = input("  Record Type: ").strip()
            category = input("  Category: ").strip()
            description = input("  Description: ").strip()
            status = input("  Status: ").strip()
            try:
                svc.create(record_type=record_type, category=category, description=description, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), record_type=record_type, category=category, description=description, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
