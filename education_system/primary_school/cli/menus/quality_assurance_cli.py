...
def quality_assurance_menu(auth):
    ...
    from education_system.primary_school.modules.domain.admin.quality_assurance.services.quality_assurance_service import QualityAssuranceService

    svc = QualityAssuranceService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            review_type = input("  Review Type: ").strip()
            subject_area = input("  Subject Area: ").strip()
            reviewer = input("  Reviewer: ").strip()
            rating = input("  Rating: ").strip()
            try:
                svc.create(review_type=review_type, subject_area=subject_area, reviewer=reviewer, rating=rating)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            rating = input("  Rating: ").strip()
            try:
                svc.update(int(pk), review_type=review_type, subject_area=subject_area, reviewer=reviewer, rating=rating)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
