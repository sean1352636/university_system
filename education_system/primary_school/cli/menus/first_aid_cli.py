...
def first_aid_menu(auth):
    ...
    svc = FirstAidService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            patient_type = input("  Patient Type: ").strip()
            incident_date = input("  Date: ").strip()
            location = input("  Location: ").strip()
            description = input("  Description: ").strip()
            treatment_given = input("  Treatment: ").strip()
            try:
                svc.create(patient_name=patient_name, patient_type=patient_type, incident_date=incident_date, location=location, description=description, treatment_given=treatment_given)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            treatment_given = input("  Treatment: ").strip()
            try:
                svc.update(int(pk), patient_name=patient_name, patient_type=patient_type, incident_date=incident_date, location=location, description=description, treatment_given=treatment_given)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
