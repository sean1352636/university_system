...
def phonics_menu(auth):
    """Phonics management menu."""
    ...
    svc = PhonicsService(get_db_path())

    while True:
        print_header("Phonics")
        print_menu([
            ("1", "List results"),
            ("2", "View details"),
            ("3", "Add Result"),
        ...
        if choice == "0":
            ...
            try:
                svc.create_result(pupil_id=pupil_id, score=score, year=year)
                print("\n  Result created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            year = input("  Year: ").strip()
            try:
                svc.update_result(int(pk), pupil_id=pupil_id, score=score, year=year)
                print("\n  Result updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
