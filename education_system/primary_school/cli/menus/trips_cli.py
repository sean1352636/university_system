"""Trips CLI menu."""

from education_system.primary_school.infrastructure.database.db import get_db_path


def trips_menu(auth):
    """Trips management menu."""
    from education_system.primary_school.cli.cli_main import print_header, print_menu, get_choice
    from education_system.primary_school.modules.domain.pupil_life.trips.services.trip_service import TripService

    svc = TripService(get_db_path())

    while True:
        print_header("Trips")
        print_menu([
            ("1", "List trips"),
            ("2", "View details"),
            ("3", "Add Trip"),
            ("4", "Update Trip"),
            ("5", "Delete Trip"),
            ("0", "Back"),
        ])

        choice = get_choice()
        if choice == "0":
            break
        elif choice == "1":
            items = svc.list_trips()
            if not items:
                print("\n  No records found.")
            else:
                for item in items:
                    print(f"  {dict(item) if hasattr(item, 'keys') else item}")
        elif choice == "2":
            pk = input("  Enter ID: ").strip()
            try:
                item = svc.get_trip(int(pk))
                if item:
                    for k, v in (dict(item) if hasattr(item, "keys") else {}).items():
                        print(f"  {k}: {v}")
                else:
                    print("\n  Not found.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "3":
            title = input("  Title: ").strip()
            destination = input("  Destination: ").strip()
            date = input("  Date: ").strip()
            try:
                svc.create_trip(title=title, destination=destination, date=date)
                print("\n  Trip created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            pk = input("  Enter ID to update: ").strip()
            title = input("  Title: ").strip()
            destination = input("  Destination: ").strip()
            date = input("  Date: ").strip()
            try:
                svc.update_trip(int(pk), title=title, destination=destination, date=date)
                print("\n  Trip updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
            pk = input("  Enter ID to delete: ").strip()
            try:
                svc.delete_trip(int(pk))
                print("\n  Trip deleted.")
            except Exception as e:
                print(f"\n  Error: {e}")
        else:
            print("\n  Invalid option.")
