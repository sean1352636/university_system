...
def behaviour_menu(auth):
    """Behaviour management menu."""
    ...
    svc = BehaviourService(get_db_path())

    while True:
        print_header("Behaviour")
        print_menu([
            ("1", "List records"),
            ("2", "View details"),
            ("3", "Add Record"),
        ...
        if choice == "0":
            ...
            try:
                svc.create_record(pupil_id=pupil_id, incident_type=incident_type, description=description)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            description = input("  Description: ").strip()
            try:
                svc.update_record(int(pk), pupil_id=pupil_id, incident_type=incident_type, description=description)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
