...
def dbs_checks_menu(auth):
    ...
    svc = DbsChecksService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            certificate_number = input("  Certificate Number: ").strip()
            issue_date = input("  Issue Date: ").strip()
            expiry_date = input("  Expiry Date: ").strip()
            status = input("  Status: ").strip()
            try:
                svc.create(certificate_number=certificate_number, issue_date=issue_date, expiry_date=expiry_date, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), certificate_number=certificate_number, issue_date=issue_date, expiry_date=expiry_date, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
