...
def send_menu(auth):
    ...
    while True:
        ...
        if choice == "0":
            ...
            need_type = input("  Need Type: ").strip()
            try:
                svc.create_record(pupil_id=pupil_id, need_type=need_type)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            need_type = input("  Need Type: ").strip()
            try:
                svc.update_record(int(pk), pupil_id=pupil_id, need_type=need_type)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
