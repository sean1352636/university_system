...
def sms_email_menu(auth):
    ...
    svc = SmsEmailService(get_db_path())
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                svc.create(message_type=message_type, recipient=recipient, subject=subject, status=status)
                print("\n  Record created.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "4":
            ...
            status = input("  Status: ").strip()
            try:
                svc.update(int(pk), message_type=message_type, recipient=recipient, subject=subject, status=status)
                print("\n  Record updated.")
            except Exception as e:
                print(f"\n  Error: {e}")
        elif choice == "5":
