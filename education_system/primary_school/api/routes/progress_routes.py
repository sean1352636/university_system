...
@progress_bp.route("/<int:pk>", methods=["GET"])
@token_required
def get_record(pk):
    svc = ProgressService(_db_path)
...
def create_record():
    data = get_json_body()
    require_fields(data, "pupil_id", "subject_code", "current_level")
    svc = ProgressService(_db_path)
    result = svc.create_record(**data)
    return jsonify({"message": "Record created.", "data": result}), 201


@progress_bp.route("/<int:pk>", methods=["PUT"])
@token_required
@role_required("admin", "teacher")
...
@progress_bp.route("/<int:pk>", methods=["DELETE"])
@token_required
@role_required("admin")
