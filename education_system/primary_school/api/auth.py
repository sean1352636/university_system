"""JWT authentication decorators for the Primary School API."""
    ...
    def decorator(f):
        ...
        def decorated(*args, **kwargs):
            ...
            if g.current_user["role"] not in roles:
                logger.warning("Access denied: user '%s' (role=%s) requires %s",
                             g.current_user["username"], g.current_user["role"], roles)
                return jsonify({
                    "error": "Access denied",
