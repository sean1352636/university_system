"""CLI handlers for year-group enrolment in the Secondary School System."""
...
def _print_pupils(rows) -> None:
    if not rows:
        print("  (none)")
        return
    print(f"  {'ID':<10} {'Form':<6} {'Name':<28} {'SEND'}")
    print(f"  {'-'*10} {'-'*6} {'-'*28} {'-'*5}")
    for p in rows:
        print(f"  {p.pupil_id:<10} {(p.form_group or '-'):<6} "
...
def _move_pupil() -> None:
    ...
    new_form_input = _prompt("  New form group (blank to keep): ")
    new_form = new_form_input if new_form_input else None
    p = data.move_pupil(pid, new_year, new_form=new_form)
    print(f"\n  Moved {p.full_name} to year {p.year_group} "
          f"(form {p.form_group or '-'})")
