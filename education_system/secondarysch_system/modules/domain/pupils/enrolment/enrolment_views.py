"""Tk views for year-group enrolment in the Secondary School System."""
...
def open_enrolment(host) -> None:
    ...
    # Two-panel layout: summary on the left, year roll on the right
    body = ttk.Frame(root)
    ...
    for y in YEAR_GROUPS:
        ...
        ttk.Radiobutton(row, text=f"Year {y}", variable=selected_year,
    ...
    def _refresh() -> None:
        ...
        for p in roll.get(y, []):
            tree.insert("", "end", iid=p.pupil_id, values=(
                p.pupil_id, p.form_group or "-", p.full_name,
                p.date_of_birth or "-", p.send_status or "-",
            ))
        summary_var.set(f"Year {y}: {len(roll.get(y, []))} pupil(s)")
...
def _move_selected_dialog(host, tree: ttk.Treeview, *, on_done=None) -> None:
    ...
    ttk.Label(row2, text="New form (blank = auto, e.g. 7A → 8A):").pack(
        side="left", padx=(0, 6))
    ttk.Entry(row2, textvariable=fvar, width=10).pack(side="left")

    def _save() -> None:
        try:
            new_form = fvar.get().strip() or None
            data.move_pupil(sel, yvar.get(), new_form=new_form)
        except ValidationError as e:
...
def _promote_year_dialog(host, year: str, *, on_done=None) -> None:
    ...
    if to_y is None:
        ...
        if not leavers:
            messagebox.showinfo("Promote year",
                                f"Year {year} is empty — nothing to do.",
                                parent=host.root)
            return
        messagebox.showinfo(
            "Promote year",
            f"Year {year} is the final year. "
    ...
    if count == 0:
        messagebox.showinfo("Promote year",
                            f"No pupils in year {year} — nothing to do.",
                            parent=host.root)
        return
    if not messagebox.askyesno(
            "Promote year",
            f"Move all {count} pupil(s) from year {year} to year {to_y}?\n"
            f"Forms will be bumped accordingly (e.g. {year}A → {to_y}A).",
            parent=host.root):
        return
    result = data.promote_year(year)
    messagebox.showinfo(
        "Promote year",
        f"Promoted {result['count']} pupil(s) from year {year} to year {to_y}.",
        parent=host.root,
    )
    if on_done:
        on_done()
...
def _show_leavers(host) -> None:
    ...
    cols = ("id", "form", "name", "dob")
    tree = ttk.Treeview(frm, columns=cols, show="headings")
    for c, label, w in [("id", "ID", 90), ("form", "Form", 60),
                        ("name", "Name", 220), ("dob", "DOB", 100)]:
        tree.heading(c, text=label)
        tree.column(c, width=w, anchor="w")
    tree.pack(fill="both", expand=True)
    for p in rows:
        tree.insert("", "end", values=(p.pupil_id, p.form_group or "-",
