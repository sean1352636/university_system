...
class TestAPISecurityManager:
    ...
    def test_validate_api_key_expired(self, test_db):
        ...
        from education_system.systems.university.infrastructure.security.comprehensive_security import _hash_api_key
...
class TestPasswordSecurityManager:
    ...
    def test_check_compromised_password_found(self, mock_get, test_db):
        ...
        # HIBP uses SHA-1 range hashes; derive the suffix the same way the
        # source does so the mocked API response actually matches.
        password = "password123"
        pw_hash = manager._hibp_range_hash(password.encode("utf-8"))
        suffix = pw_hash[5:]
