# Idle / Inactivity Auto-Logout — Secondary School System

This document describes the session-timeout behaviour added in
**v8.74.0** for the Secondary School Management System. The same
mechanism is also installed in the College and Primary subsystems
...
The Secondary GUI **and** CLI auto-log-out the active user after
...
|-----------|---------|------------------|
| GUI | 30 minutes | `secondary_school/main_gui.py` (`MainApplication.__init__`) |
| CLI | 30 minutes | `secondary_school/cli/cli_main.py` (`main()`) |
...
This is **real activity tracking** — a teacher clicking through marking
a register stays logged in indefinitely; only walking away triggers
the logout.
...
> long behaviour-incident note) are **not** bounded by the timeout.
> The clock starts again the next time you return to a menu prompt.
...
2. Click `OK`.
3. `request_logout()` is called and the main window is destroyed.
...
`education_system/secondary_school/main_gui.py`:
...
`education_system/secondary_school/cli/cli_main.py`:
...
| Risk | Mitigation |
|------|------------|
| Classroom workstation left logged in between lessons | Auto-logout ensures the next teacher / supply can't see the previous session's records |
| SLT laptop left open in a public area | Pastoral/safeguarding records remain protected |
| Remote-desktop session forgotten overnight | Same — the watchdog fires regardless of who is physically at the keyboard |
...
| Secondary GUI wiring | `education_system/secondary_school/main_gui.py` |
| Secondary CLI wiring | `education_system/secondary_school/cli/cli_main.py` |

---

## Related documentation

- [`SECURITY.md`](SECURITY.md) — overall secondary school security architecture
