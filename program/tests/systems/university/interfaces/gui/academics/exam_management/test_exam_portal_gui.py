...
def mock_get_connection(db_path):
    ...
    with patch(
        "education_system.systems.university.domain.academics.services.exam_management.exam_service.get_connection",
        _fake,
    ):
        yield
...
def _activate_tab(gui, label):
    """Materialize a lazily-built tab by its label.

    The portal only builds the "Available Exams" tab eagerly; the rest
    (My Results, Manage Exams, Grading, …) are placeholders that build on
    first selection. Tests that inspect those tabs' widgets must trigger
    the build first, which this does the same way the GUI's own
    tab-changed handler would.
    """
    for key, (placeholder, builder, lbl) in list(gui._lazy_tab_builders.items()):
        if lbl == label:
            del gui._lazy_tab_builders[key]
            builder()
            return
    raise AssertionError(f"No lazy tab registered with label {label!r}")
...
class TestManageTab:
    def test_manage_tree_populates(self, svc, mock_auth, mock_get_connection):
        ...
        try:
            ...
            from education_system.systems.university.interfaces.gui.academics.exam_management import ExamPortalGUI
            gui = ExamPortalGUI(parent=root, auth=mock_auth)
            _activate_tab(gui, "Manage Exams")
            gui._refresh_manage()
