...
def mock_service():
    ...
    with patch(
        'education_system.systems.university.domain.assessment.academic_progress.services.progress_service.ProgressService._ensure_tables_exist'
    ):
        with patch(
            'education_system.systems.university.domain.assessment.academic_progress.services.progress_service.get_connection'
        ) as mock_gc:
            with patch(
                'education_system.systems.university.domain.assessment.academic_progress.services.progress_service.transaction'
            ) as mock_tx:
                mock_conn = MagicMock()
...
def cli_instance(mock_auth):
    ...
    with patch(
        'education_system.systems.university.interfaces.cli.assessment.academic_progress.progress_cli.ProgressService'
    ) as MockSvc:
        with patch(
            'education_system.systems.university.interfaces.cli.assessment.academic_progress.progress_cli.get_auth',
...
class TestProgressServiceCalculateDegreeProgress:
    ...
    def test_valid_student_returns_dict(self, mock_service):
        ...
        with patch.object(svc, '_update_progress_records'):
            with patch('education_system.systems.university.domain.assessment.academic_progress.services.progress_service.log_activity'):
...
class TestAcademicProgressCLIGradesBreakdown:
    ...
    def test_no_enrolled_modules(self, cli_instance, capsys):
        ...
        with patch(
            'education_system.systems.university.infrastructure.database.db.get_connection',
...
class TestGUIImports:
    ...
    def test_gpa_calculator_gui_class_exists(self):
        ...
        import education_system.systems.university.interfaces.gui.assessment.academic_progress.progress_gui as gui_mod
