...
class TestSafeLogSecurityEvent:
    def test_returns_true_on_success(self):
        with patch(
            "education_system.systems.university.infrastructure.security.immutable_audit_log.log_security_event",
        ) as mock_log_fn:
    ...
    def test_returns_false_on_import_error(self):
        ...
        with patch(
            "education_system.systems.university.infrastructure.security.audit_helpers.safe_log_security_event",
            wraps=safe_log_security_event,
        ):
            # Force ImportError by patching the import path
            with patch.dict("sys.modules", {
                "education_system.systems.university.infrastructure.security.immutable_audit_log": None,
            }):
...
class TestGetGuiContext:
    def test_returns_none_none_on_import_error(self):
        with patch.dict("sys.modules", {
            "education_system.systems.university.infrastructure.shared_context": None,
        }):
    ...
    def test_returns_none_none_when_not_logged_in(self):
        ...
        with patch(
            "education_system.systems.university.infrastructure.security.audit_helpers.get_gui_context"
        ) as mock_fn:
    ...
    def test_returns_user_context_when_logged_in(self):
        ...
        with patch.dict("sys.modules", {
            "education_system.systems.university.infrastructure.shared_context": mock_module,
        }):
            user_id, session_id = get_gui_context()
            assert user_id == "42"
