"""CLI handlers for pupil CRUD in the Secondary School System.

Exposes ``dispatch(label)`` so the main CLI menu can route the labels
"Pupil Directory" / "Add Pupil" / "Search Pupils" / "Pupil Profile"
to this module without an explicit table.
"""
...
from education_system.systems.secondary.domain.learners.pupils import pupils as data
...
def _print_table(rows: list[data.Pupil]) -> None:
    if not rows:
        print("  (no pupils)")
        return
    print(f"  {'ID':<10} {'Year':<4} {'Form':<6} {'Name':<28} {'Email'}")
    print(f"  {'-'*10} {'-'*4} {'-'*6} {'-'*28} {'-'*30}")
    for p in rows:
        print(f"  {p.pupil_id:<10} {p.year_group:<4} "
              f"{(p.form_group or '-'):<6} {p.full_name[:28]:<28} {p.email}")
...
def _print_profile(p: data.Pupil) -> None:
    ...
    print(f"  Form group:     {p.form_group or '-'}")
...
def _collect_fields(existing: data.Pupil | None = None) -> dict[str, str]:
    ...
    fields: dict[str, str] = {}
    fields["first_name"] = ask("First name", existing.first_name if existing else None)
    fields["last_name"]  = ask("Last name",  existing.last_name  if existing else None)
    fields["year_group"] = ask(
        f"Year group ({'/'.join(YEAR_GROUPS)})",
        existing.year_group if existing else None,
    )
    fields["form_group"]    = ask("Form group",
...
def open_search() -> None:
    ...
    print("\n  ── Search Pupils ──")
    q = _prompt("  Search (id / name / form / email): ")
...
_ADV_FIELDS: list[tuple[str, str, str]] = [
    # (key, prompt, hint)
    ("name",        "Name contains",            "first or last name"),
    ("year_group",  "Year group",               "/".join(YEAR_GROUPS) + " — blank for any"),
    ("form_group",  "Form group contains",      "e.g. 9A"),
    ("email",       "Email contains",           ""),
    ("parent_name", "Parent name contains",     ""),
    ("send_status", "SEND",                     "yes / no / blank"),
    ("dob_from",    "DOB from (inclusive)",     "YYYY-MM-DD"),
    ("dob_to",      "DOB to   (inclusive)",     "YYYY-MM-DD"),
    ("has_phone",   "Has phone?",               "yes / no / blank"),
    ("has_parent",  "Has parent contact?",      "yes / no / blank"),
    ("sort_by",     "Sort by",                  "year / name / id / email / dob"),
]


def _collect_adv_filters() -> dict[str, str]:
...
@_safe
def open_advanced_search() -> None:
    logger.debug("CLI: open_advanced_search")
    while True:
        print("\n  ── Advanced Search ──")
        print("  Leave any field blank to skip it.")
        filters = _collect_adv_filters()
        try:
            rows = data.advanced_search(filters)
        except data.ValidationError as e:
            print(f"  Filter error: {e}")
            if _prompt("  Try again? (Y/n): ").lower() == "n":
                return
            continue
        print(f"\n  ── Results ({len(rows)} match(es)) ──")
        if filters:
            active = ", ".join(f"{k}={v}" for k, v in filters.items())
            print(f"  Filters: {active}")
        _print_table(rows)
        action = _prompt(
            "\n   V) View pupil   R) Refine search   Enter) Back: "
        ).lower()
        if action == "v":
            pid = _prompt("  Pupil ID: ")
            if pid:
...
def dispatch(label: str) -> bool:
    """Run the handler for ``label``; return True if handled."""
