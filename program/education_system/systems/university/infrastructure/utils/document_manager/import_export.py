...
def _read_excel_rows(file_path):
    """Read an .xlsx file into a list of header-keyed dicts using openpyxl.

    The first non-empty row is treated as the header. Fully-empty rows are
    skipped. Raises ImportError if openpyxl is unavailable, or another
    exception if the file is not a valid workbook.
    """
...
class ImportExportMixin:
    ...
    def import_from_excel(self, file_path=None):
        """Import documents from an Excel (.xlsx) spreadsheet.

        Reads the workbook with openpyxl, parses each row (header-keyed) and
        imports rows via validate_and_import_document — the same contract as
        the CSV import. Returns the list of parsed row dicts, or None on a
        read/format error.
        """
        try:
            print("\n📥 IMPORT FROM EXCEL")
            ...
            try:
                records = _read_excel_rows(file_path)
            except ImportError:
                print("❌ Excel import requires the openpyxl library, which is not installed.")
                print("Please convert your file to CSV and use the CSV import option.")
                return None
            except Exception as e:
                print(f"❌ Could not read Excel file (invalid format?): {e}")
                return None
            ...
            for row in records:
                ...
                doc_file_path = (row.get('file_path') or '').strip()
