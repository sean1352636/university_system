...
from education_system.systems.university.domain.assessment.grading.utils import _get_calculate_course_statistics, calculate_trend_slope
from education_system.systems.university.domain.assessment.grading.trends import create_course_comparison_charts
from education_system.systems.university.domain.assessment.grading.forecasting import export_comparison_data
from education_system.systems.university.domain.assessment.grading.grade_calculation import compare_by_grade_threshold
...
def compare_by_enrollment_date(cursor):
    ...
    from education_system.systems.university.domain.assessment.grading.grade_calculation import calculate_student_gpa
    ...
    early_avg = np.mean(early_gpas) if early_gpas else 0.0
    late_avg = np.mean(late_gpas) if late_gpas else 0.0

    print("\nEnrollment Period Comparison:")
    print(f"Early period (≤{early_date}): {len(early_students)} students, Avg GPA: {early_avg:.2f}")
    print(f"Late period (≥{late_date}): {len(late_students)} students, Avg GPA: {late_avg:.2f}")
