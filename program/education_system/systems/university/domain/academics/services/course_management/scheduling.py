...
from education_system.systems.university.domain.academics.services.course_management.validation import validate_time_format, validate_days_of_week
...
def _sync_derived_timetables(module_schedule_ids):
    ...
    try:
        from education_system.systems.university.domain.academics.services.course_management.calendar_lecture_sync import (
...
def _push_in_app_notification(user_id, title: str, message: str,
    ...
    """Best-effort in-app mirror of a schedule change, delivered to the
    recipient's unified inbox (the ``messages`` table).
    ...
    Never raises; returns False if the recipient can't be resolved.
    """
    if user_id is None:
        return False
    try:
        from education_system.systems.university.infrastructure.email.messages_inbox import (
            deliver_inbox_message,
        )
        return deliver_inbox_message(user_id, title, message) is not None
    except Exception as exc:
...
def create_course_schedule(auth):
    ...
    try:
        ...
        try:
            from education_system.systems.university.domain.academics.services.course_management.workload_sync import (
