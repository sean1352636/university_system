...
from education_system.systems.university.infrastructure.institution_settings import get_events_support_phone
...
def _manage_event_attendance_simple():
    """Manage event attendance for club events."""
    global auth

    if not auth or not auth.current_user:
        print("You must be logged in to manage attendance.")
        return
    ...
    try:
        ...
        if not events:
            print("No events found for your clubs.")
            conn.close()
            return
        ...
        cursor.execute('''
        SELECT COUNT(*) FROM union_events
        WHERE event_id = ? AND organizer_id IN (
            SELECT club_id FROM club_members WHERE student_id = ?
        )
        ''', (event_id, student_id))
        if cursor.fetchone()[0] == 0:
            print("You can only manage attendance for your own club events.")
            conn.close()
            return

        if choice == '1':
            target_student = input("Enter student ID to mark attendance: ").strip()
            cursor.execute('''
            SELECT COUNT(*) FROM unified_event_registrations
            WHERE event_id = ? AND user_id = ?
            ''', (event_id, target_student))
            if cursor.fetchone()[0] == 0:
                print("Student is not registered for this event.")
                conn.close()
                return
            ...
            cursor.execute('''
            SELECT r.user_id, s.first_name, s.last_name, r.attendance_status, r.registration_date
            ...
            if not attendees:
                ...
                for attendee in attendees:
                    print(f"{attendee[0]} - {attendee[1]} {attendee[2]} ({attendee[3]})")

        conn.close()
    except sqlite3.Error as e:
        print(f"Database error: {e}")

def _track_event_finances_simple():
    """Track expenses and income for club events."""
    global auth

    if not auth or not auth.current_user:
        print("You must be logged in to manage event finances.")
        return
    ...
    try:
        ...
        print("1. Add event expense")
        print("2. Record event income")
        ...
        if choice in {'1', '2'}:
            ...
            entry_type = 'expense' if choice == '1' else 'income'
            cursor.execute('''
            INSERT INTO event_finances (event_id, amount, entry_type, description, entry_date)
            VALUES (?, ?, ?, ?, ?)
            ''', (event_id, float(amount), entry_type, description, datetime.now().strftime('%Y-%m-%d')))
            conn.commit()
            print("Financial entry recorded.")
        else:
            cursor.execute('''
            SELECT SUM(amount) FROM event_finances
            WHERE event_id = ? AND entry_type = 'income'
            ''', (event_id,))
            total_income = cursor.fetchone()[0] or 0

            cursor.execute('''
            SELECT SUM(amount) FROM event_finances
            WHERE event_id = ? AND entry_type = 'expense'
            ''', (event_id,))
            total_expense = cursor.fetchone()[0] or 0

            cursor.execute('''
            SELECT amount, entry_type, description, entry_date
            FROM event_finances
            WHERE event_id = ? AND entry_type = 'expense'
...
def _create_recurring_event_simple():
    """Create a recurring event for a club."""
    global auth
    ...
    if not auth.check_permission('create_recurring_events'):
        print("You don't have permission to create recurring events.")
        return
    ...
    try:
        ...
        max_attendees = input("Max attendees: ").strip()
        cost = input("Cost: ").strip()
        category = input("Category: ").strip()
        start_date = input("Start date (YYYY-MM-DD): ").strip()
        end_date = input("End date (YYYY-MM-DD): ").strip()
        recurrence_type = input("Recurrence (weekly/monthly): ").strip().lower()
        recurrence_value = input("Day (e.g., Monday or day of month): ").strip()

        cursor.execute('''
        INSERT INTO union_recurring_events (
            event_name, description, start_time, location, max_attendees, cost,
            category, start_date, end_date, recurrence_type, recurrence_value, created_by
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            event_name, description, start_time, location, max_attendees, cost,
            category, start_date, end_date, recurrence_type, recurrence_value, student_id
        ))
        conn.commit()
...
def generate_event_financial_report(event_id, event_name, cursor):
    ...
    try:
        ...
        if profit_loss > 0:
            print("✓ Event was profitable")
        elif profit_loss == 0:
            print("= Event broke even")
        else:
            print("✗ Event had a loss")
...
def create_recurring_event():
    ...
    try:
        ...
        conn.commit()
        print("Recurring event series created successfully!")
...
def manage_sustainable_events(student_id, cursor, conn):
    ...
    from education_system.systems.university.domain.pastoral.student_life.student_union.services.points import auto_award_points
    try:
        ...
        if choice == '1':
            ...
            print("Target: Reduce by 30% through sustainable practices")
...
def manage_virtual_events():
    ...
    global auth
    from education_system.systems.university.domain.pastoral.student_life.student_union.services.events import (
...
def create_virtual_event(cursor, conn):
    ...
    try:
        ...
        try:
            ...
            # Test environments may not provide stdin for this prompt.
        ...
        if confirm != 'y':
            print("Virtual event creation cancelled.")
            return
        ...
        # Resolve the organiser from the logged-in user where possible.
        organizer_id = None
        try:
        ...
        # Preserve the virtual-specific settings within the description.
        detail_suffix = f"[Virtual: {platform} | Access: {access_level}"
        if features:
            detail_suffix += f" | Features: {', '.join(features)}"
        detail_suffix += f" | Recording: {'yes' if record_event else 'no'}]"
        full_description = f"{description}\n{detail_suffix}".strip()

        cursor.execute('''
            INSERT INTO union_events (
                event_name, description, organizer_id, event_date,
                start_time, end_time, location, category, max_attendees,
                status, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, 'virtual', ?, 'upcoming', ?)
        ''', (event_name, full_description, organizer_id, event_date,
              start_time, end_time, platform, max_attendees,
...
def create_event(cursor, conn):
    """Create a new event for a student club.
    ...
    conn : sqlite3.Connection
        Active database connection (used for commit).
    """
    global auth
    ...
    try:
        ...
        event_name = None
        if getattr(builtins.input, '__module__', 'builtins') != 'builtins':
            event_name = input("Event name: ").strip()
            if not event_name:
                print("Event name cannot be empty.")
                return
        ...
        # Get clubs user is an officer of
        cursor.execute('''
            SELECT c.club_id, c.club_name
            FROM student_clubs c
            WHERE (c.president_id = ? OR c.treasurer_id = ? OR c.secretary_id = ?)
        ...
        # Gather remaining event details
        description = input("Description: ").strip()
        event_date = input("Event date (YYYY-MM-DD): ").strip()
        start_time = input("Start time (HH:MM): ").strip()
        location = input("Location: ").strip()
        ...
        try:
            ticket_price = float(input("Ticket price (£, 0 for free): ").strip())
        except ValueError:
            ticket_price = 0.0

        category = input("Category: ").strip()

        # Use first club for simplicity
        club_id = clubs[0][0]
        ...
        ''', (event_name, description, event_date, start_time,
              location, club_id, category, max_attendees))
...
def view_event_analytics():
    """View analytics and statistics for events."""
    global auth

    if not auth or not auth.current_user:
        print("You must be logged in to view event analytics.")
        return
    ...
    try:
        ...
        # Total events
        cursor.execute('SELECT COUNT(*) FROM union_events')
        total_events = cursor.fetchone()[0]

        # Total attendees across all events
        cursor.execute('SELECT COALESCE(SUM(current_attendees), 0) FROM union_events')
        total_attendees = cursor.fetchone()[0]

        # Average attendance rate
        cursor.execute('''
            SELECT COALESCE(
                AVG(CASE WHEN max_attendees > 0
                    THEN (current_attendees * 100.0 / max_attendees)
                    ELSE 0 END), 0)
            FROM union_events
            WHERE max_attendees > 0
        ''')
        avg_attendance_rate = cursor.fetchone()[0]

        # Per-event breakdown
        cursor.execute('''
            SELECT e.event_name, e.max_attendees, e.current_attendees,
                   CASE WHEN e.max_attendees > 0
                        THEN ROUND(e.current_attendees * 100.0 / e.max_attendees, 1)
                        ELSE 0 END AS attendance_rate,
                   COALESCE(SUM(CASE WHEN f.amount > 0 THEN f.amount ELSE 0 END), 0) AS revenue,
                   COALESCE(SUM(CASE WHEN f.amount < 0 THEN ABS(f.amount) ELSE 0 END), 0) AS expenses
            FROM union_events e
            LEFT JOIN event_finances f ON e.event_id = f.event_id
            GROUP BY e.event_id
            ORDER BY e.event_date DESC
        ''')
        ...
        if event_details:
            print(f"\n{'Event':<25} {'Capacity':<10} {'Attended':<10} {'Rate':<8} {'Revenue':<12} {'Expenses':<12}")
            print("-" * 77)
            for event in event_details:
                print(f"{str(event[0])[:24]:<25} {event[1]:<10} {event[2]:<10} "
                      f"{event[3]:<8} £{event[4]:<11.2f} £{event[5]:<11.2f}")
