...
def display_helpdesk_menu(auth):
    ...
    from education_system.systems.university.domain.pastoral.services.helpdesk.database import init_helpdesk_db
    init_helpdesk_db()

    # Setup permissions
    from education_system.systems.university.domain.pastoral.services.helpdesk.permissions import setup_enhanced_helpdesk_permissions
        ...
        try:
            ...
            if choice_idx == len(options):  # Return option
                ...
                if action == 'create_ticket':
                    from education_system.systems.university.domain.pastoral.services.helpdesk.tickets.creation import create_ticket_enhanced
                    create_ticket_enhanced(auth)
                elif action == 'view_own_tickets':
                    view_user_tickets_enhanced(auth)
                elif action == 'advanced_search':
                    from education_system.systems.university.domain.pastoral.services.helpdesk.tickets.search import advanced_search_tickets
                    advanced_search_tickets(auth)
                elif action == 'saved_searches':
                    from education_system.systems.university.domain.pastoral.services.helpdesk.tickets.search import load_saved_searches
                    load_saved_searches(auth)
                elif action == 'knowledge_base':
                    from education_system.systems.university.domain.pastoral.services.helpdesk.knowledge_base import manage_knowledge_base
                    manage_knowledge_base(auth)
                elif action == 'view_all_tickets':
                    from education_system.systems.university.domain.pastoral.services.helpdesk.bulk import view_all_tickets_enhanced
                    view_all_tickets_enhanced(auth)
                elif action == 'analytics':
                    from education_system.systems.university.domain.pastoral.services.helpdesk.analytics.dashboard import generate_analytics_dashboard
                    generate_analytics_dashboard(auth)
                elif action == 'generate_report':
                    from education_system.systems.university.domain.pastoral.services.helpdesk.analytics.reports import generate_enhanced_ticket_report
...
def system_management_menu(auth):
    ...
    while True:
        ...
        if choice == '1':
            from education_system.systems.university.domain.pastoral.services.helpdesk.admin.sla import manage_sla_policies
            manage_sla_policies(auth)
        elif choice == '2':
            from education_system.systems.university.domain.pastoral.services.helpdesk.admin.templates import manage_ticket_templates
            manage_ticket_templates(auth)
        elif choice == '3':
            from education_system.systems.university.domain.pastoral.services.helpdesk.admin.admin_workflows import manage_workflows
            manage_workflows(auth)
        elif choice == '4':
            from education_system.systems.university.domain.pastoral.services.helpdesk.admin.departments import manage_departments
            manage_departments(auth)
        elif choice == '5':
            from education_system.systems.university.domain.pastoral.services.helpdesk.admin.organizations import manage_organizations
            manage_organizations(auth)
        elif choice == '6':
            from education_system.systems.university.domain.pastoral.services.helpdesk.maintenance import system_maintenance
            system_maintenance(auth)
        elif choice == '7':
            from education_system.systems.university.domain.pastoral.services.helpdesk.data_io import data_import_export
