...
def reply_to_ticket_enhanced(auth, ticket_id, is_internal=False):
    ...
    try:
        ...
        from education_system.systems.university.domain.pastoral.services.helpdesk.tickets.attachments import handle_file_attachments
        handle_file_attachments(ticket_id, reply_id, auth.current_user['id'])

        # Log the action
        from education_system.systems.university.domain.pastoral.services.helpdesk.workflows import log_ticket_action, run_ticket_workflows
...
def change_ticket_status_enhanced(auth, ticket_id):
    ...
    try:
        ...
        if new_status in ['resolved', 'closed']:
            ...
            if send_survey == 'y':
                from education_system.systems.university.domain.pastoral.services.helpdesk.notifications import send_satisfaction_survey
...
def assign_ticket_enhanced(auth, ticket_id):
    ...
    try:
        ...
        # Log the action
        from education_system.systems.university.domain.pastoral.services.helpdesk.workflows import log_ticket_action
