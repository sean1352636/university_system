...
try:
    ...
    from education_system.systems.university.infrastructure.email.templates import load_template, render_template
...
try:
    ...
    from education_system.systems.university.domain.pastoral.services.student_support import (
...
class TicketDetailMixin:
    ...
    def create_ticket_overview(self, parent, ticket):
        ...
        ticket_title = ticket.get('subject', 'Untitled') if ticket else 'Untitled'
        ticket_student_id = ticket.get('user_id', 'Unknown') if ticket else 'Unknown'
        ...
        ticket_created = ticket.get('created_at', 'N/A') if ticket else 'N/A'
        ...
        right_details = [
            ...
            ("⏰ Last Updated:", ticket.get('updated_at', 'N/A')),
    ...
    def create_ticket_attachments(self, parent, attachments):
        ...
        for attachment in attachments:
            ...
            uploaded_datetime = attachment.get('created_at', 'N/A')
    ...
    def edit_ticket(self, ticket, parent_window=None):
        ...
        edit_title_var = tk.StringVar(value=ticket.get('subject', ''))
        ...
        def save_ticket_changes():
            try:
                ...
                if new_title != ticket.get('subject', ''):
                    update_details += "- Title updated\n"
    ...
    def _send_ticket_update_notification(self, ticket_id, update_type, update_details=''):
        ...
        def send_notification():
            ...
            try:
                ...
                cursor.execute('''
                    SELECT st.subject, st.category, st.status, st.priority, st.user_id,
                           u.email, u.first_name, u.last_name, u.username
                    FROM support_tickets st
                    LEFT JOIN users u ON st.user_id = u.student_id
                    WHERE st.ticket_id = ?
                ''', (ticket_id,))
    ...
    def _send_ticket_status_change_notification(self, ticket_id, old_status, new_status, changed_by='Support Staff'):
        ...
        def send_notification():
            ...
            try:
                ...
                # Get ticket and user details
                cursor.execute('''
                    SELECT st.subject, st.category, st.priority, st.user_id,
                           u.email, u.first_name, u.last_name, u.username
                    FROM support_tickets st
                    LEFT JOIN users u ON st.user_id = u.student_id
                    WHERE st.ticket_id = ?
