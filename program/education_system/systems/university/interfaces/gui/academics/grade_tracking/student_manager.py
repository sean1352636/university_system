...
class StudentManager:
    ...
    def refresh_students(self):
        ...
        try:
            ...
            # Legacy/standalone grade databases create ``students`` without a
            # ``registration_datetime`` column (only ``enrollment_date``). The
            # COALESCE below still references it, and SQLite raises "no such
            # column" when it is absent — so add it on demand before querying.
            ensure_column_exists(cursor, 'students', 'registration_datetime', 'TEXT')
