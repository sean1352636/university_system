...
class DonationDialog:
    """Dialog for making a donation to a fundraising campaign."""

    def __init__(self, parent, children):
        self.result = None

        dialog = tk.Toplevel(parent)
        dialog.title("Make a Donation")
        ...
        ttk.Label(main_frame, text="Make a Donation", font=('Arial', 14, 'bold')).pack(pady=(0, 15))

        # Campaign selection
        ttk.Label(main_frame, text="Campaign:").pack(anchor='w')
        campaign_var = tk.StringVar()
        campaign_combo = ttk.Combobox(main_frame, textvariable=campaign_var, width=45, state="readonly")
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='fundraising_campaigns'")
            if cursor.fetchone():
                cursor.execute("SELECT campaign_name FROM fundraising_campaigns WHERE status = 'active' OR status IS NULL")
                campaigns = [row[0] for row in cursor.fetchall()]
            conn.close()
        except Exception:
            pass
        campaign_combo['values'] = campaigns if campaigns else ['General Fund']
        if campaign_combo['values']:
            campaign_combo.current(0)
        campaign_combo.pack(fill=tk.X, pady=(0, 10))
        ...
        # Child selection
        ttk.Label(main_frame, text="On behalf of student:").pack(anchor='w')
        child_var = tk.StringVar()
        child_combo = ttk.Combobox(main_frame, textvariable=child_var, width=45, state="readonly")
        child_combo['values'] = [f"{c[1]} {c[3]} (ID: {c[0]})" for c in children] if children else ['N/A']
        if child_combo['values']:
            child_combo.current(0)
        child_combo.pack(fill=tk.X, pady=(0, 15))
        ...
        def submit():
            ...
            child = children[idx] if children and idx >= 0 else None
            self.result = (campaign, amount, child)
            dialog.destroy()

        btn_frame = ttk.Frame(main_frame)
        btn_frame.pack(pady=15)
        ttk.Button(btn_frame, text="Donate", command=submit).pack(side=tk.LEFT, padx=5)
