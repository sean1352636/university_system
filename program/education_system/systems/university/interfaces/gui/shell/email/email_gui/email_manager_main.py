...
from education_system.systems.university.infrastructure.email.admin import display_communication_dashboard

# Import get_stored_emails first - this is critical for the inbox functionality
try:
    from education_system.systems.university.infrastructure.email.email_service import get_stored_emails
...
try:
    from education_system.systems.university.infrastructure.email.email_db_utilities import optimize_database, execute_db_operation
    logger.debug("Imported database functions")
except ImportError as e:
    print(f"⚠️ Could not import database functions: {e}")

try:
    from education_system.systems.university.infrastructure.email.template_utils import list_templates, load_template, create_template
    logger.debug("Imported template functions")
except ImportError as e:
    print(f"⚠️ Could not import template functions: {e}")

try:
    from education_system.systems.university.infrastructure.email.reports import get_system_health_info, get_user_communication_stats
...
if CommunicationDashboard is None:
    try:
        from education_system.systems.university.infrastructure.email.admin import CommunicationDashboard as DirectCommunicationDashboard
...
try:
    from education_system.systems.university.infrastructure.email.announcements import send_batch_announcement
...
try:
    ...
    from education_system.systems.university.infrastructure.email.config import config, save_config
...
from education_system.systems.university.interfaces.gui.shell.email.email_gui.utility_dialogs import HelpDialog, AboutDialog
...
class EmailManagerGUI:
    ...
    def initialize_system(self):
        ...
        try:
            ...
            try:
                from education_system.systems.university.infrastructure.email import set_auth
                set_auth(self.auth)
    ...
    def fix_email_senders(self):
        ...
        if messagebox.askyesno(_t("common.confirm", default="Confirm"), _t("email.dialogs.fix_senders_confirm", default="Fix email sender attribution? This will update existing messages.")):
            try:
                ...
                from education_system.systems.university.infrastructure.email.email_service import fix_existing_email_senders
    ...
    def test_sender_attribution(self):
        ...
        try:
            # Import the function from email_manager
            from education_system.systems.university.infrastructure.email.email_service import test_sender_attribution as test_func
            result = test_func()
            if result:
...
try:
    from education_system.systems.university.interfaces.gui.shell.email.email_gui import email_tab
    from education_system.systems.university.interfaces.gui.shell.email.email_gui import messages_tab
    from education_system.systems.university.interfaces.gui.shell.email.email_gui import dashboard_tab
    from education_system.systems.university.interfaces.gui.shell.email.email_gui import sms_tab
    from education_system.systems.university.interfaces.gui.shell.email.email_gui import announcements_tab
    from education_system.systems.university.interfaces.gui.shell.email.email_gui import chat_tab
    from education_system.systems.university.interfaces.gui.shell.email.email_gui import reports_tab
except ImportError as e:
