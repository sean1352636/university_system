...
try:
    from education_system.systems.university.domain.academics.services.parent_portal.menu import (
...
def show_career_services_gui(self):
    ...
    if callable(opener):
        from education_system.systems.university.interfaces.gui.pastoral.career_services.career_services_gui import CareerServicesGUI
...
def open_internship_portal_gui(self):
    ...
    try:
        from education_system.systems.university.interfaces.gui.pastoral.internship_management import InternshipGUI
    except Exception as e:
...
def show_admissions_crm_gui(self):
    ...
    opener = getattr(self, "open_in_workspace", None)
    if callable(opener):
        from education_system.systems.university.interfaces.gui.admissions.admissions_crm_gui import AdmissionsCRMGUI
...
def show_police_station_gui(self):
    ...
    try:
        from education_system.systems.university.interfaces.gui.operations.campus.security.police_station_gui import PoliceStationApp
...
def show_security_desk_gui(self):
    ...
    otherwise — same pattern as Student Records (8.117.38)."""
    try:
        from education_system.systems.university.interfaces.gui.operations.campus.security.security_desk_gui import SecurityDesk
    except Exception as e:
...
def show_church_management_gui(self):
    ...
    try:
        from education_system.systems.university.interfaces.gui.operations.campus.community.church_management_gui import ChurchManagementSystem
        window = tk.Toplevel(self.root)
        _install_clean_close(window)
