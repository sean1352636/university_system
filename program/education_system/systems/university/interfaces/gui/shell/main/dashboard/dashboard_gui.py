...
# Module-level auth handle for standalone launchers; None means no session is
# attached (the standalone analytics window then runs without an auth context).
auth = None

# CLI fallback menu for analytics, used when the analytics GUI is unavailable.
try:
    from education_system.systems.university.interfaces.cli.shell.admin_tools import display_analytics_menu
except ImportError:
    def display_analytics_menu(*args, **kwargs):
        print("Analytics menu is unavailable")
...
from education_system.systems.university.interfaces.gui.shell.main.imports import gui_imports
...
def show_integrated_dashboard(self):
    ...
    try:
        ...
        if role == 'student':
            ...
            from education_system.systems.university.interfaces.gui.shell.main.dashboard.admin_dashboard import create_admin_dashboard
    ...
    if role == 'admin':
        try:
            from education_system.systems.university.services.dashboard.dashboard_service import DashboardService as _DS
            _admin_svc = _DS()
            ...
            from education_system.systems.university.interfaces.gui.shell.main.dashboard.login_analytics_dashboard import create_login_analytics_tab
            ...
            from education_system.systems.university.interfaces.gui.shell.main.dashboard.operations_dashboard import create_operations_tab
            ...
            from education_system.systems.university.interfaces.gui.shell.main.dashboard.system_health_dashboard import create_system_health_tab
...
def create_overview_tab(self, parent):
    ...
    # ---------------------- ANNOUNCEMENTS (#7) ------------------------
    # Announcements created in the Communications (Email) GUI land in the
    # ``announcements`` table; surface the active, role-relevant ones here via
    # the shared panel so the main dashboard and every role portal stay in sync.
    try:
        ...
        create_announcements_panel(overview_container, self.auth,
                                   limit=5, padx=0, pady=(0, 15))
    except Exception as e:
        logging.warning(f"Announcements panel unavailable: {e}")
...
def create_health_tab(self, parent):
    ...
    def refresh_health():
        try:
            ...
            try:
                from education_system.systems.university.infrastructure.database.pool_metrics import get_pool_metrics
