...
def _launch_profile(parent, auth):
    ...
    from education_system.systems.university.interfaces.gui.pastoral.student_profile.profile_gui import StudentProfileGUI
    StudentProfileGUI(parent, auth=auth)


def _launch_security(parent, auth):
    """Launch Account Security Dashboard."""
    from education_system.systems.university.interfaces.gui.pastoral.account_security.security_gui import AccountSecurityGUI
    AccountSecurityGUI(parent, auth=auth)


def _launch_notifications(parent, auth):
    """Launch Notification Preferences."""
    from education_system.systems.university.interfaces.gui.pastoral.notification_prefs.notification_prefs_gui import NotificationPrefsGUI
...
def _launch_messaging(parent, auth):
    ...
    import tkinter as tk
    from education_system.systems.university.interfaces.gui.shell.email.email_gui.email_manager_main import EmailManagerGUI
...
def _launch_roommate_finder(parent, auth):
    ...
    """
    import tkinter as tk
    from education_system.systems.university.interfaces.gui.operations.campus.housing.housing_accommodation_gui.main_gui import HousingGUI
...
def _launch_documents(parent, auth):
    ...
    from education_system.systems.university.interfaces.gui.pastoral.document_center.document_center_gui import DocumentCenterGUI
...
def _launch_graduation(parent, auth):
    ...
    from education_system.systems.university.interfaces.gui.academics.course_management_gui.graduation_ceremony_gui import GraduationCeremonyGUI
...
def _launch_feature(root, auth, feature_name):
    ...
    feature_map = {
        'marketplace': (
            'Marketplace',
            'education_system.systems.university.interfaces.gui.operations.commerce.marketplace.marketplace_gui',
            'MarketplaceGUI', 'kwarg', True,
        ),
        'lost_found': (
            'Lost & Found',
            'education_system.systems.university.interfaces.gui.operations.campus.lost_found.lost_found_gui',
            'LostFoundGUI', 'kwarg', True,
        ),
        'campus_navigation': (
            'Campus Navigation',
            'education_system.systems.university.interfaces.gui.operations.campus.campus_navigation.navigation_gui',
            'NavigationGUI', 'none', True,
        ),
        'social_matching': (
            'Social Matching',
            'education_system.systems.university.interfaces.gui.pastoral.social_matching.social_matching_gui',
            'SocialMatchingGUI', 'none', True,
        ),
        'mail_post': (
            'Mail & Post',
            'education_system.systems.university.interfaces.gui.operations.communications.mail.mail_post_gui',
            'MailPostGUI', 'positional', True,
        ),
        'printing_services': (
            'Printing Services',
            'education_system.systems.university.interfaces.gui.operations.campus.printing.printing_gui',
            'PrintingServicesGUI', 'none', True,
        ),
        'study_room_booking': (
            'Study Room Booking',
            'education_system.systems.university.interfaces.gui.operations.campus.study_rooms.study_room_gui',
            'StudyRoomBookingGUI', 'none', True,
        ),
        'student_id': (
            'Student ID Card',
            'education_system.systems.university.interfaces.gui.learners.student_id.student_id_gui',
            'StudentIDGUI', 'none', True,
        ),
        'achievement_badges': (
            'Achievement Badges',
            'education_system.systems.university.interfaces.gui.pastoral.achievement_badges.achievement_badge_gui',
            'AchievementBadgeGUI', 'kwarg', True,
        ),
        'wellness_hub': (
            'Wellness Hub',
            'education_system.systems.university.interfaces.gui.pastoral.wellbeing.wellness.wellness_gui',
            'WellnessGUI', 'kwarg', True,
        ),
        'todo_app': (
            'Todo App',
            'education_system.systems.university.interfaces.gui.shell.tools.todo_app_gui',
            'TodoApp', 'none', False,
        ),
        'my_visa_status': (
            'My Visa Status',
            'education_system.systems.university.interfaces.gui.governance.compliance.international_compliance.my_visa_status_gui',
            'MyVisaStatusGUI', 'none', True,
        ),
        'enrolment_letters': (
            'Status Letters',
            'education_system.systems.university.interfaces.gui.pastoral.student_app.documentation.documentation_gui',
...
def create_student_dashboard(parent_frame, auth, service):
    ...
    try:
        from education_system.systems.university.interfaces.gui.shell.main.dashboard.announcements_widget import (
    ...
    try:
        from education_system.systems.university.interfaces.gui.shell.main.dashboard.student_widgets import (
