"""Tkinter views for Prevent Duty (Nursery System).

Renders into the shared content pane of ``gui_main.NurseryMainGUI`` (the
``host``); the GUI counterpart of ``prevent_duty_cli.py``.
"""
...
from education_system.systems.nursery.domain.safeguarding.prevent_duty import prevent_duty as data
from education_system.systems.nursery.domain.safeguarding.prevent_duty.prevent_duty import ValidationError
...
def open_manager(host) -> None:
    root = _clear(host)
    ttk.Label(root, text="Prevent Duty", font=("", 16, "bold")).pack(anchor="w", pady=(0, 8))
    ...
    cols = ('record_id', 'record_type', 'record_date', 'channel_referral', 'status',)
    tree = ttk.Treeview(root, columns=cols, show="headings", height=18)
    for c, label, w in [
        ("record_id", "ID", 70),
        ("record_type", "Type", 150),
        ("record_date", "Date", 110),
        ("channel_referral", "Channel", 90),
        ("status", "Status", 110),
    ]:
    ...
    host.status_var.set("Prevent Duty loaded")
...
def _refresh(tree, summary) -> None:
    ...
    try:
        ...
        return
    for rec in rows:
        tree.insert("", "end", iid=rec.record_id, values=(_cell(getattr(rec, 'record_id', None)), _cell(getattr(rec, 'record_type', None)), _cell(getattr(rec, 'record_date', None)), _cell(getattr(rec, 'channel_referral', None)), _cell(getattr(rec, 'status', None)),))
...
def _sel(tree, host, verb):
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("Prevent Duty", f"Select a record to {verb}.", parent=host.root)
        return None
    return sel
...
def _form_dialog(host, title, *, initial=None):
    ...
    ttk.Label(frm, text="Record type:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("record_type")
    v = tk.StringVar(value=str(cur or ""))
    ttk.Combobox(frm, textvariable=v, values=[""] + list(data.RECORD_TYPES), state="readonly", width=32).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["record_type"] = v
    row += 1
    ttk.Label(frm, text="Staff (optional):").grid(row=row, column=0, sticky="nw", pady=2)
    ...
    vars_["description"] = v
    row += 1
    ttk.Label(frm, text="Action taken:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("action_taken")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["action_taken"] = v
    row += 1
    v = tk.BooleanVar(value=bool(initial.get("channel_referral")))
    ttk.Checkbutton(frm, text="Channel referral made", variable=v).grid(row=row, column=0, columnspan=2, sticky="w", pady=2)
    vars_["channel_referral"] = v
    row += 1
...
def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    ...
    ttk.Label(frame, text="Prevent Duty", font=("", 14, "bold")).pack(anchor="w", pady=(0, 6))
    ttk.Label(frame, text="Open Prevent Duty from the navigation menu.").pack(anchor="w")
    return frame
