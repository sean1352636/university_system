...
from education_system.systems.nursery.domain.safeguarding.looked_after import looked_after as data
from education_system.systems.nursery.domain.safeguarding.looked_after.looked_after import ValidationError
...
def open_manager(host) -> None:
    root = _clear(host)
    ttk.Label(root, text="Looked-After Children", font=("", 16, "bold")).pack(anchor="w", pady=(0, 8))
    ...
    cols = ('record_id', 'child_name', 'legal_status', 'social_worker', 'status',)
    tree = ttk.Treeview(root, columns=cols, show="headings", height=18)
    for c, label, w in [
        ("record_id", "ID", 70),
        ("child_name", "Child", 170),
        ("legal_status", "Legal status", 160),
        ("social_worker", "Social worker", 150),
        ("status", "Status", 90),
    ]:
    ...
    host.status_var.set("Looked-After Children loaded")
...
def _refresh(tree, summary) -> None:
    ...
    try:
        ...
        return
    for rec in rows:
        tree.insert("", "end", iid=rec.record_id, values=(_cell(getattr(rec, 'record_id', None)), _cell(getattr(rec, 'child_name', None)), _cell(getattr(rec, 'legal_status', None)), _cell(getattr(rec, 'social_worker', None)), _cell(getattr(rec, 'status', None)),))
...
def _sel(tree, host, verb):
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("Looked-After Children", f"Select a record to {verb}.", parent=host.root)
        return None
    return sel
...
def _form_dialog(host, title, *, initial=None):
    ...
    vars_["pupil_id"] = v
    row += 1
    ttk.Label(frm, text="Legal status:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("legal_status")
    v = tk.StringVar(value=str(cur or ""))
    ttk.Combobox(frm, textvariable=v, values=[""] + list(data.LEGAL_STATUSES), state="readonly", width=32).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["legal_status"] = v
    row += 1
    ttk.Label(frm, text="Social worker:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("social_worker")
    ...
    row += 1
    ttk.Label(frm, text="Virtual school head:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("virtual_school_head")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["virtual_school_head"] = v
    row += 1
    ttk.Label(frm, text="Carer name:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("carer_name")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["carer_name"] = v
    row += 1
    ttk.Label(frm, text="PEP review date (YYYY-MM-DD):").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("pep_review_date")
    v = tk.StringVar(value="" if cur is None else str(cur))
    ttk.Entry(frm, textvariable=v, width=34).grid(row=row, column=1, sticky="ew", pady=2)
    vars_["pep_review_date"] = v
    row += 1
    ttk.Label(frm, text="Status:").grid(row=row, column=0, sticky="nw", pady=2)
    cur = initial.get("status")
    v = tk.StringVar(value=str(cur or "current"))
...
def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    ...
    ttk.Label(frame, text="Looked-After Children", font=("", 14, "bold")).pack(anchor="w", pady=(0, 6))
    ttk.Label(frame, text="Open Looked-After Children from the navigation menu.").pack(anchor="w")
    return frame
