"""Tkinter views for Parent Contacts (Nursery System).

Renders into the shared content pane of ``gui_main.NurseryMainGUI`` (the
``host``). Shows the parent / carer list with a tree + toolbar and an add/edit
form dialog — the GUI counterpart of ``parent_contacts_cli.py``.
"""
...
from education_system.systems.nursery.domain.operations.communications.parent_contacts.parent_contacts import (
    RELATIONSHIPS,
    ValidationError,
)
...
_FIELDS: list[tuple[str, str, str]] = [
    ...
    ("is_primary",   "Primary contact",         "bool"),
    ("parental_responsibility", "Parental responsibility", "bool"),
...
def open_manager(host) -> None:
    logger.debug("GUI: parent_contacts open_manager")
    root = _clear(host)
    _header(root, "Parent Contacts")
    ...
    cols = ("id", "child", "name", "rel", "phone", "primary", "pr", "collect")
    tree = ttk.Treeview(root, columns=cols, show="headings", height=16)
    for c, label, w in [
        ("id", "ID", 70), ("child", "Child", 170), ("name", "Name", 170),
        ("rel", "Relationship", 110), ("phone", "Phone", 130),
        ("primary", "Primary", 70), ("pr", "PR", 50), ("collect", "Collect", 70),
    ]:
    ...
    _refresh(tree)
    host.status_var.set("Parent contacts loaded")
...
def _refresh(tree: ttk.Treeview) -> None:
    ...
    try:
        ...
        logger.exception("Could not refresh parent contacts")
        return
    for c in rows:
        tree.insert("", "end", iid=c.contact_id, values=(
            c.contact_id, c.child_name or "-", c.full_name,
            c.relationship or "-", c.phone or "-",
            "Yes" if c.is_primary else "", "Yes" if c.parental_responsibility else "",
            "Yes" if c.can_collect else ""))


def _selected(tree: ttk.Treeview, host, verb: str) -> str | None:
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("Parent contacts", f"Select a contact to {verb}.",
                            parent=host.root)
        return None
    return sel
...
def _delete_selected(tree: ttk.Treeview, host) -> None:
    ...
    try:
        ...
        logger.exception("Failed to delete parent contact %s", sel)
...
def open_add(host) -> None:
    fields = _form_dialog(host, "Add Parent Contact",
    ...
    try:
        ...
        open_manager(host)
        return
    host.status_var.set(f"Added parent contact {c.contact_id}")
    open_manager(host)
...
def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    ...
    ttk.Label(frame, text="Parent Contacts", font=("", 14, "bold")).pack(
        anchor="w", pady=(0, 6))
    ttk.Label(frame, text="Open Parent Contacts from the navigation menu.").pack(
        anchor="w")
    return frame
