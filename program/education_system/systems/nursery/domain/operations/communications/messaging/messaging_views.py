"""Tkinter views for Parent Messaging (Nursery System).

Renders into the shared content pane of ``gui_main.NurseryMainGUI`` (the
``host``). Shows the message log with a tree + toolbar and an add/edit form
dialog — the GUI counterpart of ``messaging_cli.py``.
"""
...
from education_system.systems.nursery.domain.operations.communications.messaging import (
    messaging as data,
)
from education_system.systems.nursery.domain.operations.communications.messaging.messaging import (
    CHANNELS,
    DIRECTIONS,
    STATUSES,
    ValidationError,
...
# (key, label, kind) — kind one of entry/direction/channel/status/staff
_FIELDS: list[tuple[str, str, str]] = [
    ("direction",    "Direction",                "direction"),
    ("channel",      "Channel",                  "channel"),
    ("subject",      "Subject",                  "entry"),
    ("body",         "Body",                     "entry"),
    ("message_date", "Date (YYYY-MM-DD)",         "entry"),
...
def open_manager(host) -> None:
    logger.debug("GUI: messaging open_manager")
    root = _clear(host)
    _header(root, "Parent Messaging")

    bar = ttk.Frame(root)
    bar.pack(fill="x", pady=(0, 8))
    ttk.Button(bar, text="Add Message",
    ...
    cols = ("id", "child", "dir", "channel", "subject", "date", "status")
    tree = ttk.Treeview(root, columns=cols, show="headings", height=18)
    for c, label, w in [
        ("id", "ID", 70), ("child", "Child", 170), ("dir", "Dir", 80),
        ("channel", "Channel", 90), ("subject", "Subject", 200),
        ("date", "Date", 100), ("status", "Status", 80),
    ]:
    ...
    _refresh(tree)
    host.status_var.set("Parent messages loaded")
...
def _refresh(tree: ttk.Treeview) -> None:
    ...
    try:
        ...
        logger.exception("Could not refresh parent messages")
        return
    for m in rows:
        tree.insert("", "end", iid=m.message_id, values=(
            m.message_id, m.child_name or "-", m.direction,
            m.channel or "-", m.subject, m.message_date or "-", m.status))


def _selected(tree: ttk.Treeview, host, verb: str) -> str | None:
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("Parent Messaging", f"Select a message to {verb}.",
                            parent=host.root)
        return None
    return sel
...
def _delete_selected(tree: ttk.Treeview, host) -> None:
    ...
    try:
        ...
        logger.exception("Failed to delete parent message %s", sel)
...
def _form_dialog(host, title: str, *, initial: dict[str, Any] | None = None,
    ...
    for key, label, kind in _FIELDS:
        ...
        cur = initial.get(key)
        if kind == "direction":
            v = tk.StringVar(value=str(cur or "outgoing"))
            ttk.Combobox(frm, textvariable=v, values=list(DIRECTIONS),
                         state="readonly", width=32).grid(
                row=row, column=1, sticky="ew", pady=2)
        elif kind == "channel":
            v = tk.StringVar(value=str(cur or ""))
            ttk.Combobox(frm, textvariable=v, values=[""] + list(CHANNELS),
                         state="readonly", width=32).grid(
                row=row, column=1, sticky="ew", pady=2)
        elif kind == "status":
            v = tk.StringVar(value=str(cur or "sent"))
...
def open_add(host) -> None:
    fields = _form_dialog(host, "Add Parent Message",
                          pupil_choices=_pupil_choices())
    if not fields:
        host.status_var.set("Add message cancelled")
        open_manager(host)
        return
    if not fields.get("pupil_id"):
        messagebox.showerror("Add message", "Please choose a child.",
                             parent=host.root)
        open_manager(host)
        return
    try:
        ...
        messagebox.showerror("Add message", str(e), parent=host.root)
        open_manager(host)
        return
    host.status_var.set(f"Added message {m.message_id}")
    open_manager(host)
...
def open_edit(host, message_id: str) -> None:
    ...
    if m is None:
        messagebox.showerror("Edit message", f"No message with id {message_id}",
                             parent=host.root)
        return
    initial = {key: getattr(m, key) for key, _l, _k in _FIELDS}
    fields = _form_dialog(host, f"Edit message — {m.subject}",
    ...
    try:
        ...
        messagebox.showerror("Edit message", str(e), parent=host.root)
        return
...
def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    ...
    ttk.Label(frame, text="Parent Messaging", font=("", 14, "bold")).pack(
        anchor="w", pady=(0, 6))
    ttk.Label(frame, text="Open Parent Messaging from the navigation menu.").pack(
        anchor="w")
    return frame
