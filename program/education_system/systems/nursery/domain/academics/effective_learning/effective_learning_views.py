...
from education_system.systems.nursery.domain.academics.effective_learning import (
    effective_learning as data,
)
from education_system.systems.nursery.domain.academics.effective_learning.effective_learning import (
    CHARACTERISTICS,
    ValidationError,
)
...
# (key, label, kind) — kind one of entry/characteristic/staff
_FIELDS: list[tuple[str, str, str]] = [
    ("observation_date", "Observation date (YYYY-MM-DD)", "entry"),
...
def open_manager(host) -> None:
    logger.debug("GUI: effective_learning open_manager")
    root = _clear(host)
    _header(root, "Characteristics of Effective Learning")
    ...
    _refresh(tree)
    host.status_var.set("Effective learning loaded")
...
def _refresh(tree: ttk.Treeview) -> None:
    ...
    try:
        ...
        logger.exception("Could not refresh effective-learning records")
        return
    for rec in rows:
        tree.insert("", "end", iid=rec.record_id, values=(
            rec.record_id, rec.child_name or "-", rec.observation_date or "-",
            rec.characteristic or "-", rec.aspect or "-"))


def _selected(tree: ttk.Treeview, host, verb: str) -> str | None:
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("Effective learning",
...
def _delete_selected(tree: ttk.Treeview, host) -> None:
    ...
    try:
        ...
        logger.exception("Failed to delete effective-learning %s", sel)
...
def open_add(host) -> None:
    fields = _form_dialog(host, "Add Effective Learning Record",
    ...
    try:
        ...
        open_manager(host)
        return
    host.status_var.set(f"Added effective learning record {rec.record_id}")
    open_manager(host)
...
def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    ...
    ttk.Label(frame, text="Characteristics of Effective Learning",
              font=("", 14, "bold")).pack(anchor="w", pady=(0, 6))
    ttk.Label(frame,
              text="Open Characteristics of Effective Learning from the navigation menu.").pack(
        anchor="w")
    return frame
