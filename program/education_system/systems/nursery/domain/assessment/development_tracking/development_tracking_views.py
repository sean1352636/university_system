...
from education_system.systems.nursery.domain.assessment.development_tracking import (
    development_tracking as data,
)
from education_system.systems.nursery.domain.assessment.development_tracking.development_tracking import (
    AGE_BANDS,
    AREAS,
    JUDGEMENTS,
    ValidationError,
...
def open_manager(host) -> None:
    logger.debug("GUI: development_tracking open_manager")
    root = _clear(host)
    _header(root, "Development Tracking (Prime & Specific Areas)")
    ...
    cols = ("id", "child", "area", "band", "judgement", "date")
    tree = ttk.Treeview(root, columns=cols, show="headings", height=16)
    for c, label, w in [
        ("id", "ID", 70), ("child", "Child", 170), ("area", "Area", 240),
        ("band", "Age Band", 150), ("judgement", "Judgement", 90),
        ("date", "Date", 100),
    ]:
    ...
    _refresh(tree)
    host.status_var.set("Development tracking loaded")
...
def _refresh(tree: ttk.Treeview) -> None:
    ...
    try:
        ...
        logger.exception("Could not refresh development tracking records")
        return
    for r in rows:
        tree.insert("", "end", iid=r.record_id, values=(
            r.record_id, r.child_name or "-", r.area or "-",
            r.age_band or "-", r.judgement or "-",
...
def _selected(tree: ttk.Treeview, host, verb: str) -> str | None:
    sel = tree.focus()
    if not sel:
        messagebox.showinfo("Development Tracking",
...
def _delete_selected(tree: ttk.Treeview, host) -> None:
    ...
    if not sel:
        return
    if not messagebox.askyesno("Delete record", f"Delete record {sel}?",
                               parent=host.root):
        return
    try:
        data.delete_record(sel)
    except Exception as e:  # noqa: BLE001
        logger.exception("Failed to delete development tracking %s", sel)
...
def _form_dialog(host, title: str, *, initial: dict[str, Any] | None = None,
    ...
    for key, label, kind in _FIELDS:
        ...
        if kind == "area":
            ...
            v = tk.StringVar(value=str(cur or ""))
            ttk.Combobox(frm, textvariable=v, values=[""] + list(AGE_BANDS),
                         state="readonly", width=32).grid(
                row=row, column=1, sticky="ew", pady=2)
        elif kind == "judgement":
            v = tk.StringVar(value=str(cur or ""))
            ttk.Combobox(frm, textvariable=v, values=[""] + list(JUDGEMENTS),
                         state="readonly", width=32).grid(
                row=row, column=1, sticky="ew", pady=2)
        elif kind == "staff":
...
def open_add(host) -> None:
    fields = _form_dialog(host, "Add Development Tracking Record",
                          pupil_choices=_pupil_choices())
    if not fields:
        host.status_var.set("Add record cancelled")
        open_manager(host)
        return
    ...
    try:
        ...
        open_manager(host)
        return
    host.status_var.set(f"Added development tracking record {rec.record_id}")
    open_manager(host)
...
def open_edit(host, record_id: str) -> None:
    ...
    if rec is None:
        messagebox.showerror("Edit record", f"No record with id {record_id}",
                             parent=host.root)
        return
    initial = {key: getattr(rec, key) for key, _l, _k in _FIELDS}
    fields = _form_dialog(host, f"Edit record — {rec.child_name}",
    ...
    try:
        ...
        return
    host.status_var.set(f"Updated record {record_id}")
    open_manager(host)


def build(parent: tk.Misc, auth=None) -> ttk.Frame:
    frame = ttk.Frame(parent, padding=16)
    ttk.Label(frame, text="Development Tracking (Prime & Specific Areas)",
              font=("", 14, "bold")).pack(anchor="w", pady=(0, 6))
    ttk.Label(frame, text="Open Development Tracking from the navigation menu.").pack(
        anchor="w")
    return frame
