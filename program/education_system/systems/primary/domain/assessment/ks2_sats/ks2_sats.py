"""KS2 SATs data layer (Year 6).

One row per (pupil, academic_year, subject). End-of-KS2 statutory
tests are Reading, Maths and English Grammar, Punctuation & Spelling
(EGPS); Science is teacher-assessed. We accept all four as subjects.
Each row carries an optional scaled score (DfE range 80-120) and an
outcome marker:
    ...
    EXS — expected standard (scaled score >= 100)
    GDS — greater depth / working at greater depth (~110+)
"""
...
SUBJECTS: tuple[str, ...] = ("Reading", "Maths", "EGPS", "Science")
...
EXPECTED_SCALED = 100
GREATER_DEPTH_SCALED = 110

_ACADEMIC_YEAR_RE = re.compile(r"^\d{4}(-\d{2}|-\d{4})?$")

_SCHEMA = """
CREATE TABLE IF NOT EXISTS ks2_sats_results (
...
CREATE INDEX IF NOT EXISTS idx_ks2_pupil    ON ks2_sats_results(pupil_id);
CREATE INDEX IF NOT EXISTS idx_ks2_year     ON ks2_sats_results(academic_year);
CREATE INDEX IF NOT EXISTS idx_ks2_subject  ON ks2_sats_results(subject);
"""
...
def init_db() -> None:
    ...
    try:
        ...
        logger.exception("Failed to initialise ks2_sats_results table")
        raise
    logger.info("Primary ks2_sats_results table ready")
    _DB_READY = True


def _row(r: sqlite3.Row) -> KS2Result:
    return KS2Result(
...
def suggest_outcome(scaled_score: int | None) -> str | None:
    if scaled_score is None:
        return None
    if scaled_score < EXPECTED_SCALED:
        return "WTS"
    if scaled_score < GREATER_DEPTH_SCALED:
        return "EXS"
    return "GDS"
...
def create(data: dict[str, Any]) -> KS2Result:
    init_db()
    ...
    try:
        with _connect() as conn:
            dup = conn.execute(
                "SELECT result_id FROM ks2_sats_results "
            ...
            if dup:
                raise ValidationError(
                    f"A KS2 result for pupil {payload['pupil_id']} in "
                    f"{payload['academic_year']} {payload['subject']} "
                    f"already exists (#{dup['result_id']})")
            cur = conn.execute(
                """INSERT INTO ks2_sats_results
        ...
        logger.exception("Failed to create KS2 SATs result")
        raise
    rec = get(new_id)
    assert rec is not None
    logger.info("Created KS2 result #%d (pupil %s, %s %s, %s, score=%s)",
                rec.result_id, rec.pupil_id, rec.academic_year,
                rec.subject, rec.outcome, rec.scaled_score)
    return rec


def get(result_id: int) -> KS2Result | None:
    init_db()
    try:
        with _connect() as conn:
            r = conn.execute(
                "SELECT * FROM ks2_sats_results WHERE result_id = ?",
                (result_id,),
            ).fetchone()
    except sqlite3.Error:
        logger.exception("get KS2(%s) failed", result_id)
        raise
    return _row(r) if r else None


def update(result_id: int, data: dict[str, Any]) -> KS2Result:
    init_db()
    existing = get(result_id)
    if existing is None:
        raise ValidationError(f"No KS2 SATs result #{result_id}")
    merged = {
    ...
    try:
        with _connect() as conn:
            ...
            if dup:
                raise ValidationError(
                    f"Another KS2 result for that pupil / year / subject "
                    f"already exists (#{dup['result_id']})")
            conn.execute(
                """UPDATE ks2_sats_results SET
        ...
        logger.exception("Failed to update KS2 #%d", result_id)
        raise
    rec = get(result_id)
    assert rec is not None
    logger.info("Updated KS2 result #%d", result_id)
    return rec
...
def delete(result_id: int) -> bool:
    ...
    try:
        with _connect() as conn:
            cur = conn.execute(
                "DELETE FROM ks2_sats_results WHERE result_id = ?",
                (result_id,))
            conn.commit()
            deleted = cur.rowcount > 0
    except sqlite3.Error:
        logger.exception("Failed to delete KS2 #%d", result_id)
        raise
    if deleted:
        logger.info("Deleted KS2 result #%d", result_id)
    return deleted
...
def list_results(
    *, academic_year: str | None = None,
...
) -> list[tuple[KS2Result, Pupil | None]]:
    init_db()
    ...
    sql = "SELECT * FROM ks2_sats_results"
    if where:
    ...
    try:
        ...
        raise
    out: list[tuple[KS2Result, Pupil | None]] = []
    for r in rows:
        rec = _row(r)
...
def list_for_pupil(pupil_id: str) -> list[KS2Result]:
    init_db()
    ...
    try:
        with _connect() as conn:
            rows = conn.execute(
                "SELECT * FROM ks2_sats_results WHERE pupil_id = ? "
...
def cohort_combined_summary(academic_year: str) -> dict[str, Any]:
    """How many pupils met expected standard in *all four* statutory
    measures (Reading, Maths, EGPS) for a given year. A common DfE
    headline figure.
    """
    init_db()
    ...
    core = ("Reading", "Maths", "EGPS")
    try:
        with _connect() as conn:
            rows = conn.execute(
                "SELECT pupil_id, subject, outcome FROM ks2_sats_results "
                "WHERE academic_year = ?", (ay,),
            ).fetchall()
    except sqlite3.Error:
        logger.exception("cohort_combined_summary(%s) failed", ay)
        raise
    by_pupil: dict[str, dict[str, str]] = {}
    for r in rows:
        by_pupil.setdefault(r["pupil_id"], {})[r["subject"]] = r["outcome"]
    total_pupils = len(by_pupil)
    all_three_exs = 0
    for pid, subs in by_pupil.items():
        if all(subs.get(s) in ("EXS", "GDS") for s in core):
            all_three_exs += 1
    return {
        "academic_year": ay,
        "pupils_recorded": total_pupils,
        "exs_in_RWM": all_three_exs,
        "exs_in_RWM_pct": (all_three_exs / total_pupils * 100.0)
                          if total_pupils else 0.0,
    }
...
def known_years() -> list[str]:
    ...
    try:
        with _connect() as conn:
            rows = conn.execute(
                "SELECT DISTINCT academic_year FROM ks2_sats_results "
