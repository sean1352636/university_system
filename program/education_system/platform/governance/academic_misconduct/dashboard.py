...
from education_system.platform.governance.academic_misconduct._imports import tk, ttk, _t, sqlite3, DEFAULT_DB_PATH
...
class MisconductDashboardMixin:
    ...
    def create_dashboard_content(self, parent):
        ...
        # Dashboard title with system name
        system_key = getattr(self, 'system_key', None)
        if system_key:
            system_name = SYSTEM_DISPLAY_NAMES.get(system_key, system_key.title())
            title_text = f"{system_name} - Misconduct Dashboard"
        else:
            title_text = _t("misconduct.sections.dashboard_overview")
