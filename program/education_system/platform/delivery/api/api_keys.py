...
Usage:
    # In route handlers:
    from education_system.platform.delivery.api.api_keys import api_key_required
    ...
    # Admin: create a key
    from education_system.platform.delivery.api.api_keys import APIKeyManager
        ...
        try:
            ...
            if row["expires_at"]:
                if datetime.fromisoformat(row["expires_at"]) < datetime.utcnow():
                    logger.warning("Expired auth token used: %s", row["key_prefix"][:8])  # nosemgrep: python-logger-credential-disclosure
                    return None
            # Update last_used_at
            conn.execute(
