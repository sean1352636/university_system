...
- [8.78.0 — 2026-04-19](#8780---2026-04-19)
...
### Auth recovery overhaul; exam scheduler ↔ rest-of-university integration sweep

#### Fixed

- **Temp passwords issued by forgot-password were rejected on the next change-password attempt.** `shared/auth/forgot_password.verify_answers_and_reset` stored a fresh bcrypt hash in `users.password_hash` but left `users.legacy_salt` populated for migrated PBKDF2 users; `verify_password` then ran PBKDF2 with the old salt against the new bcrypt hash and always returned False. Reset now nulls `legacy_salt` alongside `password_changed_at`. (`shared/auth/forgot_password.py`)
- **`UserAuth.change_password` wrapper masked real errors** by catching every exception from shared auth and silently falling back to the legacy `user_accounts` table — which then re-verified against an outdated hash and surfaced a misleading "Current password is incorrect." Wrapper now propagates `AuthError` from shared auth when the user authenticated via shared auth, only falling back to legacy when no `shared_auth_id` is set. The university change-password GUI was updated to display the propagated error string instead of a generic message. (`university_system/infrastructure/auth/core.py`, `university_system/modules/shared/gui/main/auth_gui.py`)
- **Stray `_tkinter.TclError: invalid command name "...!button2"`** from the temp-password screen. The "Copy" / 30s auto-hide / 60s clipboard-clear callbacks scheduled with `self.after(...)` fired after the user navigated past the screen and the widgets had been destroyed. Replaced lambdas with named functions guarded by `winfo_exists()` and `try/except tk.TclError`. (`shared/gui/login_gui.py`)

#### Added

- **MFA-aware temp-password delivery.** When the user has MFA enabled and an email on file, `forgot_password` now sends the temp password to that email via `EmailService` instead of returning it for on-screen display; the result dict carries `delivered_via_email=True` and `temp_password=None`. The login GUI renders a new "Check Your Email" screen with a partially-masked recipient (`s****9@gmail.com`). Falls back to the existing on-screen flow when MFA is off, no email is on file, or SMTP is not configured. (`shared/auth/forgot_password.py`, `shared/gui/login_gui.py`)
- **MFA recovery codes now work as a login bypass.** Typing an unused recovery code into the login password field is accepted in place of the password. Single-use (consumed on success), bypasses the MFA challenge (the code itself is the second factor), and forces a password change on that login regardless of the `force_password_reset` admin setting so the user can set a password they actually know. Rate-limit lockouts now surface as `AuthError` instead of being swallowed. Bad-input fast path: only inputs in the 4–20 char range are tested against recovery codes, so this doesn't slow ordinary failed logins. (`shared/auth/core.py`)
- **Forced-change screen prefills the temp password.** `_do_login` stashes the password just used at login on `self._last_login_password`; `_show_change_password` consumes it as the default value for the "Current Password" field (clearing the cache after one use), and focus jumps straight to the New Password entry. (`shared/gui/login_gui.py`)
- **Post-change confirmation email + email-module hand-off.** After a successful mandatory password change, the login GUI sends a "your password was changed" email and tags the session with `_open_email_after_login=True`. The university `UnifiedManagementGUI.run()` consumes the flag and schedules `show_email_manager` 300 ms after the dashboard appears, so the user lands directly in the in-app email module. (`shared/gui/login_gui.py`, `university_system/modules/shared/gui/main/main_gui.py`)
- **Exam scheduler now syncs to the academic calendar end-to-end.** `add_exam_to_calendar` uses a deterministic `EXAM-{exam.id}` event id and upserts (UPDATE if the row exists, INSERT otherwise), so re-saving the same exam refreshes the calendar entry instead of creating duplicates. New `remove_exam_from_calendar` helper. `update_exam` and `delete_exam` now keep the calendar row in sync (previously only `add_exam` did). (`university_system/modules/domain/academics/gui/exam_management/notifications.py`, `data_manager.py`, `tabs/exams_tab.py`)
- **Scheduling events fan out to the in-app notifications service.** `send_exam_notifications` (scheduled), `send_exam_update_notifications` (updated), and the new `send_exam_cancellation_notifications` all push per-user notifications via `NotificationsService.create_notification` for each enrolled student plus the instructor, alongside the existing email path. Channel `academic`, `source_system="exam_management"`, `source_id=exam.id`. Cancellation uses inline subject/body so no extra template is required. The exams-tab `delete_exam` snapshots the `Exam` before removal and fires cancellation notifications. (`notifications.py`, `data_manager.py`, `tabs/exams_tab.py`)
- **Roster auto-derived at save time.** `add_exam` and `update_exam` re-call `data_manager.get_enrolled_students(module_code)` instead of trusting `self.enrolled_student_ids`, so the snapshot stored on the exam matches current enrolment in `student_modules` (covers students who enrolled after the form was opened). New "🔄 Refresh roster from enrolment" button on the form re-pulls the listbox without changing the selected module. (`tabs/exams_tab.py`)
- **Shared room reservations.** New `room_booking.py` shim delegates to the canonical `RoomBookingService` (`campus.room_booking`) instead of writing raw SQL into `room_bookings`. Maps exam→booking via `[exam:<id>]` marker in the booking purpose so add/update/delete address the same row without a schema change. `exams_tab` cross-checks shared `room_bookings` + recurring `module_schedule` lectures and shows a confirmation dialog listing the overlapping bookings before scheduling. Calls `reserve_room_for_exam` on add/update and `release_room_for_exam` on delete so other systems see exam slots as taken. (`room_booking.py`, `data_manager.py`, `tabs/exams_tab.py`)
- **Exam results writeback.** New `results.py` service: `apply_exam_results(exam, results, graded_by, pass_mark)` upserts `student_grades` (matched by `student_id + module_code + assessment_name`, `assessment_type='exam'`, `is_final=1`, computed letter grade), updates `student_modules.grade` + `completion_date` + `status` (`Completed` / `Failed` against `pass_mark`, default 40), and appends to `grade_audit_log` with old → new percentage. Idempotent on re-save. New "📝 Results" tab + `ResultsEntryDialog`: lists exams with graded progress (`3/15`, colour-coded green/orange/grey), double-click opens a per-student marks-entry dialog (pre-filled from existing grades), Save calls the service. Records `graded_by` from the logged-in user when available. (`results.py`, `tabs/results_tab.py`, `tabs/__init__.py`, `app.py`)
- **Deferred-exam approvals honour student-support accommodations.** New `accommodations.py` service merges general `student_accommodations` (with `time_multiplier`, expiry-aware) and per-exam `exam_accommodations` (`extended_time` minutes, `separate_room`, `reader_scribe`, `assistive_technology`) into one view. `compute_extended_end_time` applies flat minutes first, then multiplier on the original duration. `clone_exam_accommodation` idempotently carries the per-exam row onto a new resit `exam_id`. `deferred_tab._autocreate_resit` now looks up active accommodations once, uses the extended end time for conflict checks and the inserted resit row, clones the per-exam arrangement onto the new exam_id, and lists the applied accommodations in the confirmation dialog. (`accommodations.py`, `tabs/deferred_tab.py`)
- **External-examiner attachments per exam.** New `external_examiners_link.py` bridge: auto-creates `exam_external_examiners(exam_id, examiner_id, role)` with a UNIQUE pair so attach is idempotent. Schema-aware — detects whether `external_examiners` uses `id` or `examiner_id` as PK at runtime, and which of `specialisation` / `expertise_area` the deployment ships, with JOINs adapting accordingly. `data_manager` exposes `list_active_examiners`, `attach_external_examiner`, `detach_external_examiner`, `list_examiners_for_exam`. New "External examiners…" button on the Exams tab opens an `ExternalExaminersDialog` (Toplevel) showing currently-attached examiners with Name / Institution / Role columns, plus a Combobox of active examiners + role text + Attach. (`external_examiners_link.py`, `data_manager.py`, `tabs/exams_tab.py`)
- **Advanced-search student detail dialog renders enrolled modules as a `ttk.Treeview` table** instead of plain text. 5 columns (Type / Code / Module / Grade / Enrolled), zebra striping, scrollbar at >12 modules, "In Progress" rows in soft grey, module type `.title()`-cased (`Compulsory` instead of `compulsory`). Failure to load shows a red error label inside the same frame instead of error text mixed with module data. (`university_system/modules/shared/gui/advanced_search/results.py`)

#### Changed

- **Login flow now stops swallowing AuthErrors silently.** Where the previous wrapper would mask password / strength / history errors with a misleading legacy fallback, errors from shared auth are propagated when the user is identifiably a shared-auth user (`shared_auth_id` set on `current_user`). Reduces "Current password is incorrect" red herrings.
- **Forgot-password reset now also clears `failed_login_attempts` and `locked_until`** in the same UPDATE that sets the new hash, so a user mid-lockout can use the temp password without waiting out the lockout window.
...
### Printing payment flow, Certificates / Transcript merge, Portfolio FK migration, role-dispatch fix

#### Added

- **Printing — "Buy Credits" payment dialog with Cash / Card / Student Finance Account.** Replaced the single-click confirm with a Toplevel that asks how to pay. Cash and Card paths record the payment to the unified `payments` table via `record_payment_to_finance(...)`; Card additionally validates number (12-19 digits), expiry (MM/YY), CVV (3-4 digits) and cardholder name. Student Finance Account debits `student_finance_accounts.balance` via `process_student_finance_account_payment(...)`, refusing if funds are insufficient and showing the live balance when that method is selected. All three paths then top up `print_quotas.total_pages`, log a `credit` row in `print_credit_transactions`, and email a receipt rendered from the new JSON template `templates/email/finance/print_credits_receipt.json` (vars: first/last name, pages, price, payment_method, transaction_id, date, new_total, balance_line).
- **Certificates GUI — role-aware layout.** When `auth.current_user` is a student, the window shows only **My Certificate** (their own certs, Refresh + Export HTML) and **My Transcript** (cross-system transcript locked to their name). Staff / admin / unauthenticated callers keep the full 4-tab management surface. Role detection reads `current_user['role']` first, falling back to `current_user['systems'][*].role` and only treating the user as a student when *every* role they hold is `student` — a mixed staff+student account gets the admin GUI.
- **`DigitalTranscriptFrame(restrict_to_student=…)` mode** — hides the search bar + results tree and auto-generates the transcript for a fixed student name on construction, leaving only Save / Print (Copy) / Summary View controls. Used by the student Certificates tab.
- **Portfolio — Request Endorsement dialog** (replaced the "dialog would open here" placeholder). Skill picker over the student's own skills, endorser username, role (Faculty / Peer / Employer / Mentor), optional message. Calls `service.request_endorsement(...)` and refreshes the list; best-effort emails the endorser if their identifier looks like an address.

#### Changed

- **Digital Transcript GUI merged into Certificates GUI.** `shared/transcript/transcript_gui.py` duplicated the Transcripts tab already inside `CertificatesGUI` (differed only in cross-system vs institution-only search). Merge direction: `CertificatesGUI._build_transcripts_tab` now embeds `DigitalTranscriptFrame`; the institution-only `_generate_transcript` / `_export_transcript_html` / `_export_transcript_csv` helpers and `_get_transcript_svc` lazy accessor were removed; `DigitalTranscriptFrame` was relocated to `shared/certificates/digital_transcript_frame.py` (still backed by `shared.transcript.transcript_service`); the old `transcript_gui.py` was deleted; and all four systems' GUI imports (primary / secondary / college / university) were repointed. The separate university "Digital Transcript" sidebar button, `show_digital_transcript_gui` function + binding in `main_gui.py`, and the `digital_transcript` entry in `core/gui_setup.py` were removed — a single "Certificates" entry opens the combined surface.
- **Finance launcher from the student dashboard** — `_launch_finance` in `dashboard/student_dashboard.py:65` created a bare `tk.Toplevel(parent)` with no geometry, so the window rendered at Tk's default ~200px size. Now sets `title="Finance"`, `geometry("1280x800")`, `minsize(1024, 640)` before constructing `FinanceGUI`.
- **Student Records — Academic Records tab redesigned.** The Courier-monospaced `ScrolledText` blob is gone. Three `LabelFrame` + `Treeview` panels now render **Enrolled Modules** (grouped by module_type with shaded subheader rows), **Grades & Assessments** (colour-coded: ≥70 green / 40-69 amber / <40 red), and **Recent Attendance** (Present green / Late amber / Absent red). A summary line at the top reads "*X module(s) · Y grade(s) · Z recent attendance entry(ies)*". The whole panel stack is wrapped in a scrollable `Canvas` with mouse-wheel binding so the bottom panel is reachable on a 700px-tall detail window.
- **Student Records — Personal Information tab redesigned.** Hero header (full name + `ID · Course · Email` sub-line), horizontal separator, and three `LabelFrame` cards (Personal Info / Demographics / Academic Info) in a scrollable Canvas, each a two-column label/value grid with bold keys. Replaces the Courier `ScrolledText` block.
- **Student sidebar trimmed** — removed the "Cross-System Calendar" button (and its empty "Communication" heading) from `student_portal.py`. Feature remains available to instructors and admins via their portals.

#### Fixed

- **Superadmin role picker always rendered the admin dashboard.** `run_university_gui` (`launcher/systems.py:94-98`) accepted the `role` argument from the role-picker dialog but immediately overwrote it with the user's stored role for the university system — which is always `admin` for superadmin. Every Student / Staff / Teacher / Parent pick collapsed back to admin. Fix: only resolve from `user_info["systems"]` when no explicit `role` was passed; honour the picker's choice otherwise. College / secondary / primary launchers already did this correctly.
- **Cross-system transcript reported 0 / 0% attendance even when records existed.** `attendance_records.status` is stored TitleCase (`'Present'`, `'Late'`) by the Manual GUI Entry path, but `_collect_secondary` / `_collect_college` / `_collect_university` filtered `status IN ('present','late')` — SQLite's `IN` is case-sensitive, so the present count was always 0 even when `total_sessions` was non-zero. Switched all four collectors to `LOWER(status) IN ('present','late')`. Verified: Demo Student `S12345` now reports 1 / 1 = 100%, matching the admin attendance view.
- **Portfolio creation failed with `FOREIGN KEY constraint failed`** for any non-student account. Legacy `public_profiles`, `badges`, and `user_resumes` tables still carried `FOREIGN KEY (student_id) REFERENCES students(student_id)` from an older schema. `_remove_foreign_key_constraints` detected this but the loop body was `pass`. Now rebuilds each table into an FK-free `*_new` replica using a column-aware `INSERT ... SELECT` that tolerates schema drift, drops the old table, and renames. Also added `user_resumes` to `validate_table_name`'s allowlist in `core/sql_safety.py`. Ran the migration once against the live `student_records.db` — all three tables verified FK-free.
- **Portfolio — "Generate Resume" always rejected with "Student not found".** `generate_resume` looked the user up in the `students` table only; any admin / staff / non-student account in the `users` table failed immediately. Now falls back to `users` (by `username` or `student_id`), then to a minimal synthesised info block, so generation never fails purely because of which table the account lives in.
- **Portfolio — Settings tab NULL-field crash.** `load_settings` fed `self.portfolio.get('linkedin_url', '')` directly into `ttk.Entry.insert(0, …)`. `dict.get(key, default)` returns the stored value even if it's `None` (SQLite NULL → Python None), and Tk rejects `None`: *"wrong # args: should be … insert index text"*. Swapped `get(k, '')` → `get(k) or ''` for all six text fields (title, headline, bio, linkedin_url, github_url, personal_website). Also wrapped the Settings tab contents in a scrollable `Canvas` so the long form is reachable on short windows.
- **Finance GUI — empty amount raised `ValueError` trace noise.** `process_withdrawal`, `process_top_up`, and `process_use` called `float(amount_entry.get())` directly; clicking with the field empty raised a `ValueError` that was caught but logged before the catch ran. Now strip + guard empty input up-front and show the `enter_valid_amount` dialog without the traceback.
- **Audit Log Viewer — `NameError: AUDIT_LOG_VIEWER_AVAILABLE`.** `config_gui.show_audit_log_viewer` referenced the availability flag and the opener helper, but `config_gui.py` imported only `SECURITY_DASHBOARD_AVAILABLE` / `ACTIVITY_LOGGER_GUI_AVAILABLE` from `gui_imports`. Added the missing imports; aliased the opener as `_open_audit_log_viewer` to break a name collision with the wrapper method (which would have infinitely recursed after the flag check).
- **Academic Records — leaking `({module_type})` placeholder.** The i18n key `student_details.modules_type_suffix` in `locales/en/system/students.json` was the literal `" ({module_type})"` but the code never substituted `module_type`, so the rendered text read *"DS ({module_type})"*. Replaced by the new structured Treeview layout — the dead i18n key no longer has any Python callers.
...
### Assignment submission pipeline, unified grading, AI Auto-Grading portal, Grade Tracking rewire, student feedback viewer, plus a pile of follow-on fixes

#### Added

- **AI Auto-Grading staff portal** — new `modules/domain/academics/gui/assignment_system/auto_grading_portal.py` (`AutoGradingStaffPortal`) plus a pure-logic scorer at `modules/domain/academics/services/ai_grading/`. The scorer weights five criteria (Length 20, Rubric keyword coverage 30, Structure 20, Readability 15, Originality 15) and returns an `AutoGradingResult` with per-criterion rationale and a low/medium/high confidence tag. `grade_submission_by_id(submission_id, grader_user_id)` is the DB-backed wrapper — resolves the stored file path (healing legacy paths), extracts text, seeds the plagiarism repo with other final submissions for the assignment, runs a real similarity scan, and produces a scored result with `.error` instead of raising on unsupported formats. No external API, no new dependencies — keyword extraction uses a compact built-in English stopword list. The portal lists every ungraded final submission for a selected assignment; double-click runs the grader in a daemon thread and hands the result to the shared `GradeSubmissionDialog` pre-filled with score + rationale. Accepted grades route through the same `save_submission_grade` + `send_grade_release_emails_async` as every other grading path, so released auto-grades are indistinguishable from manual ones downstream. The "AI Auto-Grading" sidebar button in `staff_portal.py` and `instructor_portal.py` is rebound to a new `_open_auto_grading_portal` method rather than going through the legacy `UnifiedManagementGUI.show_auto_grading` shim.
- **Plagiarism + email on student submit** — after a successful INSERT into `assignment_submissions`, a daemon thread in `AssignmentStudentPortal._submit_file` (a) queues the submission-confirmation email for the student, (b) extracts text, ingests every other final submission for the assignment into the plagiarism repository, runs `PlagiarismChecker.check_plagiarism` against the populated repo, and (c) sends the scan result to the student. The resulting similarity score reflects a real comparison instead of a vacuous "no match" against an empty repo. Native text extraction supports `.txt / .md / .rst / .log / .csv / .tsv / .json / .xml / .html / .yml / .yaml` plus common source-code extensions; other formats degrade to a "skipped" email if `textract` isn't installed.
- **Duplicate-file protection on submit** — SHA-256 is computed before the file is copied, and `assignment_submissions` is queried for any prior submission by this student (any assignment) with the same `file_hash`. If found, a popup names the previous submission's filename, timestamp, and which assignment it was submitted to, and blocks the upload. The `file_hash` column was already in the schema but had never been populated; it's now stored alongside every new submission.
- **New-assignment notifications for enrolled students** — `AssignmentEditorDialog._save` now spawns a daemon thread on a successful INSERT (not UPDATE) that looks up every student in `student_modules` with lowercase-tolerant status matching, picks the best available email via `COALESCE(users.email, students.email_address)`, and queues the new template for each. New JSON template `academics/assignment_created_student.json` carries the subject / `body_text` / `body_html` for that email. Editing an existing assignment does not re-notify.
- **In-app submission file viewer** — new `assignment_system/_file_viewer.py` shared by both portals. `preview_file(parent, path)` renders plain-text formats inline in a scrollable `Toplevel` with word-wrap, and offers "Open Externally" for anything else; the non-plain path first tries the `textract`-backed extractor from the plagiarism checker. Staff portal got a "View File" button alongside "Grade / Feedback…"; student portal got "View My File" next to "Submit / Resubmit…", pre-loading the current assignment's final-submission file path via `_on_selected`. `resolve_submission_path` heals legacy stored paths in three passes: use-as-is, re-root any `submissions/submitted/...` suffix onto the current `SUBMISSIONS_DIR`, and finally look inside the submission directory for a file whose name shares the same timestamp prefix (`v1_YYYYMMDD_HHMMSS_` or `YYYYMMDD_HHMMSS_`) — so pre-reorganisation rows and pre-rename rows both become previewable.
- **Student Grade Tracking feedback viewer** — `AssessmentDetailsDialog` opens on double-click (or Enter) from the assessments table. Shows title / type / due date, a prominent grade line (colour-coded: green when graded, amber for "Submitted — not yet graded", muted for "Not submitted"), populated-only meta rows (submitted timestamp, file, late flag + days, graded timestamp), a scrollable read-only feedback pane with a helpful placeholder when no feedback has been left, and a "View My Submission" button that delegates to the file-viewer so the same plain-text preview + legacy-path healing applies here.
- **Unified grading module** — new `assignment_system/_grading.py` is the single source of truth across portals. It exports `GradeSubmissionDialog` (portal-agnostic modal, captures `(score, feedback)` via `on_save`, never touches the DB itself), `save_submission_grade(submission_id, score, feedback, grader_user_id)` (the UPDATE, returns bool on success/failure), `send_grade_release_emails_async(...)` (daemon-thread wrapper for the email pipeline), and the sync `_send_grade_release_emails` worker (moved in from `assignment_system/staff_portal.py`). `assignment_system/staff_portal.py` and `grade_tracking/staff_portal.py` both call into it — the batch vs. immediate flow differs between portals, but the DB write and the grade-released + staff-confirmation emails are identical.
- **Email templates (new)** — `academics/plagiarism_report_student.json` (per-criterion scan result), `academics/grade_release_confirmation_staff.json` (grader confirmation after a student grade is released), `academics/assignment_created_student.json` (new-assignment notification to enrolled students). All three use `$var` placeholders matching the `string.Template.safe_substitute` renderer used by the email service, and carry both `body_text` and `body_html`.

#### Changed

- **Grade Tracking portals rewired onto `assignments` + `assignment_submissions`** — both the student and staff portals were reading `module_grades` / `assessments` / `grades`, all of which are empty in the live DB. The real data lives in `assignments` (created via the assignment portal) and `assignment_submissions.grade` (written by the grading dialog). Student side now computes each module's percentage as `sum(submission.grade) / sum(assignment.max_marks)` across the student's final submissions; letter and credit-weighted GPA derive from that. Status column surfaces e.g. *"Enrolled (1/3 graded)"*. Assessments pane joins assignments + submissions and shows per-assignment grade or a submission-status label. Staff side's assessment dropdown now lists assignments for the selected module ordered by due date; the grade table joins `students` + `student_modules` + `assignment_submissions` keyed on `submission_id`; Save writes directly to `assignment_submissions` (same columns the assignment portal grading dialog writes). On save, the same grade-release emails fire as via the assignment portal.
- **Grade Tracking staff portal scoped to the logged-in user's own assignments** — the assignment portal already filtered `WHERE created_by = <my user id>`, but the grade tracking portal loaded every module + every active assignment in the system. Now resolves the numeric `users.id` the same way `_resolve_user_id` does elsewhere, filters the module dropdown to modules where this staff user has at least one active assignment, and applies `created_by = ?` to the assignment dropdown query. Empty state surfaces a "create one in the Assignment portal first" hint when a user has no assignments.
- **Auto-grader text extraction covers more formats** — `_extract_submission_text` used to delegate straight to `PlagiarismChecker.extract_text_from_file`, which only handles `.txt` natively. `.md` submissions (and `.csv`, `.py`, `.json`, etc.) raised "Unsupported file type" even though `open()` would read them just fine. The helper now recognises the same plain-text extensions the file viewer supports and reads them with a utf-8 → utf-8-sig → latin-1 → cp1252 encoding fallback; `_run_plagiarism_for_submission` and its repo-seeding loop use the same helper so the plagiarism side of auto-grading also works on text-ish formats.
- **GPA falls back to equal weight when `modules.credits` isn't populated** — every row in `modules.credits` is 0 in the live DB, so the credit-weighted GPA collapsed to 0.00 for every student. Each graded module now contributes `max(credits, 1)` to the weighted sum, so a zero/NULL credits column degrades to an unweighted mean rather than erasing the result. The summary line's qualifier reflects which mode was used: either `credit-weighted` or `equal weight — credits not set`.
- **"Return to login" flow moved out of the button callback** — all four GUI portals (student / staff / instructor / parent) previously did `self.root.destroy()` + open `UniversalLoginWindow` + call `run_university_gui` (which creates another `Tk()`) inline from the button callback. Triple-nested mainloops with destroy-in-flight would hang the relaunched portal. `_return_to_login` now just sets `self._relaunch_after_logout = True` and destroys the root; `run()` checks the flag after `mainloop()` returns and drives the re-login from the top of the stack, matching the pattern the CLI portals have always used.
- **Localization module collapsed** (carried over from 8.80.0 design) — assignment emails that were inline Python strings (plagiarism result to student, grade-release confirmation to staff) are now JSON templates. The existing `academics/assignment_submission_student.json` was rewritten from `{var}` placeholders to `$var` because `string.Template.safe_substitute` doesn't recognise the braces — emails had been going out with literal `{first_name}` text.

#### Fixed

- **Crash when grading a student who had submitted but not been graded** — the grade tracking staff portal's `EditGradeDialog` seeded its score input with whatever was in the row's Score column. Rows with status `"Submitted"` leaked that string into `float(…)` and crashed the save. The seed is now only pre-filled when the displayed value parses as a float; everything else yields an empty input.
- **"Return to login" raised `ImportError`** — eight portal files (four GUI, four CLI) had `from run import run_university_{gui,cli}` which no longer resolves; those functions live in `education_system.launcher.systems`. Updated every call site so the logout-and-reauth flow doesn't silently fail.
- **Staff / instructor dashboards hit an empty stray DB** — both dashboards built a DB path with a hand-rolled `dirname` chain that resolved to `education_system/data/db_files/student_records.db` instead of the real `education_system/university_system/data/db_files/student_records.db`. SQLite auto-created the empty target on first connect, so every subsequent query failed with `no such table: students`. Both now import `DEFAULT_DB_PATH` alongside `connect` and drop the homemade paths; the empty stray DB was deleted.
- **`st.email` / `students.user_id` were wrong columns** — five queries across the assignment portals read `students.email` and `students.user_id`; neither column exists. The real shape is `students.email_address`, and the linkage between users and students is `users.student_id` → `students.student_id` (reversed). `_send_grade_release_emails`, `_lookup_student_and_module`, `_resolve_numeric_user_id`, `_resolve_student_id`, and the plagiarism-repo seeding query were all rewritten to join `users` on `users.student_id` and `COALESCE(users.email, students.email_address)`. All five verified against the live schema via `EXPLAIN QUERY PLAN`.
- **ChatbotGUI ignored its `auth_system` argument** — the constructor accepted the parameter then immediately replaced it with `shared_context.get_auth()`. When AI Features launched the chatbot with a fully authenticated `self.auth`, the check at line 74 ran against a different auth instance whose `current_user` was None and raised "Authentication required - not logged in". The constructor now prefers the passed-in auth when it has a `current_user`, falls back to `get_auth()` otherwise, and only raises the auth-required error when no path yields a logged-in user.
- **File viewer failed on pre-restructure paths** — `assignment_submissions.file_path` was stored as an absolute path, so legacy rows from before the `education_system/` directory move pointed at e.g. `/home/.../university_system/data/submissions/submitted/...` (no `education_system/` prefix), and some had filename drift (stored `..._requirements.txt` vs on-disk `..._submission.txt` for the same timestamp). `resolve_submission_path` handles both by re-rooting the `submissions/submitted/...` suffix onto `SUBMISSIONS_DIR` and then, if needed, matching a file in the same submission directory by timestamp prefix.
- **Duplicate-file check was scoped too narrowly** — initially keyed on `(assignment_id, student_id, file_hash)`, so a student could still upload the same file to a different assignment. Widened to `(student_id, file_hash)`. The warning message joins to `assignments` so it can name the other assignment, e.g. *"already submitted for 'Lab 2'"*.

---

## [8.80.0] — 2026-04-21

### File organisation cleanup — relocate misplaced files into their correct architectural layers across the education_system tree

#### Changed

- **`university_system/utils/logging/` → `infrastructure/logging/`** — logging is infrastructure, not a utility. The whole package (`log_config.py`, `log_management/`, and the GUI `tabs/` + `features/`) moved into `infrastructure/logging/`, and every absolute import of `education_system.university_system.utils.logging.*` across the codebase (193 files) was rewritten to the new path.
- **`university_system/utils/ai/` → `infrastructure/ai/`** — consolidated the real AI code (the `ai_detector/`, `gui/`, and `university_chatbot/` packages plus `voice_interface.py`) into the existing `infrastructure/ai/` namespace, which previously held only compat-shim files (`infrastructure/ai/__init__.py` and `infrastructure/ai/university_chatbot.py`) that forwarded to `utils/ai`. Those two shims were deleted along with the flat `utils/ai/university_chatbot.py` bridge (shadowed by the `university_chatbot/` package anyway), and all 97 absolute imports of `education_system.university_system.utils.ai.*` were rewritten. Also updated two `dependency=` label strings in `infrastructure/auth/optional_dependencies.py` (around lines 237 and 350) that identified the optional chatbot/voice modules by their old path.
- **`modules/setup_unified_database.py` → `modules/scripts/setup_unified_database.py`** — the 891-line schema setup script sat at the root of `modules/` alongside domain code; moved next to the other setup/seed scripts already living in `modules/scripts/`. Updated the two lazy imports in `infrastructure/database/database_utils.py:init_db` and the two corresponding test files (`tests/cli/domain/setup/test_setup_unified_database.py`, `tests/cli/infrastructure/database/test_database_utils.py`).
- **`secondary_school/main_gui.py` and `primary_school/main_gui.py` → `modules/shared/gui/main_gui.py`** — GUI entry points belong under `modules/shared/gui/` alongside `mfa_gui.py`, matching the existing `college_system/modules/shared/gui/main_gui.py` layout rather than sitting at the subsystem root. Updated the four import sites: `launcher/systems.py:170,182` and each subsystem's `__main__.py`.
- **`secondary_school/seed_subjects.py` and `primary_school/seed_subjects.py` → `<subsystem>/scripts/seed_subjects.py`** — seeding is operational setup, not a module. Created a `scripts/` package in each subsystem (matching the university layout) and updated the `from education_system.<subsystem>.seed_subjects import seed_subjects` call at the top of each new `main_gui.py`.
- **`university_system/utils/{reset_password,fix_user_accounts,email_scheduler_control}.py` → `university_system/scripts/`** — three one-off admin scripts moved alongside the existing `fix_*/generate_*/init_*` scripts. Updated the test imports in `tests/cli/shared/utils/test_utils_email_scheduler_control.py` (~20 references), the scheduler's own docstring self-references in `email_scheduler_control.py`, the admin guide at `docs/university_system/guides/administration/EMAIL_SYSTEM_ADMIN_GUIDE.md`, `docs/university_system/infrastructure/EMAIL_SCHEDULER.md`, and the `scheduler_info` translation strings across all 10 locale `communication.json` files (the legacy `CHANGELOG-legacy-notes.md` was left unchanged as historical record).
- **Three domain smoke-test scripts → `tests/cli/domain/`** — `modules/domain/student_affairs/social_matching/test_social_matching.py` and `modules/domain/commerce/marketplace/test_marketplace.py` moved under `tests/cli/domain/` mirroring the source layout, and had their broken `sys.path.insert` hacks removed (they had the wrong depth — absolute `education_system.*` imports resolve via the normal path). Because both files use `print`/`return True/False` instead of assertions (they are developer smoke-test scripts, not pytest tests), the two new target directories were added to the `--ignore` list in `pyproject.toml [tool.pytest.ini_options]` so pytest doesn't auto-collect them.
- **`university_system/infrastructure/localization.py` reduced to a thin shim** — the 169-line standalone JSON translation loader duplicated functionality already provided by `education_system.shared.i18n` (accessed via the university wrapper at `core/i18n.py`, which registers the university's `data/locales/` directory alongside the shared one). The module is now ~13 lines and re-exports `get_translation` / `_t` from `core.i18n.get_text`. Signature matches exactly (`(key, default=None, **kwargs)`), so all 11 external callers (GUI files under `modules/domain/academics/`, `staff_hr/`, `commerce/`, `campus/`, `student_affairs/wellness/`) transparently route through the shared engine without code changes.

#### Removed

- **`university_system/extensions/`** — a bare docstring stub (`__init__.py` only) that claimed to prevent `ImportError` for code referencing an old namespace, with zero actual external callers.
- **`university_system/extras/`** — a thin wrapper `__init__.py` that re-exported `education_system.shared.extras`, also with zero external callers.

#### Note

- `modules/domain/academics/services/plagiarism/tests.py` was initially moved alongside the other two smoke tests but had to be **reverted** in the same release — unlike `test_social_matching.py` and `test_marketplace.py`, this file isn't a smoke test at all. `plagiarism_main.py` imports `test_document_repository`, `test_plagiarism_check`, `test_error_handling`, `test_edge_cases`, and `test_plagiarism_checker` from it to drive the interactive "Test System" menu option, so it's a runnable component of the plagiarism module. It stays in the domain tree; the empty `tests/cli/domain/academics/services/` placeholder dirs that had been created for it were deleted.

---

## [8.79.0] — 2026-04-19

### Role-specific Grade Tracking, Assignment, Library, Academic Calendar, Health, Student Union, Campus Events, Student Jobs, Clubs & Societies, Cafe, Bar, Takeaway, Cinema, and Barbershop portals; timetable, financial aid, LMS, scholarship-schema, and student dashboard fixes

#### Added

- **Grade Tracking portals** — new `modules/domain/academics/gui/grade_tracking/staff_portal.py` (`GradeTrackingStaffPortal`: module + assessment selector, editable student grade table with save/discard batching, class-average stats) and `grade_tracking/student_portal.py` (`GradeTrackingStudentPortal`: read-only per-module grades, credit-weighted GPA, assessment-level breakdown; resolves the current student from `auth.current_user` by `student_id` → `username` → `email`). Admin flow unchanged — admins still get the full `GradeTrackingApp` through the launcher.
- **Assignment portals** — new `modules/domain/academics/gui/assignment_system/staff_portal.py` (`AssignmentStaffPortal`: my-assignments list filtered by `created_by = user_id`, create/edit/soft-delete dialog, submissions pane with inline grading that updates `grade`, `feedback`, `graded_by`, `graded_date`, `status`) and `assignment_system/student_portal.py` (`AssignmentStudentPortal`: assignments for enrolled modules with status tags Pending/Submitted/Graded/Overdue, file-picker submit that copies into `paths.SUBMISSIONS_DIR/submitted/<student_id>/assignment_<id>/`, marks prior submissions `is_final_submission = 0`, bumps `version_number`, and computes `late_submission` + `late_days`). Admin flow unchanged.
- **Library portals** — new `modules/domain/academics/gui/library/staff_portal.py` (`LibraryStaffPortal`: tabbed Books / Checkout / Returns / All Loans / Reservations; add & edit books; checkout flips `books.status` to `checked_out` and writes `book_loans`; returns read `library_settings.fine_per_day`, compute fine based on overdue days, set `return_date` + `status = 'returned'`, restore book to `available`) and `library/student_portal.py` (`LibraryStudentPortal`: tabbed Browse / My Loans / My Reservations; search + category filter + available-only toggle; self-service reserve that honors `library_settings.reservation_expiry_days`, auto-computes priority queue, blocks duplicates and already-available books; self-cancel own reservations). Admin flow unchanged.
- **Academic Calendar portals** — new `modules/domain/academics/gui/academic_calendar/staff_portal.py` (`CalendarStaffPortal`: filterable event list by type and date range, create/edit/delete dialog against `academic_calendar_events`, stamps `created_by` / `date_added` / `last_modified`, validates date ordering) and `academic_calendar/student_portal.py` (`CalendarStudentPortal`: read-only scope selector Upcoming / This week / This month / Past / All, event-type filter, row tags for today / this week / past, description preview pane). Admin flow unchanged — admins still get the full `CalendarGUI` (dashboard, academic views, recurring events, workflows, bulk import).
- **Cafe / Bar / Takeaway portals** — new `modules/domain/commerce/services/commerce_portals.py` with a single `CommerceStaffPortal` + `CommerceStudentPortal` pair parameterised by `source_type` (`'cafe'` / `'bar'` / `'takeaway'`), plus six thin `launch_*_{staff,student}_portal` entry points. Cafe, Bar, and Takeaway share the same `products` / `orders` / `order_items` tables with a `source_type` discriminator, so one implementation covers all three. Staff portal: orders queue (Open / Today / All), one-click status transitions (pending → preparing → ready → delivered / cancelled), linked order-items pane, menu/stock tab with low-stock highlighting. Student portal: Menu (search + availability filter), Cart (add/remove/quantity prompt, running total), Place Order (writes `orders` + `order_items` atomically with correct `source_type`), My Orders.
- **Cinema portals** — new `modules/domain/commerce/cinema/cinema_portals.py`. Base schema only ships `cinema_referrals` + `cinema_settings`, so both portals call an idempotent `ensure_cinema_tables()` helper on open that `CREATE TABLE IF NOT EXISTS`s `cinema_showings` and `cinema_bookings` (with seat-count tracking). Staff portal: showing list with full/cancelled row tags, add showing dialog, cancel showing (cascades to active bookings). Student portal: Upcoming Showings tab with seats-left, book dialog validates capacity before insert and increments `booked_seats`, My Bookings tab with cancel (decrements seat count).
- **Barbershop portals** — new `modules/domain/commerce/barber/barber_portals.py`. Staff portal: tabbed Appointments (Today / Upcoming / Past / All filters) + Services & Staff reference; one-click status transitions (scheduled → confirmed → completed / no_show / cancelled), today-row highlight, completed-row highlight. Student portal: Book tab lists available `barber_services` and opens a booking dialog with date/time validation, optional preferred barber dropdown populated from active `barber_staff`, captures phone + special requests; My Appointments tab with Cancel (only for non-completed, non-cancelled rows, and only the student's own appointments).
- **Student Union portals** — new `modules/domain/student_affairs/gui/student_union_gui/staff_portal.py` (`StudentUnionStaffPortal`: tabbed Events + Elections + Clubs; create union events, open/close election voting, roster view of all active clubs) and `student_union_gui/student_portal.py` (`StudentUnionStudentPortal`: read-only Upcoming Events / Elections / Clubs tabs with active-voting highlight). Admins retain the full `StudentUnionGUI`.
- **Campus Events portals** — new `modules/domain/campus/services/campus_events_staff_portal.py` (`CampusEventsStaffPortal`: filterable event list by scope (Upcoming/Today/Past/All) and status, create new event dialog writing to `unified_events` with correct `source_type = 'campus_events'`, one-click cancel, live registrations pane) and `campus_events_student_portal.py` (`CampusEventsStudentPortal`: Browse Events + My Registrations tabs; register writes a row to `unified_event_registrations` with `user_type = 'student'`, blocks duplicate registration, unregister from "My Registrations"). Admins retain the full `CampusEventsGUI`.
- **Student Jobs portals** — new `modules/domain/career/student_jobs/gui/staff_portal.py` (`StudentJobsStaffPortal`: two-pane layout — job postings (filter includes-inactive toggle, post-job / close-posting actions) + per-job applications pane with one-click status transitions through `received` → `reviewing` → `interview` → `offered` / `accepted` / `rejected`; writes to `campus_job_postings` + `campus_job_applications`) and `student_jobs/gui/student_portal.py` (`StudentJobsStudentPortal`: Browse Jobs with search, "View Details" dialog shows description + required/preferred skills, Apply dialog captures cover letter + availability + expected hours, My Applications tab with Withdraw). Admins retain the full `StudentJobsGUI`.
- **Clubs & Societies portals** — new `modules/domain/student_affairs/gui/student_union_gui/clubs/staff_portal.py` (`ClubsStaffPortal`: two-pane — clubs table with create/edit/archive dialogs + per-club members pane) and `clubs/student_portal.py` (`ClubsStudentPortal`: Browse Clubs with search + category filter, Join writes a `club_memberships` row and bumps `student_clubs.member_count`, My Clubs tab with Leave). Because `club_memberships` wasn't in the repo schema, both portals call a shared `ensure_club_memberships_table()` helper on open that `CREATE TABLE IF NOT EXISTS`s it (with `UNIQUE(club_id, student_id)` to block double-join). The `show_clubs_gui` method was never bound anywhere (hence the old "Coming Soon" for that button); the new student portal replaces it end-to-end.
- **Health portals** — new `modules/domain/health/gui/health_portal/staff_portal.py` (`HealthStaffPortal`: tabbed Appointments + Student Lookup; appointment queue filterable by scope (Today / Upcoming / Past / All) and status (scheduled / completed / cancelled / no_show), create/edit dialog with date+time validation, one-click Complete / Cancel / No-show status transitions, Student Lookup tab shows any student's allergies and emergency contacts) and `health_portal/student_portal.py` (`HealthStudentPortal`: tabbed My Appointments + Vaccinations + Allergies + Emergency Contacts; read-only panels plus a Request Appointment dialog that inserts into `health_appointments` with status `scheduled` and a "Requested by student" note for staff to confirm). Full medical records, prescriptions, vitals, lab results, and audit/reports remain admin-only — admins still get the full `HealthPortalGUI` via `open_health_portal_gui`.

#### Changed

- **Role-aware sidebar wiring** in `modules/shared/gui/main/instructor_portal.py`, `staff_portal.py`, and `student_portal.py`: the Grade Tracking, Assignments, Library, Academic Calendar, Health Portal, Student Union, Campus Events, Clubs & Societies, Student Jobs, Cafe, Bar, Takeaway, Cinema, and Barber buttons now invoke dedicated `_open_*_portal` methods that open the new role-scoped portals directly, instead of routing through `UnifiedManagementGUI.show_*`. A new "Student Life" sidebar section was added to `instructor_portal.py` and `staff_portal.py` with the four Student Union / Campus Events / Clubs & Societies / Student Jobs buttons (previously only the student portal exposed them), and the Bar / Takeaway / Cinema / Barber buttons were added alongside the existing Cafe button in the Services / Resources section so staff and instructors can now reach all five commerce surfaces. Admins continue to open the full management GUIs via `academic_launchers_gui.py` / `student_affairs_gui.py` / `commerce_facilities_gui.py` / `student_success_gui.py` as before.

#### Fixed

- **My Timetable reported no enrolled modules** for students whose `student_modules` rows used lowercase status. `modules/domain/academics/gui/student_timetable_gui.py:197` filtered `status = 'Enrolled'`, but the table stores a mix of `'Enrolled'` and `'enrolled'` — students in the lowercase bucket were filtered out entirely. Changed the clause to `LOWER(status) = 'enrolled'` so both capitalisations match.
- **Financial Aid reported as "Coming Soon / not available"** in the student portal. The sidebar button called `show_financial_aid_gui`, but the method bound onto `UnifiedManagementGUI` is `show_financial_aid` (no `_gui` suffix, defined in `features/finance_gui.py:380`). Updated the `_launch` target in `modules/shared/gui/main/student_portal.py:159` so the existing handler resolves.
- **Financial Aid then crashed with `TypeError: 'NoneType' object is not callable`** once the button reached `show_financial_aid`. `features/finance_gui.py` imports `FinancialAidGUI` at module load time from `imports/gui_imports.py`, but that module declares `FinancialAidGUI = None` and only populates it on first `_lazy_import('FinancialAidGUI')` call — so the top-level `from ... import FinancialAidGUI` captured `None` and never updated. Rewrote `show_financial_aid` to resolve the class via `_lazy_import('FinancialAidGUI')` at call time (with a graceful fallback message if the load fails) instead of using the stale module-level alias.
- **LMS button reported "Coming Soon / not yet available"** in the student portal. `show_lms_gui` was already implemented in `features/academic_launchers_gui.py:677` (opens `education_system.shared.lms.lms_gui.LMSFrame` in a Toplevel), but it was never imported into `main_gui.py` and never bound onto `UnifiedManagementGUI`, so the portal's `_launch('show_lms_gui')` helper fell through to the "not yet available" fallback. Added the import and the `UnifiedManagementGUI.show_lms_gui = show_lms_gui` binding alongside the other `academic_launchers_gui` methods.
- **Student dashboard Quick Actions all failed** for Roommate Finder, Marketplace, Lost & Found, Campus Navigation, Social Matching, Mail & Post, Printing Services, Study Room Booking, Student ID Card, Achievement Badges, Wellness Hub, and Todo App — each button raised `ModuleNotFoundError: No module named '...student_services'`, `...wellness_hub_gui`, or `TypeError: TodoApp.__init__() got an unexpected keyword argument 'auth'`. The `_launch_feature` `feature_map` in `modules/shared/gui/main/dashboard/student_dashboard.py` still referenced the pre-8.77.0 paths (`modules.domain.student_services.*`, `...domain.health.gui.wellness_hub_gui`) and hard-coded `cls(window, auth=auth)` even for classes whose `__init__` doesn't take `auth` or takes it positionally. Rewrote the `feature_map` to reflect the current reorganised paths — `student_affairs/roommate_finder`, `commerce/marketplace`, `campus/lost_found`, `campus/campus_navigation` (→ `NavigationGUI`), `student_affairs/social_matching`, `communications/mail`, `campus/printing`, `campus/study_rooms`, `student_affairs/student_id`, `student_affairs/achievement_badges`, `student_affairs/wellness` (→ `WellnessGUI`), `shared/gui/tools/todo_app_gui` — plus an `auth_style` tag per entry (`'kwarg'` / `'positional'` / `'none'`) so the call site passes auth the way each class actually accepts it. Fixed the `RoomateFinderGUI` → `RoommateFinderGUI` misspelling at the same time. Also added a `creates_window` tag: 11 of the 12 target classes build their own `Toplevel`/`Tk` internally, so the launcher was opening a blank extra window each time; now only `TodoApp` (which fills whatever window is handed to it) gets a pre-made `Toplevel`, while the rest receive `root` directly as their parent.
- **Admin Cafe inventory and refund actions crashed** with `sqlite3.OperationalError: no such column: id`. Three queries in `modules/domain/commerce/gui/cafe_inventory.py` (load inventory at line 77, add-stock UPDATE at line 130, remove-stock UPDATE at line 178) and one in `cafe_refunds.py:263` referenced a column named `id` on the `products` / `orders` tables, but those tables' primary keys are `product_id` and `order_id` respectively (the sibling `BarGUI` uses the correct names throughout). Updated all four queries to use the correct column names; verified the SELECT returns rows and both UPDATE plans resolve against the live DB via the INTEGER PRIMARY KEY index.
- **Submitting a scholarship application crashed** with `table scholarship_applications has no column named essay_text`. Three conflicting CREATE TABLE definitions exist in the repo: `modules/domain/finance/services/financial_aid/schema.py` and the test helper both declare `essay_text` (matching `ScholarshipManager.submit_application`'s INSERT at `scholarship_manager.py:94`), but `infrastructure/database/schemas/aggregators.py:1319` declared the column as `essay` — and because aggregators' `init_additional_missing_tables()` runs first, live databases were shaped with `essay` (and missing `gpa` / `transcript_url` / `resume_url` too). Renamed the aggregator column to `essay_text` so fresh DBs get the canonical shape, and added an idempotent PRAGMA-guarded migration after the CREATE that `ALTER TABLE ... ADD COLUMN`s `essay_text` / `gpa` / `transcript_url` / `resume_url` on legacy DBs (copying existing `essay` data into `essay_text` where present). The legacy `essay` column is left in place to avoid breaking any reader that still references it.

---

## [8.78.0] — 2026-04-19

### Extend advanced search (GUI and CLI) with multi-entity coverage, real audit trail, and bulk actions

#### Added

- **Multi-entity search** in the university advanced search: students, staff (`staff_profiles`), modules, and courses. The GUI multi-criteria form (`modules/shared/gui/advanced_search/search_basic.py`) now has an Entity Type selector that dynamically rebuilds the criteria fields; `perform_database_search` routes to the correct table, validates identifiers via `core.sql_safety`, and skips criteria whose column doesn't exist in the live schema. The CLI (`modules/shared/services/analytics/advanced_search/search.py:multi_criteria_search`) gained a matching `ENTITY_SEARCH_CONFIGS` dict and an entity-type prompt.
- **Results-tree bulk actions** (GUI, `base.py`): `selectmode='extended'`, right-click context menu with Select All / Copy IDs / Copy Names / Copy Emails / Export Selected to CSV, plus `<Control-a>` for select-all. Four new methods on `AdvancedSearchGUI`: `_show_results_context_menu`, `select_all_results`, `copy_selected_results_column`, `export_selected_results_csv`.
- **Bulk actions in the CLI results view** (`display.py:display_search_results`): new `b) Bulk actions on selected rows` option that accepts ranges like `1-3,5,8` or `all`, with copy-IDs / copy-emails / copy-names / export-CSV sub-actions. Helpers: `_parse_row_selection`, `_copy_column`, `_export_rows_csv`, `_bulk_actions_menu`.
- **Recent-searches surface** in smart suggestions (both GUI and CLI): new `_load_recent_search_terms(limit)` helper queries `search_analytics` for the current user's distinct recent `search_query` values (schema-tolerant — also accepts `search_criteria`), with a global fallback when the user has no history. GUI shows them as buttons in `show_smart_suggestions`; CLI appends them to `smart_suggestions` with a re-run action that executes a student name/email/ID text search.

#### Changed

- **Audit trail is now DB-backed.** GUI `show_audit_trail` (`utils.py`) is a real Treeview dialog with limit/user filter, refresh, and CSV export, reading from `search_analytics` (handles both `timestamp`/`search_datetime` and `search_query`/`search_criteria` schema variants). CLI `view_search_audit_trail` (`admin.py`) prints a formatted audit table from the same source, falling back to the log file when the table is missing/empty. Both replace prior stubs that returned a hardcoded two-line string.
- **Advanced-search results tree** (GUI `base.py:1339`) now allows multi-row selection, enabling the new bulk actions without breaking the existing double-click-for-details flow.

#### Fixed

- **Fallback analytics stubs** in `gui/advanced_search/base.py` (inside the email-infrastructure `ImportError` branch) no longer return hardcoded placeholder strings. `search_analytics_dashboard`, `student_demographics_reports`, `academic_performance_analysis`, `duplicate_detection`, `data_quality_reports`, `export_system_statistics`, `view_search_audit_trail`, `manage_user_permissions`, `manage_scheduled_reports`, and `performance_optimization` now run real SQL against the current DB so the fallback path yields useful output.
