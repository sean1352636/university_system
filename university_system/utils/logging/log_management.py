...
class LogDatabase:
    ...
    def __init__(self, db_path=None):
        self.db_path = db_path if db_path else str(LOG_DIR / "log_database.db")
...
class LogRetention:
    ...
    def cleanup_old_logs(self):
        ...
        print(f"Deleted {deleted_count} logs older than {retention_days} days")
...
def toggle_api(log_manager):
    ...
    if new_status:
        ...
        print("Use /api/health for health checks")
...
def show_database_stats(log_manager):
    ...
    try:
        ...
        for table in tables:
            ...
            # Re-verify the table exists in sqlite_master
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?", (table,))
            if cursor.fetchone():
                # Table name validated - safe to use
