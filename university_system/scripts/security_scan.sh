...
if $DETAILED; then
    ...
    bandit -r university_system/ \
        --exclude university_system/tests/ \
        -f txt \
...
if grep -r -E "(password|secret|api_key|token)\s*=\s*['\"][^'\"]+['\"]" university_system/ \
    --exclude-dir=tests \
    --exclude-dir=__pycache__ \
    --exclude="*.pyc" 2>/dev/null | grep -v "# nosec" > /dev/null; then
    echo -e "  ${RED}✗ Potential hardcoded secrets found${NC}"
    grep -r -E "(password|secret|api_key|token)\s*=\s*['\"][^'\"]+['\"]" university_system/ \
        --exclude-dir=tests \
        --exclude-dir=__pycache__ \
...
if grep -r "execute.*%.*\|.*format.*(" university_system/ \
    --include="*.py" \
...
if grep -r "DEBUG\s*=\s*True" university_system/ \
    --include="*.py" \
...
if grep -r "import random$\|from random import" university_system/infrastructure/security/ \
