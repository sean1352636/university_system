...
try:
    from university_system.infrastructure.security.immutable_audit import (
        AuditAction, log_security_event
    )
    from university_system.modules.shared.utils.gui_context import get_gui_context
...
def create_payment_form(gui_instance, parent):
    ...
    gui_instance.payment_amount_entry.grid(row=0, column=1, padx=10)
    ...
    gui_instance.payment_method_combo.grid(row=1, column=1, padx=10)
    ...
    gui_instance.transaction_ref_entry.grid(row=2, column=1, padx=10)
...
def load_active_assignments_for_payment(gui_instance):
    ...
    from university_system.modules.domain.housing.gui.housing_accommodation_gui.assignment_manager import load_active_assignments
    ...
    try:
        ...
        if payment_method == "Student Finance Account" and FINANCE_ACCOUNT_AVAILABLE:
            ...
            payment_result = process_student_finance_account_payment(
                ...
                amount=amount,
                description=f"Housing payment for period {period_start} to {period_end}",
                transaction_source="Housing",
                transaction_ref=f"RENT_{datetime.now().strftime('%Y%m%d%H%M%S')}",
        ...
        base_msg = f"✓ Payment recorded successfully!\n\nPayment ID: {payment_id}\nAmount: £{amount:.2f}"
        ...
        gui_instance.period_start_entry.delete(0, tk.END)
        gui_instance.period_start_entry.insert(0, datetime.now().strftime('%Y-%m-%d'))
        gui_instance.period_end_entry.delete(0, tk.END)
        next_month = datetime.now().replace(day=28) + timedelta(days=4)
        end_of_month = next_month - timedelta(days=next_month.day)
        gui_instance.period_end_entry.insert(0, end_of_month.strftime('%Y-%m-%d'))
