...
# Import immutable audit logging for HIPAA compliance
try:
...
class HealthPortalGUI:
    ...
    def log_audit_event(self, action, resource_type, resource_id, details=None):
        ...
        if self.auth.current_user:
            ...
            # Also log to immutable audit log for HIPAA compliance
            if IMMUTABLE_AUDIT_AVAILABLE:
                # Map health portal actions to audit actions
                action_map = {
...
"""
    ...
    def load_audit_log(self):
        ...
        try:
            # Try health portal specific audit log first, fallback to main activity log
    ...
    def filter_audit_log(self):
        ...
        try:
            # Use same logic as load_audit_log
            log_file = paths.LOG_DIR / "health_portal_audit.log"
            if not os.path.exists(log_file) or os.path.getsize(log_file) == 0:
                log_file = paths.LOG_DIR / "activity.log"
    ...
    def generate_selected_report(self):
        ...
        # Allow admins and users with view_health_records permission
        ...
        def email_report_to_admin():
            ...
            try:
                ...
                if result:
                    ...
                    # You would integrate with university_system.infrastructure.email.email_service here
    ...
    def load_vaccination_display(self):
        ...
        try:
            ...
            with sqlite3.connect(str(DEFAULT_DB_PATH)) as conn:
                ...
                if user_id:
                    # Get vaccination records from vaccination_records table
                    cursor.execute("""
                    ...
                    if vaccination_records:
                        ...
                        for record in vaccination_records:
                            ...
                            if adverse:
                                display_data += f"  ⚠ Adverse reaction reported"
                                if reaction_desc:
