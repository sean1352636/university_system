...
class TrainingGUI:
    ...
    def _create_manage_tab(self):
        ...
        columns = (
            _t("staff_hr.training.manage.columns.id", default="ID"),
            _t("staff_hr.training.manage.columns.name", default="Name"),
            _t("staff_hr.training.manage.columns.category", default="Category"),
            _t("staff_hr.training.manage.columns.duration", default="Duration"),
            _t("staff_hr.training.manage.columns.mandatory", default="Mandatory"),
            _t("staff_hr.training.manage.columns.active", default="Active"),
            _t("staff_hr.training.manage.columns.enrollments", default="Enrollments")
        )
        ...
        for col in columns:
            ...
            self.manage_tree.column(col, width=100, anchor=tk.CENTER)
...
{_t("staff_hr.training.dialogs.certification_details.status", default="Status: {status}").format(status=values[6])}
    ...
    def _edit_course(self):
        ...
        if not selection:
            messagebox.showwarning(
                _t("staff_hr.training.dialogs.enroll.warning_title", default="Warning"),
                _t("staff_hr.training.dialogs.deactivate_course.warning_select", default="Please select a course to edit")
            )
            return

        messagebox.showinfo(
            _t("staff_hr.training.dialogs.edit_course.title", default="Edit"),
            _t("staff_hr.training.dialogs.edit_course.message", default="Edit dialog would open here")
        )
    ...
    def _load_manage_courses(self):
        ...
        try:
            ...
            with get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT c.course_id, c.name, c.category, c.duration_hours,
                           c.is_mandatory, c.is_active,
                ...
                for row in cursor.fetchall():
                    values = (row[0], row[1], row[2] or _t("staff_hr.training.catalog.values.na", default="-"),
                              _t("staff_hr.training.catalog.values.duration_hours", default="{hours}h").format(hours=f"{row[3]:.1f}") if row[3] else _t("staff_hr.training.catalog.values.na", default="-"),
                              _t("staff_hr.training.catalog.values.yes", default="Yes") if row[4] else _t("staff_hr.training.catalog.values.no", default="No"),
                              _t("staff_hr.training.catalog.values.yes", default="Yes") if row[5] else _t("staff_hr.training.catalog.values.no", default="No"), row[6])
                    self.manage_tree.insert('', 'end', values=values)

        except Exception as e:
            messagebox.showerror(
                _t("staff_hr.training.errors.error_generic", default="Error"),
                _t("staff_hr.training.errors.load_courses_failed", default="Failed to load courses: {error}").format(error=str(e))
            )
    ...
    def _export_certifications(self):
        ...
        try:
            ...
            filename = filedialog.asksaveasfilename(
                ...
                initialfilename=f"{_t('staff_hr.training.dialogs.export_certifications.filename_prefix', default='certifications')}_{datetime.now().strftime('%Y%m%d')}.csv"
            )
