...
def create_enhanced_course(auth):
    ...
    if not auth or not auth.current_user:
        print("You must be logged in to create courses.")
        return False
    ...
    try:
        ...
        while True:
            ...
            if not validate_course_code(course_code):
                print("Error: Invalid course code format. It should be 2-4 letters followed by 2-3 numbers.")
                continue
        ...
        while True:
            ...
            if len(course_name) < 5:
                print("Error: Course name must be at least 5 characters.")
                continue
            break
        ...
        # Duration
        duration = None
        duration_input = input("Enter course duration in years (e.g., 3, 4): ").strip()
        if duration_input:
            try:
                duration = int(duration_input)
                if duration <= 0:
                    print("Warning: Duration must be positive. Setting to null.")
                    duration = None
            except ValueError:
                print("Warning: Duration must be a number. Setting to null.")
        ...
        level = None
        level_choice = input("Enter choice (1-5) or type level: ").strip()
        if level_choice:
...
def manage_course_status(auth):
    ...
    try:
        ...
        cursor.execute("UPDATE courses SET status = ?, updated_at = ? WHERE id = ?", 
                      (new_status, timestamp, course_id))
        
        # Log the change in history
        cursor.execute('''
        INSERT INTO course_history (course_id, field_name, old_value, new_value, changed_by, changed_at)
        VALUES (?, ?, ?, ?, ?, ?)
        ''', (course_id, 'status', current_status, new_status, auth.current_user, timestamp))
...
def display_enhanced_course_menu(auth):
    ...
    if not auth or not auth.current_user:
        print("You must be logged in to access course management.")
        return
    ...
    while True:
        print("\n" + "="*60)
        print("ENHANCED COURSE MANAGEMENT SYSTEM")
        print("="*60)
        
        if auth.check_permission('manage_courses'):
            print("\n📚 COURSE MANAGEMENT:")
            print("1.  Create new course (Enhanced)")
            print("2.  View all courses")
            print("3.  Update course")
            print("4.  Delete course")
            print("5.  Manage course status")
            print("6.  Search courses (Advanced)")
            
            print("\n🔗 PREREQUISITES & RELATIONSHIPS:")
            print("7.  Add prerequisite")
            print("8.  View prerequisites")
            print("9.  Remove prerequisite")
            
            print("\n👨‍🏫 INSTRUCTOR MANAGEMENT:")
            print("10. Create instructor")
            print("11. View instructors")
            print("12. Assign instructor to course")
            
            print("\n📅 SCHEDULING:")
            print("13. Create course schedule")
            print("14. View course schedules")
            print("15. Update schedule")
            
            print("\n📊 ENROLLMENT & WAITLISTS:")
            print("16. Add student to waitlist")
            print("17. View waitlists")
            print("18. Process waitlist")
            
            print("\n📈 ANALYTICS & REPORTING:")
            print("19. Course analytics dashboard")
            print("20. Generate enrollment report")
            print("21. Department statistics")
            
            print("\n💾 BULK OPERATIONS:")
            print("22. Import courses from CSV")
            print("23. Export courses to CSV")
            print("24. Bulk update courses")
            
            print("\n🎯 RECOMMENDATIONS:")
            print("25. Course recommendations")
            print("26. Find alternative courses")
            
            print("\n🔧 UTILITIES:")
            print("27. Course history")
            print("28. System maintenance")

            print("\n📋 MODULE MANAGEMENT:")
            print("29. Module Management")
            ...
            print("3. View prerequisites")
            print("4. Course recommendations")
            print("5. Course analytics")
            print("0. Return to Main Menu")
            
            max_option = 5
        else:
            print("You don't have permission to manage courses.")
            return
        ...
        if auth.check_permission('manage_courses'):
            if choice == '1':
                ...
                # Import module management menu from cli_main
                from university_system.cli_main import display_module_management_menu
...
def find_alternative_courses(auth):
    ...
    try:
        ...
        while True:
            try:
                ref_course_id = int(input("\nEnter course ID to find alternatives for: "))
                ref_course = next((c for c in courses if c[0] == ref_course_id), None)
                if ref_course:
                    break
                print("Invalid course ID.")
            except ValueError:
...
def view_all_courses(auth):
    ...
    if not auth or not auth.current_user:
        print("You must be logged in to view courses.")
        return
    ...
    try:
        ...
        print("\nAll Courses (Enhanced View):")
        print(f"{'ID':<5} {'Code':<8} {'Name':<25} {'Dept':<12} {'Level':<12} {'Type':<12} {'Credits':<8} {'Enrolled':<10} {'Status':<10}")
        print("-" * 102)
        ...
        for course in courses:
            ...
            dept_display = dept[:9] + "..." if dept and len(dept) > 12 else dept or "N/A"
            level_display = level[:9] + "..." if level and len(level) > 12 else level or "N/A"
            type_display = ctype[:9] + "..." if ctype and len(ctype) > 12 else ctype or "N/A"
        ...
        detail_choice = input("\nEnter a course ID for detailed view (or press Enter to return): ")
...
def update_course(auth):
    ...
    try:
        ...
        # Current values (handling both old and new schema)
        if len(course) >= 21:  # Enhanced schema
            (id, code, name, desc, duration, level, dept, credit_hours, contact_hours, 
             learning_outcomes, assessment_methods, textbooks, course_fee, lab_required, 
             online_available, max_enrollment, current_enrollment, status, course_type, 
             tags, availability, created_at, updated_at) = course
        else:  # Legacy schema
            code, name, desc, duration, level, dept = course[1:7]
            credit_hours = course[7] if len(course) > 7 else 3.0
...
if __name__ == "__main__":
    print("Enhanced Course Management System")
    print("This module should be imported and used with an authentication system.")
    print("Run initialize_enhanced_database() to set up the database schema.")
