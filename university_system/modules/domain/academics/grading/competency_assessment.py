...
from university_system.modules.domain.academics.grading.grade_calculation import calculate_student_gpa
...
def generate_student_competency_report(cursor, student_id):
    ...
    try:
        ...
        elements.append(Paragraph(f"Student Competency Report", title_style))
...
def generate_course_competency_report(cursor, course):
    ...
    try:
        ...
        for comp_id, comp_name, category in competencies:
            ...
            FROM student_competencies sc
            JOIN competency_levels cl ON sc.level_id = cl.level_id
            WHERE sc.competency_id = ? AND sc.student_id IN ({student_ids})
            ''', (comp_id,))
        ...
        if competencies_with_data:
            ...
            plt.ylabel('Competency')
            plt.title(f"Average Competency Levels")
            plt.xlim(0, 5.5)
            ...
            plt.xlabel('Coverage (%)')
            plt.ylabel('Competency')
            plt.title(f"Lowest Competency Coverage")
            plt.xlim(0, 105)
            ...
            if low_level_competencies:
                elements.append(Paragraph(
                    f"• Focus on improving overall proficiency in these competencies:",
                    styles['Normal']))
            ...
            if low_coverage_competencies:
                elements.append(Paragraph(
                    f"• Increase assessment coverage for these competencies:",
                    styles['Normal']))
            ...
            elements.append(Paragraph(
                f"• Document best practices from competencies with high achievement levels to apply to other areas.",
                styles['Normal']))
        else:
