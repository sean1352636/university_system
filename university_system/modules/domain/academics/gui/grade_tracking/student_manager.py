...
class StudentManager:
    ...
    def generate_individual_transcript(self):
        ...
        try:
            ...
            student_dialog = tk.Toplevel(self.root)
            student_dialog.title("Select Student for Transcript")
            student_dialog.geometry("400x200")
            safe_grab_set(student_dialog)
                
            ttk.Label(student_dialog, text="Select Student:", font=('Arial', 12, 'bold')).pack(pady=10)
                
            student_var = tk.StringVar()
            student_combo = ttk.Combobox(student_dialog, textvariable=student_var, width=40)
            ...
            students = [f"{s[0]} - {s[1]} {s[2]}" for s in cursor.fetchall()]
            ...
            def generate_selected():
                if student_var.get():
                    ...
                    messagebox.showinfo("Success", f"Transcript generation initiated for student {student_id}")
                    student_dialog.destroy()
                else:
                    messagebox.showwarning("Warning", "Please select a student")
                
            ttk.Button(student_dialog, text="Generate Transcript", command=generate_selected).pack(pady=10)
            ttk.Button(student_dialog, text="Cancel", command=student_dialog.destroy).pack(pady=5)
