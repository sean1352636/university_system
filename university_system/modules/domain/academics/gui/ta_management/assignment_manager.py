...
class AssignmentManager:
    ...
    def _assign_ta(self):
        ...
        ttk.Entry(main_frame, textvariable=student_var, width=30).grid(row=0, column=1, pady=5, padx=(10, 0))

        # Module Code
        ttk.Label(main_frame, text="Module Code:").grid(row=1, column=0, sticky=tk.W, pady=5)
        module_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=module_var, width=30).grid(row=1, column=1, pady=5, padx=(10, 0))
        ...
        def _do_assign():
            ...
            if not student_id:
                messagebox.showwarning("Validation", "Student ID is required.", parent=dialog)
                return
            if not module_code:
                messagebox.showwarning("Validation", "Module Code is required.", parent=dialog)
                return
