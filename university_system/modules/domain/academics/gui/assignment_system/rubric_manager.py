...
class RubricManager:
    ...
    def manage_rubrics(self):
        ...
        title = ttk.Label(self.gui.layout.content_area, text="Manage Rubrics", style='Title.TLabel')
        title.pack(anchor='w', pady=(0, 20))
        
        # Rubrics list
        rubrics_frame = ttk.LabelFrame(self.gui.layout.content_area, text="Existing Rubrics", padding=10)
        ...
        ttk.Button(buttons_frame, text="Create New Rubric", 
                  command=self.show_create_rubric).pack(side='left', padx=(0, 10))
        ttk.Button(buttons_frame, text="Edit Rubric", 
                  command=lambda: self.edit_rubric(rubric_tree)).pack(side='left', padx=(0, 10))
        ttk.Button(buttons_frame, text="Delete Rubric", 
    ...
    def load_rubrics_data(self, tree):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to load rubrics: {e}")
    ...
    def edit_rubric(self, tree):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to load rubric: {exc}")
            return
    
        if not record:
            messagebox.showerror("Error", "Selected rubric no longer exists.")
            return
        ...
        ttk.Label(form_frame, text="Total Points:").grid(row=1, column=0, sticky='w', pady=5)
        ...
        ttk.Checkbutton(form_frame, text="Active", variable=active_var).grid(row=2, column=1, sticky='w', pady=5)
        ...
        def save_rubric_changes():
            try:
                ...
                messagebox.showinfo("Success", "Rubric updated successfully.")
            except ValueError as ve:
                messagebox.showerror("Validation Error", str(ve))
            except Exception as exc:
                messagebox.showerror("Error", f"Failed to update rubric: {exc}")
    
        button_frame = ttk.Frame(editor)
        button_frame.pack(fill='x', pady=(0, 10))
        ttk.Button(button_frame, text="Save Changes", command=save_rubric_changes, style='Accent.TButton').pack(side='left')
    ...
    def delete_rubric(self, tree):
        ...
        if messagebox.askyesno("Confirm Delete", f"Delete rubric '{rubric_name}'?"):
            try:
                conn = sqlite3.connect(str(DEFAULT_DB_PATH))
                cursor = conn.cursor()
                ...
                messagebox.showinfo("Success", "Rubric deleted successfully")
                self.load_rubrics_data(tree)
                
            except Exception as e:
                messagebox.showerror("Error", f"Failed to delete rubric: {e}")
    ...
    def show_rubric_editor(self):
        ...
        try:
            ...
            dialog.title("Rubric Editor")
            dialog.geometry("800x600")
            dialog.transient(self.root)
    
            ttk.Label(dialog, text="Rubric Editor",
                     font=('TkDefaultFont', 14, 'bold')).pack(pady=10)
    
            # Rubric details
            details_frame = ttk.LabelFrame(dialog, text="Rubric Details", padding=10)
            details_frame.pack(fill='x', padx=10, pady=5)
    
            ttk.Label(details_frame, text="Rubric Name:").grid(row=0, column=0, sticky='w', pady=5)
            ...
            ttk.Label(details_frame, text="Total Points:").grid(row=1, column=0, sticky='w', pady=5)
            ...
            # Criteria section
            criteria_frame = ttk.LabelFrame(dialog, text="Rubric Criteria", padding=10)
            ...
            ttk.Button(button_frame, text="Add Criterion",
                      command=lambda: messagebox.showinfo("Info", "Add Criterion - To be implemented")).pack(side='left', padx=2)
            ttk.Button(button_frame, text="Edit Selected",
                      command=lambda: messagebox.showinfo("Info", "Edit - To be implemented")).pack(side='left', padx=2)
            ttk.Button(button_frame, text="Delete Selected",
                      command=lambda: messagebox.showinfo("Info", "Delete - To be implemented")).pack(side='left', padx=2)
    
            # Bottom buttons
            bottom_frame = ttk.Frame(dialog)
            bottom_frame.pack(pady=10)
    
            ttk.Button(bottom_frame, text="Save Rubric",
                      command=lambda: messagebox.showinfo("Info", "Rubric saved")).pack(side='left', padx=5)
            ttk.Button(bottom_frame, text="Close", command=dialog.destroy).pack(side='left', padx=5)
    
        except Exception as e:
            messagebox.showerror("Error", f"Failed to open rubric editor: {e}")
    ...
    def show_create_rubric(self):
        ...
        title = ttk.Label(self.gui.layout.content_area, text="Create Rubric", style='Title.TLabel')
        title.pack(anchor='w', pady=(0, 20))
    
        form_frame = ttk.LabelFrame(self.gui.layout.content_area, text="Rubric Details", padding=20)
        form_frame.pack(fill='x', pady=(0, 20))
    
        ttk.Label(form_frame, text="Rubric Name:").grid(row=0, column=0, sticky='w', pady=5)
        rubric_name_var = tk.StringVar()
        ...
        criteria_frame = ttk.LabelFrame(self.gui.layout.content_area, text="Rubric Criteria", padding=10)
        ...
        def open_criteria_dialog(existing=None, index=None):
            dialog = tk.Toplevel(self.root)
            dialog.title("Criterion" if existing is None else "Edit Criterion")
            dialog.geometry("420x320")
            dialog.transient(self.root)
            dialog.grab_set()
    
            ttk.Label(dialog, text="Criterion Name:").pack(anchor='w', padx=10, pady=(10, 5))
            name_var = tk.StringVar(value=(existing['name'] if existing else ""))
            ttk.Entry(dialog, textvariable=name_var).pack(fill='x', padx=10)
    
            ttk.Label(dialog, text="Description:").pack(anchor='w', padx=10, pady=(10, 5))
            ...
            ttk.Label(points_frame, text="Points:").grid(row=0, column=0, sticky='w')
            ...
            ttk.Label(points_frame, text="Weight (%):").grid(row=0, column=2, sticky='w', padx=(20, 0))
            ...
            def save():
                ...
                description = desc_text.get("1.0", tk.END).strip()
                if not name:
                    messagebox.showerror("Validation Error", "Criterion name is required.", parent=dialog)
                    return
                try:
                    points_value = float(points_var.get())
                    weight_value = float(weight_var.get())
                except ValueError:
                    messagebox.showerror("Validation Error", "Points and weight must be numbers.", parent=dialog)
                    return
                criterion = {
                    'name': name,
            ...
            ttk.Button(button_frame, text="Save", command=save, style='Accent.TButton').pack(side='left')
        ...
        def edit_selected_criterion():
            ...
            if not selection:
                messagebox.showwarning("No Selection", "Select a criterion to edit.")
                return
            idx = int(selection[0])
        ...
        def delete_selected_criterion():
            selection = criteria_tree.selection()
            if not selection:
                messagebox.showwarning("No Selection", "Select a criterion to delete.")
                return
            idx = int(selection[0])
            if messagebox.askyesno("Confirm Delete", "Delete selected criterion?"):
                criteria_data.pop(idx)
                refresh_criteria()
    
        buttons_frame = ttk.Frame(criteria_frame)
        buttons_frame.pack(fill='x', pady=(10, 0))
        ttk.Button(buttons_frame, text="Add Criterion", command=add_criterion).pack(side='left')
        ttk.Button(buttons_frame, text="Edit Selected", command=edit_selected_criterion).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Delete Selected", command=delete_selected_criterion).pack(side='left')
        ...
        def save_rubric():
            ...
            if not name:
                show_status("Rubric name is required.", 'Error.TLabel')
                return
            try:
                ...
                show_status("Total points must be a positive number.", 'Error.TLabel')
                return
            if not criteria_data:
                show_status("Add at least one criterion to the rubric.", 'Error.TLabel')
                return
            ...
            if sum_points != total_points:
                if not messagebox.askyesno(
                        "Point Mismatch",
                        f"Total points ({total_points}) do not match the sum of criteria ({sum_points}).\n"
                        "Do you want to save anyway?"):
                    show_status("Rubric not saved. Adjust point values to continue.", 'Warning.TLabel')
                    return
            ...
            try:
                ...
                conn.commit()
                conn.close()
                show_status(f"Rubric '{name}' created successfully.", 'Success.TLabel')
                messagebox.showinfo("Rubric Created", f"Rubric '{name}' saved with {len(criteria_data)} criteria.")
                self.manage_rubrics()
            except Exception as exc:
                show_status(f"Failed to save rubric: {exc}", 'Error.TLabel')
    
        action_frame = ttk.Frame(self.gui.layout.content_area)
        action_frame.pack(fill='x', pady=(10, 0))
        ttk.Button(action_frame, text="Save Rubric", command=save_rubric, style='Accent.TButton').pack(side='left')
        ttk.Button(action_frame, text="Cancel", command=self.manage_rubrics).pack(side='left', padx=10)
