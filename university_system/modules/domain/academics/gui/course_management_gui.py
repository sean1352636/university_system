...
class CourseManagementGUI:
    ...
    def show_analytics(self):
        """
        Entry point for the 'Course Analytics' menu item.
        Tries to launch the analytics GUI if available; otherwise shows an info dialog.
        """
        log = logging.getLogger(__name__)

        # If you have a dedicated analytics window/module, try it first.
        try:
            # Adjust import/path to whatever you actually have
            from university_system.modules.domain.academics.gui.course_analytics_gui import CourseAnalyticsGUI
            db = getattr(self, "db", None) or getattr(self, "conn", None)
            CourseAnalyticsGUI(self.root, db=db)
            return
        except ImportError:
            # No separate analytics module—fall back to a lightweight window.
            pass
        except Exception as e:
            log.exception("Failed to open analytics GUI: %s", e)

        # Minimal, safe fallback so the app doesn't crash
        try:
            win = Toplevel(self.root)
            win.title("Course Analytics")
            win.geometry("420x220")
            ttk.Label(win, text="Course analytics isn’t available in this build.",
                      font=("TkDefaultFont", 11)).pack(padx=16, pady=(20, 8))
            ttk.Label(win, text="Hook up refactored.modules.course_analytics_gui.CourseAnalyticsGUI to enable.",
                      wraplength=380, justify="left").pack(padx=16, pady=4)
            ttk.Button(win, text="Close", command=win.destroy).pack(pady=(12, 16))
        except Exception:
            # Last resort: simple message box
            messagebox.showinfo("Course Analytics", "Course analytics isn’t available in this build.")
