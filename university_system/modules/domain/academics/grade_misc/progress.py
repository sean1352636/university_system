...
from university_system.modules.domain.academics.grade_misc.utils import select_student
...
def student_progress_tracking():
    ...
    try:
        ...
        print(f"\nProgress Tracking for: {first_name} {last_name} ({course})")
        ...
        if not grades:
            print("No assessment grades found for this student.")
            conn.close()
            return
        ...
        visualize = input("\nGenerate progress visualization? (y/n): ").strip().lower()
        if visualize == 'y':
