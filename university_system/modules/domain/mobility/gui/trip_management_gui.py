...
class TripManagementGUI:
    ...
    def show_main_interface(self):
        ...
        ttk.Label(header_frame, text="Trip Management System", 
    ...
    def add_trips_tab(self):
        ...
        self.notebook.add(trips_frame, text="All Trips")
        
        # Toolbar
        toolbar = ttk.Frame(trips_frame)
        toolbar.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Button(toolbar, text="Refresh", command=self.refresh_trips).pack(side=tk.LEFT, padx=(0, 5))
        
        if self.auth.check_permission('create_trips'):
            ttk.Button(toolbar, text="Create Trip", command=self.show_create_trip).pack(side=tk.LEFT, padx=5)
    ...
    def get_db_connection(self, timeout=30.0, max_retries=3):
        """Get a database connection - integrated from original"""
        retry_delay = 0.1
            ...
            def generate_report_thread():
                ...
                try:
                    ...
                    def generate_operation(conn):
                        ...
                        filename = report_generator.generate_filename(self.report_type, format_type)

                        if format_type == "TXT":
                            report_generator.generate_txt_report(data, self.report_type, filename)
                        elif format_type == "PDF":
                            report_generator.generate_pdf_report(data, self.report_type, filename)
                        else:
                            raise ValueError(f"Unsupported format: {format_type}")

                        return filename, data['generated_at']
                    ...
                    filename, generated_at = result
                    log_message(f"✓ Report generated: {filename}")
                    try:
                        file_size = os.path.getsize(filename)
                        log_message(f"File size: {file_size:,} bytes")
                    except OSError:
                        log_message("Unable to determine file size.")
