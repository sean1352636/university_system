...
def calculate_late_fees():
    ...
    if not auth or not auth.current_user:
        print("You must be logged in to calculate late fees.")
        return
    
    if not auth.check_permission('manage_finances'):
        print("You don't have permission to calculate late fees.")
        return
    ...
    try:
        ...
        if not overdue_fees:
            print("No overdue fees found that require late fee calculation.")
            conn.close()
            return
        ...
        print(f"\nCalculating late fees for {len(overdue_fees)} overdue fees...")
        ...
        for fee_data in overdue_fees:
            ...
            if calculated_late_fee > 0:
                ...
                print(f"Applied £{calculated_late_fee:.2f} late fee to {first_name} {last_name} for {fee_name} ({effective_days_overdue} days overdue)")
        
        conn.commit()
        
        print(f"\nLate fee calculation completed!")
        print(f"Late fees applied: {late_fees_applied}")
...
def waive_late_fee():
    ...
    if not auth or not auth.current_user:
        print("You must be logged in to waive late fees.")
        return
    
    if not auth.check_permission('manage_finances'):
        print("You don't have permission to waive late fees.")
        return
