...
class SettingsManager:
    ...
    def create_admin_tab(self):
        ...
        admin_buttons = [
            ...
            ("🔍 Analyze Admin Menu", self.update_admin_menu_with_missing_functions),
            ("📋 Analyze Reports Menu", self.update_reports_menu_with_missing_functions),
        ]        
    ...
    def update_admin_menu_with_missing_functions(self):
        ...
        try:
            ...
            # Display report
            report = "Admin Menu Function Analysis\n"
            report += "=" * 60 + "\n\n"
            report += f"Available Functions: {len(available_functions)}\n"
            report += f"Missing Functions: {len(missing_functions)}\n\n"
            ...
            result_window.title("Admin Menu Analysis")
            ...
            self.update_status("Admin menu analysis completed")

        except Exception as e:
            messagebox.showerror("Error", f"Failed to analyze admin menu: {e}")
    ...
    def update_reports_menu_with_missing_functions(self):
        ...
        try:
            ...
            # Display report
            report = "Reports Menu Function Analysis\n"
            report += "=" * 60 + "\n\n"
            report += f"Available Functions: {len(available_functions)}\n"
            report += f"Missing Functions: {len(missing_functions)}\n\n"
            ...
            report += "Available Functions by Category:\n"
            report += "-" * 60 + "\n"
            current_category = None
            for category, func_name, display_name in sorted(available_functions):
                if category != current_category:
            ...
            # Show report
            result_window = tk.Toplevel(self.gui.root)
            result_window.title("Reports Menu Analysis")
            ...
            ttk.Button(btn_frame, text="Close", command=result_window.destroy).pack(side='left', padx=5)
                    ...
                    try:
                        ...
                        if filename:
                            with open(filename, 'w') as f:
                                f.write("Missing Report Functions\n")
                                f.write("=" * 60 + "\n\n")
                                for category, func_name, display_name in missing_functions:
                                    f.write(f"{category}: {func_name} - {display_name}\n")
                            messagebox.showinfo("Success", f"Missing functions exported to {filename}")
                    except Exception as e:
                        messagebox.showerror("Error", f"Export failed: {e}")

                ttk.Button(btn_frame, text="Export Missing", command=export_missing).pack(side='left', padx=5)

            self.update_status("Reports menu analysis completed")

        except Exception as e:
            messagebox.showerror("Error", f"Failed to analyze reports menu: {e}")
