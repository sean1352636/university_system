...
def create_search_filter_bar(parent, on_search_callback, filters: Optional[List[str]] = None) -> Tuple[tk.Entry, ttk.Combobox]:
    ...
    # Search button
    ttk.Button(filter_frame, text="Search", command=on_search_callback).pack(side='left', padx=(0, 20))
...
def validate_date(date_str: str) -> bool:
    ...
    from university_system.modules.shared.utils.input_validation import is_valid_date
...
def export_to_csv(data: List[Dict], filename: str):
    ...
    """
    if not data:
        show_warning("Export", "No data to export")
        return
    ...
    try:
        ...
        if filepath:
            ...
            show_success("Export", f"Data exported to {filepath}")
