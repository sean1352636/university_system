...
class BatchOperationManager:
    ...
    def display_batch_menu(self):
        ...
        while True:
            print("\n" + "="*60)
            print("ENHANCED BATCH OPERATIONS MENU")
            print("="*60)
            print("\n📊 IMPORT OPERATIONS")
            print("1.  Import Students from CSV")
            print("2.  Import Students from Excel") 
            print("3.  Multi-file Import")
            print("4.  Import with Duplicate Detection")
            print("5.  Preview Import (No Changes)")
            print("6.  Resume Failed Import")
            
            print("\n✏️  UPDATE OPERATIONS")
            print("7.  Batch Update Existing Records")
            print("8.  Bulk Module Operations")
            print("9.  Import Grade Data")
            
            print("\n📤 EXPORT OPERATIONS")
            print("10. Export Students to File")
            print("11. Export Enrollment Statistics")
            print("12. Generate Import Reports")
            
            print("\n🔍 DATA QUALITY")
            print("13. Validate and Clean Data")
            print("14. Find Duplicate Students")
            print("15. Data Quality Dashboard")
            
            print("\n🛠️  UTILITIES")
            print("16. Generate Import Template")
            print("17. Create Database Backup")
            print("18. Undo Last Import")
            print("19. Import History")
            
            print("\n🤖 AUTOMATION")
            print("20. Schedule Automated Imports")
            print("21. Start API Server")
            print("22. External System Integration")
    ...
    def get_import_file_path(self, file_type: str) -> Optional[str]:
        ...
        while True:
            file_path = input(f"Enter the path to your {file_type} file: ")
    ...
    def import_from_csv(self):
        ...
        try:
            ...
            with open(file_path, 'r', newline='', encoding='utf-8-sig') as csvfile:
                ...
                print(f"\n✅ Validation complete: {len(valid_records)} valid records, {len(error_records)} records with errors")
                
                if error_records:
                    self.display_validation_errors(error_records)
                    
                    choice = input("\nWhat would you like to do?\n1. Import valid records only\n2. Fix errors interactively\n3. Cancel\nChoice (1-3): ")
    ...
    def import_from_excel(self):
        ...
        try:
            ...
            if len(xl_file.sheet_names) > 1:
                ...
                while True:
                    try:
                        sheet_choice = int(input("Select sheet number: ")) - 1
            ...
            if df.empty:
                print("❌ Error: Excel sheet has no data")
                return
            ...
            total_records = len(records)
            print(f"📋 Found {total_records} records in sheet '{sheet_name}'")
    ...
    def multi_file_import(self):
        ...
        directory = input("Enter directory path containing import files: ")
        ...
        if not files:
            print("❌ No supported files found (CSV, Excel)")
            return
        ...
        for file_path in files:
            ...
            try:
                ...
                if records:
                    ...
                    print(f"✅ {result.successful_imports} records imported from {file_path.name}")
    ...
    def import_with_duplicate_detection(self):
        ...
        print(f"\n📋 Processing {len(records)} records for duplicates...")
        ...
        if duplicates:
            ...
            print("2. Update existing records with new data")
            print("3. Handle each duplicate individually")
            print("4. Import as new records anyway")
            ...
            print("✅ No duplicates found, proceeding with import...")
            result = self.import_valid_records(records)
            self.save_import_history(result, file_path, 'Import with Duplicate Detection')

    def preview_import(self):
        """Preview import changes without committing"""
        print("\n👁️  Preview Import (No Changes)")
        ...
        print(f"\n📋 Preview of {len(records)} records:")
        print("-" * 80)
        ...
        duplicates = self.find_duplicates_in_import(records)
        if duplicates:
            print(f"\n⚠️  {len(duplicates)} potential duplicates detected")
        
        # Show what modules would be assigned
        sample_record = records[0]
        course = sample_record.get('course', 'CS')
        print(f"\n📚 Modules that would be assigned to {course} students:")
    ...
    def resume_failed_import(self):
        ...
        if not resume_files:
            print("❌ No failed imports found to resume")
            return
        
        print("\n📋 Available imports to resume:")
        for i, file in enumerate(resume_files, 1):
            # Extract timestamp from filename
            timestamp = file.stem.replace('import_resume_', '')
            print(f"{i}. Import from {timestamp}")
        
        try:
            choice = int(input("Select import to resume (number): ")) - 1
            if 0 <= choice < len(resume_files):
                ...
                print(f"\n📊 Resuming import: {processed_count}/{original_total} records already processed")
                print(f"🔄 {len(remaining_records)} records remaining")
                
                confirm = input("Continue with remaining records? (y/n): ")
                if confirm.lower() == 'y':
                    result = self.import_valid_records(remaining_records)
                    
                    # Clean up resume file
                    os.remove(resume_file)
                    print(f"✅ Resume file cleaned up")
            ...
            print(f"❌ Error resuming import: {e}")
    ...
    def display_validation_errors(self, error_records: List[Dict], max_display: int = 10):
        ...
        print(f"\n❌ Records with errors (showing first {min(len(error_records), max_display)}):")
        print("-" * 80)
        
        for i, record in enumerate(error_records[:max_display], 1):
            print(f"\n{i}. Row {record['row']}:")
            print(f"   Name: {record['data'].get('first_name', 'N/A')} {record['data'].get('last_name', 'N/A')}")
    ...
    def interactive_error_resolution(self, error_records: List[Dict]) -> List[Dict]:
        ...
        print("\n🛠️  Interactive Error Resolution")
        fixed_records = []
        
        for i, error_record in enumerate(error_records, 1):
            print(f"\n--- Error {i}/{len(error_records)} ---")
            print(f"Row {error_record['row']}: {error_record['data'].get('first_name', 'N/A')} {error_record['data'].get('last_name', 'N/A')}")
            print(f"Errors: {'; '.join(error_record['errors'])}")
            
            print("\nOptions:")
            print("1. Fix this record")
            print("2. Skip this record")
    ...
    def fix_record_interactive(self, record: Dict) -> Optional[Dict]:
        ...
        for field in editable_fields:
            ...
            if field == 'gender':
                print("Valid options: male, female, other")
            elif field == 'dob':
                print("Format: YYYY-MM-DD")
            elif field == 'course':
                print("Valid options: CS, DS")
        ...
        if errors:
            print(f"\n❌ Still has errors: {'; '.join(errors)}")
            retry = input("Try fixing again? (y/n): ")
            if retry.lower() == 'y':
                return self.fix_record_interactive(fixed_record)
            else:
                return None
        else:
            print("✅ Record fixed successfully!")
            return fixed_record
    ...
    def import_valid_records(self, records: List[Dict]) -> ImportResult:
        ...
        try:
            ...
            conn.commit()
            print(f"\n✅ Successfully imported {result.successful_imports} student records")
            
            if result.failed_imports > 0:
                print(f"❌ {result.failed_imports} records failed to import")
    ...
    def batch_update_records(self):
        ...
        print("\n✏️  Batch Update Existing Records")
        
        file_type = input("Update from (1) CSV or (2) Excel file? Enter 1 or 2: ")
        ...
        try:
            ...
            if file_type == '1':  # CSV
                with open(file_path, 'r', newline='', encoding='utf-8-sig') as csvfile:
                    ...
                    if 'student_id' not in reader.fieldnames:
                        print("❌ Error: File must contain 'student_id' column for updates")
                        return
            ...
            print(f"\n📊 Validation complete: {len(valid_records)} valid, {len(error_records)} errors")
            
            if error_records:
                self.display_validation_errors(error_records)
                
                proceed = input("\nProceed with valid records only? (y/n): ")
                if proceed.lower() != 'y':
            ...
            if valid_records:
                ...
                print("❌ No valid records to update")
    ...
    def update_batch_records(self, records: List[Dict]) -> ImportResult:
        ...
        try:
            ...
            conn.commit()
            print(f"\n✅ Successfully updated {result.successful_imports} records")
            
            if result.failed_imports > 0:
                print(f"❌ {result.failed_imports} updates failed")
    ...
    def bulk_module_operations(self):
        ...
        print("\nOptions:")
        print("1. Add module to multiple students")
        print("2. Remove module from multiple students")
        print("3. Replace module for multiple students")
        print("4. Import module enrollments from file")
    ...
    def bulk_add_modules(self):
        ...
        filter_choice = input("Select students by:\n1. Course (CS/DS)\n2. Upload student ID list\n3. All students\nChoice (1-3): ")
        
        student_ids = []
        if filter_choice == '1':
            course = input("Enter course (CS/DS): ").upper()
            if course in ['CS', 'DS']:
        ...
        module_type = input("Enter module type (compulsory/optional/CS/DS): ")
        
        # Confirm operation
        confirm = input(f"Add module {module_code} to {len(student_ids)} students? (y/n): ")
        if confirm.lower() != 'y':
            return
    ...
    def bulk_remove_modules(self):
        ...
        module_code = input("Enter module code to remove: ")
        ...
        try:
            ...
            if not student_ids:
                print(f"❌ No students found with module {module_code}")
                return
            
            print(f"📋 Found {len(student_ids)} students with module {module_code}")
            
            confirm = input(f"Remove module {module_code} from {len(student_ids)} students? (y/n): ")
            if confirm.lower() == 'y':
    ...
    def bulk_replace_modules(self):
        ...
        old_module = input("Enter module code to replace: ")
        new_module_code = input("Enter new module code: ")
        new_module_name = input("Enter new module name: ")
        new_module_type = input("Enter new module type: ")
        ...
        try:
            ...
            if not student_ids:
                print(f"❌ No students found with module {old_module}")
                return
            
            print(f"📋 Found {len(student_ids)} students with module {old_module}")
            
            confirm = input(f"Replace {old_module} with {new_module_code} for {len(student_ids)} students? (y/n): ")
            if confirm.lower() == 'y':
                # Remove old module
    ...
    def import_module_enrollments(self):
        ...
        try:
            ...
            confirm = input("Import these enrollments? (y/n): ")
            if confirm.lower() == 'y':
    ...
    def execute_bulk_module_operation(self, operation: str, student_ids: List[str], module_code: str, module_name: str = None, module_type: str = None):
        ...
        try:
            ...
            conn.commit()
            print(f"\n✅ Successfully {operation}ed module for {success_count} students")
                ...
                for row in reader:
                    ...
                    try:
                        ...
                        if 0 <= grade_value <= 100:
                            ...
                            print(f"⚠️  Invalid grade {grade_value} for student {row['student_id']}")
                    except ValueError:
                        print(f"⚠️  Invalid grade format '{row['grade']}' for student {row['student_id']}")
            
            print(f"📋 Found {len(grades)} valid grade records")
            
            if grades:
                confirm = input("Import these grades? (y/n): ")
                if confirm.lower() == 'y':
    ...
    def process_grade_data(self, grades: List[Dict]):
        ...
        try:
            ...
            conn.commit()
            print(f"\n✅ Successfully imported {success_count} grade records")
    ...
    def export_students_to_file(self):
        ...
        format_choice = input("Export format:\n1. CSV\n2. Excel\n3. JSON\nChoice (1-3): ")
        ...
        print("3. Filter by registration date range")
        ...
        try:
            ...
            print(f"📋 Found {len(results)} students to export")
    ...
    def export_enrollment_statistics(self):
        ...
        try:
            ...
            print(f"✅ Enrollment statistics exported to {filename}")
            
            # Display summary
            print(f"\n📊 Summary:")
            print(f"Course Enrollment: {stats['course_enrollment']}")
            print(f"Total Students: {sum(stats['course_enrollment'].values())}")
            print(f"Most Popular Module: {stats['module_enrollment'][0]['module_name']} ({stats['module_enrollment'][0]['enrollment']} students)")
    ...
    def generate_import_reports(self):
        ...
        if not self.import_history:
            print("❌ No import history available")
            return
        
        print("\nReport options:")
        print("1. Import success rates")
        print("2. Common error analysis")
        print("3. Performance trends")
        print("4. Comprehensive report")
        
        choice = input("Choose report type (1-4): ")
    ...
    def generate_success_rate_report(self):
        ...
        print(f"✅ Success rate report exported to {filename}")
    ...
    def generate_error_analysis_report(self):
        ...
        if not all_errors:
            print("✅ No errors found in import history")
            return
        ...
        print(f"✅ Error analysis report exported to {filename}")
    ...
    def generate_performance_report(self):
        ...
        print(f"⚡ Average performance: {avg_performance:.1f} records/second")
    ...
    def generate_comprehensive_report(self):
        ...
        print("✅ Comprehensive report generated - check individual report files")
    ...
    def validate_and_clean_data(self):
        ...
        try:
            ...
            print(f"📋 Analyzing {len(students)} student records...")
            ...
            print(f"🔧 Auto-fixes applied: {fixed_count}")
            
            if issues:
                print(f"\n🔍 First 10 issues found:")
                for i, issue in enumerate(issues[:10], 1):
                    print(f"{i}. Student {issue['student_id']} ({issue['name']}): {', '.join(issue['issues'])}")
                ...
                print(f"📄 Full issues report saved to {filename}")
    ...
    def find_duplicate_students(self):
        ...
        try:
            ...
            print(f"📋 Analyzing {len(students)} students for duplicates...")
            ...
            if duplicates:
                ...
                print(f"📄 Full duplicates report saved to {filename}")
                
                # Offer to merge duplicates
                merge_choice = input("\nWould you like to merge any duplicates? (y/n): ")
    ...
    def interactive_duplicate_merger(self, duplicates: List[Dict]):
        ...
        print("\n🔧 Interactive Duplicate Merger")
        
        for i, dup in enumerate(duplicates, 1):
            print(f"\n--- Duplicate Pair {i}/{len(duplicates)} ---")
            print(f"Student 1: {dup['student1']['name']} (ID: {dup['student1']['id']})")
            print(f"           Email: {dup['student1']['email']}, DOB: {dup['student1']['dob']}")
            print(f"Student 2: {dup['student2']['name']} (ID: {dup['student2']['id']})")
            print(f"           Email: {dup['student2']['email']}, DOB: {dup['student2']['dob']}")
            print(f"Confidence: {dup['confidence']:.0%}")
            
            print("\nOptions:")
            print("1. Keep Student 1, delete Student 2")
            print("2. Keep Student 2, delete Student 1")
            print("3. Merge (manual selection of best data)")
            print("4. Skip this pair")
            print("5. Skip all remaining")
    ...
    def generate_import_template(self):
        ...
        print("1. New Student Import")
        print("2. Student Update")
        print("3. Module Enrollment")
        print("4. Grade Import")
        print("5. Custom Template")
        
        template_type = input("Choose template type (1-5): ")
        
        if template_type not in ['1', '2', '3', '4', '5']:
            print("❌ Invalid choice")
            return
        
        file_format = input("Format: (1) CSV or (2) Excel? Enter 1 or 2: ")
        ...
        # Get output location
        output_path = input(f"Save as (default: {filename}): ").strip()
        if not output_path:
    ...
    def undo_last_import(self):
        ...
        if not self.import_history:
            print("❌ No imports to undo")
            return
        
        last_import = self.import_history[-1]
        
        print(f"Last import operation:")
        print(f"📅 Date: {last_import['timestamp']}")
        print(f"📄 Operation: {last_import['operation_type']}")
        print(f"📊 Records imported: {last_import['successful_imports']}")
        print(f"📁 File: {last_import['file_path']}")
        
        confirm = input("\n⚠️  This will delete all records from the last import. Continue? (y/n): ")
        if confirm.lower() != 'y':
        ...
        try:
            ...
            if not student_ids_to_delete:
                print("❌ No records found to undo")
                return
            
            print(f"🗑️  Found {len(student_ids_to_delete)} records to delete")
            ...
            conn.rollback()
            print(f"❌ Database error during undo: {e}")
        finally:
            conn.close()
    ...
    def show_import_history(self):
        ...
        print(f"\nShowing last {min(20, len(self.import_history))} operations:")
        print("-" * 100)
        ...
        print(f"Success Rate: {(total_successful/total_records*100):.1f}%" if total_records > 0 else "Success Rate: N/A")
    ...
    def process_module_enrollments(self, enrollments: List[Dict]):
        ...
        try:
            ...
            conn.commit()
            print(f"\n✅ Successfully processed {success_count} enrollments")
            if error_count > 0:
                print(f"❌ {error_count} enrollments failed")
    ...
    def schedule_automated_imports(self):
        ...
        print("1. Daily import from specific directory")
        print("2. Weekly import with email notifications")
        print("3. Custom schedule")
        print("4. View scheduled tasks")
        print("5. Cancel scheduled task")
    ...
    def setup_daily_import(self):
        ...
        import_dir = input("Enter directory to monitor for import files: ")
        if not os.path.exists(import_dir):
            print("❌ Directory does not exist")
            return
        
        import_time = input("Enter time for daily import (HH:MM, 24-hour format): ")
        try:
        ...
        print(f"✅ Scheduled daily import from {import_dir} at {import_time}")
        print("📝 Note: This schedule is active only while the application is running")

    def setup_weekly_import(self):
        """Set up weekly automated import with notifications"""
        print("\n📅 Setup Weekly Import")
        
        import_dir = input("Enter directory to monitor: ")
        if not os.path.exists(import_dir):
            print("❌ Directory does not exist")
            return
        
        day_of_week = input("Enter day of week (monday, tuesday, etc.): ").lower()
        import_time = input("Enter time (HH:MM): ")
        email = input("Enter email for notifications (optional): ")
        ...
        print(f"✅ Scheduled weekly import on {day_of_week} at {import_time}")

    def setup_custom_schedule(self):
        """Set up custom schedule using cron-like syntax"""
        print("\n🔧 Setup Custom Schedule")
        print("Enter cron-like schedule (e.g., '0 9 * * 1' for Monday 9 AM)")
        cron_expr = input("Cron expression: ")
        task_name = input("Task name: ")
        print(f"✅ Custom schedule created: {task_name}")
    ...
    def cancel_scheduled_task(self):
        ...
        jobs = schedule.get_jobs()
        if not jobs:
            print("❌ No scheduled tasks to cancel")
            return
        ...
        try:
            choice = int(input("Select task to cancel (number): ")) - 1
    ...
    def send_notification_email(self, email: str, message: str):
        ...
        try:
            ...
            try:
                ...
                print(f"📧 Email notification would be sent to {email}: {message}")
                return False
    ...
    def external_system_integration(self):
        ...
        print("1. Database connection (MySQL, PostgreSQL)")
        print("2. REST API integration")
        print("3. File share monitoring (FTP, SFTP)")
        print("4. Export to external system")
        
        choice = input("Choose integration type (1-4): ")
