...
def _get_state():
    ...
    if state is None:
        try:
            from university_system.infrastructure.email import state as _state
            state = _state
        except ImportError:
            try:
                from university_system.modules.shared.utils import state as _state
                state = _state
            except ImportError:
                pass
    return state
...
def _init_log_management():
    ...
    try:
        from university_system.utils.logging.log_management import (
