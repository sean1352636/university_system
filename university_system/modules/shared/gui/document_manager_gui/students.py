    ...
    def send_report_to_admin(self, title, columns, data):
        ...
        def send_email():
            ...
            try:
                ...
                try:
                    from university_system.infrastructure.email.email_service import send_email_via_smtp
                    ...
                    cursor = conn.cursor()
                    cursor.execute('''
                        INSERT INTO notifications (recipient_id, notification_type, title, message, created_date, is_sent)
                        VALUES (?, ?, ?, ?, datetime('now'), 0)
                    ''', (recipient, 'report_email', subject, body))
                    conn.commit()
                    conn.close()
    ...
    def load_students_data(self):
        ...
        try:
            ...
            for student in students:
                self.gui.students_tree.insert('', 'end', values=student)
