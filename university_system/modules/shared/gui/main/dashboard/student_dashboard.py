...
def _launch_profile(parent, auth):
    ...
    from university_system.modules.domain.student_affairs.gui.student_profile.profile_gui import StudentProfileGUI
    StudentProfileGUI(parent, auth=auth)


def _launch_security(parent, auth):
    """Launch Account Security Dashboard."""
    from university_system.modules.domain.student_affairs.gui.account_security.security_gui import AccountSecurityGUI
    AccountSecurityGUI(parent, auth=auth)


def _launch_notifications(parent, auth):
    """Launch Notification Preferences."""
    from university_system.modules.domain.student_affairs.gui.notification_prefs.notification_prefs_gui import NotificationPrefsGUI
    NotificationPrefsGUI(parent, auth=auth)


def _launch_grades_breakdown(parent, auth):
    """Launch Grades Breakdown by Module."""
    from university_system.modules.domain.academics.gui.grades_breakdown.grades_breakdown_gui import GradesBreakdownGUI
    GradesBreakdownGUI(parent, auth=auth)


def _launch_degree_progress(parent, auth):
    """Launch Degree Progress Tracker."""
    from university_system.modules.domain.academics.gui.degree_progress.degree_progress_gui import DegreeProgressGUI
    DegreeProgressGUI(parent, auth=auth)


def _launch_course_catalog(parent, auth):
    """Launch Course Catalog & Registration."""
    from university_system.modules.domain.academics.gui.course_catalog.course_catalog_gui import CourseCatalogGUI
    CourseCatalogGUI(parent, auth=auth)


def _launch_gpa_calculator(parent, auth):
    """Launch What-If GPA Calculator."""
    from university_system.modules.domain.academics.gui.gpa_calculator.gpa_calculator_gui import GPACalculatorGUI
    GPACalculatorGUI(parent, auth=auth)


def _launch_messaging(parent, auth):
    """Launch Student Messaging Hub."""
    from university_system.modules.domain.student_affairs.gui.messaging_hub.messaging_hub_gui import MessagingHubGUI
    MessagingHubGUI(parent, auth=auth)


def _launch_forums(parent, auth):
    """Launch Course Discussion Forums."""
    from university_system.modules.domain.academics.gui.course_forums.course_forums_gui import CourseForumsGUI
    CourseForumsGUI(parent, auth=auth)


def _launch_finance(parent, auth):
    """Launch Student Financial Dashboard."""
    from university_system.modules.domain.finance.gui.student_finance.student_finance_gui import StudentFinanceGUI
    StudentFinanceGUI(parent, auth=auth)


def _launch_help_center(parent, auth):
    """Launch Integrated Help Center."""
    from university_system.modules.domain.student_affairs.gui.help_center.help_center_gui import HelpCenterGUI
    HelpCenterGUI(parent, auth=auth)


def _launch_documents(parent, auth):
    """Launch Personal Document Center."""
    from university_system.modules.domain.student_affairs.gui.document_center.document_center_gui import DocumentCenterGUI
...
def create_student_dashboard(parent_frame, auth, service):
    ...
    try:
        from university_system.modules.shared.gui.main.dashboard.student_widgets import (
