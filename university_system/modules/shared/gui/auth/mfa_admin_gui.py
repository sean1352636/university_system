...
def _ensure_mfa_tables_exist():
    ...
    try:
        from university_system.infrastructure.database.migrations.add_mfa_system import run_migration
        run_migration()
...
class MFAAdminPanel(tk.Toplevel):
    ...
    def _load_settings(self):
        ...
        try:
            from university_system.infrastructure.auth.sms_provider import get_sms_service
        ...
        try:
            from university_system.infrastructure.auth.email_otp_service import get_email_service
    ...
    def _view_user_details(self):
        ...
        username = item['values'][0] if item['values'] else "Unknown"
        ...
        try:
            ...
            for row in cursor.fetchall():
                status = "Primary" if row[1] else "Secondary"
                methods_tree.insert('', 'end', values=(
                    row[0].upper(), status, row[2] or 'N/A', row[3] or 'Never'
                ))
            ...
            for row in cursor.fetchall():
                result = "Success" if row[2] else "Failed"
                history_tree.insert('', 'end', values=(row[0], row[1].upper(), result, row[3] or 'N/A'))
