...
class StatusBar(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Button(
            right_frame,
            text="🏠 " + _t("activity_logger.return_main"),
...
"""
            ...
            ttk.Button(button_frame, text="📄 " + _t("activity_logger.report.formats.txt"), command=lambda: save_as_format('txt')).pack(side=tk.LEFT, padx=2)
            ttk.Button(button_frame, text="📋 " + _t("activity_logger.report.formats.json"), command=lambda: save_as_format('json')).pack(side=tk.LEFT, padx=2)
            ttk.Button(button_frame, text="📊 " + _t("activity_logger.report.formats.csv"), command=lambda: save_as_format('csv')).pack(side=tk.LEFT, padx=2)

            ttk.Separator(button_frame, orient='vertical').pack(side=tk.LEFT, fill=tk.Y, padx=10)

            ttk.Button(button_frame, text="📧 " + _t("activity_logger.report.send_to_admin"), command=send_to_admin).pack(side=tk.LEFT, padx=5)
            ttk.Button(button_frame, text="🏠 " + _t("activity_logger.report.close"), command=report_window.destroy).pack(side=tk.RIGHT, padx=5)
...
class EnhancedActivityLoggerGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(
            title_frame,
            text="🌐 " + _t("activity_logger.language.change"),
        ...
        ttk.Button(
            status_frame,
            text="🏠 " + _t("activity_logger.return_home"),
        ...
        self.notebook.add(self.log_viewer_tab, text="📊 " + _t("activity_logger.tabs.live_logs"))

        self.analytics_tab = AnalyticsTab(self.notebook, self)
        self.notebook.add(self.analytics_tab, text="📈 " + _t("activity_logger.tabs.analytics"))

        self.security_tab = SecurityTab(self.notebook, self)
        self.notebook.add(self.security_tab, text="🔒 " + _t("activity_logger.tabs.security"))
        ...
        self.notebook.add(self.plugin_tab, text="🔌 " + _t("activity_logger.tabs.plugins"))

        self.query_tab = QueryTab(self.notebook, self)
        self.notebook.add(self.query_tab, text="🔍 " + _t("activity_logger.tabs.query"))
