...
class StudentAnalytics:
    ...
    def analyze_course_enrollments(self):
        ...
        summary_text += f"Total courses offered: {students_df['course'].nunique()}\n"
        summary_text += f"Total enrolled students: {len(students_df)}\n"
        summary_text += f"Average students per course: {len(students_df) / students_df['course'].nunique():.1f}\n"
    ...
    def analyze_registration_timeline(self):
        ...
        summary_text += f"Analysis period: {students_df['registration_date'].min().strftime('%Y-%m-%d')} to {students_df['registration_date'].max().strftime('%Y-%m-%d')}\n"
    ...
    def analyze_engagement(self):
        ...
        ax6 = fig.add_subplot(336)
        age_groups = pd.cut(students_df['age'], bins=[0, 25, 35, 45, 100], 
                           labels=['18-25', '26-35', '36-45', '46+'])
        age_engagement = students_df.groupby(age_groups)['engagement_score'].agg(['mean', 'std'])
        
        x_pos = range(len(age_engagement))
        ax6.bar(x_pos, age_engagement['mean'], yerr=age_engagement['std'], 
    ...
    def analyze_cohorts(self):
        ...
        age_cohort_completion = students_df.groupby(['age_group', 'cohort_month']).apply(
            lambda x: (x['completion_status'] == 'Completed').mean() * 100
    ...
    def analyze_module_popularity(self):
        ...
        ax1 = fig.add_subplot(331)
        module_counts = modules_df['module_name'].value_counts().head(15)
        ...
        ax2 = fig.add_subplot(332)
        type_stats = modules_df.groupby('module_type').agg({
            'student_id': 'count',
            'module_grade': lambda x: (x.isin(['A', 'B'])).mean() * 100
        })
        ...
        ax4 = fig.add_subplot(334)
        course_module_prefs = modules_df.groupby(['course', 'module_type']).size().unstack(fill_value=0)
        ...
        ax5 = fig.add_subplot(335)
        module_analysis = modules_df.groupby('module_name').agg({
            'student_id': 'count',
            'difficulty_rating': 'mean',
            'module_grade': lambda x: (x.isin(['A', 'B'])).mean() * 100
        })
        ...
        scatter = ax5.scatter(module_analysis['Avg_Difficulty'], module_analysis['Enrollment'], 
                             s=module_analysis['Success_Rate']*3, alpha=0.6, 
        ...
        ax6 = fig.add_subplot(336)
        attendance_module = modules_df.groupby('module_type')['attendance_rate'].mean()
        ...
        ax7 = fig.add_subplot(337)
        completion_rates = modules_df.groupby('module_name').apply(
        ...
        module_scores = modules_df.groupby('module_name').agg({
            'student_id': 'count',  # Popularity
            'module_grade': lambda x: (x.isin(['A', 'B'])).mean(),  # Success rate
            'attendance_rate': 'mean',  # Engagement
            'difficulty_rating': lambda x: 5 - x.mean()  # Inverse difficulty
        })
        
        # Normalize scores
        for col in module_scores.columns:
            module_scores[col] = (module_scores[col] - module_scores[col].min()) / (module_scores[col].max() - module_scores[col].min())
    ...
    def generate_complete_report(self):
        ...
        with PdfPages(filename) as pdf:
            ...
            insights = [
                f"Highest performing course: {students_df.groupby('course')['gpa'].mean().idxmax()}",
                f"Most popular age group: {pd.cut(students_df['age'], bins=[0, 25, 35, 45, 100], labels=['18-25', '26-35', '36-45', '46+']).mode().iloc[0]}",
                f"Engagement-GPA correlation: {students_df['engagement_score'].corr(students_df['gpa']):.3f}",
