...
from university_system.modules.core.services.health_misc.security import truthy
...
def import_templates_from_path(auth, source_path: str | None = None):
    ...
    try:
        ...
        print(f"Template import aborted (schema): {e}")
        return
    ...
    try:
        ...
        try:
            ...
            for r in records:
                try:
                    ...
                    skipped += 1
                    print(f"Skipping '{r.get('name')}': {ie}")
            conn.commit()
        ...
        if updated == 0 and skipped == 0 and inserted > 0:
            print(f"Imported templates: {inserted} processed (inserted/updated).")
        else:
            print(f"Imported templates: {inserted} inserted, {updated} updated, {skipped} skipped.")
    except Exception as e:
        try:
            conn.rollback()
        except Exception:
