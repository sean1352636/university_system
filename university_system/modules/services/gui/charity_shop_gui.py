        ...
        with self.get_connection() as conn:
            ...
            if 'sold' not in columns:
                ...
                if 'id' in columns:
                    ...
                    conn.execute(f"ALTER TABLE {self.TABLE_NAME} ADD COLUMN sold INTEGER DEFAULT 0")
                    conn.execute(f"ALTER TABLE {self.TABLE_NAME} ADD COLUMN sold_date TEXT")
                    conn.execute(f"ALTER TABLE {self.TABLE_NAME} ADD COLUMN sold_quantity INTEGER DEFAULT 0")
                else:
                    # Create new table
                    conn.execute(f"""
