...
try:
    from university_system.infrastructure.auth.sms_provider import send_otp as send_sms_otp
...
try:
    from university_system.infrastructure.auth.email_otp_service import send_otp as send_email_otp
...
class MFAService:
    ...
    def _ensure_schema(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            # Add verification_disabled column if it doesn't exist
    ...
    def generate_email_otp(self, user_id: int, email: str) -> Dict:
        ...
        is_in_use, existing_user = self._is_mfa_contact_in_use('email', email, exclude_user_id=user_id)
        if is_in_use:
            return {
                'success': False,
                'error': f'This email address is already registered for MFA by another user (User ID: {existing_user}). Each email can only be linked to one account.'
            }
