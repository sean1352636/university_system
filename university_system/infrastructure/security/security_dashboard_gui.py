...
class SecurityDashboard(tk.Toplevel):
    ...
    def _load_incidents(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to load incidents: {str(e)}")
    ...
    def _generate_compliance_report(self, report_type: str):
        ...
        def send_to_admin():
            ...
            try:
                ...
                subject = f"{report_type.upper()} Compliance Report - {datetime.now().strftime('%Y-%m-%d')}"

                body = f"""Dear {admin_name},

Please find attached the {report_type.upper()} compliance report generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}.
...
This report was automatically generated by the Security & Compliance Dashboard.
...
"""
        ...
        ttk.Button(btn_frame, text="📧 Send to Admin", command=send_to_admin).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="🏠 Return to Home", command=report_window.destroy).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="❌ Close", command=report_window.destroy).pack(side=tk.RIGHT, padx=5)
