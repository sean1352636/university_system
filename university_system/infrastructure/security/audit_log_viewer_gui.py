...
class AuditLogViewerGUI(tk.Toplevel):
    ...
    def _update_statistics(self):
        ...
        try:
            with get_connection(self.audit_logger.db_path) as conn:
                # Total entries
                cursor = conn.execute(f"SELECT COUNT(*) FROM [{self.audit_logger.TABLE_NAME}]")
                self.stat_labels['total_entries'].config(text=str(cursor.fetchone()[0]))

                # Unique users
                cursor = conn.execute(f"""
                    SELECT COUNT(DISTINCT username) FROM [{self.audit_logger.TABLE_NAME}]
                    WHERE username IS NOT NULL
                """)
                ...
                # Logins today
                cursor = conn.execute(f"""
                    SELECT COUNT(*) FROM [{self.audit_logger.TABLE_NAME}]
...
"""
    ...
    def _export_csv(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            initialfilename=f"audit_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    ...
    def _export_pdf(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            initialfilename=f"audit_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        )
