...
class EmailQueueSchedulerGUI:
    ...
    def __init__(self, parent=None):
        ...
        self.window.title("Email Queue & Scheduler Manager")
    ...
    def create_worker_tab(self):
        ...
        self.notebook.add(tab, text="Worker Control")

        # Title
        title = tk.Label(tab, text="Email Worker Thread Control", font=("Arial", 16, "bold"))
        title.pack(pady=10)

        # Status frame
        status_frame = ttk.LabelFrame(tab, text="Worker Status", padding=10)
...
"""
    ...
    def create_queue_tab(self):
        ...
        tab = ttk.Frame(self.notebook)
        self.notebook.add(tab, text="Queue Emails")

        # Title
        title = tk.Label(tab, text="Queue Emails for Background Processing", font=("Arial", 16, "bold"))
        title.pack(pady=10)
        ...
        queue_notebook.add(direct_tab, text="Queue Direct Email")
        self.create_direct_queue_form(direct_tab)

        # Template Email Queue
        template_tab = ttk.Frame(queue_notebook)
        queue_notebook.add(template_tab, text="Queue Template Email")
    ...
    def create_direct_queue_form(self, parent):
        ...
        # Recipient
        ttk.Label(form_frame, text="Recipient Email:", font=("Arial", 10, "bold")).grid(
        ...
        # Subject
        ttk.Label(form_frame, text="Subject:", font=("Arial", 10, "bold")).grid(
        ...
        # CC
        ttk.Label(form_frame, text="CC (comma-separated):", font=("Arial", 10, "bold")).grid(
        ...
        # BCC
        ttk.Label(form_frame, text="BCC (comma-separated):", font=("Arial", 10, "bold")).grid(
        ...
        # Body
        ttk.Label(form_frame, text="Email Body:", font=("Arial", 10, "bold")).grid(
    ...
    def create_template_queue_form(self, parent):
        ...
        # Template selection
        ttk.Label(form_frame, text="Template:", font=("Arial", 10, "bold")).grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        ...
        # Template Variables
        ttk.Label(form_frame, text="Template Variables (JSON):", font=("Arial", 10, "bold")).grid(
        ...
        ttk.Button(
            form_frame,
            text="Add Template Email to Queue",
    ...
    def create_scheduler_tab(self):
        ...
        tab = ttk.Frame(self.notebook)
        self.notebook.add(tab, text="Schedule Emails")

        # Title
        title = tk.Label(tab, text="Schedule Emails for Future Delivery", font=("Arial", 16, "bold"))
        title.pack(pady=10)
    ...
    def create_schedule_form(self, parent):
        ...
        # Time selection
        ttk.Label(form_frame, text="Scheduled Time (HH:MM):", font=("Arial", 10, "bold")).grid(
            row=2, column=0, sticky=tk.W, pady=5
        )
        ...
        # Recipients
        ttk.Label(form_frame, text="Recipients (one per line):", font=("Arial", 10, "bold")).grid(
        ...
        ttk.Label(form_frame, text="Template Variables (JSON array):", font=("Arial", 10, "bold")).grid(
            row=4, column=0, sticky=tk.NW, pady=5
        )
    ...
    def create_scheduled_view(self, parent):
        ...
        list_frame = ttk.LabelFrame(parent, text="Scheduled Emails", padding=10)
        list_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=10)

        # Create treeview
        columns = ("ID", "Template", "Recipient", "Scheduled For", "Status")
    ...
    def create_monitor_tab(self):
        ...
        # Title
        title = tk.Label(tab, text="Email Queue & Worker Monitoring", font=("Arial", 16, "bold"))
        title.pack(pady=10)
        ...
        self.monitor_queue_size = tk.Label(status_frame, text="Queue Size: 0", font=("Arial", 12))
        self.monitor_queue_size.pack()

        self.monitor_workers = tk.Label(status_frame, text="Active Workers: 0", font=("Arial", 12))
        self.monitor_workers.pack()

        self.monitor_scheduled = tk.Label(status_frame, text="Pending Scheduled: 0", font=("Arial", 12))
        ...
        # Details
        details_frame = ttk.LabelFrame(tab, text="Details", padding=10)
...
"""
    ...
    def start_workers(self):
        ...
        try:
            ...
            if result:
                messagebox.showinfo("Success", "Email worker threads started successfully")
            else:
                messagebox.showwarning("Warning", "Workers already running or database-only mode enabled")
            self.refresh_status()
        except Exception as e:
            messagebox.showerror("Error", f"Failed to start workers: {str(e)}")
    ...
    def stop_workers(self):
        ...
        try:
            ...
            if result:
                messagebox.showinfo("Success", "Email worker threads stopped successfully")
            self.refresh_status()
        except Exception as e:
            messagebox.showerror("Error", f"Failed to stop workers: {str(e)}")
    ...
    def queue_direct_email(self):
        ...
        if not recipient or not subject or not body:
            messagebox.showerror("Error", "Recipient, subject, and body are required")
            return
        ...
        try:
            ...
            if result:
                messagebox.showinfo("Success", "Email queued successfully")
                # Clear form
                ...
                messagebox.showwarning("Warning", "Email was sent immediately (database-only mode)")

        except Exception as e:
            messagebox.showerror("Error", f"Failed to queue email: {str(e)}")
    ...
    def queue_template_email_action(self):
        ...
        if not template or not recipient:
            messagebox.showerror("Error", "Template and recipient are required")
            return
        ...
        try:
            ...
            if result:
                messagebox.showinfo("Success", "Template email queued successfully")
                # Clear form
            ...
            messagebox.showerror("Error", "Invalid JSON in template variables")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to queue email: {str(e)}")
    ...
    def schedule_emails(self):
        ...
        if not template or not recipients_str:
            messagebox.showerror("Error", "Template and recipients are required")
            return
        ...
        try:
            ...
            if result:
                messagebox.showinfo("Success", f"Scheduled {len(recipients)} emails successfully")
                self.refresh_scheduled_list()
            else:
                messagebox.showerror("Error", "Failed to schedule emails")

        except json.JSONDecodeError:
            messagebox.showerror("Error", "Invalid JSON in template variables")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to schedule emails: {str(e)}")

    def process_due_emails(self):
        """Process due scheduled emails"""
        try:
            count = process_scheduled_emails()
            messagebox.showinfo("Success", f"Processed {count} due scheduled emails")
            self.refresh_scheduled_list()
        except Exception as e:
    ...
    def refresh_scheduled_list(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to load scheduled emails: {str(e)}")
    ...
    def refresh_status(self):
        ...
        try:
            ...
            self.worker_count_label.config(text=f"Active Workers: {worker_count}")
    ...
    def refresh_monitor(self):
        ...
        try:
            ...
            # Update labels
            self.monitor_queue_size.config(text=f"Queue Size: {queue_size}")
            self.monitor_workers.config(text=f"Active Workers: {worker_count}")
            self.monitor_scheduled.config(text=f"Pending Scheduled: {scheduled_count}")
...
Queue Information:
- Current Queue Size: {queue_size} emails
- Worker Threads: {worker_count} active
...
Note: Queue is only used when not in database-only mode.
In database-only mode, emails are sent immediately.
"""
