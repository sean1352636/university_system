...
def safe_log_email(cursor, recipient, subject, sent_date, status, related_to=None, student_id=None, 
    ...
    try:
        ...
        log_event('warning', f"Could not log with extended columns: {e}")
        try:
            ...
            log_event('error', f"Could not log to email_log at all: {e2}")
            return False
...
def send_email_db_only(recipient_email, subject, body, cc, bcc, attachments, current_time):
    ...
    def _store_email(cursor):
        ...
        if user_row:
            ...
            if not sender_id:
                ...
                try:
                    ...
                    log_event('warning', f"System sender fallback failed: {e}")
        ...
        try:
            ...
            # Fallback to minimal log if extended columns aren’t available
            log_event('warning', f"Extended email_log insert failed, falling back: {e}")
            cursor.execute('''
    ...
    try:
        ...
        log_event('error', f"Error storing email: {e}")
        return False
...
def fix_inbox_display_issue():
    ...
    def _fix_missing_inbox_messages(cursor):
        ...
        print(f"Found {len(missing_messages)} stored emails missing from inboxes...")
        ...
        for email_data in missing_messages:
            ...
            try:
                ...
                if recipient_result:
                    ...
                    fixed_count += 1
                    print(f"✅ Added message to inbox: '{subject[:50]}...' for {recipient_email}")
                
            except Exception as e:
                print(f"❌ Failed to fix message for {recipient_email}: {e}")
        
        return fixed_count
    
    try:
        count = execute_db_operation(_fix_missing_inbox_messages)
        print(f"\n🎉 Fixed {count} missing inbox messages!")
        return count
    except Exception as e:
        print(f"❌ Error fixing inbox messages: {e}")
        return 0
...
def get_stored_emails(limit=50, offset=0, recipient_filter=None, date_filter=None):
    ...
    try:
        ...
        log_event('error', f"Error retrieving stored emails: {e}")
...
def delete_stored_email(email_id):
    ...
    try:
        ...
        log_event('error', f"Error deleting stored email: {e}")
        return False
...
def clear_stored_emails(older_than_days=None):
    ...
    try:
        ...
        log_event('error', f"Error clearing stored emails: {e}")
        return 0
    ...
    while not stop_workers_event.is_set():
        try:
            ...
            try:
                ...
                log_event('error', f"Worker processing error: {e}")
                success = False
            ...
            log_event('error', f"Unexpected error in email worker: {e}")
            # Add delay before continuing to prevent rapid error loops
            time.sleep(2.0)
    ...
    if config.get('database_only_mode', True):
        for i, recipient in enumerate(recipients):
            try:
                ...
                log_event('error', f"Error processing email for {recipient}: {e}")
                failure_count += 1
    else:
...
def schedule_send(datetime_obj, recipients, template_name, template_vars_list=None):
    ...
    def _schedule_emails(cursor):
        ...
        for i, recipient in enumerate(recipients):
            try:
                ...
                log_event('error', f"Error scheduling email for {recipient}: {e}")
                failure_count += 1
    ...
    try:
        ...
        log_event('error', f"Error scheduling emails: {e}")
        return {
            'total': len(recipients),
            'success': 0,
    ...
    try:
        ...
        for email in scheduled_emails:
            ...
            try:
                ...
                log_event('error', f"Error processing scheduled email {scheduled_id}: {e}")
                update_scheduled_email_status(scheduled_id, 'failed')
        ...
        log_event('error', f"Error processing scheduled emails: {e}")
        return 0
...
def run_scheduler():
    ...
    while True:
        try:
            ...
            log_event('error', f"Error in scheduler: {e}")
...
def update_scheduled_email_status(scheduled_id, status):
    ...
    try:
        ...
        log_event('error', f"Error updating scheduled email status: {e}")
        return False
...
def send_registration_confirmation(student_id):
    ...
    try:
        ...
        if result:
            ...
            # Queue the email
            return queue_template_email('registration_confirmation', email_address, template_vars)
        return False
    except Exception as e:
        log_event('error', f"Error sending registration confirmation: {e}")
        return False
...
def send_password_reset(student_id, reset_code):
    ...
    try:
        ...
        if result:
            template_vars, email_address = result
            # Queue the email
            return queue_template_email('password_reset', email_address, template_vars)
        return False
    except Exception as e:
...
def send_assignment_notification(assignment_id, assignment_title, module_code, due_date, description=None):
    ...
    def _send_notifications(cursor):
        ...
        WHERE sm.module_code = ? AND u.email IS NOT NULL
        ''', (module_code,))
...
def send_batch_email_form():
    ...
    print("\nEnter announcement body (type 'END' on a new line when finished):")
    body_lines = []
    while True:
        line = input()
        if line == 'END':
    ...
    # Get filter criteria
    print("\nFilter Recipients (leave empty to send to all students):")
    ...
    if config.get('database_only_mode', True):
        print(f"\nEmails stored in database for {total} students.")
        print(f"Success: {success}, Failed: {failed}")
    else:
        print(f"\nEmails queued for {total} students.")
        print(f"Success: {success}, Failed: {failed}")
...
def schedule_email_form():
    ...
    # Get recipients
    print("\nEnter recipients (one email per line, type 'END' to finish):")
    recipients = []
    while True:
        line = input()
        if line == 'END':
    ...
    try:
        ...
        print("Invalid date/time. Please enter valid numbers.")
        return
    ...
    if confirm.lower() != 'y':
        print("Email scheduling cancelled.")
        return
    
    # Schedule the emails
    result = schedule_send(scheduled_date, recipients, template_name)
    
    print(f"\nScheduled {result['success']} emails for {scheduled_date}")
    if result['failure'] > 0:
        print(f"Failed to schedule {result['failure']} emails")
    
    print(f"Scheduled IDs: {', '.join(map(str, result['scheduled_ids']))}")
...
def send_ticket_notification(ticket_id, subject, username, admin_list=None):
    ...
    def _send_ticket_notification(cursor):
        ...
        user_subject = f"Support Ticket #{ticket_id} Created"
        user_body = f"""Dear {username},
...
You will be notified when there are updates to your ticket.
            ...
            for admin in admin_list:
                ...
                def _notify_admin(cursor):
                    ...
                    if admin_result:
                        ...
                        admin_subject = f"New Support Ticket #{ticket_id}"
                        admin_body = f"""New support ticket has been created.
...
Please log in to the support system to view and respond to this ticket.
    ...
    def _send_reply_notification(cursor):
        ...
        if username:
            ...
            if user_result:
                ...
                if status_update:
                    user_subject = f"Support Ticket #{ticket_id} Status Changed"
                    user_body = f"""Dear {username},
...
New Status: {status_update.upper()}
Updated by: {responder}

You can log in to view the ticket details.

Thank you,
IT Support Team
"""
                else:
                    user_subject = f"New Reply to Support Ticket #{ticket_id}"
                    user_body = f"""Dear {username},
...
You can log in to view the reply and respond.
                ...
                if admin_username != responder:  # Don't notify the responder
                    def _notify_admin(cursor):
                        ...
                        if admin_result:
                            ...
                            if status_update:
                                admin_subject = f"Support Ticket #{ticket_id} Status Changed"
                                admin_body = f"""Support ticket status has been updated.
...
Please log in to the support system to view this ticket.

IT Support System
"""
                            else:
                                admin_subject = f"New Reply to Support Ticket #{ticket_id}"
                                admin_body = f"""There is a new reply to a support ticket.
...
def send_appointment_confirmation(student_id, appointment_id, appointment_date, appointment_time, provider, appointment_type):
    ...
    def _send_appointment_confirmation(cursor):
        ...
        subject = "University Health Portal - Appointment Confirmation"
        body = f"""Dear {first_name} {last_name},

Your appointment has been successfully scheduled with the University Health Center.
...
If you need to reschedule or cancel your appointment, please log in to the Health Portal
or contact the Health Center at health@university.edu or call 555-123-4567.
...
def send_health_notification(student_id, advisory_title, advisory_description, severity):
    ...
    def _send_health_notification(cursor):
        ...
        subject = f"HEALTH ADVISORY: {advisory_title}"
        body = f"""Dear {first_name} {last_name},

The University Health Center has issued the following health advisory:
...
Please take appropriate precautions and contact the Health Center if you have any questions.
...
def send_internship_notification(student_id, internship_id, status, feedback=None):
    ...
    def _send_internship_notification(cursor):
        ...
        if status == 'approved':
            template_name = "internship_application_approved"
        elif status == 'rejected':
            template_name = "internship_application_rejected"
        else:
...
def send_application_confirmation(student_id, internship_id):
    ...
    def _send_application_confirmation(cursor):
        ...
        # Construct message
        subject = f"Internship Application Confirmation: {internship_title} at {company}"
        
        message = f"""Dear {first_name} {last_name},

Thank you for applying to the '{internship_title}' internship at {company}.

Your application has been received and is currently under review. The application deadline is {deadline}.

We will notify you once your application has been reviewed.

Best regards,
University Internship Team"""
...
def send_alumni_welcome_email(alumni_id, email_address, full_name):
    ...
    if not (subject and message):
        ...
        subject = "Welcome to the University Alumni Network!"
        message = f"""Dear {full_name},

Welcome to the University Alumni Network! We're delighted that you've joined our community of graduates.

As a member of our alumni network, you'll receive:
- Invitations to exclusive alumni events and networking opportunities
- Regular updates on university news and achievements
- Access to career resources and continuous learning opportunities
- Chances to connect with fellow alumni across the globe

You can update your profile information and preferences at any time through the Alumni Portal.
...
def send_mentorship_notification(mentor_email, mentee_email, mentor_name, mentee_name, focus_area, start_date, end_date=None):
    ...
    A new mentorship has been set up between:

    Mentor: {mentor_name}
    Mentee: {mentee_name}
    Focus Area: {focus_area}
    Start Date: {start_date}{end_text}

    Please connect and schedule your first meeting soon.
...
def send_event_invitation(alumni_id, event_id=None, email_address=None, event_name=None, event_date=None, event_location=None):
    ...
    def _send_event_invitation(cursor):
        ...
        if not (subject and message):
            # Fallback if template fails
            subject = f"Invitation: {event_name} - University Alumni Event"
            message = f"""Dear Alumni,

We are pleased to invite you to our upcoming alumni event:

Event: {event_name}
Date: {event_date}
Location: {event_location}

This is a wonderful opportunity to reconnect with fellow alumni and stay engaged with the university community.
...
def send_donation_receipt(alumni_id, donation_id=None, email_address=None, amount=None, donation_date=None, purpose=None):
    ...
    def _send_donation_receipt(cursor):
        ...
        if not (subject and message):
            # Fallback if template fails
            subject = f"Thank You for Your Donation to the University"
            message = f"""Dear University Supporter,

Thank you for your generous donation of £{amount:.2f} on {donation_date}.

Your contribution helps us to continue our mission of providing excellent education and research opportunities.
"""
...
Thank you again for your support and commitment to our university community.
...
def send_permit_confirmation(permit_id, email, zone, permit_type, start_date, end_date):
    ...
    subject = f"Parking Permit Confirmation - {permit_id}"
    
    body = f"""Dear Permit Holder,

Your parking permit has been successfully created.
...
Please keep this confirmation for your records. Your permit is now active and ready for use.

If you have any questions, please contact the Parking Services office.
...
def send_permit_update_confirmation(permit_id, email, updated_fields):
    ...
    subject = f"Parking Permit Update Confirmation - {permit_id}"
...
Your parking permit has been successfully updated.
...
If you did not request these changes or notice any errors, please contact Parking Services immediately.
...
def send_book_checkout_confirmation(user_id, book_id, book_title, due_date):
    ...
    def _send_checkout_confirmation(cursor):
        ...
        body = f"""Dear {title or ''} {first_name} {last_name},

You have successfully checked out '{book_title}' (ID: {book_id}).
Please return the book by {due_date}.
...
This is a friendly reminder that '{book_title}' (ID: {book_id}) is due on {due_date}.
...
def send_overdue_notification(user_id, book_id, book_title, due_date, days_overdue):
    ...
    def _send_overdue_notification(cursor):
        ...
        subject = "OVERDUE NOTICE - Library Book"
        body = f"""Dear {title or ''} {first_name} {last_name},

The book '{book_title}' (ID: {book_id}) is overdue by {days_overdue} days.
It was due on {due_date}.
Please return the book as soon as possible to minimize fines.
...
@handle_exception
def display_stored_emails_menu():
    """Interactive menu to view stored emails"""
    while True:
        print("\nStored Emails Management:")
        print("=========================")
        print("1. View Recent Stored Emails")
        print("2. Search Stored Emails")
        print("3. View Email Details")
        print("4. Delete Stored Email")
        print("5. Clear Old Stored Emails")
        print("6. Clear All Stored Emails")
        print("7. Export Stored Emails to CSV")
        print("8. Back to Email Menu")
        
        choice = input("Choose an option (1-8): ")
        
        if choice == '1':
            # View recent emails
            emails_data = get_stored_emails(limit=20)
            
            if emails_data['emails']:
                print(f"\nRecent Stored Emails ({emails_data['total_count']} total):")
                print("=" * 100)
                print(f"{'ID':<5}{'Recipient':<30}{'Subject':<35}{'Date':<20}{'Template'}")
                print("-" * 100)
            ...
            # Search emails
            search_term = input("Enter recipient email to search for: ")
            emails_data = get_stored_emails(limit=50, recipient_filter=search_term)
            
            if emails_data['emails']:
                print(f"\nFound {len(emails_data['emails'])} emails for '{search_term}':")
                print("=" * 100)
                print(f"{'ID':<5}{'Recipient':<30}{'Subject':<35}{'Date':<20}{'Template'}")
                print("-" * 100)
                ...
                print(f"No stored emails found for '{search_term}'.")
            ...
            try:
                ...
                print("Invalid email ID. Please enter a number.")
            ...
            try:
                ...
                confirm = input(f"Are you sure you want to delete email ID {email_id}? (y/n): ")
                
                if confirm.lower() == 'y':
                    if delete_stored_email(email_id):
                        print("Email deleted successfully.")
                    else:
                        print("Failed to delete email or email not found.")
                else:
            ...
            if confirm.lower() == 'y':
                ...
                if confirm2 == 'DELETE ALL':
                    ...
                    print(f"Deleted all {deleted_count} stored emails.")
                else:
                    print("Operation cancelled.")
            else:
                print("Operation cancelled.")
            ...
            try:
                ...
                print(f"Exported {len(emails_data['emails'])} emails to {filename}")
...
def send_sla_alert(ticket_id, alert_type='overdue'):
    ...
    def _send_sla_alert(cursor):
        ...
        if alert_type == 'overdue':
            alert_subject = f"OVERDUE TICKET ALERT: #{ticket_id} - {subject}"
            alert_body = f"""URGENT: Ticket #{ticket_id} is now OVERDUE

Ticket Details:
- Subject: {subject}
- Priority: {priority.upper()}
- Due Date: {due_date}
- Submitted by: {submitter} ({submitter_email})
- Assigned to: {assignee or 'Unassigned'}
- Department: {department or 'None'}
- Created: {created_at}

This ticket has exceeded its SLA deadline and requires immediate attention.

Please take action to resolve this ticket as soon as possible.

This is an automated alert from the Support Ticket System.
"""
        elif alert_type == 'escalation':
            alert_subject = f"ESCALATION ALERT: #{ticket_id} - {subject}"
            alert_body = f"""Ticket #{ticket_id} requires escalation
...
This ticket is approaching its SLA deadline and may require escalation.
...
"""
            ...
            alert_subject = f"SLA ALERT: #{ticket_id} - {subject}"
            alert_body = f"""SLA Alert for Ticket #{ticket_id}
...
This ticket requires attention regarding SLA compliance.
...
def send_satisfaction_survey(ticket_id, custom_message=None):
    ...
    def _send_satisfaction_survey(cursor):
        ...
        cursor.execute('''
        SELECT t.ticket_id, t.subject, t.resolved_at, t.status,
        ...
        ticket_id, subject, resolved_at, status, username, email, first_name, last_name = ticket_info
        ...
        if custom_message:
            ...
            survey_body = f"""Dear {customer_name},

Your support ticket has been resolved and we'd love to hear about your experience!
...
1. Replying to this email with your rating (1-5) and any comments
2. Logging into the support portal and rating your ticket
3. Contacting us at support@university.edu
...
---
This is an automated message. If you have additional questions about ticket #{ticket_id}, please reply to this email or create a new support ticket.
"""
...
def fix_existing_email_senders():
    ...
    def _fix_senders(cursor):
        ...
        print(f"Found {len(messages_to_fix)} messages that might need sender fixes...")
        ...
        for msg_data in messages_to_fix:
            ...
            if real_user and real_user[0] != current_sender_id:
                try:
                    ...
                    print(f"✅ Fixed message #{msg_id}: '{subject[:50]}...' - Now from {real_user[1]} instead of {current_sender}")
                    fixed_count += 1
                except Exception as e:
                    print(f"❌ Failed to fix message #{msg_id}: {e}")
        
        return fixed_count
    
    try:
        count = execute_db_operation(_fix_senders)
        print(f"\n🎉 Fixed {count} messages with proper sender attribution!")
        return count
    except Exception as e:
        print(f"❌ Error fixing senders: {e}")
        return 0
...
def test_sender_attribution(auth_instance=None):
    ...
    if not _auth:
        print("❌ Authentication not initialized. Please ensure the communication system is properly set up.")
        return False

    if not hasattr(_auth, 'current_user') or not _auth.current_user:
        print("❌ No user is currently logged in. Please log in first to test sender attribution.")
        return False

    current_user = _auth.current_user
    print(f"Testing sender attribution for user: {current_user['username']} ({current_user.get('email', 'no email')})")
    
    # Test 1: Send email as current user
    print("\n1. Testing email as current logged-in user...")
    ...
    print("2. Testing email as named system...")
    ...
    if result1 and result2:
        print("✅ Test emails sent! Check the inbox to verify sender attribution.")
        ...
        for i, msg in enumerate(inbox.get('messages', [])[:5], 1):
            print(f"  {i}. From: '{msg['sender']}' - Subject: '{msg['subject'][:50]}...'")
...
def send_schedule_change_notification(schedule_id: int, old_data: dict, new_data: dict) -> bool:
    ...
    try:
        ...
        log_event('error', f"Error sending schedule change notifications: {e}")
        return False
...
def _get_room_location(room_id: int | None) -> str:
    ...
    try:
        ...
        log_event('error', f"Error getting room location: {e}")
...
def _get_instructor_info(instructor_id: int | None) -> dict:
    ...
    try:
        ...
        log_event('error', f"Error getting instructor info: {e}")
    ...
    try:
        ...
        with get_connection() as conn:
            ...
            cursor.execute('''
                ...
                WHERE e.module_code = ? AND e.status = 'enrolled'
            ''', (module_code,))
        ...
        log_event('error', f"Error getting enrolled students: {e}")
...
def _get_module_staff(module_code: str) -> list:
    ...
    try:
        ...
        log_event('error', f"Error getting module staff: {e}")
...
def _notify_datetime_change(recipients: list, module_info: dict, old_data: dict, new_data: dict) -> bool:
    ...
    for recipient in recipients:
        try:
            ...
            template = load_template('schedule_change_datetime')
            if template:
            ...
            log_event('error', f"Error sending datetime change email to {recipient['email']}: {e}")
            return False
...
def _notify_instructor_change(recipients: list, module_info: dict, old_data: dict, new_data: dict) -> bool:
    ...
    for recipient in recipients:
        try:
            ...
            template = load_template('schedule_change_instructor')
            if template:
                subject = template.get('subject', 'Schedule Change Notification')
...
def _notify_room_change(recipients: list, module_info: dict, old_data: dict, new_data: dict) -> bool:
    ...
    for recipient in recipients:
        try:
            ...
            template = load_template('schedule_change_room')
            if template:
                subject = template.get('subject', 'Schedule Change Notification')
...
def _notify_new_instructor_assignment(instructor_id: int, module_info: dict, schedule_data: dict) -> bool:
    ...
    try:
        ...
        template = load_template('instructor_assignment_notification')
        if template:
