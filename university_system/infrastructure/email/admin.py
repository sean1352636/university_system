...
from university_system.infrastructure.email.templates import template_management_menu, save_default_templates
...
class CommunicationDashboard:
    ...
    def __init__(self, db_path=None, auth=None):
        ...
        try:
            # Use auth database path if available
        ...
        def _create_tables(cursor):
            ...
            for column_name, definition in chat_room_column_definitions:
                if column_name not in chat_room_columns:
                    try:
                        cursor.execute(f'ALTER TABLE chat_rooms ADD COLUMN {column_name} {definition}')
        ...
        if message_id and send_email_copy:
            ...
            try:
                ...
                if recipient_email:
                    ...
                    email_subject = f"Message: {subject}"
                    email_body = f"""You have received a new message.
    
    From: {dashboard.auth.current_user['username']}
    Subject: {subject}
    ...
    ---
    You can reply to this message by logging into the system.
    This email copy was sent from the University Communication System.
    """
    ...
    def send_message_with_debug(self, recipient_id, subject, content, attachment_path=None):
        ...
        logger.debug(f"DEBUG: Attempting to send message to user ID {recipient_id}")
        logger.debug(f"DEBUG: Subject: {subject}")
        logger.debug(f"DEBUG: Current user: {self.auth.current_user['username']} (ID: {self.auth.current_user['id']})")
        
        result = self.send_message(recipient_id, subject, content, attachment_path)
        
        if result:
            logger.debug(f"DEBUG: Message sent successfully with ID: {result}")
        else:
            logger.error("DEBUG: Failed to send message")
        
        return result
        
    def _log_communication_action(self, user_id, action_type, action_details):
        """Enhanced communication action logging using both systems"""
        
        def _log_action(cursor):
    ...
    def update_message_status(self, message_id, action):
        ...
        def _update_status(cursor):
            ...
            self._log_communication_action(user_id, f"{action}_message", f"Message ID {message_id} {action_desc}")
    ...
    def get_inbox(self, include_archived=False, page=1, limit=10):
        ...
        def _get_inbox(cursor):
            ...
            logger.debug(f"DEBUG: User {user_id} has {total_messages_for_user} total messages")
            ...
            logger.debug(f"DEBUG: {non_deleted_count} messages for this user")
            ...
            logger.debug(f"DEBUG: Query returned {len(rows)} messages")
            ...
            logger.debug(f"DEBUG: Total count: {total_count}, Unread: {unread_count}")
    ...
    def debug_check_messages(self, user_id=None):
        ...
        def _check_messages(cursor):
            logger.debug(f"\nDEBUG: Checking messages for user ID {user_id}")
    ...
    def create_announcement(self, title, content, target_audience, is_urgent=0, start_date=None, end_date=None):
        ...
        def _create_announcement_op(cursor):
            ...
            # Log the action for auditing
            self._log_communication_action(creator_id, "create_announcement", f"Announcement created: {title}")
        ...
        if choice == '1':
            ...
            if config.get('database_only_mode', True):
                ...
                if recipient:
                    ...
                    body = f"""This is a test email from the Student Records System.
                    
Mode: Database Only (stored in database)
Sender: {config['sender_email']}
Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

This email was stored in the database instead of being sent.
...
def test_email_system():
    ...
    if send_template_email('registration_confirmation', test_email, template_vars):
...
def send_system_notification(dashboard, user_id, title, message, notification_type='info'):
    ...
    system_subject = f"System Notification: {title}"
    system_message = f"""System Notification
