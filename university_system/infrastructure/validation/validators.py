...
class Validators:
    ...
    Usage:
        from university_system.infrastructure.validation import Validators
    ...
    def validate_phone(cls, phone: str, allow_international: bool = True) -> ValidationResult:
        ...
        if len(cleaned) < 10:
            errors.append("Phone number is too short (minimum 10 digits)")
    ...
    def validate_credits(cls, credits: Union[str, int]) -> ValidationResult:
        ...
        if credits_value < cls.CREDITS_MIN:
            errors.append(f"Credits cannot be negative")
