...
from education_system.platform.delivery.api.auth import token_required, decode_token
from education_system.platform.identity.auth.db import AUTH_DB_FILE
from education_system.platform.services.parent_child_link import ParentChildLinkService
...
def _get_child_db(system_key: str) -> str | None:
    ...
    try:
        if system_key == "university":
            from education_system.systems.university.infrastructure.database.database_utils import get_db_path
...
def _verify_child_access(child_student_id: str) -> tuple[dict | None, tuple | None]:
    ...
    if link is None:
        ...
        try:
            from education_system.platform.identity.auth.db import AUTH_DB_FILE, connect
...
def list_children_journeys():
    ...
    parent_user_id = g.current_user["user_id"]
    try:
        from education_system.platform.services.parent_overview import (
