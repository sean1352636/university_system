...
class MisconductAnalyticsMixin:
    ...
    def refresh_analytics_tab(self):
        ...
        conn = None
        try:
            conn = sqlite3.connect(str(getattr(self, '_db_path', None) or DEFAULT_DB_PATH))
            cursor = conn.cursor()
            _sk = getattr(self, 'system_key', None)

            # Build fully parameterized queries (no f-string SQL interpolation)
            if _sk:
                p_all = (_sk,)
                q_total = "SELECT COUNT(*) FROM academic_misconduct_cases WHERE system_key = ?"
                q_resolved = "SELECT COUNT(*) FROM academic_misconduct_cases WHERE status = 'Resolved' AND system_key = ?"
                q_active = "SELECT COUNT(*) FROM academic_misconduct_cases WHERE status != 'Resolved' AND system_key = ?"
                q_pending = "SELECT COUNT(*) FROM academic_misconduct_cases WHERE status = 'Pending Hearing' AND system_key = ?"
            else:
                p_all = ()
                q_total = "SELECT COUNT(*) FROM academic_misconduct_cases"
                q_resolved = "SELECT COUNT(*) FROM academic_misconduct_cases WHERE status = 'Resolved'"
                q_active = "SELECT COUNT(*) FROM academic_misconduct_cases WHERE status != 'Resolved'"
                q_pending = "SELECT COUNT(*) FROM academic_misconduct_cases WHERE status = 'Pending Hearing'"

            # Overall statistics
            cursor.execute(q_total, p_all)
            total_cases = cursor.fetchone()[0]

            cursor.execute(q_resolved, p_all)
            resolved_cases = cursor.fetchone()[0]

            cursor.execute(q_active, p_all)
            active_cases = cursor.fetchone()[0]

            cursor.execute(q_pending, p_all)
            ...
            if _sk:
                cursor.execute("""
                    ...
                    FROM academic_misconduct_cases
                    WHERE date_filed >= date('now', '-6 months') AND system_key = ?
                    GROUP BY month
                    ORDER BY month DESC
                """, p_all)
            else:
    ...
    def generate_analytics_report(self):
        ...
        try:
            ...
            and_clause = "AND system_key = ?" if params else ""

            report_lines = []
            report_lines.append("=" * 80)
            system_key = getattr(self, 'system_key', None)
            if system_key:
                from education_system.platform.governance.academic_misconduct.dashboard import SYSTEM_DISPLAY_NAMES
                sys_name = SYSTEM_DISPLAY_NAMES.get(system_key, system_key.title())
                report_lines.append(f"ACADEMIC MISCONDUCT ANALYTICS REPORT - {sys_name}")
            else:
                report_lines.append("ACADEMIC MISCONDUCT ANALYTICS REPORT - ALL SYSTEMS")
            report_lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            report_lines.append("=" * 80)
            report_lines.append("")

            # Overall statistics
            cursor.execute(f"SELECT COUNT(*) FROM academic_misconduct_cases {where}", params)
            total = cursor.fetchone()[0]
            report_lines.append(f"Total Cases: {total}")

            cursor.execute(f"SELECT COUNT(*) FROM academic_misconduct_cases WHERE status = 'Resolved' {and_clause}", params)
