...
from education_system.systems.university.domain.operations.campus.housing.services.housing_accommodation.applications import select_student
...
def record_payment():
    ...
    try:
        ...
        # Payment type — distinguishes rent from deposit/damage/late fee so finance,
        # reports, and the deposit-refund flow can treat them differently.
        ...
        while True:
            try:
                pt_choice = int(input("\nSelect type (enter number): "))
                if 1 <= pt_choice <= len(payment_types):
                    payment_type = payment_types[pt_choice - 1]
                    break
                print(f"Please enter a number between 1 and {len(payment_types)}.")
            except ValueError:
                print("Please enter a valid number.")
        ...
        # Payment period — only meaningful for rent. Deposits, damage charges and
        # late fees don't cover a period; leave NULL so reports don't average them in.
        period_start = None
        period_end = None
        ...
        # Deposits start the 30-day TDP clock under Housing Act 2004 s.213.
        # Stamp the deadline on the assignment (idempotent — first deposit wins)
        # and warn the operator so protection isn't forgotten. Also set the
        # deposit lifecycle state to Held on the first deposit row.
        if payment_type == 'Deposit':
            try:
                ...
                deadline = set_deposit_deadline_if_unset(cursor, assignment_id, timestamp)
                if deadline:
                    print(f"\nTDP deadline: protect & send prescribed info by {deadline} "
                          f"(30 days from receipt). Use Payment Menu → "
                          f"Record Deposit Protection.")
        ...
        # Cross-domain: only an actual rent payment clears arrears holds —
        # deposits, damage charges and late fees don't reduce rent owed.
        if payment_type == 'Rent':
            try:
        ...
        print(f"Type: {payment_type} | Amount: £{payment_amount} | Method: {payment_method}")
    ...
    try:
        ...
        if current_role == 'student':
            ...
            SELECT p.source_payment_id, p.reference_id, p.amount, p.payment_date, p.payment_method,
                   ...
                   r.room_number, b.building_name, COALESCE(p.payment_type, 'Rent')
            FROM payments p
            ...
            for i, payment in enumerate(payments, 1):
                print(f"{i}. [{payment[10]}] £{payment[2]} - Paid on {payment[3]} via {payment[4]}")
                print(f"   Room {payment[8]} in {payment[9]}")
                if payment[5] and payment[6]:
                    print(f"   Period: {payment[5]} to {payment[6]} | Status: {payment[7]}")
                else:
                    print(f"   Status: {payment[7]}")
                print()

            # Per-type breakdown — separates rent collected from refundable deposits
            # held and one-off charges.
            cursor.execute('''
            SELECT COALESCE(payment_type, 'Rent') AS pt,
                   COUNT(payment_id), SUM(amount)
            FROM payments
            ...
            grand_total = 0.0
            grand_count = 0
            for pt, cnt, total in cursor.fetchall():
                total = total or 0
                grand_total += total
                grand_count += cnt
                print(f"  {pt}: {cnt} payment(s), £{total:,.2f}")
            print(f"Total Payments: {grand_count}")
            print(f"Total Amount Paid: £{grand_total:,.2f}")
            ...
            if filter_choice == '1':
                ...
                SELECT p.source_payment_id, p.student_id, s.first_name, s.last_name,
                       ...
                       p.payment_period_start, p.payment_period_end, p.status,
                       r.room_number, COALESCE(p.payment_type, 'Rent')
                FROM payments p
                ...
                for i, payment in enumerate(payments, 1):
                    print(f"{i}. [{payment[11]}] {payment[2]} {payment[3]} ({payment[1]}) - £{payment[4]}")
                    print(f"   Room: {payment[10]} | Paid on: {payment[5]} via {payment[6]}")
                    if payment[7] and payment[8]:
                        print(f"   Period: {payment[7]} to {payment[8]} | Status: {payment[9]}")
                    else:
                        print(f"   Status: {payment[9]}")
                    print()
                ...
                for i, payment in enumerate(payments, 1):
                    print(f"{i}. [{payment[10]}] {payment[2]} {payment[3]} ({payment[1]}) - £{payment[4]}")
