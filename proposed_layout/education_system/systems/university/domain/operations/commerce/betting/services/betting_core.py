...
class AccountManager:
    ...
    @staticmethod
    def credit_funds(user_id: str, amount: float, payment_method: str, processed_by: str = None) -> Optional[int]:
        """Credit funds to an account without the card-deposit cap.

        Unlike :meth:`deposit`, this is for internal transfers (e.g. cashing
        arcade credits back to betting funds) that are not real card buy-ins,
        so the MAX_DEPOSIT ceiling must not apply. Only a positive amount is
        required; the balance and transaction record are updated identically.
        """
        if amount <= 0:
            return None
        ...
        try:
            ...
            print(f"Error crediting funds: {e}")
            return None
