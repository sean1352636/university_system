...
from education_system.systems.university.infrastructure.sql_safety import validate_identifier, escape_like  # nosec B608
...
logger = logging.getLogger("legal.emails")


def _send_legal_email(template_name: str, recipient_email: str,
                      vars_: Dict[str, Any]) -> bool:
    """Render ``legal/<template_name>`` and dispatch best-effort. Skips when
    *recipient_email* is empty."""
    ...
    try:
        ...
        return False
    subject, body = render_template(f"legal/{template_name}", vars_)
    ...
    try:
        ...
        logger.exception("send_email failed legal/%s recipient=%s",
                         template_name, recipient_email)
        return False


def send_policy_acknowledgement(recipient_email: str, recipient_name: str,
                                policy_name: str, policy_version: str,
                                effective_date: str, deadline: str,
                                audience: str = "user",
                                issued_by: str = "Legal & Compliance",
                                policy_url: str = "(see staff/student portal)",
                                acknowledgement_url: str = "(see staff/student portal)",
                                reference_id: str = "",
                                changes_summary: str = "(see full policy for details)",
                                contact_person: str = "Legal Services",
                                contact_email: str = "legal@university.edu") -> bool:
    """Email a staff member or student to request acknowledgement of a policy."""
    return _send_legal_email('policy_acknowledgement', recipient_email, {
        ...
        'reference_id':         reference_id or f"POL-{uuid.uuid4().hex[:8].upper()}",
...
def send_compliance_notice(recipient_email: str, recipient_name: str,
                           notice_subject: str, notice_body: str,
                           required_actions: str,
                           legal_basis: str = "(internal policy)",
                           audience: str = "user",
                           issued_by: str = "Legal & Compliance Office",
                           effective_date: str = "",
                           response_required: str = "Yes",
                           response_deadline: str = "(see notice)",
                           non_compliance_consequences: str = "escalation under the relevant policy",
                           contact_name: str = "Legal Services",
                           contact_email: str = "legal@university.edu",
                           appeal_policy: str = "the University Appeals Procedure",
                           notice_reference: str = "") -> bool:
    """Email a formal compliance notice to a user."""
    return _send_legal_email('compliance_notice', recipient_email, {
        'recipient_name':              recipient_name,
        'notice_reference':            notice_reference or f"COMP-{uuid.uuid4().hex[:8].upper()}",
        ...
        'non_compliance_consequences': non_compliance_consequences,
