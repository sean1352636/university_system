...
class AchievementBadgeService:
    ...
    def _ensure_tables_exist(self):
        with transaction() as conn:
            ...
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS achievement_student_badges (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    def award_badge(self, student_id: str, badge_id: int, awarded_by: str = None, reason: str = None) -> int:
        with transaction() as conn:
            existing = conn.execute("SELECT id FROM achievement_student_badges WHERE student_id = ? AND badge_id = ?", (student_id, badge_id)).fetchone()
            if existing:
                raise ValueError("Student already has this badge.")
            cursor = conn.execute(
                "INSERT INTO achievement_student_badges (student_id, badge_id, awarded_by, reason) VALUES (?, ?, ?, ?)",
    ...
    def revoke_badge(self, student_id: str, badge_id: int) -> bool:
        with transaction() as conn:
            conn.execute("DELETE FROM achievement_student_badges WHERE student_id = ? AND badge_id = ?", (student_id, badge_id))
            conn.commit()
            return True
    ...
    def get_student_badges(self, student_id: str) -> List[Dict]:
        with get_connection() as conn:
            rows = conn.execute("""
                ...
                FROM achievement_student_badges sb JOIN badge_definitions bd ON sb.badge_id = bd.id
    ...
    def get_student_badge_count(self, student_id: str) -> Dict:
        with get_connection() as conn:
            count = conn.execute("SELECT COUNT(*) FROM achievement_student_badges WHERE student_id = ?", (student_id,)).fetchone()[0]
            points = conn.execute("""
                SELECT COALESCE(SUM(bd.points), 0) FROM achievement_student_badges sb
    ...
    def toggle_badge_display(self, student_id: str, badge_id: int) -> bool:
        with transaction() as conn:
            row = conn.execute("SELECT is_displayed FROM achievement_student_badges WHERE student_id = ? AND badge_id = ?", (student_id, badge_id)).fetchone()
            if row:
                new_val = 0 if row[0] == 1 else 1
                conn.execute("UPDATE achievement_student_badges SET is_displayed = ? WHERE student_id = ? AND badge_id = ?", (new_val, student_id, badge_id))
                conn.commit()
                return True
            return False
    ...
    def get_badge_statistics(self) -> Dict:
        with get_connection() as conn:
            ...
            total_awarded = conn.execute("SELECT COUNT(*) FROM achievement_student_badges").fetchone()[0]
            unique_students = conn.execute("SELECT COUNT(DISTINCT student_id) FROM achievement_student_badges").fetchone()[0]
            by_category = {}
            for row in conn.execute("SELECT bd.category, COUNT(*) FROM achievement_student_badges sb JOIN badge_definitions bd ON sb.badge_id = bd.id GROUP BY bd.category").fetchall():
