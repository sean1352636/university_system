...
from education_system.systems.university.infrastructure import paths as sys_paths
...
def load_config(path: str | Path | None = None) -> Dict[str, Any]:
    ...
    Delegates to the shared email config loader which checks:
      1. Explicit path
    ...
    # Use shared config loader for resolution
    try:
        from education_system.platform.integrations.email.config import load_email_config as _shared_load
        loaded = _shared_load(path)
    except ImportError:
        loaded = None
    ...
    if loaded is None:
        ...
        if path is None:
            path = sys_paths.EMAIL_CONFIG_FILE
        else:
            path = Path(path)
...
def save_config(path: str | Path | None = None) -> None:
    ...
    """
    try:
        from education_system.platform.integrations.email.config import save_email_config
        save_email_config(dict(config), path)
        return
    except ImportError:
        pass
