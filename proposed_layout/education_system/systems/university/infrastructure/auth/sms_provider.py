...
from education_system.systems.university.infrastructure.validation.validators import Validators
...
class TwilioSMSProvider(SMSProvider):
    ...
    def _normalize_phone(self, phone_number: str) -> str:
        ...
        # Validate E.164 format: digits (excluding +) must be 7-15
        digits_only = ''.join(c for c in clean if c.isdigit())
        if len(digits_only) < 7 or len(digits_only) > 15:
            raise ValueError(f"Invalid phone number length after normalization: {len(digits_only)} digits (expected 7-15)")
...
class EmailToSMSProvider(SMSProvider):
    ...
    def _load_smtp_config(self) -> dict:
        """Load SMTP config from shared email_config.json"""
        try:
        ...
        try:
            ...
            logger.warning("Failed to load SMTP config from %s: %s", config_path, e)
        return {}
    ...
    def _send_via_smtp(self, to_email: str, message: str) -> bool:
        ...
        smtp_server = config.get('smtp_server') or os.getenv('SMTP_SERVER', 'smtp.gmail.com')
        smtp_port = config.get('smtp_port') or int(os.getenv('SMTP_PORT', '587'))
        ...
        sender = config.get('sender_email') or os.getenv('SENDER_EMAIL', smtp_user)
...
class SMSService:
    ...
    def _create_provider(self, provider_name: str) -> Optional[SMSProvider]:
        ...
        try:
            if provider_name == 'twilio':
                ...
                logger.warning("Unknown SMS provider: %s, using mock", provider_name)
                return MockSMSProvider()
        except Exception as e:
            logger.warning("Failed to create %s SMS provider: %s, using mock", provider_name, e)
            return MockSMSProvider()
    ...
    def send_otp(self, phone_number: str, code: str) -> Dict:
        ...
        validation = Validators.validate_phone(phone_number)
        if not validation.is_valid:
            return {'success': False, 'error': validation.error_message}

        # Use the cleaned/validated number
        phone_number = validation.value
        ...
        if self.fallback:
            logger.warning("Primary SMS provider failed, trying fallback")
...
def load_sms_config() -> Dict:
    ...
    if os.path.exists(config_file):
        try:
            ...
            from education_system.systems.university.infrastructure.validation.config_validators import validate_sms_config
            result = validate_sms_config(file_config)
            if not result.is_valid:
                logger.warning("SMS config validation warnings: %s", result.error_message)
            return file_config
        except (json.JSONDecodeError, OSError) as e:
            logger.warning("Failed to load SMS config from %s: %s", config_file, e)
    ...
    # Check if SMTP is configured (for email gateway) - check config file too
    email_config = {}
    try:
        from education_system.platform.integrations.email.config import load_email_config
        ...
        try:
            email_cfg_path = os.path.join(paths.CONFIG_DIR, 'email_config.json')
            if os.path.exists(email_cfg_path):
                with open(email_cfg_path, 'r') as f:
                    email_config = json.load(f)
        except (json.JSONDecodeError, OSError) as e:
            logger.warning("Failed to load email config for SMS fallback detection: %s", e)
    smtp_configured = all([
        email_config.get('smtp_username') or os.getenv('SMTP_USERNAME') or os.getenv('SMTP_USER'),
        email_config.get('smtp_password') or os.getenv('SMTP_PASSWORD')
    ])
