...
def _overlay_shared_last_login(users: List[Dict]) -> None:
    ...
    if not users:
        return
    try:
        from education_system.platform.identity.auth.db import connect as _shared_connect
    except Exception:
        return
...
class UserManager:
    ...
    def _purge_user_references(self, cursor, user_id) -> int:
        """Delete every row in this database that references ``users`` via a
        foreign key, using each table's *actual* FK column(s) discovered from
        the schema (``PRAGMA foreign_key_list``).

        This replaces the previous hardcoded table list, which covered only a
        handful of the 130+ tables that reference users and assumed a
        ``user_id`` column — so most dependent rows (messages, chat
        membership, created_by/sender_id/… columns) were left orphaned on
        delete. Discovering references from the schema covers every current
        and future user-referencing table, by its correct column name.

        The caller owns the transaction. With FK enforcement ON, each delete
        additionally cascades to that table's descendants (per their ON DELETE
        CASCADE policy); the explicit per-table deletes make the purge robust
        even if enforcement is off. Returns rows directly deleted.
        """
        deleted = 0
        ...
        for table in tables:
            ...
            try:
                ...
                continue
            # fk row: (id, seq, parent_table, from_col, to_col, ...)
            for col in {fk[3] for fk in fks if fk[2] == 'users' and fk[3]}:
                try:
                    cursor.execute(f'DELETE FROM "{table}" WHERE "{col}" = ?', (user_id,))
                    deleted += cursor.rowcount or 0
                except sqlite3.OperationalError:
                    pass  # view / transient lock / odd schema — skip
        return deleted
        ...
        try:
            with self.db_manager.get_connection() as conn:
                ...
                if not user_data:
                    ...
                    try:
                        from education_system.platform.identity.auth.core import UserAuth as SharedAuth
                        shared = SharedAuth()
                        ...
                        if shared_user:
                            ...
                            try:
                                ...
                                shared_conn.close()
                            # The shared-auth delete cannot reach this (records)
                            # database, so purge the user's dependent rows here too.
                            try:
                                # No local users row to cascade from, so purge
                                # dependent rows directly; FK enforcement ON lets
                                # each delete cascade to its descendants.
                                cursor.execute("PRAGMA foreign_keys = ON")
                                removed = self._purge_user_references(cursor, user_id)
                                conn.commit()
                                logger.info("delete_user(fallback): purged %d dependent row(s) "
                                            "for user_id=%s", removed, user_id)
                            except sqlite3.Error:
                                logger.warning("fallback purge of local refs failed for "
                                               "user_id=%s", user_id, exc_info=True)
                            return True
                    except Exception:
                        logger.warning(
                            "delete_user(fallback): unexpected error while purging "
                            "references for user_id=%s", user_id, exc_info=True)
                    return False
                ...
                # FK enforcement ON so each foreign key's ON DELETE policy
                # fires: deleting the user row cascades to all dependent rows
                # (and NULLs the few SET NULL columns) across the schema. The
                # PRAGMA must be set before any write opens a transaction.
                cursor.execute("PRAGMA foreign_keys = ON")
                cursor.execute('DELETE FROM users WHERE id = ?', (user_id,))
                logger.info("delete_user: cascaded delete for user_id=%s", user_id)
