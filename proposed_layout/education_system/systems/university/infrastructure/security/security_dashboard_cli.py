...
from education_system.systems.university.infrastructure.security import session_management as session_mgmt
from education_system.systems.university.infrastructure.security import data_encryption as encryption
from education_system.systems.university.infrastructure.security import comprehensive_security as comp_security
from education_system.systems.university.infrastructure.security import init_security_tables as security_tables

# Import individual classes for backwards compatibility
from education_system.systems.university.infrastructure.security.session_management import SessionManager
from education_system.systems.university.infrastructure.security.data_encryption import EncryptionManager
...
try:
    import education_system.systems.university.infrastructure.auth as auth_module
    from education_system.systems.university.infrastructure.auth import mfa_service as mfa_module
    from education_system.systems.university.infrastructure.auth import mfa_integration as mfa_integration_module
...
class SecurityDashboardCLI:
    ...
    def __init__(self, admin_user_id: int):
        ...
        init_security_tables(self.db_path)

        # Initialize managers against the same configured database so they all
        # operate on self.db_path (critical for tests that patch DEFAULT_DB_PATH).
        self.session_mgr = SessionManager(self.db_path)
        self.encryption_mgr = EncryptionManager(self.db_path)
        self.api_mgr = APISecurityManager(self.db_path)
        self.password_mgr = PasswordSecurityManager(self.db_path)
        self.audit_mgr = SecurityAuditManager(self.db_path)
        self.dlp_mgr = DataLossPreventionManager(self.db_path)
        self.incident_mgr = IncidentResponseManager(self.db_path)
        self.vuln_scanner = VulnerabilityScanner(self.db_path)
...
def user_management_cli_menu(dashboard):
    ...
    if not AUTH_MODULE_AVAILABLE:
        ...
        if auth_module:
            ...
            print("   Key functions: display_user_management_menu, UserAuth, create_user, etc.")
        return
