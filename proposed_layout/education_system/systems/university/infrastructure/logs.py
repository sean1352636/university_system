...
def _get_state():
    ...
    if state is None:
        try:
            from education_system.systems.university.infrastructure.email import state as _state
            state = _state
        except ImportError:
            try:
                from education_system.systems.university.infrastructure.utils import state as _state
                state = _state
            except ImportError:
                pass
    return state
...
def display_communication_analytics_menu(dashboard):
    ...
    from education_system.systems.university.infrastructure.utils.database import execute_db_operation  # Local import to avoid circular dependency
    from education_system.systems.university.infrastructure.utils.email_service import get_stored_emails  # Local import to avoid circular dependency
