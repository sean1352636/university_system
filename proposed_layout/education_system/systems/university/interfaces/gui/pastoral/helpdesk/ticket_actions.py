...
def add_ticket_reply(self, ticket_id, message, time_spent=0, is_internal=False):
    ...
    try:
        ...
        if not is_internal:  # Only send for public replies
            try:
                from education_system.systems.university.infrastructure.email.email_service import send_reply_notification
...
def update_ticket_status(self, ticket_id, new_status, resolution=None):
    ...
    try:
        ...
        if new_status in ['resolved', 'closed']:
            ...
            try:
                from education_system.systems.university.infrastructure.email.email_service import send_satisfaction_survey
