...
try:
    from education_system.systems.university.domain.pastoral.services.helpdesk import (
...
def export_analytics_json(self):
    ...
    try:
        ...
        filename = filedialog.asksaveasfilename(
            ...
            initialfile=f"analytics_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def _select_and_upload_file(self, ticket_id, reply_id, tree):
    ...
    try:
        ...
        cursor.execute('''
            ...
            (ticket_id, reply_id, filename, original_filename, file_size, mime_type, uploaded_by, created_at, file_path)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            ticket_id,
            reply_id,
...
def _send_ticket_created_emails(self, ticket_id, subject, category, priority, admin_list, user_id):
    ...
    try:
        from education_system.systems.university.infrastructure.email.email_service import send_ticket_notification
