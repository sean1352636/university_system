    ...
    def process_payment():
        ...
        try:
            ...
            if conn:
                ...
                messagebox.showinfo("Payment Complete",
                                  "Meal plan payment processed successfully!\n\n" +
...
def send_payment_receipt_email(order_id, amount, payment_method, **kwargs):
    ...
    try:
        ...
        conn = get_db_connection()
        if not conn:
            print("[Restaurant] Could not send receipt - database connection failed")
            return False
...
def send_refund_receipt_email(order_id, refund_amount, payment_method, refund_type, reason):
    ...
    try:
        ...
        conn = get_db_connection()
        if not conn:
            print("[Restaurant] Could not send refund receipt - database connection failed")
            return False
