...
class DonationsMixin:
        ...
        def load_donations_data(self):
            """Load donation records from the database (no sample data)."""
            try:
                # Clear existing data
                ...
                with db_get_connection() as conn:
                    rows = conn.execute(
                        ...
                        SELECT
                            d.donation_date,
                            TRIM(COALESCE(a.first_name,'') || ' ' || COALESCE(a.last_name,'')) AS donor_name,
                            d.alumni_id,
                            d.amount,
                            COALESCE(NULLIF(d.campaign, ''), d.donation_type, '') AS campaign,
                            COALESCE(d.payment_method, '') AS payment_method,
                            CASE WHEN d.receipt_sent THEN 'Receipted' ELSE 'Recorded' END AS status
                        FROM donations d
                        LEFT JOIN alumni a ON a.alumni_id = d.alumni_id
                ...
                count = 0
                for r in rows:
                    date_, name, alumni_id, amount, campaign, method, status = r
                    try:
                        amount_str = f"£{float(amount):.2f}"
                    except (TypeError, ValueError):
                        amount_str = str(amount or "")
                    self.donations_tree.insert('', tk.END, values=(
                        date_ or "", (name or "").strip() or str(alumni_id or ""),
                        alumni_id or "", amount_str, campaign or "", method or "", status,
                    ))
                    count += 1

                if count == 0:
                    self.update_status("No donations recorded yet")
                else:
                    self.update_status(f"Loaded {count} donation records")
    ...
    """
        ...
        def show_record_donation(self):
            ...
            payment_combo = ttk.Combobox(payment_frame, textvariable=self.donation_vars['payment_method'],
                                        values=["Credit Card", "Check", "Bank Transfer", "Cash",
                                                "Online", "Finance Account"])
            payment_combo.pack(side=tk.LEFT)
            ttk.Label(payment_frame,
                      text="  (Finance Account debits the alumnus's university account)",
        ...
        def submit_donation(self):
            ...
            alumni_id = self.donation_vars['alumni_id'].get().strip()
            campaign = self.donation_vars['campaign'].get().strip()
            payment_method = self.donation_vars['payment_method'].get().strip() or "Online"
            is_recurring = 1 if self.donation_vars['is_recurring'].get() else 0
            frequency = self.donation_vars['frequency'].get().strip()
            notes = self.donation_notes.get(1.0, tk.END).strip()
            donation_date = datetime.now().strftime('%Y-%m-%d')

            # Resolve the alumnus (name/email and linked finance student_id).
            alum = None
            try:
                with db_get_connection() as conn:
                    alum = conn.execute(
                ...
                messagebox.showerror("Database Error", f"Could not look up alumnus: {e}")
                return
            if not alum:
                messagebox.showerror("Not Found",
                                     f"No alumni record found for ID {alumni_id}.")
                return
            student_id, donor_email, donor_name = alum
            donor_name = (donor_name or "").strip() or alumni_id

            # Pay-by-finance-account: debit the alumnus's university finance
            # account (linked via alumni.student_id -> student_finance_accounts).
            if payment_method == "Finance Account":
                if not student_id:
                    messagebox.showerror(
                        "No Finance Account",
                        "This alumnus has no linked student/finance account, so the "
                        "donation cannot be paid from a finance account.")
                    return
                try:
                    with db_get_connection() as conn:
                    ...
                    messagebox.showerror("Database Error", f"Finance account lookup failed: {e}")
                    return
                if bal_row is None:
                    messagebox.showerror(
                        "No Finance Account",
                        f"No finance account exists for student {student_id}.")
                    return
                if float(bal_row[0] or 0) < amount:
                    messagebox.showerror(
                        "Insufficient Funds",
                        f"Finance account balance (£{float(bal_row[0] or 0):.2f}) is less "
                        f"than the donation amount (£{amount:.2f}).")
                    return
                try:
                    from education_system.systems.university.services.bus.finance_bus import raise_charge
                    tx = raise_charge(
                        student_id, amount,
                        source="alumni_donation",
                        description=f"Alumni donation{(' - ' + campaign) if campaign else ''}",
                        reference_id=f"DONATION-{alumni_id}-{donation_date}",
                        processed_by=str(getattr(self, 'current_user', {}).get('username', 'alumni_gui')),
                    )
                    if tx is None:
                    ...
                    messagebox.showerror("Payment Failed",
                                         f"Finance account charge failed: {e}")
                    return

                # Mirror the donation into the central finance system as revenue.
                try:
                    ...
                    record_revenue_to_finance(
                        ...
                        transaction_ref=f"DONATION-{alumni_id}-{donation_date}",
                        payment_method="Finance Account",
                        notes=f"Alumni donation{(' - ' + campaign) if campaign else ''}",
                    )
                except Exception as e:
                    import logging
                    logging.warning(f"record_revenue_to_finance failed (donation still debited): {e}")
            ...
            try:
                with db_get_connection() as conn:
                    cur = conn.execute(
                        ...
                        INSERT INTO donations
                            (alumni_id, amount, donation_date, campaign, payment_method,
                             is_recurring, recurring_frequency, notes, donation_type, receipt_sent)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'general', 0)
                        """,
                        (alumni_id, amount, donation_date, campaign, payment_method,
                ...
                messagebox.showerror("Database Error", f"Failed to record donation: {e}")
                return

            # Send a thank-you receipt email (best-effort) and flag receipt_sent.
            receipt_sent = self._send_donation_receipt(
                donor_email, donor_name, amount, donation_date,
                campaign or "General Fund", payment_method)
            if receipt_sent:
                try:
                    with db_get_connection() as conn:
                        conn.execute("UPDATE donations SET receipt_sent = 1 WHERE donation_id = ?",
                                     (donation_id,))
                        conn.commit()
                except sqlite3.Error:
                    pass

            msg = f"Donation of £{amount:.2f} recorded for {donor_name}."
            if payment_method == "Finance Account":
                msg += "\nFinance account debited."
            msg += ("\nThank-you receipt emailed." if receipt_sent
                    else "\n(Receipt email could not be sent.)")
            messagebox.showinfo("Donation Recorded", msg)
        ...
        def _send_donation_receipt(self, donor_email, donor_name, amount,
                                   donation_date, campaign, payment_method):
            """Email a thank-you donation receipt using the JSON template
            alumni/donation_thank_you. Returns True if sent/queued."""
            ...
            try:
                ...
                subject, body = render_template("alumni/donation_thank_you", {
                ...
                if not subject or not body:
                    return False
                return bool(send_email(recipient_email=donor_email, subject=subject, body=body))
            except Exception as e:
                import logging
                logging.warning(f"Donation receipt email failed: {e}")
                return False

        def update_donation_summary(self):
            """Update donation summary from the database."""
            total_amount = 0.0
            total_donations = 0
            try:
                with db_get_connection() as conn:
                ...
                import logging
                logging.warning(f"Donation summary query failed: {e}")

            avg_donation = (total_amount / total_donations) if total_donations else 0.0
            summary_text = (f"Total Donations: {total_donations} | "
                            f"Total Amount: £{total_amount:,.2f} | "
                            f"Average: £{avg_donation:,.2f}")
