...
def create_analysis_tab(self):
    ...
    analysis_combo = ttk.Combobox(controls_frame, textvariable=self.analysis_type,
                                 values=["forecasting", "risk_analysis", "cash_flow", "scenario_planning",
                                         "anomaly_detection", "lifecycle", "comparative"],
                                 state="readonly")
    ...
    def run_analysis():
        try:
            if analysis_type == "forecasting":
                ...
                result = "Payment Risk Analysis Report\n"
                result += "=" * 40 + "\n\n"
                ...
                anomalies = detector.detect_payment_anomalies()
                result = "Payment Anomaly Detection Report\n"
                result += "=" * 40 + "\n\n"
                result += f"Threshold: {detector.threshold_multiplier}x average payment\n"
                result += f"Anomalous transactions found: {len(anomalies)}\n\n"
                if anomalies:
                    total_flagged = sum((a[1] or 0) for a in anomalies)
                    result += f"Total value flagged: £{total_flagged:,.2f}\n\n"
                    result += f"{'Student ID':<20} {'Amount':<15} {'Reason'}\n"
                    result += "-" * 40 + "\n"
                    for student_id, amount in anomalies[:25]:
                        reason = detector.get_anomaly_reason((student_id, amount), anomalies)
                        result += f"{str(student_id):<20} £{(amount or 0):<14,.2f} {reason}\n"
                    if len(anomalies) > 25:
                        result += f"\n... and {len(anomalies) - 25} more.\n"
                else:
                    result += "No anomalous payment patterns detected.\n"

            elif analysis_type == "lifecycle":
                analyzer = StudentLifecycleAnalyzer()
                data = analyzer.analyze_student_lifecycle()
                result = "Student Lifecycle Analysis Report\n"
                result += "=" * 40 + "\n\n"
                stats = data.get('summary_stats', {}) if data else {}
                if stats:
                    def _pct(v):
                        # Guard against inf/nan from students with payments but no recorded fees.
                        return f"{v:.1f}%" if isinstance(v, (int, float)) and np.isfinite(v) else "n/a"
                    result += f"Total students:        {stats.get('total_students', 0)}\n"
                    result += f"Paying students:       {data.get('paying_students', 0)}\n"
                    result += f"Payment rate:          {_pct(data.get('payment_rate', 0))}\n"
                    result += f"Avg collection rate:   {_pct(stats.get('avg_collection_rate', 0))}\n"
                    result += f"High-risk students:    {stats.get('high_risk_students', 0)}\n\n"
                    student_data = data.get('student_data')
                    if student_data is not None and not student_data.empty:
                        worst = student_data.nsmallest(10, 'collection_rate')
                        result += "Lowest Collection Rates:\n"
                        result += "-" * 40 + "\n"
                        for _, row in worst.iterrows():
                            name = f"{row['first_name']} {row['last_name']}".strip()
                            result += (f"  {name or row['student_id']}: "
                                       f"{row['collection_rate']:.1f}% "
                                       f"(£{row['total_paid']:,.2f} of £{row['total_fees']:,.2f})\n")
                else:
                    result += "Insufficient data for lifecycle analysis.\n"

            elif analysis_type == "comparative":
                analyzer = ComparativeAnalyzer()
                yoy = analyzer.year_over_year_analysis()
                result = "Comparative (Year-over-Year) Analysis Report\n"
                result += "=" * 40 + "\n\n"
                if yoy:
                    result += f"{'Year':<8} {'Students':<10} {'Collected':<18} {'Rate':<10}\n"
                    result += "-" * 40 + "\n"
                    years_sorted = sorted(yoy.keys(), reverse=True)
                    for year in years_sorted:
                        d = yoy[year]
                        result += (f"{year:<8} {d['student_count']:<10} "
                                   f"£{d['total_collected']:<17,.2f} "
                                   f"{d['collection_rate']:<9.1f}%\n")
                    if len(years_sorted) >= 2:
                        this_y = yoy[years_sorted[0]]['total_collected']
                        last_y = yoy[years_sorted[1]]['total_collected']
                        growth = ((this_y - last_y) / last_y * 100) if last_y else 0
                        result += f"\nYear-over-year growth: {growth:+.1f}%\n"
                else:
                    result += "Insufficient historical data for comparison.\n"

            else:
                available = ("forecasting, risk_analysis, cash_flow, scenario_planning, "
                             "anomaly_detection, lifecycle, comparative")
                result = (f"Analysis type '{analysis_type}' is not recognised.\n\n"
                          f"Available analysis types:\n  {available}")
    ...
    def save_as_txt():
        ...
        if filename:
            try:
                ...
                with open(filename, 'w') as f:
                    f.write("Year-over-Year Financial Analysis Report\n")
