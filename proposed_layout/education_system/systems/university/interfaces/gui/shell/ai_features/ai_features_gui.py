...
from education_system.systems.university.infrastructure.database.schemas.ai_features_schemas import init_ai_features_system_db
from education_system.systems.university.infrastructure.auth import UserAuth
from education_system.systems.university.infrastructure.activity_logger import log_activity
from education_system.systems.university.services.ai_features.ai_features_core import (
...
from education_system.systems.university.infrastructure.i18n import get_text as _t, _

# Import chatbot components
try:
    from education_system.systems.university.infrastructure.ai.university_chatbot import UniversityChatbot
...
"""
    ...
    def create_autograding_tab(self):
        """Create the auto-grading results tab"""
        tab = ttk.Frame(self.notebook)
        ...
        v_scroll = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL)
        self.grading_tree = ttk.Treeview(
            tree_frame,
            columns=('ID', 'Submission', 'Type', 'Score', 'Max', 'Confidence', 'Review', 'Graded'),
            show='tree headings',
            yscrollcommand=v_scroll.set,
        )
    ...
    def load_grading_results(self):
        """Load auto-grading results into the grading tree."""
        try:
            ...
            self.update_status(_t("ai_features.autograding.loaded",
                                  count=len(self.grading_tree.get_children())))
        except Exception as e:
            messagebox.showerror(_t("common.error"),
                                 _t("ai_features.autograding.load_error", error=str(e)))

    def view_grading_details(self, event=None):
        """Show details for the selected grading result."""
        ...
        try:
            with get_connection() as conn:
                ...
                max_score = grading['max_score'] or 0
                percentage = (grading['auto_score'] / max_score * 100) if max_score else 0
                confidence = f"{grading['confidence_score']:.2f}" if grading['confidence_score'] else 'N/A'
...
{grading['feedback_generated'] or 'No feedback'}
                ...
                ttk.Button(main_frame, text=_t("ai_features.buttons.close"),
                           command=dialog.destroy).pack(pady=10)
        except Exception as e:
            messagebox.showerror(_t("common.error"),
                                 _t("ai_features.autograding.details_error", error=str(e)))

    def create_plagiarism_tab(self):
        """Create the plagiarism detection results tab"""
        tab = ttk.Frame(self.notebook)
        ...
        ttk.Button(header_frame, text=_t("ai_features.plagiarism.launch_full"),
                   command=self.launch_full_plagiarism_gui).pack(side=tk.RIGHT, padx=5)
        ...
        v_scroll = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL)
        self.plagiarism_tree = ttk.Treeview(
            tree_frame,
            columns=('ID', 'Submission', 'Student', 'Similarity', 'Flagged', 'Sources', 'Checked'),
            show='tree headings',
            yscrollcommand=v_scroll.set,
        )
    ...
    def load_plagiarism_checks(self):
        """Load plagiarism detection results into the plagiarism tree."""
        try:
            ...
            with get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    sources = row['matched_sources'] or 'None'
                    values = (
                        row['check_id'],
                        row['submission_id'],
                        row['student_id'],
                        ...
                        (sources[:30] + '...') if len(sources) > 30 else sources,
                        row['checked_at'][:16] if row['checked_at'] else '',
                    )
                    self.plagiarism_tree.insert('', 'end', values=values, tags=(tag,))

            self.update_status(_t("ai_features.plagiarism.loaded",
                                  count=len(self.plagiarism_tree.get_children())))
        except Exception as e:
            messagebox.showerror(_t("common.error"),
                                 _t("ai_features.plagiarism.load_error", error=str(e)))
    ...
    def generate_ai_response(self, user_message: str) -> str:
        ...
        Uses the shared NLP engine: a Claude call when the Anthropic SDK and
        credentials are configured, otherwise a deterministic, database-grounded
        response. Never returns random canned text.
        """
        try:
            from education_system.systems.university.services.ai_features.nlp_engine import (
                generate_chatbot_reply,
            )
            return generate_chatbot_reply(user_message)
        except Exception as e:
            print(f"Chatbot reply generation failed: {e}")
            return ("I'm having trouble answering right now. Please try again, or "
    ...
    def analyze_sentiment(self):
        ...
        def perform_analysis():
            try:
                ...
                # Real sentiment analysis via NLTK VADER (keyword fallback if
                # the lexicon is unavailable) — see nlp_engine.analyze_sentiment_text.
                from education_system.systems.university.services.ai_features.nlp_engine import (
