...
from education_system.systems.university.infrastructure.auth import UserAuth, set_auth_instance
from education_system.systems.university.infrastructure.shared_context import set_auth as set_shared_auth, get_auth
...
from education_system.systems.university.infrastructure.logging.log_config import patch_messagebox_logging
...
def set_auth(auth_instance):
    ...
    try:
        from education_system.systems.university.domain.governance.legal.disciplinary._db_init import (  # noqa: E501
    ...
    try:
        from education_system.systems.university.domain.governance.legal.disciplinary.fitness_to_practise._db_init import (  # noqa: E501
            ensure_ftp_schema,
        )
        ensure_ftp_schema()
    except Exception:
...
def init_gui(session_user=None):
    ...
    if role == 'student':
        from education_system.systems.university.interfaces.gui.shell.main.student_portal import StudentPortalGUI
        app = StudentPortalGUI(auth)
    elif role == 'staff':
        from education_system.systems.university.interfaces.gui.shell.main.staff_portal import StaffPortalGUI
        app = StaffPortalGUI(auth)
    elif role in ('instructor', 'teacher'):
        from education_system.systems.university.interfaces.gui.shell.main.instructor_portal import InstructorPortalGUI
        app = InstructorPortalGUI(auth)
    elif role == 'parent':
        from education_system.systems.university.interfaces.gui.shell.main.parent_portal_wrapper import ParentPortalWrapper
        app = ParentPortalWrapper(auth)
    elif role == 'alumni':
        from education_system.systems.university.interfaces.gui.shell.main.alumni_portal_wrapper import AlumniPortalWrapper
        app = AlumniPortalWrapper(auth)
    else:
...
from education_system.systems.university.interfaces.gui.shell.main.core.gui_setup import (
    ...
    restart_gui,
    _cancel_timers,
    _notification_user_id,
    _get_unread_notification_count,
    _refresh_notification_badge,
    _open_notifications,
    _update_clock,
)
from education_system.systems.university.interfaces.gui.shell.main.auth_gui import (
    logout_user,
    toggle_login_logout,
...
from education_system.systems.university.interfaces.gui.learners.students.student_records_gui import (
...
from education_system.systems.university.interfaces.gui.learners.students.student_crud_gui import (
...
from education_system.systems.university.interfaces.gui.learners.students.student_export_gui import (
    export_student_data,
...
from education_system.systems.university.interfaces.gui.shell.main.admin.user_management_gui import (
...
from education_system.systems.university.interfaces.gui.shell.main.admin.system_admin_gui import (
    show_system_admin,
...
from education_system.systems.university.interfaces.gui.shell.main.admin.database_admin_gui import (
    show_backup,
...
from education_system.systems.university.interfaces.gui.shell.main.admin.config_gui import (
...
from education_system.systems.university.interfaces.gui.shell.main.admin.admin_tools_gui import (
...
from education_system.systems.university.interfaces.gui.shell.main.features.academic_launchers_gui import (
    show_library_management,
    show_academic_calendar,
    show_course_management,
    show_module_management,
    show_module_scheduling,
    show_timetable_management,
    show_grades,
...
from education_system.systems.university.interfaces.gui.shell.main.features.student_affairs_gui import (
...
from education_system.systems.university.interfaces.gui.shell.main.features.finance_gui import (
...
from education_system.systems.university.interfaces.gui.shell.main.features.commerce_facilities_gui import (
...
from education_system.systems.university.interfaces.gui.shell.main.features.student_success_gui import (
...
from education_system.systems.university.interfaces.gui.shell.main.email.email_helpers_gui import (
    compose_email,
...
from education_system.systems.university.interfaces.gui.shell.main.dashboard.dashboard_gui import (
...
UnifiedManagementGUI.create_status_bar = create_status_bar
...
UnifiedManagementGUI._notification_user_id = _notification_user_id
UnifiedManagementGUI._get_unread_notification_count = _get_unread_notification_count
UnifiedManagementGUI._refresh_notification_badge = _refresh_notification_badge
UnifiedManagementGUI._open_notifications = _open_notifications
UnifiedManagementGUI._update_clock = _update_clock
...
def show_security_questions(self):
    ...
    try:
        from education_system.platform.delivery.gui.security_questions_gui import SecurityQuestionsFrame
...
UnifiedManagementGUI.show_timetable_management = show_timetable_management
...
def show_analytics_dashboard_gui(self):
    ...
    try:
        from education_system.systems.university.interfaces.gui.academics.research.external_quality_assurance.qa_widgets import EQAStatusStrip
    ...
    frame.pack(fill='both', expand=True)
    try:
        from education_system.systems.university.interfaces.gui.academics.research.external_quality_assurance.qa_receivers import consume_eqa_context
...
def show_reporting_warehouse_gui(self):
    ...
    from education_system.platform.features.analytics.warehouse_gui import WarehouseFrame
...
def show_information_rights_gui(self):
    ...
    drill-throughs work."""
    try:
        from education_system.systems.university.interfaces.gui.governance.legal.information_rights.information_rights_gui import (
...
def show_fitness_to_practise_gui(self):
    ...
    handler)."""
    try:
        from education_system.systems.university.interfaces.gui.governance.legal.disciplinary.fitness_to_practise.ftp_portal_gui import (
...
from education_system.systems.university.interfaces.gui.shell.main.features.academic_link_bar import (  # noqa: E402
...
_NEW_FEATURE_MODULES = [
    ...
    ("complaints_portal", "Complaints Portal",
     "education_system.systems.university.domain.operations.communications.feedback.complaints_portal"),
    ("course_evaluation_system", "Course Evaluation",
     "education_system.systems.university.interfaces.gui.academics.course_management_gui.course_evaluation_system"),
    ("lecturer_evaluation", "Lecturer Evaluation",
     "education_system.systems.university.interfaces.gui.academics.course_management_gui.lecturer_evaluation"),
    ("module_evaluation_portal", "Module Evaluation",
     "education_system.systems.university.interfaces.gui.academics.course_management_gui.module_evaluation_portal"),
    ("disciplinary_portal", "Disciplinary Portal",
     "education_system.systems.university.domain.governance.legal.disciplinary.disciplinary_portal"),
    ("fitness_to_practise", "Fitness to Practise",
     "education_system.systems.university.domain.governance.legal.disciplinary.fitness_to_practise.fitness_to_practise"),
    ("risk_management", "Risk Management",
     "education_system.systems.university.domain.governance.legal.risk_management.university_risk_management"),
    ("information_rights", "SAR / FOI Requests",
     "education_system.systems.university.interfaces.gui.governance.legal.information_rights.information_rights_gui"),
    ("first_aid_portal", "First Aid Portal",
     "education_system.systems.university.domain.pastoral.health.first_aid.first_aid_portal"),
    ("health_safety_portal", "Health & Safety",
     "education_system.systems.university.domain.governance.health_safety.health_safety_portal"),
    ("intervention_support", "Intervention Support",
     "education_system.systems.university.domain.pastoral.wellbeing.student_wellbeing.intervention_support"),
    ("safeguarding_system", "Safeguarding",
     "education_system.systems.university.domain.safeguarding.safeguarding_system"),
    ("lesson_planner", "Lesson Planner",
     "education_system.systems.university.domain.academics.course_planning.lesson_planner"),
    ("background_checker", "Background Checker",
     "education_system.systems.university.domain.staff.staff_hr.background_checks.university_bg_checker"),
    ("university_research", "Research Portal",
     "education_system.systems.university.domain.academics.research.services.university_research"),
    # Cross-system ports added 2026-04 — services with their own Tk launchers.
    ("employer_portal", "Employer Portal",
     "education_system.systems.university.domain.progression.employer_portal.employer_portal_app"),
    ("intervention_outcomes", "Intervention Outcomes",
     "education_system.systems.university.domain.pastoral.services.early_warning.outcomes.intervention_outcomes_app"),
    ("kpi_dashboard", "KPI Dashboard",
     "education_system.systems.university.domain.operations.reporting.kpi_dashboard.kpi_dashboard_app"),
    ("bursary", "Bursary Management",
     "education_system.systems.university.domain.finance.bursary.bursary_app"),
    ("mentoring_matching", "Peer Mentoring Matching",
     "education_system.systems.university.domain.pastoral.student_life.student_union.services.mentoring_matching.mentoring_matching_app"),
    ("room_booking", "Room Booking",
     "education_system.systems.university.interfaces.gui.operations.campus.facilities.room_booking_app"),
    ("building_management", "Building Management",
     "education_system.systems.university.interfaces.gui.operations.campus.facilities.building_management_app"),
    ("tutor_groups", "Tutor Groups",
     "education_system.systems.university.domain.pastoral.tutor_groups.tutor_groups_app"),
    # Standalone Tk apps moved from /add 2026-04
    ("apprenticeship_system", "Apprenticeships",
     "education_system.systems.university.domain.progression.apprenticeships.apprenticeship_system"),
    ("placement_tracker", "Placement Hours",
     "education_system.systems.university.domain.progression.placements.placement_tracker"),
    ("bakery_shop", "Bakery Shop",
     "education_system.systems.university.domain.operations.commerce.bakery_shop.app"),
]
...
def show_new_feature_university_research(self):
    ...
    if callable(opener):
        try:
            from education_system.systems.university.domain.academics.research.services.university_research import UniversityApp
...
def show_new_feature_module_evaluation_portal(self):
    ...
    def _build(host):
        from education_system.systems.university.interfaces.gui.academics.course_management_gui.module_evaluation_portal import ModuleEvaluationPortal
...
def show_new_feature_lecturer_evaluation(self):
    ...
    def _build(host):
        from education_system.systems.university.interfaces.gui.academics.course_management_gui.lecturer_evaluation import App
        app = App(host=host)
        app.run()
        return app
    self._embed_or_subprocess(title, module_dotted, _build)
...
def show_new_feature_lesson_planner(self):
    ...
    def _build(host):
        from education_system.systems.university.domain.academics.course_planning.lesson_planner import LessonPlannerApp
...
def show_new_feature_course_evaluation_system(self):
    ...
    import tkinter as tk
    from education_system.systems.university.interfaces.gui.academics.course_management_gui.course_evaluation_system import (
        CourseEvaluationApp,
    )
...
def show_new_feature_complaints_portal(self):
    ...
    module_dotted = (
        "education_system.systems.university.domain.operations.communications."
        "feedback.complaints_portal"
    )

    def _build(host):
        from education_system.systems.university.domain.operations.communications.feedback.complaints_portal import ComplaintsPortal
...
def show_new_feature_intervention_outcomes(self):
    ...
    def _build(host):
        from education_system.systems.university.domain.pastoral.services.early_warning.outcomes.intervention_outcomes_app import (
...
def show_new_feature_safeguarding_system(self):
    ...
    try:
        from education_system.systems.university.domain.safeguarding.safeguarding_system import (
            SafeguardingApp,
        )
    except Exception as e:
        from tkinter import messagebox
...
def show_new_feature_mentoring_matching(self):
    ...
    def _build(host):
        from education_system.systems.university.domain.pastoral.student_life.student_union.services.mentoring_matching.mentoring_matching_app import (
...
def show_new_feature_tutor_groups(self):
    ...
    def _build(host):
        from education_system.systems.university.domain.pastoral.tutor_groups.tutor_groups_app import (
...
def show_new_feature_employer_portal(self):
    ...
    def _build(host):
        from education_system.systems.university.interfaces.gui.progression.employer_portal.employer_portal_gui import (
            EmployerPortalFrame,
        )
        from education_system.systems.university.domain.progression.employer_portal.employer_portal_app import _get_current_user
...
def show_new_feature_kpi_dashboard(self):
    ...
    def _build(host):
        from education_system.systems.university.domain.operations.reporting.kpi_dashboard.kpi_dashboard_app import (
            _Frame as _KpiFrame,
            _get_current_user,
        )
...
def show_new_feature_institutional_analytics(self):
    title = "Institutional Analytics"
    module_dotted = (
        "education_system.systems.university.domain.operations.reporting."
        "institutional_analytics.institutional_analytics_app"
    )

    def _build(host):
        from education_system.systems.university.domain.operations.reporting.institutional_analytics.institutional_analytics_app import (
            _Frame as _AnalyticsFrame,
            _get_current_user,
        )
        frame = _AnalyticsFrame(host, user=_get_current_user())
        frame.pack(fill="both", expand=True)
        return frame
    self._embed_or_subprocess(title, module_dotted, _build)
...
def show_new_feature_apprenticeship_system(self):
    ...
    def _build(host):
        from education_system.systems.university.domain.progression.apprenticeships.apprenticeship_system import ApprenticeshipApp
...
UnifiedManagementGUI.show_new_feature_institutional_analytics = show_new_feature_institutional_analytics
...
def show_new_feature_first_aid_portal(self):
    """Launch the First Aid Portal in a standalone Toplevel sized like the
    main GUI (1400x900, minsize 1200x800) rather than embedding it in the
    main content frame — matches the Safeguarding/Finance convention."""
    try:
        from education_system.systems.university.domain.pastoral.health.first_aid.first_aid_portal import (
            FirstAidPortal,
        )
    except Exception as e:
        from tkinter import messagebox
        messagebox.showerror("First Aid Portal",
                             f"Could not load First Aid Portal module: {e}")
        return

    parent = getattr(self, 'root', None) or self
    win = tk.Toplevel(parent)
    win.title("University First Aid Portal")
    w, h = 1400, 900
    ...
    try:
        ...
        from tkinter import messagebox
        messagebox.showerror("First Aid Portal",
                             f"Could not start First Aid Portal: {e}")
        try:
            win.destroy()
        except Exception:
            pass
...
def show_new_feature_room_booking(self):
    ...
    def _build(host):
        from education_system.systems.university.interfaces.gui.operations.campus.facilities.room_booking_app import (
...
def show_new_feature_background_checker(self):
    ...
    def _build(host):
        from education_system.systems.university.domain.staff.staff_hr.background_checks.university_bg_checker import BGCheckerApp
