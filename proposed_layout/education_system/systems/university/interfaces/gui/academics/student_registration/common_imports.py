...
def get_user_role(auth) -> Optional[str]:
    """Return the current user's role (lowercased) from the auth
    instance, robust to dict- or object-shaped ``current_user``.
    Returns None when unauthenticated or role is absent."""
    if auth and getattr(auth, 'current_user', None):
        ...
        if isinstance(user, dict):
            ...
            user_dict = {}
        role = user_dict.get('role') or getattr(user, 'role', None)
