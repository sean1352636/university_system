...
Lecturers are the university's `instructors`, so the app reads the
directory straight from the `instructors` table (the old standalone
`lecturers` table has been retired). It adds a single
`lecturer_evaluations` table (none of the existing evaluation tables
match the 7-criterion lecturer schema).
"""
...
CREATE TABLE IF NOT EXISTS lecturer_evaluations (
    ...
    FOREIGN KEY (lecturer_id) REFERENCES instructors(id),
...
class Database:
    ...
    the same `instructors`, `modules`, `users`, and `students` data as
    every other module. Lecturers ARE the university's instructors, so
    the app reads them straight from the `instructors` table; the old
    standalone `lecturers` table has been retired (dropped on init).
    The only table this app introduces is `lecturer_evaluations`
    (idempotently created / migrated on init).
    """
    ...
    def _ensure_schema(self):
        ...
        try:
            ...
            conn.commit()
            self._migrate_off_lecturers_table(conn)
        finally:
            conn.close()

    def _migrate_off_lecturers_table(self, conn):
        """One-off migration: retire the standalone `lecturers` table.

        Older installs created `lecturer_evaluations` with a
        ``FOREIGN KEY (lecturer_id) REFERENCES lecturers(lecturer_id)``
        and stored lecturers in their own table. Lecturers are really the
        university's `instructors`, so we rebuild the evaluations table to
        reference ``instructors(id)`` and drop the now-unused `lecturers`
        table. Idempotent — a no-op once migrated or on a fresh install.
        """
        try:
            fks = conn.execute(
                "PRAGMA foreign_key_list(lecturer_evaluations)").fetchall()
            # PRAGMA row layout: (id, seq, table, from, to, ...)
            refs_lecturers = any((row[2] == "lecturers") for row in fks)
            has_lecturers = conn.execute(
                "SELECT 1 FROM sqlite_master "
                "WHERE type='table' AND name='lecturers'"
            ).fetchone() is not None
        except Exception:
            logger.exception("lecturer_evaluations migration probe failed")
            return

        if not refs_lecturers and not has_lecturers:
            return  # already migrated / fresh install

        # FK enforcement can't be toggled inside a transaction; do it first.
        conn.execute("PRAGMA foreign_keys = OFF")
        try:
            conn.execute("BEGIN")
            if refs_lecturers:
                conn.execute(
                    _LECTURER_EVALS_SCHEMA.replace(
                        "lecturer_evaluations", "lecturer_evaluations_new", 1))
                # Preserve any existing evaluations (column list is unchanged).
                conn.execute(
                    "INSERT INTO lecturer_evaluations_new "
                    "SELECT * FROM lecturer_evaluations")
                conn.execute("DROP TABLE lecturer_evaluations")
                conn.execute(
                    "ALTER TABLE lecturer_evaluations_new "
                    "RENAME TO lecturer_evaluations")
            conn.execute("DROP TABLE IF EXISTS lecturers")
            conn.execute("COMMIT")
            logger.info("Migrated lecturer_evaluations onto instructors; "
                        "dropped legacy lecturers table")
        except Exception:
            conn.execute("ROLLBACK")
            logger.exception("Failed migrating off lecturers table")
        finally:
            conn.execute("PRAGMA foreign_keys = ON")

    # -- lecturers (sourced from the shared instructors table) --------------
    def get_lecturers(self):
        """Return the university's instructors, shaped like the old
        lecturer rows: (lecturer_id, full_name, department, email, title).
        ``lecturer_id`` is the instructor's ``id``; ``title`` falls back to
        the instructor's specialization, else 'Lecturer'."""
        conn = self._connect()
        try:
            return conn.execute(
                "SELECT i.id, "
                "       TRIM(COALESCE(i.first_name,'') || ' ' || "
                "            COALESCE(i.last_name,'')) AS full_name, "
                "       COALESCE(i.department,''), "
                "       COALESCE(i.email,''), "
                "       COALESCE(NULLIF(TRIM(i.specialization),''), 'Lecturer') "
                "FROM instructors i "
    ...
    def get_all_lecturer_summaries(self):
        ...
        try:
            return conn.execute("""
                ...
                FROM instructors i
                LEFT JOIN lecturer_evaluations e ON i.id = e.lecturer_id
                WHERE COALESCE(i.is_active,1) = 1
                GROUP BY i.id
                ORDER BY full_name
            """).fetchall()
