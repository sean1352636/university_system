...
class TemplatesMixin:
    ...
    @staticmethod
    def _ensure_drafts_schema(cursor):
        """Create assignment_drafts and migrate legacy tables missing draft_name."""
        cursor.execute('''
        ...
        ''')
        # Legacy tables (created elsewhere) may lack draft_name; add it if missing.
    ...
    def save_assignment_draft(self):
        ...
        try:
            ...
            try:
                ...
                # Ensure schema (creates table / migrates legacy schema)
                self._ensure_drafts_schema(cursor)
                ...
                cursor.execute('''
                INSERT INTO assignment_drafts (user_id, draft_name, draft_data, created_at, updated_at)
    ...
    def load_assignment_template(self):
        ...
        try:
            ...
            # Ensure schema (migrates legacy tables that lack draft_name)
            self._ensure_drafts_schema(cursor)

            # Get templates and drafts
            cursor.execute('''
            SELECT id, COALESCE(draft_name, 'Draft #' || id), draft_data, created_at
    ...
    def save_assignment_template(self):
        ...
        try:
            ...
            template_name = simpledialog.askstring("Template Name",
                                                   ...
                                                   initialvalue=f"Template - {self.title_var.get()[:30]}")
