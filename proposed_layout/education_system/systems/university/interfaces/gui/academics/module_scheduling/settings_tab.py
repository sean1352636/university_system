...
from education_system.systems.university.interfaces.gui.academics.module_scheduling.dialogs import AddHolidayDialog
...
def _sync_holiday_to_academic_calendar(self):
    ...
    try:
        ...
        if holiday:
            ...
            # Sync into the single canonical calendar table.
            try:
                import uuid
                from datetime import datetime
                with get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='academic_calendar_events'")
                    if cursor.fetchone():
                        cursor.execute('''
                            INSERT OR IGNORE INTO academic_calendar_events
                                (id, name, description, date_start, date_end, event_type, date_added)
                            VALUES (?, ?, ?, ?, ?, 'holiday', ?)
                        ''', (f"HOL-{uuid.uuid4()}", event_title, description or '',
                              start_date, end_date or start_date, datetime.now().isoformat()))
                        conn.commit()
            except Exception as e:
                print(f"Could not sync to academic_calendar_events: {e}")
...
def _save_template_json_file(template_name, description, schedules):
    ...
    import json
    from education_system.systems.university.infrastructure.paths import TEMPLATES_DIR
