...
from education_system.systems.university.domain.pastoral.wellbeing.student_wellbeing.services.wellbeing_service import (
    WellbeingService,
)
...
class StudentWellbeingCLI:
    ...
    # ------------------------------------------------------------ check-ins
    def _checkins_menu(self):
        """Display the wellbeing check-ins submenu and handle user input."""
        while True:
            print("\n" + "=" * 60)
            print(" " * 16 + "WELLBEING CHECK-INS")
            print("=" * 60)

            print("\n  1. Log Check-in")
            print("  2. List Check-ins")
            print("  3. Check-in / Referral Summary")
            ...
            try:
                if choice == "0":
                    ...
                    self._checkin_summary()
                else:
                    print("Invalid choice. Please try again.")
            except Exception as e:
                print(f"\nError: {e}")

    def _log_checkin(self):
        """Log a wellbeing check-in against a student."""
        print("\nEnter check-in details (press Enter to skip optional fields):")

        student_id = input("  Student ID: ").strip()
        if not student_id:
            print("Student ID is required. Check-in not logged.")
            return

        mood_raw = input("  Mood rating (1-10, optional): ").strip()
        mood_rating = None
        if mood_raw:
            try:
                mood_rating = int(mood_raw)
            except ValueError:
                print("Mood rating must be a whole number. Check-in not logged.")
                return

        notes = input("  Notes (optional): ").strip() or None
        logged_by = input("  Logged by (staff name/ID, optional): ").strip() or None
        ...
        print(f"\nCheck-in logged successfully (ID: {checkin_id}).")

    def _list_checkins(self):
        """List wellbeing check-ins, optionally filtered by student."""
        student_id = input("  Filter by Student ID (Enter for all): ").strip() or None
        limit_input = input("  Max records (default 100): ").strip()
        limit = int(limit_input) if limit_input else 100

        records = self.service.list_checkins(student_id=student_id, limit=limit)
        if not records:
            print("\nNo check-ins found.")
            return
        self._print_records(records)

    def _checkin_summary(self):
        """Print aggregate wellbeing referral + check-in figures."""
        data = self.service.summary()
        print("\n--- Wellbeing Summary ---")
        print(f"  Referrals total: {data['referrals_total']}")
        print(f"    Auto (absence tracker): {data['referrals_auto']}")
        print(f"    Manual: {data['referrals_manual']}")
        print("  Referrals by status:")
        for k, v in sorted(data["referrals_by_status"].items()):
            print(f"    {k}: {v}")
        print("  Referrals by urgency:")
        for k, v in sorted(data["referrals_by_urgency"].items()):
            print(f"    {k}: {v}")
        print(f"  Check-ins total: {data['checkins_total']}")
        avg = data["avg_mood"] if data["avg_mood"] is not None else "-"
        print(f"  Average mood: {avg}")
        print("  Most recent referrals:")
        if not data["recent_referrals"]:
            print("    (none)")
        for r in data["recent_referrals"]:
            print(f"    #{r['id']}  {r['created_at']}  {r['student_id']}  "
                  f"{r['concern_type']}  [{r['urgency']}/{r['status']}]")
