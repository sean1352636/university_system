...
class CinemaCLI:
    ...
    # ---------- Export / Email helpers (same data source as GUI reports) ----------
    ...
    def _choose_report_type(self):
        """Prompt for a report type to export/email."""
        print("\nReport type:")
        print("  1. Sales Summary")
        print("  2. Revenue by Movie")
        print("  3. Daily Sales")
        print("  4. Occupancy")
        print("  5. Payment Methods")
        print("  6. Booking Status")
        mapping = {
            '1': 'sales_summary', '2': 'revenue_by_movie', '3': 'daily_sales',
            '4': 'occupancy', '5': 'payment_methods', '6': 'booking_status',
        }
        return mapping.get(input("Select report type [1]: ").strip() or '1', 'sales_summary')

    def _fetch_report_data(self, report_type, from_date, to_date):
        """Run the SAME query the cinema GUI reports use; return (headers, rows)."""
        ...
        if report_type == "sales_summary":
            ...
            ''', (from_date, to_date))
            headers = ["Booking Ref", "Customer", "Email", "Amount", "Payment", "Status", "Date"]
    ...
    def _export_report_csv(self):
        """Export a report to CSV (same data source as the GUI export)."""
        report_type = self._choose_report_type()
        from_date, to_date = self._get_date_range()
        headers, data = self._fetch_report_data(report_type, from_date, to_date)
        if not data:
            print("\nNo data to export for this period.")
            return

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        default_name = f"cinema_report_{report_type}_{timestamp}.csv"
        path = input(f"Output file path [{default_name}]: ").strip() or default_name
        ...
        print(f"\nExported {len(data)} record(s) to {os.path.abspath(path)}")

    def _email_report_to_admin(self):
        """Email a report to an admin (same report content as the GUI email)."""
        report_type = self._choose_report_type()
        from_date, to_date = self._get_date_range()
        headers, data = self._fetch_report_data(report_type, from_date, to_date)
        if not data:
            print("\nNo data to email for this period.")
            return
        ...
        if not row or not row[0]:
            print("\nNo admin email configured. Cannot send report.")
            return
        admin_email = row[0]

        title = self._REPORT_TITLES.get(report_type, "Cinema Report")
        report = f"{title}\n"
        ...
        report += "-" * 80 + "\n"
        for r in data:
            report += "\t".join(str(val) for val in r) + "\n"
        report += "\n" + "=" * 80 + "\n"
        ...
        send_email(
            ...
            subject=f"Cinema {title} - {today}",
            body=report,
        )
        print(f"\nReport emailed to {admin_email}")
