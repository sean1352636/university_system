"""Tk views for reading levels in the Primary School System."""
...
from education_system.systems.primary.domain.assessment.reading_levels.reading_levels import (
    BANDS, STATUSES,
)
...
def _safe_view(func: Callable[..., None]) -> Callable[..., None]:
    ...
    def wrapper(host, *args, **kwargs):
        try:
            ...
            try:
                messagebox.showerror("Reading Levels", str(e),
...
@_safe_view
def open_reading_levels(host) -> None:
    logger.debug("GUI: open_reading_levels")

    win = tk.Toplevel(host.root)
    win.title("Reading Levels")
    ...
    ttk.Label(filt, text="Band:").pack(side="left")
    band_var = tk.StringVar(value="All")
    ttk.Combobox(filt, textvariable=band_var,
                 values=["All"] + list(BANDS),
    ...
    cols = ("pupil_id", "name", "year", "band", "status",
            "last_assessed", "book")
    ...
    def _refresh() -> None:
        try:
            ...
            b = None if band_var.get() == "All" else band_var.get()
            s = None if status_var.get() == "All" else status_var.get()
            rows = data.list_records(year_group=y, band=b, status=s)
        except ValidationError as e:
            messagebox.showerror("Reading Levels", str(e), parent=win)
            return
        except Exception:
            logger.exception("reading levels refresh failed")
            messagebox.showerror("Error",
                                 "Could not load — see logs.", parent=win)
            return
        ...
        nonzero = [f"{b}: {n}" for b, n in counts.items() if n > 0]
        summary_var.set(
            f"Tracked: {total}" + ("   " + "   ".join(nonzero) if nonzero else "")
        )

    def _selected() -> str | None:
        sel = tree.selection()
        if not sel:
            messagebox.showinfo("Reading Levels", "Select a pupil first.",
                                parent=win)
            return None
        return sel[0]
    ...
    def _clear() -> None:
        ...
        if not messagebox.askyesno(
                "Clear record",
                f"Clear current reading-level record for {pid}? "
        ...
        try:
            ...
            messagebox.showerror("Error",
                                 "Could not clear — see logs.", parent=win)
            return
        _refresh()
    ...
    band_var.trace_add("write", lambda *_: _refresh())
...
def _open_record_dialog(parent, pupil_id: str,
    ...
    if pupil is None:
        messagebox.showerror("Reading Levels",
    ...
    dlg.title(f"Record reading level — {pupil.full_name}")
    ...
    if existing is not None:
        ttk.Label(frm,
                  text=f"Current: {existing.band} ({existing.status}), "
                       f"last {existing.last_assessed or '-'}",
                  foreground="#666").pack(anchor="w", pady=(0, 8))

    grid = ttk.Frame(frm)
    grid.pack(fill="x")
    ttk.Label(grid, text="Band *").grid(row=0, column=0, sticky="w", pady=4)
    band_var = tk.StringVar(value=existing.band if existing else BANDS[0])
    ttk.Combobox(grid, textvariable=band_var, values=list(BANDS),
    ...
    ttk.Label(grid, text="Current book title").grid(
        row=3, column=0, sticky="w", pady=4)
    book_var = tk.StringVar()
    ttk.Entry(grid, textvariable=book_var, width=30).grid(
        row=3, column=1, sticky="ew", pady=4)
    ttk.Label(grid, text="Notes").grid(row=4, column=0, sticky="w", pady=4)
    ...
    def _save() -> None:
        try:
            ...
            messagebox.showerror("Reading Levels", str(e), parent=dlg)
            return
        except Exception:
...
def _open_pupil_dialog(parent, pupil_id: str,
    ...
    try:
        ...
        logger.exception("loading pupil %s reading data failed", pupil_id)
        messagebox.showerror("Error",
                             "Could not load — see logs.", parent=parent)
        return

    dlg = tk.Toplevel(parent)
    dlg.title(f"Reading — {pupil.full_name}")
    dlg.transient(parent)
    ...
    if rec is None:
        ttk.Label(frm, text="No reading-level record yet.",
                  foreground="#666").pack(anchor="w", pady=(2, 8))
    else:
        ttk.Label(frm,
                  text=f"Current band: {rec.band} ({rec.status})   "
                       f"Last assessed: {rec.last_assessed or '-'}",
                  foreground="#444").pack(anchor="w", pady=(2, 2))
        if rec.book_title:
            ttk.Label(frm, text=f"Current book: {rec.book_title}",
    ...
    cols = ("id", "date", "band", "status", "book", "notes")
    ...
    for h in history:
        tree.insert("", "end", iid=str(h.assessment_id), values=(
            h.assessment_id, h.assessed_on, h.band, h.status,
            h.book_title or "", h.notes or ""))

    def _delete() -> None:
        sel = tree.selection()
        if not sel:
            messagebox.showinfo("Reading Levels",
                                "Select a history row first.", parent=dlg)
            return
        aid = int(sel[0])
        ...
        try:
            ...
            messagebox.showerror("Error",
                                 "Could not delete — see logs.", parent=dlg)
            return
        dlg.destroy()
        on_saved()
