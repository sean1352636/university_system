...
from education_system.systems.primary.domain.assessment.ks1_sats.ks1_sats import (
    KS1Result, OUTCOMES, OUTCOME_LABELS, SUBJECTS,
...
def _safe_view(func: Callable[..., None]) -> Callable[..., None]:
    ...
    def wrapper(host, *args, **kwargs):
        try:
            ...
            try:
                messagebox.showerror("KS1 SATs", str(e),
...
@_safe_view
def open_ks1_sats(host) -> None:
    logger.debug("GUI: open_ks1_sats")
    ...
    def _refresh() -> None:
        try:
            ...
            messagebox.showerror("KS1 SATs", str(e), parent=win)
            return
        except Exception:
            logger.exception("KS1 refresh failed")
    ...
    def _selected_id() -> int | None:
        sel = tree.selection()
        if not sel:
            messagebox.showinfo("KS1 SATs", "Select a result first.",
                                parent=win)
            return None
        return int(sel[0])
    ...
    def _outcome_help() -> None:
        ...
        messagebox.showinfo("KS1 outcomes", msg, parent=win)
...
def _open_form_dialog(parent, *, result_id: int | None,
    ...
    if result_id is not None:
        try:
            ...
            return
        if existing is None:
            messagebox.showerror("KS1 SATs",
                                 f"No result #{result_id}", parent=parent)
            return

    dlg = tk.Toplevel(parent)
    dlg.title("KS1 result" if existing else "New KS1 result")
    ...
    def _save() -> None:
        ...
        try:
            ...
            messagebox.showerror("KS1 SATs", str(e), parent=dlg)
            return
        except Exception:
            logger.exception("save KS1 failed")
