"""Mobile Dashboard aggregation layer for the Primary School System.
...
def _announcement_feed(limit: int = 5) -> list[FeedItem]:
    def _():
        from education_system.systems.primary.domain.operations.communications.announcements import (
            announcements,
        )
...
def _high_risk_feed(limit: int = 10) -> list[FeedItem]:
    def _():
        from education_system.systems.primary.domain.progress_dashboard import (
            progress_dashboard,
        )
...
def _open_alerts_feed(limit: int = 10) -> list[FeedItem]:
    def _():
        from education_system.systems.primary.domain.assessment.early_warning import (
            early_warning,
        )
