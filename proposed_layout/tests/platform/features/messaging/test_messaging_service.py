...
from education_system.platform.features.messaging.messaging_service import InterSystemMessagingService
...
def test_send_message_returns_dict(svc):
    msg = svc.send_message(
        sender_id=1, sender_system="sixth_form", sender_name="Alice",
...
def test_get_inbox(svc):
    svc.send_message(1, "sixth_form", "A", 2, "uni", "B", "", "", "s1", "b1")
    svc.send_message(3, "secondary", "C", 2, "uni", "B", "", "", "s2", "b2")
    svc.send_message(1, "sixth_form", "A", 9, "uni", "X", "", "", "s3", "b3")
...
def test_get_sent(svc):
    svc.send_message(1, "sixth_form", "A", 2, "uni", "B", "", "", "s1", "b1")
    svc.send_message(1, "sixth_form", "A", 3, "uni", "C", "", "", "s2", "b2")
...
def test_unread_count_with_system_filter(svc):
    ...
    svc.send_message(2, "c", "C", 5, "secondary", "B", "", "", "s2", "")

    assert svc.get_unread_count(user_id=5, system="uni") == 1
    assert svc.get_unread_count(user_id=5, system="secondary") == 1
    assert svc.get_unread_count(user_id=5) == 2
