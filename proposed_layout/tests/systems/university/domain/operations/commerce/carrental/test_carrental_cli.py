...
class TestEmailAndExport:
    def test_email_admin_report_sends(self, cli, capture, monkeypatch):
        ...
        monkeypatch.setattr(
            "education_system.systems.university.infrastructure.database.db.get_db_connection",
            _fake_conn,
        )
        monkeypatch.setattr(
            "education_system.systems.university.infrastructure.email.email_service.send_email",
            _fake_send,
        )
        buf = capture([])
