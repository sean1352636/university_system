...
from education_system.systems.university.interfaces.gui.academics.grade_tracking.dialogs.grade_edit_dialog import (
    GradeEditDialog,
    GRADE_SYSTEMS,
...
class TestGradeEditDialog:
    ...
    def test_ui_components_exist(self, root):
        ...
        # Tkinter returns combobox values as a tuple of strings
        assert list(dialog.grade_combo['values']) == expected_grades

        dialog.dialog.destroy()

    @patch('education_system.systems.university.interfaces.gui.academics.grade_tracking.dialogs.grade_edit_dialog.messagebox')
    ...
    def test_boundary_score_values(self, root):
        ...
        # save_grade() destroys the dialog on success, so use a fresh dialog
        # for each boundary value.
        for score, grade in (("0", "F"), ("100", "A+")):
            callback = Mock()
            ...
            dialog.score_var.set(score)
            dialog.grade_var.set(grade)
            dialog.save_grade()
