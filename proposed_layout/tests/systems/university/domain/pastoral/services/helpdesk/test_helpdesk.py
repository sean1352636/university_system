...
class TestTicketManagement:
    ...
    def test_update_ticket_status(self, setup_helpdesk_database, sample_user):
        ...
        # Create ticket
        cursor.execute('''
            INSERT INTO support_tickets (user_id, subject, description, category, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
...
class TestTicketReplies:
    ...
    def test_add_reply_to_ticket(self, setup_helpdesk_database, sample_user):
        ...
        # Create ticket
        cursor.execute('''
            INSERT INTO support_tickets (user_id, subject, description, category, created_at)
            VALUES (?, ?, ?, ?, ?)
...
class TestTicketAssignments:
    ...
    def test_assign_ticket(self, setup_helpdesk_database, sample_user):
        ...
        # assigned_to has TEXT affinity (user IDs may be non-numeric), so it
        # round-trips as a string.
        assert int(assigned_to) == 2001
...
class TestTicketEscalations:
    ...
    def test_escalate_ticket(self, setup_helpdesk_database, sample_user):
        ...
        # Create ticket
        cursor.execute('''
            INSERT INTO support_tickets (user_id, subject, description, category, escalation_level, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
