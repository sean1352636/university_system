"""Tests for ``education_system.systems.primary.infrastructure.paths``."""
...
def test_package_root_is_primary_package(paths_mod):
    assert paths_mod.PACKAGE_ROOT.name == "primarysch_system"
...
def test_pupils_db_lives_in_data_dir(paths_mod):
    assert paths_mod.PUPILS_DB == paths_mod.DATA_DIR / "primary.db"
...
def test_env_override_overrides_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("EDU_PRIMARY_DATA_DIR", str(override))
    from education_system.systems.primary.infrastructure import paths as paths_mod
    reloaded = importlib.reload(paths_mod)
    try:
        assert reloaded.DATA_DIR == override.resolve()
        assert reloaded.PUPILS_DB == override.resolve() / "primary.db"
    finally:
        monkeypatch.delenv("EDU_PRIMARY_DATA_DIR", raising=False)
        importlib.reload(paths_mod)
