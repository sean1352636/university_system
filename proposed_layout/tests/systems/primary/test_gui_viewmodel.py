...
@pytest.fixture
def g():
    from education_system.systems.primary import gui_main
    return gui_main
...
def test_count_pupils_counts_rows(g):
    with patch(
        "education_system.systems.primary.domain.learners.pupils.pupils.list_pupils",
        return_value=[1, 2, 3],
    ):
...
def test_count_staff_counts_rows(g):
    with patch(
        "education_system.systems.primary.domain.staff.staff.list_staff",
        return_value=[1, 2],
    ):
